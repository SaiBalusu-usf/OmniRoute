...
export async function sendAntigravityRequest(
  ...
  log.debug(
    "TELEMETRY",
    `[Antigravity] PhysicalSend - URL: ${url}, Model: ${model}, PhysicalSend: ${physicalSendOrdinal}, RetryAttempt: ${retryAttempt}`
  );
  ...
  if (response.status === HTTP_STATUS.FORBIDDEN && finalHeaders["x-goog-user-project"]) {
    ...
    log.debug(
      "TELEMETRY",
      `[Antigravity] PhysicalSend - URL: ${url}, Model: ${model}, PhysicalSend: ${retryPhysicalSendOrdinal}, RetryAttempt: ${retryAttempt}, Cause: x-goog-user-project-403`
    );
...
export async function tryCreditsRetry(
  ...
  try {
    ...
    log.debug(
      "TELEMETRY",
      `[Antigravity] PhysicalSend - URL: ${url}, PhysicalSend: ${creditsPhysicalSendOrdinal}, Cause: google-one-ai-credits-retry`
    );
