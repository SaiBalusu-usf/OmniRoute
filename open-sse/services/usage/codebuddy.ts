/**
 * CodeBuddy / WorkBuddy AI (International) usage handler — scoped to "codebuddy".
 *
 * Quota lives behind Tencent's international billing endpoint (POST, payload wrapped twice
 * under data.Response.Data). It mixes two credit types:
 *
 *  - Refill / base ("Free Plan Subscription" / monthly quota): recurring allowance.
 *    Live numbers in the *Cycle* fields; resetAt = next refresh.
 *  - Bonus ("Bonus Pack"): one-shot credits that run a single cycle and expire.
...
export async function getCodeBuddyUsage(
  accessToken?: string,
  apiKey?: string,
  ...
  try {
    ...
    return { message: "CodeBuddy error: failed to fetch quota." };
  }
}

export default getCodeBuddyUsage;
