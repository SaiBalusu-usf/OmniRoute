 ...
 * map, the dashboard, or fair-share saturation. These tests pin the mapping of
 * `limits[]` into `quotas["weekly <model> (7d)"]`.
 */
...
test("getClaudeUsage maps a weekly_scoped model limit into quotas['weekly <model> (7d)']", async () => {
  const usage = await withUsageResponse(oauthUsagePayload(), () => getClaudeUsage("tok-scoped"));

  assert.deepEqual(Object.keys(quotasOf(usage)).sort(), [
    "session (5h)",
    "weekly (7d)",
    "weekly fable (7d)",
  ]);
  assert.equal(quotasOf(usage)["weekly fable (7d)"].used, 69);
  assert.equal(quotasOf(usage)["weekly fable (7d)"].remaining, 31);
  assert.equal(quotasOf(usage)["weekly fable (7d)"].remainingPercentage, 31);
  assert.equal(quotasOf(usage)["weekly fable (7d)"].total, 100);
  assert.equal(
    quotasOf(usage)["weekly fable (7d)"].resetAt,
...
test("getClaudeUsage skips weekly_scoped rows without a usable name or percent", async () => {
  ...
  assert.equal(quotasOf(usage)["weekly cowork (7d)"].used, 5);
});
