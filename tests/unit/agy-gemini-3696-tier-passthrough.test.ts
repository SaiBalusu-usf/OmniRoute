...
test("(#3696) non-tiered ANTIGRAVITY_PUBLIC_MODELS entries resolve to distinct upstream ids", () => {
  ...
  for (const model of ANTIGRAVITY_PUBLIC_MODELS) {
    // Gemini 3.7/3.8/3.6 Flash tiers intentionally share the upstream
    // `gemini-3.7-flash-tiered` endpoint (#14017 — the backend has no distinct
    // 3.8/3.6 engine yet) with different reasoning token budgets.
    if (
      model.id.startsWith("gemini-3.7-flash") ||
      model.id.startsWith("gemini-3.8-flash") ||
      model.id.startsWith("gemini-3.6-flash")
    ) {
      continue;
    }
