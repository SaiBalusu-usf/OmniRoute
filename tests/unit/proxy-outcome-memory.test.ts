...
// A pool proxy is set aside only on a 429 the provider really returned, and only for a
// provider whose quota is tied to the egress IP. Locally generated 429s carry no upstream
// status and never accuse a proxy.
...
test("a received 429 from an IP-quota provider sets the proxy aside", () => {
...
test("a 429 from a provider outside the list writes nothing", () => {
...
test("a 2xx from an IP-quota provider clears the proxy, other statuses leave it", () => {
...
test("a 2xx from another provider only ends an unreachable period, not an IP-quota one", () => {
  noteProxyOutcome("opencode", { proxy: PROXY, upstreamStatus: 429 });
  noteProxyOutcome("openai", { proxy: PROXY, upstreamStatus: 200 });
  assert.equal(memory.isProxyAvoided(KEY), true, "a shared pool must not undo the quota");
