...
describe("OpencodeExecutor refused-route failover", () => {
  ...
  it("fails over past two refused routes to the healthy third", async () => {
    ...
    assert.strictEqual(
      ...
      200,
      "must fail over past refused routes"
    );
  ...
  it("propagates the last 403 after exhausting all proxies without marking success", async () => {
    ...
    for (const a of accountsOf(exec)) {
      ...
      assert.strictEqual(a.cooldownUntil, 0, "no cooldown from refused-route failover");
    }
  });
  ...
  it("refused-route failover never cools the account down", async () => {
  ...
  it("shares the tried-set with geo-blocked proxies in both directions", async () => {
    // A geo-blocked, B refused, C healthy: neither tried route is re-called.
    const exec = new OpencodeExecutor("opencode-zen");
    installFetch([
    ...
    // Reverse direction: A refused, B geo-blocked, C healthy.
  ...
  it("451 + signal: no failover, no success mark", async () => {
    const exec = new OpencodeExecutor("opencode-zen");
