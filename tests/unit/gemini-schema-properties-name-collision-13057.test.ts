...
test("#13477 does not inject type:object into properties map when a property is named 'required'", () => {
  const input = {
    type: "object",
    properties: {
      delivery: {
  ...
  const result = cleanJSONSchemaForAntigravity(input) as Record<string, unknown>;
  const delivery = (result.properties as Record<string, unknown>).delivery as Record<string, unknown>;
  const pin = (delivery.properties as Record<string, unknown>).pin as Record<string, unknown>;
  const pinProps = pin.properties as Record<string, unknown>;

  // Ensure "type" is NOT injected as a sibling property inside pin's properties map
  assert.equal(
    pinProps.type,
    undefined,
    "pin properties map must not have 'type: object' injected when a property is named 'required'"
  );

  // Ensure 'required' property is preserved as a boolean schema
  assert.deepEqual(
    pinProps.required,
    { type: "boolean" },
    "property named 'required' must be preserved as boolean schema"
  );
  assert.deepEqual(pin.required, ["enabled"], "object's required array must be preserved");
});
