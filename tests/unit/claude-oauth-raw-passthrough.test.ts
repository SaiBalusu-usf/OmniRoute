...
import { resolveExecutorWithProxy } from "../../open-sse/handlers/chatCore/executorProxy.ts";
...
describe("retry signing guard, marker plumbing and proxy fallback (Task 5)", () => {
  ...
  it("TC-15b: raw-passthrough claude connection bypasses proxy executors", async () => {
    const logs: string[] = [];
    const log = { info: (_tag: string, msg: string) => logs.push(msg) };
    const executor = await resolveExecutorWithProxy("claude", log, { rawPassthrough: true });
    assert.equal(executor.provider, "claude", "must fall back to the native claude executor");
    assert.ok(
