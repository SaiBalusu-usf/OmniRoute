...
**النطاق:** اتصال/حساب/مفتاح واحد لمزوّد.

**الغرض:** تجاوز مفتاح واحد معطّل مع استمرار الاتصالات الأخرى للمزوّد نفسه في تقديم الخدمة.
...
- أساس OAuth: 5 ثوانٍ
- أساس مفتاح API: 3 ثوانٍ
- استجابة 429 لمفتاح API: تفضّل ترويسات `Retry-After`/إعادة الضبط الواردة من المنبع أو نص إعادة ضبط قابلًا للتحليل
- التراجع: `baseCooldownMs * 2 ** failureIndex`

**آلية الحماية من الاندفاع المتزامن:** تمنع حالات الفشل المتزامنة من تمديد فترة التهدئة بشكل مفرط أو زيادة `backoffLevel` مرتين.

**الحالات النهائية (ليست فترات تهدئة):**

- `banned` — تُضبط عند اكتشاف كلمة مفتاحية للحظر / حظر الحساب (راجع [BAN_DETECTION](../security/BAN_DETECTION.md))
...
تستمر هذه الحالات حتى تتغير بيانات الاعتماد أو يعيد أحد المشغّلين ضبطها. لا تستبدل الحالات النهائية بحالة تهدئة عابرة.
...
**النطاق:** جلسة عميل واحدة (ترويسة `X-Session-Id` / `x-codex-session-id` / `x-omniroute-session`) مثبتة على اتصال واحد، لأي مزوّد.

**الغرض:** إبقاء وكيل متعدد الجولات (Claude Code، وaider، والوكلاء المخصصين) على الحساب نفسه عبر الطلبات، مما يقلل فقدان السياق بين الحسابات واستجابات 429 المتكررة عند البدء البارد لدى المزوّدين الذين يحتفظون بحالة جلسة لكل حساب.

**التنفيذ:**

- تحديد مدة TTL: `src/sse/services/sessionAffinityPin.ts::resolveSessionAffinityTtlMs()`
- اختيار/إنشاء التثبيت: `src/sse/services/sessionAffinityPin.ts::selectSessionAffinityConnection()`
...
- الإعداد: `sessionAffinityTtlMs` (مدة TTL عامة بالمللي ثانية، وتعطّلها القيمة `0`) — `src/lib/db/settings.ts`. أُعيدت تسميته من `codexSessionAffinityTtlMs` الخاص بـ Codex فقط بواسطة الترحيل `124_generic_session_affinity_ttl.sql`، الذي ينقل أي مدة TTL مضبوطة سابقًا لـ Codex لتصبح القيمة الافتراضية الجديدة.

قبل #7274، كانت `resolveSessionAffinityTtlMs()` تُرجع `0` فورًا لكل مزوّد باستثناء `codex`، ولذلك لم يكن لإعداد TTL (ولا لترويسات الجلسة) أي تأثير في أي مكان آخر، رغم أن آلية التثبيت واستخراج الترويسة كانا بالفعل غير مرتبطين بمزوّد محدد. أزال الإصلاح ذلك الإرجاع المبكر؛ وأصبحت مدة TTL تُطبّق الآن بصورة موحّدة على كل مزوّد بمجرد ضبطها عالميًا على قيمة أكبر من `0`.

لا تُمرَّر ترويسات ألفة الجلسة الثلاث إلى المنبع مطلقًا — إذ تنشئ المنفّذات ترويسات المنبع الخاصة بها من الصفر بدلًا من تمرير ترويسات العميل، ولذلك تبقى هذه الترويسات معرّفات ارتباط داخلية فقط.

### إيجارات اتصالات الجلسات المُدارة الحصرية

**النطاق:** يمتلك عميل/جلسة HTTP مُدارة ونشطة اتصال OmniRoute واحدًا مؤهلًا.

**الغرض:** توفير ملكية حصرية ودائمة للاتصال للعملاء الذين يحتاجون إلى حاجز توجيه صارم
عبر الطلبات. يختلف هذا عن ألفة الجلسة، التي تمثل تفضيلًا مرنًا للاستمرارية:
إذ يستمر الإيجار الحصري في حفظ حالة دورة الحياة داخل SQLite، ويفرض فرادة المالك النشط
والاتصال النشط على المستوى العام، ويرفض الجيل القديم قبل الإرسال إلى المزوّد.

تكون الميزة اختيارية لكل مفتاح API. يجب أن يمتلك المفتاح المُدار النطاق `lease:exclusive` وقائمة
`allowedConnections` صريحة وغير فارغة. يمكن لأي عميل HTTP استخدام نقطة نهاية دورة الحياة؛ ولا
يلزم اسم عميل أو وكيل مستخدم أو مزوّد أو طريقة OAuth أو نموذج. يمتلك الإيجار اتصالًا،
وليس نموذجًا، ولذلك يحتفظ تغيير النموذج بالارتباط ما دام الاتصال مؤهلًا
بشكل اعتيادي. تظل قواعد النموذج والحصة والسلامة والتهدئة وقائمة السماح العادية هي المرجع النهائي، وقد
تنقل الجيل نفسه إلى اتصال مؤهل وحر آخر.

تتمثل دورة الحياة في `POST /api/v1/session-leases` مع إجراءات JSON التالية: `acquire` و`renew` و`release`.
تقدّم طلبات الاستدلال المُدارة قيمة `X-OmniRoute-Lease-Owner` المبهمة والقيمة الدقيقة
لـ`X-OmniRoute-Lease-Generation`. يستخدم المالك البادئة `vlo_` متبوعة بـ43 محرفًا من محارف base64url؛ ولا
تُخزّن سوى قيمة تجزئة SHA-256 الخاصة به. يربط كل حاجز إرسال نهائي أيضًا معرّف مفتاح API المصادق عليه
ومعرّف الاتصال النشط. تُزال ترويسات التحكم في الإيجار من السجلات ولقطات الطلبات المحتفظ بها
وترويسات منفّذ المنبع.

إذا كان التوجيه العادي يحتوي على مرشحين مُدارين مؤهلين، لكن كل مرشح حر مشغول بإيجار
نشط أجنبي، يعيد OmniRoute استجابة HTTP `429`، ورمز lease-capacity-unavailable، وحالة
انتظار للسعة، وقيمة `Retry-After` محدودة مشتقة من أقرب وقت انتهاء ذي صلة.
لا يُعد انعدام الأهلية العادي تنافسًا على الإيجار، ويحتفظ بدلالات أخطاء التوجيه الحالية.
...
- إيجارات اتصالات الجلسات المُدارة الحصرية هي ملكية دائمة لدورة الحياة مع حاجز للجيل.

---

## 3. حظر النموذج

**النطاق:** مجموعة المزوّد + الاتصال + النموذج.

**الغرض:** تجنّب تعطيل اتصال كامل عندما يكون نموذج واحد فقط غير متاح أو خاضعًا لحدود الحصة.

**أمثلة:**

- المزوّدون ذوو الحصص المحددة لكل نموذج الذين يعيدون 429
- المزوّدون المحليون الذين يعيدون 404 عند فقدان نموذج واحد
- حالات فشل الأذونات الخاصة بالمزوّد لوضع/نموذج معيّن (مثل أوضاع Grok)

**التنفيذ:** `open-sse/services/accountFallback.ts` — `lockModel()`، `clearModelLock()`، `getAllModelLockouts()`.

### لوحة فترات تهدئة النماذج (v3.8.0)

واجهة المستخدم: الإعدادات → فترات تهدئة النماذج (`src/app/(dashboard)/dashboard/settings/components/ModelCooldownsCard.tsx`)

تعرض حالات الحظر النشطة مع: المزوّد، والاتصال، والنموذج، والسبب، وexpiresAt. يمكن للمشغّلين إعادة تمكين نموذج يدويًا من البطاقة.

**واجهة REST API:**

- `GET /api/resilience/model-cooldowns` — عرض حالات الحظر النشطة
- `DELETE /api/resilience/model-cooldowns` — إعادة التمكين يدويًا. النص: `{provider, connection, model}`. المصادقة: إدارية.

### واجهة إعدادات الحظر + الاسترداد بالتناقص عند النجاح (v3.8.23)

انتقل حظر النموذج من سلوك ثابت ومفعّل دائمًا إلى ميزة قابلة للتهيئة بالكامل،
تتطلب التفعيل صراحةً، ولها بطاقة إعدادات خاصة بها ومسار استرداد ذاتي الإصلاح.

**بطاقة الإعدادات:** الإعدادات → حظر النموذج
(`src/app/(dashboard)/dashboard/settings/components/ModelLockoutCard.tsx`).
وهي **مختلفة** عن `ModelCooldownsCard` للقراءة فقط أعلاه (التي
_تعرض_ حالات الحظر النشطة فحسب) — إذ تضبط البطاقة الجديدة _المعلمات_. توجد القيم
الافتراضية في `DEFAULT_MODEL_LOCKOUT_SETTINGS`
(`src/lib/resilience/modelLockoutSettings.ts`):

| الإعداد                 | القيمة الافتراضية                | المعنى                                                       |
| ----------------------- | -------------------------------- | ------------------------------------------------------------ |
| `enabled`               | `false`                          | مفتاح التبديل الرئيسي — حظر النموذج **معطّل افتراضيًا**.     |
| `errorCodes`            | `[403, 404, 429, 502, 503, 504]` | حالات المنبع التي تُحتسب كفشل خاص بالنموذج.                  |
| `baseCooldownMs`        | `120_000` (120 ث)                | مدة الحظر الأولية عند أول فشل.                               |
| `maxCooldownMs`         | `1_800_000` (30 دقيقة)           | الحد الأقصى لفترة التهدئة المتصاعدة.                         |
| `maxBackoffSteps`       | `10`                             | الحد الأقصى لخطوات تصعيد التراجع الأُسّي.                    |
| `useExponentialBackoff` | `true`                           | ما إذا كانت حالات الفشل المتكررة تصعّد فترة التهدئة أُسّيًا. |

تُحفَظ الإعدادات عبر مخزن الإعدادات المعتاد ويجري التحقق منها من خلال
مخطط إعدادات المرونة؛ وتقيّد البطاقة `baseCooldownMs`/`maxCooldownMs`
(بحيث يكون `maxCooldownMs ≥ baseCooldownMs`) و`maxBackoffSteps`.

**الاسترداد بالتناقص عند النجاح:** لا يعتمد الاسترداد **فقط** على انتهاء المؤقت. إذ تؤدي
الاستجابة السليمة إلى خفض عدد حالات فشل النموذج تدريجيًا، بحيث يتوقف النموذج الذي استرد عافيته
في منتصف النافذة عن التصعيد (ويُزال حظره) قبل انتهاء مؤقته. عند نجاح
هدف مركّب، يستدعي `open-sse/services/combo.ts` الدالة `decayModelFailureCount()`
(`open-sse/services/accountFallback.ts`)، التي **تنصّف** قيمة
`failureCount` المخزّنة (`Math.floor(failureCount / 2)`)؛ وعندما تصل إلى `0`، يُحذف إدخال الحظر
بالكامل. وتزيد الدالة المقابلة `recordModelLockoutFailure()`
العدد (وتصعّد فترة التهدئة) عند حدوث حالات فشل ضمن
نافذة التصعيد. ويأتي هذا التناقص عند النجاح بالإضافة إلى انتهاء المؤقت العادي —
إذ يمكن لأي من المسارين إعادة تمكين النموذج.

**الحالة:** تُحفظ حالات الحظر **في الذاكرة** (`Map`s لكل عملية من النوع
`ModelLockoutEntry` ومفاتيحها بالصيغة `provider:connectionId:model`)، ولا تُحفَظ في
قاعدة البيانات — لذا تُفقَد عند إعادة التشغيل. أما _الإعدادات_ فتُحفَظ؛ بينما تكون
...
- تم تخطي جميع مفاتيح أحد المزوّدين → تحقّق من حالة قاطع الدائرة ومن `rateLimitedUntil`/`testStatus` لكل اتصال.
- استبعاد المزوّد بشكل دائم بعد نافذة إعادة التعيين → تقرأ الشيفرة قيمة `state` الخام بدلًا من `getStatus()`/`canExecute()`.
- فشل مفتاح واحد بينما ينبغي أن تعمل المفاتيح الأخرى → فضّل فترة تهدئة الاتصال على قاطع الدائرة.
- فشل نموذج واحد فقط → فضّل حظر النموذج على فترة تهدئة الاتصال.
- ينبغي أن تتعافى الحالة ذاتيًا لكنها لا تفعل → تحقّق من وجود طابع زمني مستقبلي ومن مسار قراءة يحدّث الحالة المنتهية الصلاحية. تتطلب الحالات الدائمة تغييرات يدوية.
