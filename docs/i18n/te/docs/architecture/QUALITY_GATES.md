...
## గేట్ల జాబితా (~90 స్క్రిప్ట్లు)

స్క్రిప్ట్లు `scripts/check/` (విధాన గేట్లు) మరియు `scripts/quality/` (ratchet ఇంజిన్) కింద ఉంటాయి.
CIకి ప్రామాణిక మూలం `.github/workflows/ci.yml`.

### విడుదల PR వేగవంతమైన మార్గం (`quality.yml`)

`.github/workflows/quality.yml` అనేది `release/**`ను లక్ష్యంగా చేసుకున్న PRలపై అమలవుతుంది. ఇది path-filtered వేగవంతమైన గేట్లతో contributor
బ్రాంచ్లు ముందుకు సాగేటట్లు చేస్తుంది, అలాగే కోడ్ మార్పుల కోసం ఒక సలహాపూర్వక production-build సంకేతాన్ని
అందిస్తుంది:

| జాబ్                                             | పరిధి                                                                                                                                                                                                                         | నిరోధం                                                                                                     |
| ------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------- |
| `Build (advisory)`                               | Non-draft కోడ్ PRలు మరియు Mergify queue బ్రాంచ్లు; Node 24, `npm-ci-retry`, `check:node-runtime`, `OMNIROUTE_USE_TURBOPACK=1`తో `npm run build`; downstream నాణ్యత జాబ్ ఏదీ దాన్ని వినియోగించనందున artifact upload ఉండదు      | **సలహాపూర్వకం** (`continue-on-error: true`; స్థిరమైన release-PR అమలులు ఒక వారం పూర్తయిన తర్వాత తొలగించండి) |
| `Docs Gates (fast-path)`                         | డాక్స్/కోడ్ PRలు; API డాక్స్ refs మరియు docs-all                                                                                                                                                                              | అవును                                                                                                      |
| `Fast Quality Gates`                             | కోడ్ PRలు; static checks, typecheck, dashboard typecheck, ప్రభావిత unit tests                                                                                                                                                 | అవును                                                                                                      |
| `Forgotten sibling tests`                        | కోడ్ PRలు; మార్చబడిన modules నుంచి static consumers మరియు candidate sibling tests వరకు trace చేయబడతాయి; barrel మరియు dynamic-import మార్గాలు, సూచించబడిన allowlist exceptionsతో పాటు, సలహాపూర్వక diagnosticsగా నివేదించబడతాయి | **సలహాపూర్వకం**                                                                                            |
| `Vitest (fast-path)`                             | కోడ్ PRలు; వేగవంతమైన vitest suite                                                                                                                                                                                             | అవును                                                                                                      |
| `Unit Tests fast-path`                           | కోడ్ PRలు; 4-shard unit suite                                                                                                                                                                                                 | అవును                                                                                                      |
| `No new ESLint warnings`                         | కోడ్ PRలు; suppressions-aware lint guard                                                                                                                                                                                      | స్వంత మూలం కోసం అవును, forks కోసం సలహాపూర్వకం                                                              |
| `Merge integrity (changelog + generated skills)` | Non-draft PRలు; changelog మరియు generated skill sync                                                                                                                                                                          | స్వంత మూలం కోసం అవును, forks కోసం సలహాపూర్వకం                                                              |

#### మరచిపోయిన sibling tests నివేదిక

`npm run check:forgotten-sibling-tests` అనేది test-impact map వెనుక ఉన్న import resolverను తిరిగి ఉపయోగిస్తుంది.
మార్చబడిన ప్రతి production module కోసం, candidate
test అనేది pull-request diffలో లేనప్పుడు, ఇది నిర్దిష్టమైన
`changed module/symbol -> static consumer -> candidate sibling test` chainలను నివేదిస్తుంది. ఏదైనా నిరోధక rolloutకు ముందు calibration కోసం Markdown సారాంశం మరియు JSON ఫలితం
`forgotten-sibling-tests` workflow artifactగా ఉంచబడతాయి.

Barrel re-exports మరియు dynamic imports అనేవి resolution diagnostics మాత్రమే; అవి ఎప్పటికీ
నిరోధక findingను సృష్టించవు. సమీక్షించబడిన exceptions
`config/quality/forgotten-sibling-allowlist.json`లో ఉంటాయి. ప్రతి entry తప్పనిసరిగా consumer మరియు candidate
testను పేర్కొనాలి, నిర్దిష్ట rationaleను అందించాలి, అలాగే GitHub issue లేదా pull requestకు link చేయాలి. సరిగా లేని entries
fail closed అవుతాయి. తొలగించబడిన candidate testను లేదా `.skip`/`.todo`ను జోడించే diffను exceptions
suppress చేయలేవు;
assertion weakening మరియు ఇతర masking అంశాలు స్వతంత్రంగా నిరోధించే
`check:test-masking` గేట్ పరిధిలోనే ఉంటాయి.

### జాబ్: `lint`

`main`కు వచ్చే ప్రతి PRపై అమలవుతుంది. వైఫల్యం సంభవిస్తే mergeను నిరోధిస్తుంది.

| స్క్రిప్ట్ (`npm run ...`)        | ధృవీకరించేది                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | నిరోధకమా                                  |
| --------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------- |
| `check:node-runtime`              | Node.js వెర్షన్ మద్దతు ఉన్న పరిధిలో ఉంది                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | అవును                                     |
| `check:cycles`                    | సర్క్యులర్ ఇంపోర్ట్లు — అన్ని `src/` + `open-sse/` మాడ్యూల్లు                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | అవును                                     |
...
| `check:model-lifecycle`           | చేతితో నిర్వహించబడే మూడు routing tables, checked-in lifecycle snapshotతో (#11503) స్థిరంగా ఉంటాయి: `FITNESS_TABLE` (`taskFitness.ts`), `REGISTRY` route చేయగల retired idకు score ఇవ్వదు; ప్రతి `BUILT_IN_ALIASES` target `REGISTRY`లో ఉంటుంది మరియు retired-id snapshotలో ఉండదు; `REGISTRY`లో ఇప్పటికీ ఉన్న ప్రతి retired id forward చేయబడుతుంది లేదా `allowedRetiredInCatalog`లో జాబితా చేయబడుతుంది; అలాగే ఏ `DEFAULT_DEGRADATION_MAP` source లేదా target కూడా ఆ snapshotలో retiredగా కనిపించదు. ఒక model ప్రస్తుతం live upstream ద్వారా అందించబడుతోందని ఇది నిరూపించదు. Offline — `config/quality/model-lifecycle.json`తో పోలుస్తుంది; ఇది `npm run quality:refresh-model-lifecycle`తో చేతితో refresh చేయబడుతుంది (network; CIతో అనుసంధానించబడలేదు). `allowedRetiredInCatalog` ఒక burn-down ratchet: tracking issue ఉన్నప్పుడు మాత్రమే entryని జోడించండి. | అవును                                     |
| `check:fetch-targets`             | client-side `src/`లోని ప్రతి `fetch("/api/...")` నిజమైన `route.ts`కు resolve అవుతుంది                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | అవును                                     |
| `check:deps`                      | repoలోని ప్రతి `package.json`లో ఉన్న, `npm install` చేయగల అన్ని deps `dependency-allowlist.json`లో ఉంటాయి; కొత్తగా pin చేయని లేదా slopsquat చేయబడిన packages flag చేయబడతాయి                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | అవును                                     |
...
| `check:lockfile`                  | `package-lock.json` integrity — https registry, integrity hashes, host overrides లేవు                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | అవును                                     |
...
| `check:file-size`                 | ఏ సోర్స్ ఫైల్ కూడా ఒక్కో ఎక్స్టెన్షన్కు నిర్దేశించిన గరిష్ఠ పరిమితిని మించదు (రాచెట్: `frozen` జాబితాలో స్థిరపరచిన పెద్ద ఫైళ్లు)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | అవును                                     |
...
| `check:migration-numbering`       | మైగ్రేషన్ SQL ఫైళ్లకు వరుస క్రమంలో సంఖ్యలు కేటాయించబడ్డాయి; ఖాళీలు లేదా నకళ్లు లేవు                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | అవును                                     |
| `check:public-creds`              | `publicCreds.ts` వెలుపల లిటరల్ OAuth `client_id`/`client_secret` లేదా Firebase Web కీలు ఉండకూడదు (కఠిన నియమం #11)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | అవును                                     |
| `check:db-rules`                  | `src/lib/db/` మాడ్యూళ్ల వెలుపల రా SQL ఉండకూడదు; `localDb.ts` నుండి బ్యారెల్-ఇంపోర్ట్లు ఉండకూడదు (కఠిన నియమాలు #2/#5)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | అవును                                     |
| `check:known-symbols`             | వాటి డిస్పాచ్ పట్టికలలో నమోదు చేసిన ప్రొవైడర్ ఎగ్జిక్యూటర్లు, రూటింగ్ వ్యూహాలు మరియు అనువాదకాలు డిస్క్లోని ఫైళ్లతో సరిపోలాలి — అనాథ లేదా ప్రకటించని సింబల్లు ఉండకూడదు                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | అవును                                     |
| `check:route-guard-membership`    | చైల్డ్ ప్రాసెస్ను ప్రారంభించే ప్రతి రూట్ను `isLocalOnlyPath()` ద్వారా వర్గీకరించాలి (కఠిన నియమాలు #15/#17)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | అవును                                     |
| `check:test-discovery`            | రెపోలోని ప్రతి `*.test.ts` / `*.spec.ts` ఫైల్ను కనీసం ఒక టెస్ట్ రన్నర్ సేకరించాలి (ర్యాచెట్: `test-discovery-baseline.json`లోని అనాథల జాబితా కేవలం తగ్గాలి)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | అవును                                     |
| `check:agent-skills-sync`         | రూపొందించిన agent-skills ఆర్టిఫాక్ట్లు వాటి సోర్స్ కేటలాగ్తో సరిపోలాలి (విచలనం ఉండకూడదు)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `check:provider-asset-provenance` | ప్రొవైడర్ లోగోలు/అసెట్లకు నమోదు చేసిన మూలాధార ఎంట్రీ ఉండాలి                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `lint:json`                       | JSON కాన్ఫిగరేషన్ ఫైళ్లు పార్స్ అవుతాయి మరియు repo lint నియమాలను సంతృప్తిపరుస్తాయి                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `typecheck:core`                  | ఎలాంటి లోపాలు లేకుండా TypeScript కంపైలేషన్ (సలహా హెచ్చరికలు మాత్రమే)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | అవును                                     |
| `typecheck:noimplicit:core`       | కఠినమైన `noImplicitAny` — భవిష్యత్ను దృష్టిలో ఉంచుకున్నది; ముందే ఉన్న అనేక కాల్ సైట్లకు ఇప్పటికీ అనోటేషన్లు అవసరం                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | **సలహాత్మకం** (`continue-on-error: true`) |
| `check:dashboard-typecheck`       | `src/app/(dashboard)/**` (#7033)కు పరిమితమైన `tsc` — `typecheck:core` యొక్క ఎంపిక చేసిన 27-ఫైళ్ల అనుమతి జాబితాలో ఎలాంటి dashboard TSX లేదు, అలాగే `next build` కూడా దానిని ఎప్పుడూ టైప్-చెక్ చేయదు (`next.config.mjs`లో `ignoreBuildErrors: true` సెట్ చేయబడింది), కాబట్టి అక్కడి అనాథ-ఐడెంటిఫైయర్ రిగ్రెషన్లు (#6625/#6909) CIకి కనిపించలేదు. స్థిరీకరించిన ప్రతి-ఫైల్/ప్రతి-TS-కోడ్ గణన బేస్లైన్తో (`config/quality/dashboard-typecheck-baseline.json`, `check:known-symbols`లో ఉన్న అదే పాతదనం-నిరోధక నమూనా) తేడాలను పోలుస్తుంది — బేస్లైన్ గణనను మించిన కొత్త లోపాలు మాత్రమే గేట్ను విఫలం చేస్తాయి; ముందే ఉన్న లోపం పరిష్కరించబడినప్పుడు `--update`తో బేస్లైన్ను తగ్గించండి.                                                                                                                                                                            | అవును                                     |

### జాబ్: `quality-gate`

`test-coverage` తర్వాత నడుస్తుంది. విఫలమైతే mergeను నిరోధిస్తుంది.

| స్క్రిప్ట్                   | ధ్రువీకరించేది                                                                                                                                                                                             | నిరోధించేది                |
| ---------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------- |
...
| `quality:ratchet`            | `quality-baseline.json`లోని ప్రతి metric రిగ్రెస్ కాలేదు (ESLint హెచ్చరికలు ≤ baseline; coverage ≥ baseline)                                                                                               | అవును                      |
| `check:duplication`          | కోడ్ డూప్లికేషన్ (jscpd@4) `quality-baseline.json`లోని baselineను మించదు                                                                                                                                   | అవును                      |
...
| `check:cognitive-complexity` | Cognitive complexity ratchet (`eslint-plugin-sonarjs`) — ప్రత్యేక ESLint పాస్; CI రెండింటినీ ఒకే `check:complexity-ratchets` దశగా విలీనం చేసి నడుపుతుంది                                                   | అవును                      |
| `check:dead-code`            | ఉపయోగించని exports / files ratchet baselineతో పోలిస్తే (knip) రిగ్రెస్ కాలేదు                                                                                                                              | అవును                      |
| `check:compression-budget`   | కంప్రెషన్ బెంచ్మార్క్ బడ్జెట్ — ప్రతి-engine token-savings కనిష్ఠ పరిమితులు రిగ్రెస్ కాకూడదు                                                                                                               | అవును                      |
| `check:type-coverage`        | టైప్ చేయబడిన శాతం ratchet (`type-coverage`) రిగ్రెస్ కాలేదు; ఇది ఎక్కువగా `typecheck:noimplicit:core`ను తన పరిధిలోకి తీసుకుంటుంది                                                                          | అవును                      |
| `check:codeql-ratchet`       | తెరిచి ఉన్న CodeQL అలర్ట్ల సంఖ్య రిగ్రెస్ కాలేదు (`gh api` ద్వారా చదువుతుంది; token లేకపోతే సురక్షితంగా దాటవేస్తుంది) — రిఫ్రెష్ వ్యవధి మరియు మాన్యువల్ ట్రిగ్గర్ కోసం: దిగువనున్న "CodeQL ratchet" చూడండి | అవును                      |

### జాబ్: `quality-extended`

మొత్తం జాబ్ సలహాత్మకమైనది (`continue-on-error: true`). npm-ఆధారిత రాచెట్లు
నిజంగానే అమలవుతాయి; బాహ్య స్కానర్లు `gh release download` ద్వారా ఇన్స్టాల్ అవుతాయి మరియు
బైనరీ ఇంకా అందుబాటులో లేకపోతే స్వయంగా దాటవేస్తాయి (exit 0).

| స్క్రిప్ట్               | ధృవీకరించేది                                                                                                                                                                                          | నిరోధకం       |
| ------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------- |
...
| `check:openapi-breaking` | బేస్ బ్రాంచ్తో పోలిస్తే పబ్లిక్ API ఒప్పందంలో (`openapi.yaml`) బ్రేకింగ్ మార్పులు (oasdiff) — `openapiBreaking=N`ను ఉద్గారిస్తుంది; oasdiff లేకపోతే లేదా బేస్ స్పెక్ను పరిష్కరించలేకపోతే దాటవేస్తుంది | **సలహాత్మకం** |

### జాబ్: `docs-sync-strict`

`main`కు వచ్చే ప్రతి PRపై అమలవుతుంది. విఫలమైతే విలీనాన్ని నిరోధిస్తుంది.
...
| ↳ `check:docs-counts`          | గద్యంలో ఉన్న గణనలు (ప్రొవైడర్ల సంఖ్య, మైగ్రేషన్ల సంఖ్య మొదలైనవి) వాస్తవ గణనల రాచెట్ విండోలో ఉంటాయి                                                                | అవును                          |
| ↳ `check:env-doc-sync`         | `.env.example`లోని ప్రతి env var డాక్స్ పట్టికలో డాక్యుమెంట్ చేయబడి ఉంటుంది, అలాగే డాక్స్ పట్టికలోనివి కూడా అందులో ఉంటాయి                                         | అవును                          |
| ↳ `check:deprecated-versions`  | డాక్స్లో వాడుకలో లేని సంస్కరణ స్ట్రింగ్లు లేవు                                                                                                                    | అవును                          |
| ↳ `check:doc-links`            | డాక్స్లోని అంతర్గత markdown లింకులు వాస్తవ ఫైళ్లకు పరిష్కరించబడతాయి (`[text]`/`(path)` రూపం)                                                                      | అవును                          |
| ↳ `check:fabricated-docs`      | డాక్స్లో పేర్కొన్న రూట్లు, env vars, CLI కమాండ్లు, హుక్ పేర్లు మరియు ఫైల్ పాత్లు కోడ్బేస్లో ఉన్నాయి. `--strict` ద్వారా హార్డ్ గేట్; ఫ్లాగ్ లేకుండా సాఫ్ట్-ఫెయిల్. | అవును (CIలో `--strict` ద్వారా) |
| `check:cli-i18n`               | CLI కమాండ్ స్ట్రింగ్లు అన్ని i18n లొకేల్ ఫైళ్లలో ఉన్నాయి                                                                                                          | అవును                          |
| `check:openapi-coverage`       | OpenAPI స్పెక్ వాస్తవ రూట్లకు సంబంధించిన కనీసం రాచెట్ చేయబడిన దిగువ పరిమితిని కవర్ చేస్తుంది                                                                      | అవును                          |
| `check:openapi-security-tiers` | `openapi.yaml`లోని సెక్యూరిటీ టైర్ అనోటేషన్లు `routeGuard.ts` వర్గీకరణలకు అనుగుణంగా ఉంటాయి                                                                        | **సలహాత్మకం**                  |
| `check:openapi-routes`         | `openapi.yaml`లోని ప్రతి పాత్ వాస్తవ `route.ts`కు పరిష్కరించబడుతుంది (కల్పిత సమాచార నిరోధం)                                                                       | అవును                          |
| `check:docs-symbols`           | `docs/**/*.md`లోని ప్రతి `/api/...` సూచన వాస్తవ `route.ts`కు పరిష్కరించబడుతుంది (కల్పిత సమాచార నిరోధం)                                                            | అవును                          |
| `i18n translation drift`       | i18n లొకేల్ ఫైళ్లలో అనువదించని కీలు — హెచ్చరిక మాత్రమే                                                                                                            | **సలహాత్మకం**                  |
...
| `check-ui-value-drift` (ఇన్లైన్)   | తిరిగి వ్రాయబడిన ఆంగ్ల **విలువ** పాత అనువాదమేదీ మిగలకుండా చేస్తుంది                                                                                                                | అవును           |
| `check-new-key-coverage` (ఇన్లైన్) | ఒక **కొత్త** ఆంగ్ల కీ ప్రతి localeకు చేరుతుంది                                                                                                                                     | అవును           |
| `check-translation-ratio`          | ప్రతి localeకు వాస్తవ-అనువాద నిష్పత్తి (allowlist వెలుపల ఆంగ్లంతో సమానమైనవి / placeholder / తప్పిపోయిన leaves) `config/quality/i18n-translation-baseline.json` + slackను మించకూడదు | **సలహాపరమైనది** |

`fetch-depth: 0` అవసరం — value-drift gate, merge baseతో `en.json` తేడాలను పోలుస్తుంది.

#### `check-ui-value-drift` — పాత-అనువాద నిరోధకం

ఇతర gates నిర్మాణాత్మకంగా గుర్తించలేని ఒక i18n regressionను ఇది పట్టుకుంటుంది: ఒక ఆంగ్ల విలువను
తిరిగి వ్రాసినా, _మునుపటి_ ఆంగ్లం నుంచి రూపొందించిన అనువాదాలు అలాగే మిగిలిపోతాయి; ఫలితంగా
ఆంగ్లేతర వినియోగదారులు నమ్మకంగా వ్రాయబడిన, కానీ ఇప్పుడు తప్పైన పాఠ్యాన్నే చదువుతుంటారు.

ఇది నిజంగానే విడుదలైంది. Antigravity login helper ప్రవేశపెట్టినప్పుడు (#5203)
`oauthModal.googleOAuthWarning` తిరిగి వ్రాయబడింది; **43 localeలలో 39** localeలు ఆపరేటర్లకు
"పూర్తి URLను కాపీ చేసి క్రింద అతికించండి" అని చెప్పే పాఠ్యాన్ని అలాగే ఉంచాయి — ఆ providerకు
పూర్తి చేయలేని flow అది. ఈ కారణాల వల్ల #8463 వరకు ఇది గుర్తించబడలేదు:

- `sync-ui-keys` **లేని** keysను మాత్రమే backfill చేస్తుంది, **పాతబడిన** వాటిని ఎన్నడూ చేయదు;
- `check-ui-keys-coverage` కీ _ఉనికిని_ లెక్కిస్తుంది, కాబట్టి పాత అనువాదం coveredగా పరిగణించబడుతుంది;
- `check-translation-drift` అనేది `docs/i18n/<locale>/**.md` documentation mirrorsను ట్రాక్ చేస్తుంది —
  అది `src/i18n/messages/*.json`ను ఎప్పుడూ చదవదు. 2026-09 re-sync నుంచి job `docs-sync-strict`లో
  నిరోధకంగా ఉంది: core docను సవరించండి → `npm run i18n:run -- --files=<doc>` (section-level, తక్కువ ఖర్చుతో).

**Diff-aware, baseline-backed కాదు.** ఇది merge base వద్ద ఉన్న `en.json`ను working treeతో
పోలుస్తుంది; ఆంగ్ల విలువ మారిన ప్రతి కీకి, మార్పు చేయని అనువాదాన్నే ఇంకా కలిగి ఉన్న ఏ locale అయినా
పాతబడినదిగా పరిగణించబడుతుంది. ఇది ఉద్దేశపూర్వకంగా **ముందే ఉన్న debtను స్థిరపరుస్తుంది** — చాలా కాలంగా
ఉన్న అనువాదం ఏ పాత ఆంగ్లం నుంచి వచ్చిందో diff వెల్లడించలేదు, కాబట్టి gate ప్రస్తుత మార్పు తాకిన
వాటినే అంచనా వేస్తుంది. ప్రత్యామ్నాయం (ప్రతి కీకి hash baseline) కోసం ~600 KB generated file అవసరం;
అది ప్రస్తుతం ఉన్న అతి పెద్ద baseline కంటే 3× పెద్దది, అంతేకాక ప్రతి i18n PRలో మార్పులకు గురవుతుంది.

దీన్ని సంతృప్తిపరచడానికి రెండు మార్గాలు ఉన్నాయి:

1. ప్రభావిత అనువాదాలను నవీకరించడం, లేదా
2. వాటిని `__MISSING__:<new english>`గా సెట్ చేయడం — అప్పుడు runtime సరిచేసిన ఆంగ్లాన్ని అందిస్తుంది
   (`src/i18n/request.ts::deepMergeFallback`, #7258), అలాగే ఆ కీ అనువాదం కోసం queueలో చేరుతుంది.

string యొక్క **అర్థం** మారితే, **కీ పేరు మార్చడం** ఉత్తమం: కొత్త కీ పాత అనువాదాన్ని
వారసత్వంగా పొందలేదు. #8463లో ఉపయోగించిన pattern ఇదే.

```bash
npm run i18n:check-value-drift          # కఠినం (CI అమలు చేసేది)
npm run i18n:check-value-drift:warn     # నివేదిక మాత్రమే
BASE_REF=origin/release/vX.Y.Z npm run i18n:check-value-drift
```

base catalogను చదవలేనప్పుడు (base ref లేని shallow clone), `check-openapi-breaking`ను
అనుసరిస్తూ `SKIP reason=base-unresolved`తో 0 స్థితిలో నిష్క్రమిస్తుంది.

### Job: `i18n`

పూర్తి i18n ధ్రువీకరణ matrix (ప్రతి localeకు ఒక job). మొత్తం job సలహాపరమైనది.

| స్క్రిప్ట్                      | ధృవీకరించేది                   | నిరోధకమా                                                 |
| ------------------------------- | ------------------------------ | -------------------------------------------------------- |
| `validate_translation.py quick` | ప్రతి localeకు అనువాద సంపూర్ణత | **సలహాపరమైనది** (మొత్తం jobపై `continue-on-error: true`) |

### Job: `pr-test-policy`

pull requestsపై మాత్రమే అమలవుతుంది.
...
| `check:test-masking`   | మార్చిన test files నికర assert countను తగ్గించవు లేదా `assert.ok(true)` tautologiesను జోడించవు                                                             | అవును    |
| `check:pr-evidence`    | PR body మార్పుకు సంబంధించిన test/VPS ఆధారాలను పేర్కొంటుంది (PR proseను grep చేయడం ద్వారా Hard Rule #18ను యాంత్రీకరిస్తుంది — సున్నితమైనది, Backlog చూడండి) | అవును    |

### Job: `test-vitest`

`build` తర్వాత అమలవుతుంది. విఫలమైతే mergeను నిరోధిస్తుంది.

| సూట్             | ధృవీకరించేవి                                                | నిరోధక స్థితి                                                                                                               |
| ---------------- | ----------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------- |
| `test:vitest`    | MCP సర్వర్ (110 సాధనాలు), autoCombo, క్యాష్ — vitest రన్నర్ | అవును                                                                                                                       |
...
### రాత్రివేళ వర్క్ఫ్లోలు (షెడ్యూల్ చేయబడినవి, సూచనాత్మకమైనవి)

ఇవి cron షెడ్యూల్లో (మరియు `workflow_dispatch` ద్వారా) అమలవుతాయి, PRలపై ఎప్పుడూ అమలవవు. అన్నీ సూచనాత్మకమైనవే.

| వర్క్ఫ్లో              | ధృవీకరించేవి                                                                                                                                                      | నిరోధక స్థితి  |
| ---------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------- |
| `nightly-property`     | యాదృచ్ఛిక సీడ్ + అధిక రన్ కౌంట్తో fast-check ప్రాపర్టీ పరీక్షలు                                                                                                   | **సూచనాత్మకం** |
| `nightly-resilience`   | హీప్-వృద్ధి గేట్, కయాస్ ఫాల్ట్-ఇంజెక్షన్, k6 లోడ్/సోక్ పరీక్షలు                                                                                                   | **సూచనాత్మకం** |
| `nightly-llm-security` | promptfoo ఇంజెక్షన్ గార్డ్ (బ్లాక్ మోడ్) + garak ప్రోబ్లు (ప్రొవైడర్ సీక్రెట్ లేకుంటే దాటవేయబడతాయి)                                                               | **సూచనాత్మకం** |
| `nightly-schemathesis` | `docs/openapi.yaml`ను ఉపయోగించి లైవ్ OmniRouteపై OpenAPI కాంట్రాక్ట్ ఫజ్జింగ్ (schemathesis) — స్పెక్ ఉల్లంఘనలు / నిర్వహించని 500లను వెలికితీస్తుంది (Fase 8 B.4) | **సూచనాత్మకం** |
| `nightly-mutation`     | వేగవంతమైన యూనిట్ లేన్పై Stryker మ్యూటేషన్-టెస్టింగ్ స్కోర్ — మనుగడలో ఉన్న మ్యూటెంట్లు బలహీన అసెర్ట్లను వెలికితీస్తాయి                                             | **సూచనాత్మకం** |
| `nightly-compat`       | మద్దతు ఉన్న `engines.node` పరిధులన్నింటిలో Node ఇంజిన్ అనుకూలత మ్యాట్రిక్స్                                                                                       | **సూచనాత్మకం** |
...
- సరఫరా గొలుసు (provenance, SBOM, Trivy, Scorecard): [`docs/security/SUPPLY_CHAIN.md`](../security/SUPPLY_CHAIN.md)
...
ప్రతి `src/i18n/messages/<locale>.json` యొక్క లీఫ్ కీ సెట్ను `en.json`తో పోల్చి,
కీ ఎప్పుడు జోడించబడిందనే దానితో సంబంధం లేకుండా, ఏదైనా లీఫ్ లేకపోయినా లేదా అదనంగా ఉన్నా విఫలమవుతుంది.
`__MISSING__:` ప్లేస్హోల్డర్లు ఉన్నట్లుగానే పరిగణించబడతాయి (వాటి కంటెంట్ను పరిశీలించడం
నిష్పత్తి గేట్ పని). ఇది రెండు diff-ఆధారిత/శాతం గేట్లకు సంపూర్ణ పూరకం:
`check-ui-keys-coverage` ప్రతి localeకు 80 % కనిష్ఠ పరిమితిని అమలు చేస్తుంది
(~13,000లో 43 కీలు లేకపోయినా అది 99.7 %గానే కనిపిస్తుంది), అలాగే `check-new-key-coverage`
ఒక PR `en.json`కు జోడించే కీలను మాత్రమే అంచనా వేస్తుంది. ఒక locale బ్యాచ్కు దాని బ్రాంచ్
సృష్టించిన రోజు ఉన్న `en.json` నుంచి రూపం ఇవ్వబడుతుంది; base కొత్త కీలను జోడిస్తూనే ఉండగా,
అది అనేక రోజులపాటు అనువదించబడుతుంది. బ్యాచ్ PR స్వయంగా ఏ కీని జోడించదు, కాబట్టి బ్యాచ్ 1
(#13044) తొమ్మిది localeలలో 43 కీలు తక్కువగా, బ్యాచ్ 2 (#13660) ఎనిమిది localeలలో 10 కీలు
తక్కువగా విలీనం అయినప్పుడు కూడా (2026-09-15) సంబంధిత రెండు గేట్లు నిశ్శబ్దంగానే ఉన్నాయి.
విఫలమైన తనిఖీని `node scripts/i18n/sync-ui-keys.mjs --locale=<codes> --translate-markers`తో
పరిష్కరించండి; `extra` లీఫ్ అంటే మూలం దాన్ని తొలగించిందని అర్థం — దాన్ని locale నుంచి
తొలగించండి. `--warn` విఫలం చేయకుండా నివేదిస్తుంది. `--catalog=cli` అదే పోలికను
`bin/cli/locales`పై అమలు చేస్తుంది (`npm run i18n:check-keys:cli`); రెండు దశలూ
`i18n-ui-coverage` జాబ్లో ఉంటాయి.

#### `check-new-key-coverage` — కొత్త-కీ i18n గేట్

`check-ui-value-drift`కు అనుబంధ గేట్. అనువాదాలను నవీకరించకుండా వదిలేసినప్పుడు, **తిరిగి వ్రాయబడిన**
ఆంగ్ల విలువను అది గుర్తిస్తుంది; కొన్ని localeలకు ఎప్పటికీ అందని విధంగా **జోడించబడిన**
ఆంగ్ల కీని ఇది గుర్తిస్తుంది.

`check-ui-keys-coverage` ఈ వర్గాన్ని గుర్తించలేదు: ఇది ప్రతి localeకు శాతం కనిష్ఠ పరిమితిని
అమలు చేస్తుంది, మరియు ~13,000 లీఫ్లలో పదకొండు కీలు లేకపోయినా coverage 99.9%గానే ఉంటుంది.
భాషకు ఒక శాతం చొప్పున ఇవ్వడం ద్వారా "ఈ ఫీచర్ అనువాదం లేకుండానే విడుదలైంది" అని
వ్యక్తపరచలేం — ఒక పూర్తి ఫీచర్ ఏ వచనమూ లేకుండా కొత్త localeలో చేరినా, ఆ సంఖ్యలో ఎలాంటి
మార్పూ రాకపోవచ్చు.

ఇది నమోదు చేసిన సంఘటన: Orchestration Canvas యొక్క Phase 3, ఆ సమయంలో ఉన్న 42 localeలన్నింటిలో
దాని పదకొండు కీలను అనువదించింది. కొన్ని గంటల తర్వాత EU-భాషా బ్యాచ్ (#13044) repoను 51 localeలకు
తీసుకెళ్లింది; తొమ్మిది కొత్త localeలు (`el`, `et`, `ga`, `hr`, `lt`, `lv`, `mt`, `sl`, `sr`)
వాటిని ఎప్పటికీ పొందలేదు. లేని కీ స్థానంలో `deepMergeFallback` ఆంగ్లాన్ని ఉంచుతుంది, కాబట్టి
విఫలత ఖాళీ UIగా కాకుండా అనువదించని UIగా కనిపించింది — ఇది వాస్తవమైనదే కాక, నిర్మాణరీత్యా
నిశ్శబ్దమైనది కూడా.

దాని అనుబంధ గేట్లాగే ఇదీ **diff-aware**: merge base వద్ద ఉన్న ఆంగ్లాన్ని working treeతో
పోలుస్తుంది. అందువల్ల ముందే ఉన్న లోపాలు అలాగే స్థిరంగా ఉంటాయి, అలాగే గేట్ను ప్రారంభించడానికి
migration అవసరం కాలేదు. ప్రత్యామ్నాయ మార్గం: `__MISSING__:<english>` runtimeను సరిగ్గా ఉంచుతూనే
అనువాదాన్ని వాయిదా వేస్తుంది. `vi`లో ప్లేస్హోల్డర్లు నిషేధించబడ్డాయి
(`tests/unit/i18n-vi-completeness.test.ts`), కాబట్టి దానికి నిజమైన అనువాదం అవసరం.

#### `check-vitest-exclusions` — నిలిపి ఉంచిన పరీక్షల గేట్

`vitest.config.ts`లోని `exclude` జాబితాలో ఉన్న ఫైల్ అమలు కాని పరీక్ష; treeను చదివేవారికి మాత్రం
అది coverageలో భాగమైనట్లుగా కనిపిస్తుంది. `// #8618 — ముందే ఉన్న వైఫల్యం; పరిష్కరించినప్పుడు ఈ మినహాయింపును తొలగించండి`
అనే వ్యాఖ్య వెనుక అరవై రెండు ఫైళ్లు పేరుకుపోయాయి. Issue #8618 2026-08-11న మూసివేయబడింది,
అయితే అది ట్రాక్ చేసిన జాబితా 45 ఎంట్రీల నుంచి 62కు పెరిగింది; ప్రతి కొత్త ఎంట్రీ పనిచేయని
issueను సూచించే వ్యాఖ్యను వారసత్వంగా పొందింది. చివరకు జాబితాలోని ప్రతి ఫైల్ను విడివిడిగా
కొలిచినప్పుడు (#13204), **62లో 51 ఫైళ్లు ఎలాంటి source మార్పు లేకుండానే ప్రస్తుత treeపై
విజయవంతమయ్యాయి**.

నిజమైన ఫైల్గా resolve అయ్యే ప్రతి exclusion (a) ఒక tracking issueను పేర్కొనాలని, అలాగే
(b) దాని కొలిచిన statusతో `config/quality/vitest-exclusions.json`లో కనిపించాలని గేట్ కోరుతుంది.
దీంతో ఒక exclusionను జోడించడం అనేది 60-ఎంట్రీల arrayలో మరో పంక్తి జోడించడం కాకుండా, ప్రత్యేక
ఫైల్లో సమీక్షించగల diff అవుతుంది. ఇది ఉద్దేశపూర్వకంగా మినహాయించిన పరీక్షలను మళ్లీ అమలు చేయదు —
దానికి ~10 నిమిషాలు పడతాయి, కాబట్టి అది periodic jobలో ఉండాలి; ప్రతి పరీక్షను చివరిసారిగా
ఎప్పుడు కొలిచారో inventory నమోదు చేస్తుంది.
