...
**תחום:** חיבור/חשבון/מפתח יחיד של ספק.
...
- `rateLimitedUntil` — חותמת זמן שעד אליה תקופת הצינון בתוקף
...
- 429 במפתח API: ניתנת עדיפות לכותרות `Retry-After`/איפוס במעלה הזרם/טקסט איפוס שניתן לניתוח
- השהיה: `baseCooldownMs * 2 ** failureIndex`

**הגנה מפני עדר מתנפל:** מונעת מכשלים מקביליים להאריך יתר על המידה את תקופת הצינון או להגדיל פעמיים את `backoffLevel`.

**מצבים סופיים (אינם תקופות צינון):**

- `banned` — מוגדר בעקבות זיהוי מילת מפתח של חסימה / חסימת חשבון (ראו [BAN_DETECTION](../security/BAN_DETECTION.md))
- `expired` (עובר למצב סופי לאחר מספר מוגבל של ניסיונות חוזרים — `EXPIRED_RETRY_MAX = 3` עם השהיה מעריכית — כך ששגיאות OAuth זמניות יכולות להיפתר מעצמן לפני שהחשבון מושבת לצמיתות)
- `credits_exhausted`

מצבים אלה נשמרים עד לשינוי פרטי ההזדהות או עד שמפעיל מאפס אותם. אין לדרוס מצבים סופיים באמצעות מצב צינון זמני.

**התאוששות עצלה:** כאשר `rateLimitedUntil` חלף, החיבור נעשה כשיר שוב. לאחר שימוש מוצלח, `clearAccountError()` מנקה את כל שדות השגיאה.

### זיקה להפעלה (#7274)

**תחום:** הפעלת לקוח אחת (כותרת `X-Session-Id` / `x-codex-session-id` / `x-omniroute-session`) המוצמדת לחיבור אחד, עבור **כל** ספק.

**מטרה:** לשמור סוכן רב-תורי (Claude Code, aider, סוכנים מותאמים אישית) באותו חשבון לאורך בקשות, וכך לצמצם אובדן הקשר בין חשבונות ושגיאות 429 חוזרות של התחלה קרה אצל ספקים בעלי מצב הפעלה לפי חשבון.

**מימוש:**

- קביעת TTL:‏ `src/sse/services/sessionAffinityPin.ts::resolveSessionAffinityTtlMs()`
- בחירת/יצירת הצמדה: `src/sse/services/sessionAffinityPin.ts::selectSessionAffinityConnection()`
...
- הגדרה: `sessionAffinityTtlMs` (TTL גלובלי באלפיות שנייה, `0` משבית) — `src/lib/db/settings.ts`. השם שונה מ-`codexSessionAffinityTtlMs`, שהיה ייעודי ל-Codex בלבד, באמצעות המיגרציה `124_generic_session_affinity_ttl.sql`, שמעבירה כל TTL של Codex שהוגדר בעבר כברירת המחדל החדשה.

לפני #7274, הפונקציה `resolveSessionAffinityTtlMs()` החזירה מיד `0` עבור כל ספק מלבד `codex`, ולכן הגדרת ה-TTL (וכותרות ההפעלה) לא השפיעו בשום מקום אחר, אף שמנגנון ההצמדה וחילוץ הכותרות כבר היו בלתי תלויים בספק. התיקון הסיר את ההחזרה המוקדמת הזאת; כעת ה-TTL חל באופן אחיד על כל ספק לאחר שהוא מוגדר גלובלית לערך הגדול מ-`0`.

שלוש כותרות הזיקה להפעלה לעולם אינן מועברות במעלה הזרם — רכיבי הביצוע בונים בעצמם את הכותרות למעלה הזרם מאפס, במקום להעביר את כותרות הלקוח, ולכן הן נשארות מזהה מתאם פנימי בלבד.

### חכירות בלעדיות של חיבורים להפעלות מנוהלות

**תחום:** לקוח/הפעלת HTTP מנוהלים ופעילים יחידים מחזיקים בבעלות על חיבור OmniRoute כשיר אחד.

**מטרה:** לספק בעלות בלעדית ומתמידה על חיבור עבור לקוחות הזקוקים לגדר ניתוב קשיחה
בין בקשות. הדבר שונה מזיקה להפעלה, שהיא העדפה רכה להמשכיות:
חכירה בלעדית שומרת את מצב מחזור החיים ב-SQLite, אוכפת ייחודיות גלובלית של בעלים פעילים ושל
חיבורים פעילים, ודוחה דור מיושן לפני השיגור לספק.

התכונה מופעלת במפורש לכל מפתח API. מפתח מנוהל חייב לכלול את התחום `lease:exclusive` וכן
רשימת `allowedConnections` מפורשת שאינה ריקה. כל לקוח HTTP יכול להשתמש בנקודת הקצה של מחזור החיים; אין
צורך בשם לקוח, בסוכן משתמש, בספק, בשיטת OAuth או במודל. החכירה מחזיקה בחיבור,
לא במודל, ולכן שינוי מודל משמר את הקישור כל עוד החיבור נשאר כשיר
באופן רגיל. כללי המודל, המכסה, התקינות, תקופת הצינון ורשימת ההיתרים הרגילים נותרים בעלי הסמכות, ועשויים
להעביר את אותו דור לחיבור פנוי וכשיר אחר.
...
`X-OmniRoute-Lease-Generation` המדויק. מזהה הבעלים משתמש בקידומת `vlo_` שלאחריה 43 תווי base64url; רק
...
רכיבי הביצוע במעלה הזרם.

אם לניתוב רגיל יש מועמדים מנוהלים כשירים, אך כל מועמד פנוי תפוס בידי
חכירה פעילה זרה, OmniRoute מחזיר HTTP `429`, קוד של אי-זמינות קיבולת חכירה,
מצב של המתנה לקיבולת, ו-`Retry-After` תחום שנגזר ממועד התפוגה הרלוונטי המוקדם ביותר.
היעדר כשירות רגיל אינו תחרות על חכירה, ומשמר את הסמנטיקה הקיימת של שגיאות הניתוב.

מנגנונים קשורים נשארים נפרדים:

- תפוסת הפעלת OAuth היא חלוקה רכה ומקומית לתהליך עבור חשבונות OAuth.
- סמפורים של חשבונות מעניקים היתרי מקביליות לבקשות ומסתיימים עם השלמת הבקשה.
- חכירות בלעדיות של חיבורים להפעלות מנוהלות הן בעלות מתמידה לאורך מחזור חיים, עם גדר דור.

---

## 3. נעילת מודל

**תחולה:** שילוב של ספק + חיבור + מודל.
...
- כשלי הרשאה למצבים/מודלים ספציפיים לספק (למשל, מצבי Grok)
...
- `GET /api/resilience/model-cooldowns` — הצגת הנעילות הפעילות
- `DELETE /api/resilience/model-cooldowns` — הפעלה מחדש באופן ידני. גוף: `{provider, connection, model}`. אימות: ניהול.

### ממשק הגדרות נעילה + התאוששות באמצעות דעיכה בעקבות הצלחה (v3.8.23)

נעילת מודל השתנתה מהתנהגות קשיחה ופעילה תמיד לתכונה ניתנת להגדרה מלאה,
הדורשת הפעלה מפורשת וכוללת מסלול התאוששות שמתקן את עצמו.

**כרטיס הגדרות:** הגדרות → נעילת מודל
(`src/app/(dashboard)/dashboard/settings/components/ModelLockoutCard.tsx`).
כרטיס זה **נבדל** מהכרטיס לקריאה בלבד `ModelCooldownsCard` שלעיל (שרק
...
| ----------------------- | -------------------------------- | ------------------------------------------------------ |
| `enabled`               | `false`                          | מתג ראשי — נעילת מודל **כבויה כברירת מחדל**.           |
...
| `maxCooldownMs`         | `1_800_000` (30 דקות)            | הגבול העליון לתקופת הצינון המוגדלת.                    |
| `maxBackoffSteps`       | `10`                             | המספר המרבי של שלבי הגדלה בנסיגה מעריכית.              |
| `useExponentialBackoff` | `true`                           | האם כשלים חוזרים מגדילים את תקופת הצינון באופן מעריכי. |

ההגדרות נשמרות דרך מאגר ההגדרות הרגיל ועוברות אימות באמצעות סכמת
הגדרות העמידות; הכרטיס מגביל את `baseCooldownMs`/`maxCooldownMs`
(כאשר `maxCooldownMs ≥ baseCooldownMs`) ואת `maxBackoffSteps`.

**התאוששות באמצעות דעיכה בעקבות הצלחה:** ההתאוששות **אינה** מבוססת רק על פקיעת קוצב זמן. תגובה
תקינה מפחיתה בהדרגה את מונה הכשלים של המודל, כך שמודל שהתאושש
באמצע החלון מפסיק להסלים (והנעילה שלו מתנקה) לפני שקוצב הזמן שלו היה פוקע. לאחר הצלחה
ביעד משולב, `open-sse/services/combo.ts` קורא ל-`decayModelFailureCount()`
(`open-sse/services/accountFallback.ts`), אשר **מחלק לשניים** את
...
מגדילה את המונה (ומסלימה את תקופת הצינון) בעקבות כשלים בתוך
חלון ההסלמה. דעיכה זו בעקבות הצלחה מתווספת לפקיעה רגילה של קוצב הזמן —
כל אחד מהמסלולים יכול להפעיל מחדש מודל.

**מצב:** הנעילות נשמרות **בזיכרון** (`Map`-ים לכל תהליך של
`ModelLockoutEntry` עם מפתחות בתבנית `provider:connectionId:model`), ואינן נשמרות
...
- כל המפתחות של ספק מסוים דולגו → בדקו גם את מצב מפסק הזרם וגם את `rateLimitedUntil`/`testStatus` של כל חיבור.
- ספק הוחרג לצמיתות לאחר חלון האיפוס → הקוד קורא את `state` הגולמי במקום את `getStatus()`/`canExecute()`.
- מפתח אחד נכשל, אך האחרים אמורים לעבוד → העדיפו תקופת צינון לחיבור על פני מפסק זרם.
- רק מודל אחד נכשל → העדיפו נעילת מודל על פני תקופת צינון לחיבור.
- המצב אמור להתאושש מעצמו אך אינו עושה זאת → בדקו אם קיימת חותמת זמן עתידית ונתיב קריאה שמרענן מצב שפג תוקפו. סטטוסים קבועים דורשים שינויים ידניים.
