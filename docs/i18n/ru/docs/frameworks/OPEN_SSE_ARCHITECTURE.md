...
**Основной обработчик запросов**. Несмотря на размер, он имеет чёткую структуру:
...
Несмотря на то что это одна гигантская функция, она организована в **разделы с комментариями**, соответствующие пяти этапам конвейера.
...
export async function handleComboChat(body, comboId): Promise<ChatResult> {
  ...
  for (const target of targets) {
    try {
      ...
      log.warn("цель недоступна, попытка перехода к следующей", {
        target,
        err,
      });
    }
...
| Стратегия           | Поведение                                                                           |
| ------------------- | ----------------------------------------------------------------------------------- |
| `priority`          | Упорядоченный список с первой приоритетной целью                                    |
| `weighted`          | Вероятностный выбор на основе веса каждой цели                                      |
...
| `least-used`        | Выбор цели с наименьшим числом недавних обращений                                   |
| `cost-optimized`    | Сначала выбирается самая дешёвая работоспособная цель                               |
| `reset-aware`       | Учёт периодов сброса ограничений провайдера                                         |
| `reset-window`      | Маршрутизация на основе периодов сброса                                             |
| `headroom`          | Сначала выбирается цель с наибольшим остатком квоты                                 |
| `strict-random`     | Действительно равномерный выбор (без взвешивания по качеству)                       |
| `auto`              | Использование оценки по 16 факторам (`autoCombo/`)                                  |
| `lkgp`              | Сначала выбирается последний известный работоспособный провайдер                    |
...
**Абстрактный исполнитель**, который наследуют все 101 исполнитель. Он содержит:

- `buildUrl()` — стандартное формирование URL-адреса (подклассы переопределяют для нестандартных случаев)
- `buildHeaders()` — стандартные заголовки (аутентификация, тип содержимого)
- `transformRequest()` — по умолчанию передача без изменений
- `execute()` — основной цикл HTTP-запросов с повторными попытками, увеличивающейся задержкой и прерывателем

```ts
// open-sse/executors/default.ts
export class DefaultExecutor extends BaseExecutor {
  // Обрабатывает всех провайдеров, совместимых с OpenAI/Anthropic
  // Провайдеры регистрируют конфигурации (URL, аутентификация, заголовки), но используют общую логику исполнителя
}
```

Специфичное для провайдера поведение (заголовки аутентификации, базовый URL-адрес, заголовки версий) настраивается через реестр провайдеров, а не через отдельные классы исполнителей.
...
Сервисы — это **специализированные модули с единственной задачей**, которые объединяются обработчиками. Основные категории:

### Маршрутизация и комбинации

- `combo.ts` — точка входа для запросов с комбинированной маршрутизацией
...
- `rateLimitManager.ts` — корзина токенов для каждой комбинации ключа и провайдера
...
- `sessionManager.ts` — состояние многоходового сеанса
...
- `contextHandoff.ts` — сериализация сеанса
...
- (рассмотрены в [SKILLS.md](./SKILLS.md))

### Память

- (рассмотрена в [MEMORY.md](./MEMORY.md))

---

## Исполнители (75+ файлов)

По одному файлу на каждого провайдера. Все они расширяют `BaseExecutor` и переопределяют различающиеся части.
...
Преобразования **пользовательского тела запроса** (например, отделение `system` от `messages` для Anthropic) регистрируются отдельно для каждого провайдера в `open-sse/translator/`.
...
Разрешение выполняется через `ExecutorRegistry` (`executors/registry.ts`): каждый специализированный исполнитель объявляется во встроенной таблице `executors/index.ts` и регистрируется посредством `registerExecutor(alias, instance)` при загрузке модуля; `getExecutor()` обращается к реестру и при отсутствии специализированной записи для провайдера использует в качестве резервного варианта мемоизированный `DefaultExecutor`. Полное сопоставление псевдонимов и исполнителей описано эталонным тестом `tests/unit/executor-map-golden.test.ts`.
