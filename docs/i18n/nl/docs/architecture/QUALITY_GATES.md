...
## Inventaris van kwaliteitspoorten (~90 scripts)

Scripts bevinden zich onder `scripts/check/` (beleidspoorten) en `scripts/quality/` (ratchet-engine).
De gezaghebbende CI-bron is `.github/workflows/ci.yml`.

### Snel pad voor release-PR's (`quality.yml`)

`.github/workflows/quality.yml` wordt uitgevoerd voor PR's die zijn gericht op `release/**`. Hiermee kunnen wijzigingen
op branches van bijdragers doorgaan dankzij snelle, op paden gefilterde poorten, plus één adviserend productiebuildsignaal voor
codewijzigingen:

| Job                                              | Bereik                                                                                                                                                                                                                             | Blokkerend                                                                                   |
| ------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------- |
| `Build (advisory)`                               | Niet-conceptcode-PR's en Mergify-wachtrijbranches; Node 24, `npm-ci-retry`, `check:node-runtime`, `npm run build` met `OMNIROUTE_USE_TURBOPACK=1`; geen upload van artefacten omdat geen downstream-kwaliteitsjob deze gebruikt    | **Adviserend** (`continue-on-error: true`; verwijderen na één week stabiele release-PR-runs) |
| `Docs Gates (fast-path)`                         | Documentatie-/code-PR's; API-documentatiereferenties en docs-all                                                                                                                                                                   | Ja                                                                                           |
| `Fast Quality Gates`                             | Code-PR's; statische controles, typecontrole, dashboardtypecontrole, beïnvloede unittests                                                                                                                                          | Ja                                                                                           |
| `Forgotten sibling tests`                        | Code-PR's; gewijzigde modules herleid tot statische gebruikers en kandidaat-siblingtests; paden via barrels en dynamische imports worden gerapporteerd als adviserende diagnostiek, met verwijzingen naar allowlist-uitzonderingen | **Adviserend**                                                                               |
| `Vitest (fast-path)`                             | Code-PR's; snelle Vitest-suite                                                                                                                                                                                                     | Ja                                                                                           |
| `Unit Tests fast-path`                           | Code-PR's; unit-testsuite met 4 shards                                                                                                                                                                                             | Ja                                                                                           |
| `No new ESLint warnings`                         | Code-PR's; onderdrukkingsbewaking voor lintwaarschuwingen                                                                                                                                                                          | Ja voor PR's uit de eigen repository, adviserend voor forks                                  |
| `Merge integrity (changelog + generated skills)` | Niet-concept-PR's; synchronisatie van changelog en gegenereerde skills                                                                                                                                                             | Ja voor PR's uit de eigen repository, adviserend voor forks                                  |
...
`gewijzigde module/symbool -> statische gebruiker -> kandidaat-siblingtest` wanneer de kandidaat-
test niet voorkomt in de diff van de pull request. De Markdown-samenvatting en het JSON-resultaat worden bewaard als
het workflowartefact `forgotten-sibling-tests`, ter kalibratie vóór een eventuele blokkerende uitrol.

Barrel-re-exports en dynamische imports dienen uitsluitend als resolutiediagnostiek; ze leiden nooit tot een
blokkerende bevinding. Beoordeelde uitzonderingen bevinden zich in
`config/quality/forgotten-sibling-allowlist.json`. Elke vermelding moet de gebruiker en kandidaat-
test benoemen, een specifieke motivering geven en naar een GitHub-issue of pull request linken. Ongeldige vermeldingen leiden
standaard tot een fout. Uitzonderingen kunnen een verwijderde kandidaattest of een diff die `.skip`/`.todo`
toevoegt niet onderdrukken; verzwakking van assertions en andere maskering blijven onder de verantwoordelijkheid van de onafhankelijk blokkerende
poort `check:test-masking`.

### Job: `lint`

Wordt uitgevoerd op elke PR naar `main`. Blokkeert samenvoeging bij een fout.
...
| `check:model-lifecycle`           | De drie handmatig onderhouden routeringstabellen blijven consistent met de ingecheckte levenscyclussnapshot (#11503): `FITNESS_TABLE` (`taskFitness.ts`) kent geen score toe aan een uitgefaseerde id die `REGISTRY` kan routeren; elk doel van `BUILT_IN_ALIASES` is aanwezig in `REGISTRY` en afwezig in de snapshot met uitgefaseerde id's; elke uitgefaseerde id die nog in `REGISTRY` staat, wordt doorgestuurd of vermeld in `allowedRetiredInCatalog`; en geen enkele bron of bestemming in `DEFAULT_DEGRADATION_MAP` wordt in die snapshot als uitgefaseerd aangemerkt. Dit bewijst niet dat een model momenteel door een actieve upstream wordt aangeboden. Offline — vergelijkt met `config/quality/model-lifecycle.json`, handmatig vernieuwd met `npm run quality:refresh-model-lifecycle` (netwerk; niet opgenomen in CI). `allowedRetiredInCatalog` is een afbouwratel: voeg alleen een vermelding toe met een tracking-issue. | Ja                                         |
| `check:fetch-targets`             | Elke `fetch("/api/...")` in `src/` aan de clientzijde verwijst naar een bestaande `route.ts`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | Ja                                         |
| `check:deps`                      | Alle via `npm install` installeerbare afhankelijkheden in elke `package.json` in de repository staan in `dependency-allowlist.json`; nieuwe niet-vastgezette pakketten of pakketten met een naam die mogelijk het doelwit is van slopsquatting worden gemarkeerd                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | Ja                                         |
...
| `check:tracked-artifacts`         | Geen buildartefacten / vastgelegde `node_modules`-symbolische koppelingen (wordt ook uitgevoerd in de husky pre-commit; pre-push is bewust lichtgewicht — #6716)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | Ja                                         |
...
| `check:file-size`                 | Geen enkel bronbestand overschrijdt de limiet per extensie (ratchet: bevroren grote bestanden in de lijst `frozen`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | Ja                                         |
...
| `check:provider-asset-provenance` | Providerlogo's/-assets hebben een geregistreerde herkomstvermelding                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `lint:json`                       | JSON-configuratiebestanden kunnen worden geparseerd en voldoen aan de lintregels van de repository                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `typecheck:core`                  | TypeScript-compilatie zonder fouten (alleen adviserende waarschuwingen)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | Ja                                         |
| `typecheck:noimplicit:core`       | Strikte `noImplicitAny` — toekomstgericht; veel bestaande aanroepen moeten nog van annotaties worden voorzien                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | **Adviserend** (`continue-on-error: true`) |
| `check:dashboard-typecheck`       | `tsc` beperkt tot `src/app/(dashboard)/**` (#7033) — de zorgvuldig samengestelde toelatingslijst van 27 bestanden van `typecheck:core` bevat geen dashboard-TSX en `next build` voert hiervoor ook nooit typecontroles uit (`next.config.mjs` stelt `ignoreBuildErrors: true` in), waardoor regressies met verweesde identifiers daar (#6625/#6909) onzichtbaar waren voor CI. Vergelijkt met een bevroren baseline voor het aantal fouten per bestand/per TS-code (`config/quality/dashboard-typecheck-baseline.json`, hetzelfde patroon voor controle op veroudering als `check:known-symbols`) — alleen NIEUWE fouten boven het aantal in de baseline laten de controle mislukken; verlaag de baseline met `--update` wanneer een bestaande fout is opgelost.                                                                                                                                                                             | Ja                                         |
...
| `check:compression-budget`   | Budget voor compressiebenchmark — de minimale tokenbesparing per engine mag niet verslechteren                                                                                                                          | Ja                              |
...
| `check:codeql-ratchet`       | Het aantal openstaande CodeQL-meldingen verslechtert niet (wordt gelezen via `gh api`; wordt zonder token netjes overgeslagen) — zie 'CodeQL-ratchet' hieronder voor de vernieuwingsfrequentie en handmatige activering | Ja                              |

### Taak: `quality-extended`

De volledige job is adviserend (`continue-on-error: true`). De op npm gebaseerde ratelcontroles worden
...
| `check:openapi-breaking` | Incompatibele wijzigingen in het openbare API-contract (`openapi.yaml`) ten opzichte van de basisbranch (oasdiff) — geeft `openapiBreaking=N` als uitvoer; wordt overgeslagen als oasdiff ontbreekt of de basisspecificatie niet kan worden bepaald | **Adviserend** |

### Job: `docs-sync-strict`

Wordt uitgevoerd voor elke PR naar `main`. Blokkeert samenvoegen bij een fout.
...
| ↳ `check:docs-counts`          | Aantallen in lopende tekst (aantal providers, aantal migraties enz.) vallen binnen het ratelvenster van de werkelijke aantallen                                                             | Ja                        |
...
| `check:cli-i18n`               | CLI-opdrachttekenreeksen zijn aanwezig in alle i18n-lokaliteitsbestanden                                                                                                                    | Ja                        |
| `check:openapi-coverage`       | De OpenAPI-specificatie dekt ten minste een via een ratelmechanisme vastgestelde ondergrens van de werkelijke routes                                                                        | Ja                        |
...
| `i18n translation drift`       | Onvertaalde sleutels in i18n-lokaliteitsbestanden — alleen waarschuwen                                                                                                                      | **Adviserend**            |
...
| `check-new-key-coverage` (inline) | Een **nieuwe** Engelse sleutel bereikt elke locale                                                                                                                                                                 | Ja             |
| `check-translation-ratio`         | De verhouding echte vertalingen per locale (identiek aan Engels / tijdelijke aanduiding / ontbrekende bladeren buiten de allowlist) mag `config/quality/i18n-translation-baseline.json` + marge niet overschrijden | **Adviserend** |

Vereist `fetch-depth: 0` — de value-drift-gate vergelijkt `en.json` met de merge-base.

#### `check-ui-value-drift` — gate voor verouderde vertalingen

Detecteert de ene i18n-regressie die de andere gates structureel niet kunnen zien: een Engelse waarde
wordt herschreven en de vertalingen die van het _vorige_ Engels zijn afgeleid blijven staan, waardoor
...
loginhelper werd toegevoegd (#5203); **39 van de 43 locales** behielden tekst die beheerders opdroeg om "de
volledige URL te kopiëren en hieronder te plakken" — een proces dat voor die provider niet kan worden voltooid. Het bleef
...
- `check-translation-drift` de documentatiespiegels in `docs/i18n/<locale>/**.md` bijhoudt —
  het leest nooit `src/i18n/messages/*.json`. Blokkerend in job `docs-sync-strict` sinds de
  hersynchronisatie van 2026-09: bewerk een kerndocument → `npm run i18n:run -- --files=<doc>` (op sectieniveau, goedkoop).

**Diff-bewust, niet gebaseerd op een baseline.** Het vergelijkt `en.json` op de merge-base met de
werkboom; voor elke sleutel waarvan de Engelse waarde is gewijzigd, is elke locale die nog een
onaangeroerde vertaling bevat verouderd. Dit **bevriest bewust bestaande achterstand** — een diff
kan niet onthullen van welke oude Engelse tekst een reeds lang bestaande vertaling afkomstig is, dus beoordeelt de gate
alleen wat door de huidige wijziging wordt geraakt. Het alternatief (een hashbaseline per sleutel) zou
een gegenereerd bestand van circa 600 KB vereisen, 3× zo groot als de grootste bestaande baseline, dat bij elke i18n-PR wijzigt.

Er zijn twee manieren om eraan te voldoen:

1. werk de betreffende vertalingen bij, of
2. stel ze in op `__MISSING__:<new english>` — tijdens runtime wordt dan het gecorrigeerde Engels
   aangeboden (`src/i18n/request.ts::deepMergeFallback`, #7258) en wordt de sleutel in de vertaalwachtrij geplaatst.
...
Wordt afgesloten met code 0 en `SKIP reason=base-unresolved` wanneer de basiscatalogus niet kan worden gelezen (ondiepe
clone zonder de base-ref), overeenkomstig `check-openapi-breaking`.
...
| Script                          | Valideert                             | Blokkerend                                                  |
| ------------------------------- | ------------------------------------- | ----------------------------------------------------------- |
| `validate_translation.py quick` | Volledigheid van vertaling per locale | **Adviserend** (`continue-on-error: true` voor de hele job) |
...
| `check:pr-evidence`    | De PR-beschrijving vermeldt test-/VPS-bewijs voor de wijziging (automatiseert Harde regel #18 door PR-tekst te doorzoeken — fragiel, zie Backlog) | Ja         |
...
| `test:vitest:ui` | UI-componenttests — vitest-runner                        | **Blokkerend** — bestaande fouten worden expliciet uitgesloten in `vitest.config.ts`; nieuwe fouten doen de job mislukken |
...
| ---------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------- |
| `nightly-property`     | fast-check-eigenschapstests met een willekeurige seed en een hoog aantal uitvoeringen                                                                                        | **Adviserend** |
| `nightly-resilience`   | heap-groeicontrole, chaos-foutinjectie, k6-belastings-/duurtest                                                                                                              | **Adviserend** |
...
| `nightly-schemathesis` | OpenAPI-contractfuzzing (schemathesis) tegen een live OmniRoute met `docs/openapi.yaml` — brengt specificatieschendingen / onverwerkte 500-fouten aan het licht (Fase 8 B.4) | **Adviserend** |
| `nightly-mutation`     | Stryker-mutatie-testscore voor de snelle unit-lane — overlevende mutanten brengen zwakke assertions aan het licht                                                            | **Adviserend** |
...
- Supply chain (herkomst, SBOM, Trivy, Scorecard): [`docs/security/SUPPLY_CHAIN.md`](../security/SUPPLY_CHAIN.md)

#### `check-key-completeness` — pariteitscontrole voor sleutelsets

`scripts/i18n/check-key-completeness.mjs` (`npm run i18n:check-keys`, job `i18n-ui-coverage`).
Vergelijkt de set bladsleutels van elk bestand `src/i18n/messages/<locale>.json` met `en.json` en mislukt
bij elke ontbrekende of extra bladsleutel, ongeacht wanneer de sleutel is toegevoegd. Tijdelijke aanduidingen
met `__MISSING__:` tellen als aanwezig (hun inhoud valt onder de ratio-controle). Dit is de absolute aanvulling
op de twee op verschillen/percentages gebaseerde controles: `check-ui-keys-coverage` handhaaft per
locale een ondergrens van 80% (43 ontbrekende sleutels op ~13.000 leveren nog steeds 99,7% op) en
`check-new-key-coverage` beoordeelt alleen de sleutels die een PR aan `en.json` toevoegt. Een locale-batch
wordt gegenereerd op basis van de `en.json` van de dag waarop de branch wordt aangemaakt en wordt dagenlang
vertaald terwijl er aan de basis nieuwe sleutels blijven worden toegevoegd; de batch-PR voegt zelf geen
sleutel toe, waardoor beide verwante controles stil bleven toen batch 1 (#13044) werd gemerged met 43
ontbrekende sleutels in negen locales en batch 2 (#13660) met 10 ontbrekende sleutels in acht locales
(2026-09-15). Los een rode status op met
`node scripts/i18n/sync-ui-keys.mjs --locale=<codes> --translate-markers`; een `extra` bladsleutel
betekent dat de bron deze heeft verwijderd — verwijder deze uit de locale. `--warn` rapporteert zonder
te mislukken. `--catalog=cli` voert dezelfde vergelijking uit op `bin/cli/locales`
(`npm run i18n:check-keys:cli`); beide stappen bevinden zich in de job `i18n-ui-coverage`.

#### `check-new-key-coverage` — i18n-controle voor nieuwe sleutels

Verwante controle van `check-ui-value-drift`. Die detecteert een Engelse waarde die is **herschreven**
terwijl de vertalingen ervan niet zijn bijgewerkt; deze detecteert een Engelse sleutel die is **toegevoegd**
terwijl sommige locales deze nooit hebben ontvangen.

`check-ui-keys-coverage` kan dit soort probleem niet detecteren: deze controle handhaaft een procentuele
ondergrens per locale, en elf ontbrekende sleutels op ~13.000 laten de dekking op 99,9% staan. Een percentage
per taal kan niet uitdrukken dat „deze functie onvertaald is uitgebracht” — een volledige functie kan zonder
tekst in een nieuwe locale terechtkomen zonder dat het cijfer ooit verandert.

Het incident dat hiermee wordt vastgelegd: fase 3 van het Orchestration Canvas vertaalde zijn elf sleutels
naar de 42 locales die op dat moment bestonden. Uren later bracht de EU-talenbatch (#13044) de repository
op 51 locales, en de negen nieuwkomers (`el`, `et`, `ga`, `hr`, `lt`, `lv`, `mt`, `sl`, `sr`) hebben
deze nooit ontvangen. `deepMergeFallback` vervangt een ontbrekende sleutel door Engels, waardoor het probleem
zich uitte als een onvertaalde UI in plaats van een lege UI — reëel en inherent onopvallend.

Net als de verwante controle is deze **verschilbewust**: Engels op het merge-basispunt wordt vergeleken met
de werkboom, zodat bestaande hiaten bevroren blijven en er geen migratie nodig was om de controle in te
schakelen. Uitweg: `__MISSING__:<english>` stelt een vertaling uit terwijl de runtime correct blijft. `vi`
verbiedt tijdelijke aanduidingen (`tests/unit/i18n-vi-completeness.test.ts`) en vereist een echte vertaling.

#### `check-vitest-exclusions` — controle voor geparkeerde tests

Een bestand in de lijst `exclude` van `vitest.config.ts` is een test die niet wordt uitgevoerd, terwijl het
voor iemand die de boom bekijkt wel op dekking lijkt. Tweeënzestig bestanden verzamelden zich achter de
opmerking `// #8618 — reeds bestaande fout; verwijder deze uitsluiting zodra die is opgelost`. Issue #8618
werd op 2026-08-11 gesloten, terwijl de lijst die ermee werd bijgehouden groeide van 45 naar 62 vermeldingen,
waarbij elke nieuwe vermelding een opmerking erfde die naar een afgehandelde issue verwees. Toen de lijst
uiteindelijk bestand voor bestand werd gemeten (#13204), **slaagden 51 van de 62 tests tegen de huidige
boom zonder enige wijziging in de broncode**.

De controle vereist dat elke uitsluiting die naar een echt bestand verwijst (a) een tracking-issue noemt en
(b) voorkomt in `config/quality/vitest-exclusions.json` met de gemeten status ervan, zodat het toevoegen van
een uitsluiting een controleerbaar verschil in een afzonderlijk bestand vormt in plaats van nog één regel in
een array met 60 vermeldingen. De controle voert de uitgesloten tests bewust niet opnieuw uit — dat kost
~10 minuten en hoort thuis in een periodieke job; de inventaris registreert wanneer elke test voor het laatst
is gemeten.
