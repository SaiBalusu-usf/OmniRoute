...
Os scripts ficam em `scripts/check/` (verificações de políticas) e `scripts/quality/` (mecanismo de linha de base incremental).
A fonte oficial da CI é `.github/workflows/ci.yml`.

### Caminho rápido para PRs de release (`quality.yml`)

`.github/workflows/quality.yml` é executado em PRs direcionados a `release/**`. Ele mantém as branches dos colaboradores
em movimento com verificações rápidas filtradas por caminho, além de um sinal consultivo de build de produção para alterações
de código:
...
| `Forgotten sibling tests`                        | PRs de código; módulos alterados rastreados até consumidores estáticos e testes irmãos candidatos; caminhos de barris e de importação dinâmica são relatados como diagnósticos consultivos, com as exceções correspondentes da lista de permissões | **Consultivo**                                                                                              |
...
| `No new ESLint warnings`                         | PRs de código; proteção de lint que considera supressões                                                                                                                                                                                           | Sim para a origem própria, consultivo para forks                                                            |
| `Merge integrity (changelog + generated skills)` | PRs que não sejam rascunhos; sincronização do changelog e das skills geradas                                                                                                                                                                       | Sim para a origem própria, consultivo para forks                                                            |

#### Relatório de testes irmãos esquecidos

`npm run check:forgotten-sibling-tests` reutiliza o resolvedor de importações por trás do mapa de impacto dos testes.
...
candidato está ausente do diff da pull request. O resumo em Markdown e o resultado em JSON são mantidos como
o artefato de workflow `forgotten-sibling-tests` para calibração antes de qualquer adoção como bloqueio.

Reexportações de barris e importações dinâmicas são apenas diagnósticos de resolução; elas nunca criam uma
...
candidato, fornecer uma justificativa específica e incluir um link para uma issue ou pull request do GitHub. Entradas malformadas
resultam em falha por padrão. As exceções não podem suprimir um teste candidato excluído ou um diff que adicione `.skip`/`.todo`;
o enfraquecimento de asserções e outras formas de mascaramento continuam sob responsabilidade da verificação
`check:test-masking`, que bloqueia de forma independente.

### Job: `lint`

É executado em todos os PRs para `main`. Bloqueia o merge em caso de falha.
...
| `check:route-validation:t06`      | Schemas Zod presentes em todas as rotas (política do Tier 6)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | Sim                                         |
...
| `check:model-lifecycle`           | As três tabelas de roteamento mantidas manualmente permanecem consistentes com o snapshot de ciclo de vida versionado (#11503): `FITNESS_TABLE` (`taskFitness.ts`) não pontua nenhum id desativado que `REGISTRY` possa rotear; cada destino de `BUILT_IN_ALIASES` está presente em `REGISTRY` e ausente do snapshot de ids desativados; cada id desativado ainda presente em `REGISTRY` é encaminhado ou listado em `allowedRetiredInCatalog`; e nenhuma origem ou destino de `DEFAULT_DEGRADATION_MAP` aparece como desativado nesse snapshot. Isso não comprova que um modelo esteja sendo atendido atualmente por um upstream ativo. Offline — compara com `config/quality/model-lifecycle.json`, atualizado manualmente com `npm run quality:refresh-model-lifecycle` (rede; não integrado à CI). `allowedRetiredInCatalog` é um mecanismo progressivo de redução: adicione uma entrada somente com uma issue de acompanhamento. | Sim                                         |
| `check:fetch-targets`             | Cada `fetch("/api/...")` no `src/` do lado do cliente é resolvido para um `route.ts` real                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | Sim                                         |
| `check:deps`                      | Todas as dependências instaláveis com `npm install` em cada `package.json` do repositório estão em `dependency-allowlist.json`; pacotes novos sem versão fixada ou que possam ser resultado de slopsquatting são sinalizados                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | Sim                                         |
| `audit:deps`                      | `npm audit` (raiz + electron) — nenhum alerta de severidade alta/crítica (sobrepõe-se ao `check:vuln-ratchet` do osv; consulte o Backlog de Racionalização)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | Sim                                         |
| `check:lockfile`                  | Integridade do `package-lock.json` — registro https, hashes de integridade, sem substituições de host                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | Sim                                         |
...
| `check:vitest-exclusions`         | Cada exclusão do Vitest menciona uma issue de acompanhamento e aparece em `config/quality/vitest-exclusions.json` (#13204)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | Sim                                         |
| `check:file-size`                 | Nenhum arquivo-fonte excede o limite por extensão (ratchet: arquivos grandes congelados na lista `frozen`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | Sim                                         |
...
| `check:db-rules`                  | Nenhum SQL bruto fora dos módulos de `src/lib/db/`; nenhuma importação de barrel a partir de `localDb.ts` (Regras Rígidas nº 2/nº 5)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Sim                                         |
...
| `check:test-discovery`            | Todo arquivo `*.test.ts` / `*.spec.ts` no repositório é coletado por pelo menos um executor de testes (mecanismo de catraca: a lista de órfãos em `test-discovery-baseline.json` só pode diminuir)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | Sim                                         |
| `check:agent-skills-sync`         | Os artefatos gerados de habilidades de agentes correspondem ao catálogo de origem (sem divergências)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `check:provider-asset-provenance` | Os logotipos/recursos dos provedores possuem uma entrada de procedência registrada                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `lint:json`                       | Os arquivos de configuração JSON são analisados corretamente e atendem às regras de lint do repositório                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `typecheck:core`                  | Compilação do TypeScript sem erros (somente avisos informativos)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | Sim                                         |
| `typecheck:noimplicit:core`       | `noImplicitAny` estrito — voltado para o futuro; muitos locais de chamada preexistentes ainda precisam de anotações                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | **Informativo** (`continue-on-error: true`) |
| `check:dashboard-typecheck`       | `tsc` com escopo limitado a `src/app/(dashboard)/**` (#7033) — a lista de permissões selecionada de 27 arquivos do `typecheck:core` não inclui nenhum TSX do dashboard, e `next build` também nunca verifica seus tipos (`next.config.mjs` define `ignoreBuildErrors: true`), portanto, regressões de identificadores órfãos nessa área (#6625/#6909) não eram detectadas pela CI. Compara com uma linha de base congelada de contagem por arquivo/por código TS (`config/quality/dashboard-typecheck-baseline.json`, seguindo o mesmo padrão de imposição de obsolescência de `check:known-symbols`) — somente NOVOS erros além da contagem registrada na linha de base fazem a verificação falhar; reduza progressivamente com `--update` quando um erro preexistente for corrigido.                                                                                                                                                | Sim                                         |

### Tarefa: `quality-gate`

É executada após `test-coverage`. Bloqueia o merge em caso de falha.

| Script                       | Valida                                                                                                                                                                                                                            | Bloqueante                         |
| ---------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------- |
| `quality:collect`            | Gera `quality-metrics.json` (contagem de avisos do ESLint, cobertura proveniente do relatório consolidado dos shards)                                                                                                             | Sim (precede o ajuste progressivo) |
| `quality:ratchet`            | Nenhuma métrica em `quality-baseline.json` sofreu regressão (avisos do ESLint ≤ linha de base; cobertura ≥ linha de base)                                                                                                         | Sim                                |
| `check:duplication`          | A duplicação de código (jscpd@4) não excede a linha de base em `quality-baseline.json`                                                                                                                                            | Sim                                |
| `check:complexity`           | A complexidade ciclomática no nível do arquivo não excede o limite (`complexity` + `max-lines-per-function` do ESLint principal)                                                                                                  | Sim                                |
| `check:cognitive-complexity` | Ajuste progressivo da complexidade cognitiva (`eslint-plugin-sonarjs`) — execução separada do ESLint; a CI executa ambos combinados na única etapa `check:complexity-ratchets`                                                    | Sim                                |
| `check:dead-code`            | O ajuste progressivo de exportações/arquivos não utilizados (knip) não sofre regressão em relação à linha de base                                                                                                                 | Sim                                |
| `check:compression-budget`   | Orçamento do benchmark de compactação — os limites mínimos de economia de tokens por mecanismo não podem sofrer regressão                                                                                                         | Sim                                |
| `check:type-coverage`        | O ajuste progressivo da porcentagem tipada (`type-coverage`) não sofre regressão; substitui em grande parte `typecheck:noimplicit:core`                                                                                           | Sim                                |
| `check:codeql-ratchet`       | A contagem de alertas abertos do CodeQL não sofre regressão (lida por meio de `gh api`; ignorada normalmente sem token) — para a frequência de atualização e o acionamento manual, consulte "Ajuste progressivo do CodeQL" abaixo | Sim                                |

### Tarefa: `quality-extended`

Todo o job é consultivo (`continue-on-error: true`). Os mecanismos de controle progressivo baseados em npm são executados
de fato; os scanners externos são instalados via `gh release download` e ignoram a própria execução (exit 0)
...
| `check:vuln-ratchet`     | As vulnerabilidades das dependências (osv-scanner) não regridem — ignorada se o binário estiver ausente                                                                                                                    | **Consultivo** |
| `check:workflows`        | Lint de workflows (actionlint + zizmor) — ignorado se os binários estiverem ausentes                                                                                                                                       | **Consultivo** |
...
Executado em cada PR para `main`. Bloqueia o merge em caso de falha.

| Script                         | Valida                                                                                                                                                                                                      | Bloqueante                 |
| ------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------- |
| `check:docs-all`               | Meta-verificação que executa sequencialmente as 6 verificações secundárias abaixo                                                                                                                           | Sim                        |
| ↳ `check:docs-sync`            | Consistência das versões em CHANGELOG / OpenAPI / llm.txt                                                                                                                                                   | Sim                        |
| ↳ `check:docs-counts`          | As contagens no texto (número de provedores, número de migrações etc.) estão dentro da janela de controle progressivo das contagens reais                                                                   | Sim                        |
...
| `check:openapi-coverage`       | A especificação OpenAPI cobre pelo menos um patamar mínimo progressivo das rotas reais                                                                                                                      | Sim                        |
...
| `desvio de tradução de i18n`   | Chaves não traduzidas nos arquivos de localidade de i18n — apenas aviso                                                                                                                                     | **Consultivo**             |

### Job: `i18n-ui-coverage`

| Script                            | Valida                                                                                                                                                                                                       | Bloqueante     |
| --------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------- |
| `check-ui-keys-coverage` (inline) | A cobertura de chaves de i18n da UI é ≥ 65%                                                                                                                                                                  | Sim            |
| `check-ui-value-drift` (inline)   | Um **valor** em inglês reescrito não deixa nenhuma tradução obsoleta para trás                                                                                                                               | Sim            |
| `check-new-key-coverage` (inline) | Uma **nova** chave em inglês chega a todas as localidades                                                                                                                                                    | Sim            |
| `check-translation-ratio`         | A proporção de traduções reais por localidade (folhas idênticas ao inglês / de placeholder / ausentes fora da lista de permissões) não deve exceder `config/quality/i18n-translation-baseline.json` + margem | **Consultivo** |

Requer `fetch-depth: 0` — a verificação de divergência de valor compara `en.json` com a base de mesclagem.

#### `check-ui-value-drift` — verificação de traduções obsoletas

Detecta a única regressão de i18n que as outras verificações estruturalmente não conseguem identificar: um valor em inglês
...
Isso realmente chegou à produção. `oauthModal.googleOAuthWarning` foi reescrito quando o auxiliar
de login do Antigravity foi adicionado (#5203); **39 de 43 localidades** mantiveram o texto orientando os operadores a "copiar a
...
- `sync-ui-keys` apenas preenche chaves que estão **ausentes**, nunca as que estão **obsoletas**;
- `check-ui-keys-coverage` conta a _presença_ da chave, portanto uma tradução obsoleta é considerada coberta;
...
**Sensível ao diff, não baseado em uma linha de base.** Ele compara o `en.json` na base de mesclagem com a
árvore de trabalho; para cada chave cujo valor em inglês foi alterado, qualquer localidade que ainda mantenha uma
tradução não modificada está obsoleta. Isso deliberadamente **congela a dívida preexistente** — um diff
não pode revelar de qual versão antiga em inglês uma tradução existente há muito tempo se originou, portanto a verificação avalia
apenas o que a alteração atual afeta. A alternativa (uma linha de base de hashes por chave) custaria
um arquivo gerado de aproximadamente 600 KB, 3× maior que a maior linha de base existente, com alterações em cada PR de i18n.
...
2. defini-las como `__MISSING__:<new english>` — o runtime então fornece o inglês corrigido
   (`src/i18n/request.ts::deepMergeFallback`, #7258) e a chave entra na fila de tradução.
...
| `check:pr-evidence`    | O corpo do PR cita evidências de teste/VPS para a alteração (automatiza a Regra Rígida nº 18 procurando no texto do PR — frágil, consulte o Backlog) | Sim        |
...
| `test:vitest:ui` | Testes de componentes da UI — executor vitest                      | **Bloqueante** — falhas preexistentes são explicitamente excluídas em `vitest.config.ts`; novas falhas reprovam o job |

### Workflows noturnos (agendados, informativos)

São executados de acordo com um cronograma cron (e `workflow_dispatch`), nunca em PRs. Todos são informativos.

| Workflow               | Valida                                                                                                                                                                            | Bloqueante      |
| ---------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------- |
| `nightly-property`     | Testes de propriedades com fast-check usando uma semente aleatória e uma alta contagem de execuções                                                                               | **Informativo** |
| `nightly-resilience`   | Verificação de crescimento do heap, injeção de falhas de caos, testes de carga/soak com k6                                                                                        | **Informativo** |
| `nightly-llm-security` | Proteção contra injeção do promptfoo (modo de bloqueio) + sondas do garak (ignoradas sem um segredo de provedor)                                                                  | **Informativo** |
...
| `nightly-compat`       | Matriz de compatibilidade do mecanismo Node nos intervalos compatíveis de `engines.node`                                                                                          | **Informativo** |
...
#### `check-key-completeness` — gate de paridade do conjunto de chaves
...
em caso de qualquer chave folha ausente ou adicional, independentemente de quando a chave foi adicionada. Os placeholders `__MISSING__:`
contam como presentes (o conteúdo deles é responsabilidade do gate de proporção). Ele é o complemento absoluto
dos dois gates baseados em diff/porcentagem: `check-ui-keys-coverage` impõe um mínimo de 80 % por
localidade (43 chaves ausentes em cerca de 13.000 ainda resultam em 99,7 %), e `check-new-key-coverage` avalia
somente as chaves que um PR adiciona a `en.json`. Um lote de localidades é gerado com base no `en.json` do dia
...
nenhuma chave por si só, portanto ambos os gates relacionados permaneceram silenciosos quando o lote 1 (#13044) foi integrado com 43 chaves
...
#### `check-new-key-coverage` — gate de i18n para novas chaves

Gate relacionado a `check-ui-value-drift`. Esse detecta um valor em inglês que foi **reescrito**
enquanto suas traduções ficaram desatualizadas; este detecta uma chave em inglês que foi **adicionada**
sem jamais ser incluída em algumas localidades.

`check-ui-keys-coverage` não consegue detectar esse caso: ele impõe um percentual mínimo por localidade, e
onze chaves ausentes entre cerca de 13.000 mantêm a cobertura em 99,9%. Uma porcentagem por idioma não consegue
expressar "este recurso foi lançado sem tradução" — um recurso inteiro pode ser adicionado a uma nova localidade sem
nenhum texto e sem jamais alterar o número.

O incidente que ele codifica: a Fase 3 do Canvas de Orquestração traduziu suas onze chaves nas
42 localidades que existiam na época. Horas depois, o lote de idiomas da UE (#13044) elevou o repositório
...
uma interface não traduzida, em vez de uma interface em branco — real e silencioso por definição.

Assim como o gate relacionado, ele é **consciente de diff**, comparando o inglês na base do merge com a árvore
de trabalho, de modo que as lacunas preexistentes permanecem congeladas e o gate não exigiu migração para ser ativado. Mecanismo de escape:
`__MISSING__:<english>` adia uma tradução enquanto mantém o runtime correto. `vi` proíbe
placeholders (`tests/unit/i18n-vi-completeness.test.ts`) e exige uma tradução real.

#### `check-vitest-exclusions` — gate de testes suspensos

Um arquivo na lista `exclude` de `vitest.config.ts` é um teste que não é executado e parece
cobertura para quem examina a árvore. Sessenta e dois arquivos se acumularam sob o comentário
`// #8618 — falha preexistente; remova esta exclusão quando for corrigida`. A issue #8618 foi encerrada em
2026-08-11, enquanto a lista que ela rastreava cresceu de 45 para 62 entradas, cada nova entrada herdando um comentário
...
O gate exige que toda exclusão que aponte para um arquivo real (a) nomeie uma issue de acompanhamento e
(b) apareça em `config/quality/vitest-exclusions.json` com seu status medido, para que adicionar uma seja um
diff revisável em um arquivo dedicado, em vez de apenas mais uma linha em um array de 60 entradas. Ele deliberadamente
não executa novamente os testes excluídos — isso leva cerca de 10 minutos e deve fazer parte de um job periódico; o
