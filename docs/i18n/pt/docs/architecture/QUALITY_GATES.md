...
## Inventário de controlos (~90 scripts)

Os scripts encontram-se em `scripts/check/` (controlos de políticas) e `scripts/quality/` (motor de ratchet).
A fonte oficial de CI é `.github/workflows/ci.yml`.

### Via rápida para PRs de versões (`quality.yml`)

`.github/workflows/quality.yml` é executado em PRs destinados a `release/**`. Mantém os ramos dos
contribuidores em andamento através de controlos rápidos filtrados por caminho, além de um sinal consultivo de compilação para produção relativo a alterações de código:

| Tarefa                                           | Âmbito                                                                                                                                                                                                                                                    | Bloqueante                                                                                                  |
| ------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------- |
| `Build (advisory)`                               | PRs de código que não sejam rascunhos e ramos da fila do Mergify; Node 24, `npm-ci-retry`, `check:node-runtime`, `npm run build` com `OMNIROUTE_USE_TURBOPACK=1`; sem carregamento de artefactos, porque nenhuma tarefa de qualidade a jusante os utiliza | **Consultivo** (`continue-on-error: true`; remover após uma semana de execuções estáveis de PRs de versões) |
...
| `Forgotten sibling tests`                        | PRs de código; módulos alterados rastreados até consumidores estáticos e testes irmãos candidatos; caminhos de barris e de importações dinâmicas são comunicados como diagnósticos consultivos, com as exceções referenciadas da lista de permissões      | **Consultivo**                                                                                              |
| `Vitest (fast-path)`                             | PRs de código; conjunto rápido do vitest                                                                                                                                                                                                                  | Sim                                                                                                         |
| `Unit Tests fast-path`                           | PRs de código; conjunto de testes unitários com 4 fragmentos                                                                                                                                                                                              | Sim                                                                                                         |
| `No new ESLint warnings`                         | PRs de código; controlo de lint que considera supressões                                                                                                                                                                                                  | Sim para a própria origem, consultivo para forks                                                            |
...
`npm run check:forgotten-sibling-tests` reutiliza o resolvedor de importações subjacente ao mapa de impacto dos testes.
Para cada módulo de produção alterado, comunica cadeias determinísticas
`módulo/símbolo alterado -> consumidor estático -> teste irmão candidato` quando o teste
candidato não está presente no diff do pull request. O resumo em Markdown e o resultado JSON são mantidos como
o artefacto de workflow `forgotten-sibling-tests` para calibração antes de qualquer implementação bloqueante.

As reexportações através de barris e as importações dinâmicas são apenas diagnósticos de resolução; nunca criam uma
ocorrência bloqueante. As exceções revistas encontram-se em
`config/quality/forgotten-sibling-allowlist.json`. Cada entrada tem de indicar o consumidor e o teste
candidato, fornecer uma justificação específica e incluir uma ligação para um issue ou pull request do GitHub. As entradas
malformadas provocam uma falha por predefinição. As exceções não podem suprimir um teste candidato eliminado nem um diff que adicione `.skip`/`.todo`;
o enfraquecimento de asserções e outras formas de dissimulação continuam a ser tratados pelo controlo bloqueante independente
`check:test-masking`.

### Tarefa: `lint`

É executada em todos os PRs destinados a `main`. Bloqueia a integração em caso de falha.
...
| `check:route-validation:t06`      | Esquemas Zod presentes em todas as rotas (política de Nível 6)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Sim                                         |
...
| `check:model-lifecycle`           | As três tabelas de encaminhamento mantidas manualmente permanecem consistentes com o instantâneo do ciclo de vida incluído no repositório (#11503): `FITNESS_TABLE` (`taskFitness.ts`) não pontua nenhum id descontinuado que `REGISTRY` consiga encaminhar; cada destino de `BUILT_IN_ALIASES` está presente em `REGISTRY` e ausente do instantâneo de ids descontinuados; cada id descontinuado ainda presente em `REGISTRY` é reencaminhado ou incluído em `allowedRetiredInCatalog`; e nenhuma origem ou destino de `DEFAULT_DEGRADATION_MAP` aparece como descontinuado nesse instantâneo. Isto não prova que um modelo seja atualmente disponibilizado por um serviço a montante ativo. Offline — compara com `config/quality/model-lifecycle.json`, atualizado manualmente com `npm run quality:refresh-model-lifecycle` (rede; não integrado no CI). `allowedRetiredInCatalog` é um mecanismo de redução progressiva: adicione uma entrada apenas com uma ocorrência de acompanhamento. | Sim                                         |
| `check:fetch-targets`             | Cada `fetch("/api/...")` no `src/` do lado do cliente é resolvido para um `route.ts` real                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | Sim                                         |
| `check:deps`                      | Todas as dependências instaláveis com `npm install` em cada `package.json` do repositório constam de `dependency-allowlist.json`; pacotes novos sem versão fixa ou sujeitos a slopsquatting são assinalados                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | Sim                                         |
| `audit:deps`                      | `npm audit` (raiz + electron) — sem avisos de gravidade alta/crítica (sobrepõe-se a `check:vuln-ratchet` do osv; consulte a lista de pendências de racionalização)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | Sim                                         |
...
| `check:tracked-artifacts`         | Sem artefactos de compilação / ligações simbólicas de `node_modules` submetidas (também executado no pre-commit do husky; o pre-push é intencionalmente ligeiro — #6716)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | Sim                                         |
| `check:vitest-exclusions`         | Cada exclusão do Vitest indica uma ocorrência de acompanhamento e consta em `config/quality/vitest-exclusions.json` (#13204)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | Sim                                         |
| `check:file-size`                 | Nenhum ficheiro de código-fonte excede o limite por extensão (ratchet: ficheiros grandes fixados na lista `frozen`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | Sim                                         |
...
| `check:route-guard-membership`    | Todas as rotas que iniciam um processo-filho são classificadas por `isLocalOnlyPath()` (Regras Rígidas n.º 15/n.º 17)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | Sim                                         |
| `check:test-discovery`            | Todos os ficheiros `*.test.ts` / `*.spec.ts` no repositório são recolhidos por, pelo menos, um executor de testes (mecanismo de catraca: a lista de órfãos em `test-discovery-baseline.json` apenas pode diminuir)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | Sim                                         |
| `check:agent-skills-sync`         | Os artefactos gerados de competências dos agentes correspondem ao respetivo catálogo de origem (sem divergências)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `check:provider-asset-provenance` | Os logótipos/recursos dos fornecedores incluem uma entrada de proveniência registada                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `lint:json`                       | Os ficheiros de configuração JSON são analisados e cumprem as regras de lint do repositório                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| `check:dashboard-typecheck`       | `tsc` limitado a `src/app/(dashboard)/**` (#7033) — a lista restrita de 27 ficheiros permitidos de `typecheck:core` não inclui nenhum TSX do dashboard, e `next build` também nunca verifica os respetivos tipos (`next.config.mjs` define `ignoreBuildErrors: true`), pelo que as regressões de identificadores órfãos nessa área (#6625/#6909) eram invisíveis para a CI. Compara com uma referência congelada da contagem por ficheiro/por código TS (`config/quality/dashboard-typecheck-baseline.json`, seguindo o mesmo padrão de imposição contra obsolescência de `check:known-symbols`) — apenas erros NOVOS para além da contagem de referência fazem falhar o controlo; reduza progressivamente a referência com `--update` quando um erro preexistente for corrigido.                                                                                                                                                                                                               | Sim                                         |
...
| ---------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------- |
| `quality:collect`            | Gera `quality-metrics.json` (contagem de avisos do ESLint, cobertura proveniente do relatório combinado dos fragmentos)                                                                                         | Sim (a montante do ajuste progressivo) |
...
| `check:complexity`           | A complexidade ciclomática ao nível do ficheiro não excede o limite (regras `complexity` + `max-lines-per-function` do núcleo do ESLint)                                                                        | Sim                                    |
| `check:cognitive-complexity` | Ajuste progressivo da complexidade cognitiva (`eslint-plugin-sonarjs`) — execução separada do ESLint; a CI executa ambas em conjunto como o único passo `check:complexity-ratchets`                             | Sim                                    |
| `check:dead-code`            | O ajuste progressivo de exportações/ficheiros não utilizados (knip) não regride face à referência                                                                                                               | Sim                                    |
...
| `check:type-coverage`        | O ajuste progressivo da percentagem com tipos (`type-coverage`) não regride; substitui em grande medida `typecheck:noimplicit:core`                                                                             | Sim                                    |
| `check:codeql-ratchet`       | A contagem de alertas CodeQL abertos não regride (lê através de `gh api`; omissão controlada sem token) — para a cadência de atualização e o acionamento manual, consulte "Ajuste progressivo do CodeQL" abaixo | Sim                                    |

### Tarefa: `quality-extended`

Todo o job é consultivo (`continue-on-error: true`). Os controlos baseados em npm são executados
efetivamente; os scanners externos são instalados através de `gh release download` e omitem-se automaticamente (exit 0)
...
| `check:secrets`          | Deteção de segredos (gitleaks) — omitido se o binário estiver ausente                                                                                                                                                | **Consultivo** |
| `check:vuln-ratchet`     | As vulnerabilidades das dependências (osv-scanner) não regridem — omitido se o binário estiver ausente                                                                                                               | **Consultivo** |
| `check:workflows`        | Validação dos workflows (actionlint + zizmor) — omitido se os binários estiverem ausentes                                                                                                                            | **Consultivo** |
| `check:openapi-breaking` | Alterações incompatíveis no contrato da API pública (`openapi.yaml`) face ao branch base (oasdiff) — emite `openapiBreaking=N`; omitido se o oasdiff estiver ausente ou a especificação base não puder ser resolvida | **Consultivo** |
...
| Script                         | Valida                                                                                                                                                                                                                   | Bloqueante                        |
| ------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------- |
| `check:docs-all`               | Meta-validação que executa sequencialmente as 6 subvalidações abaixo                                                                                                                                                     | Sim                               |
| ↳ `check:docs-sync`            | Consistência das versões em CHANGELOG / OpenAPI / llm.txt                                                                                                                                                                | Sim                               |
| ↳ `check:docs-counts`          | As contagens no texto (número de fornecedores, número de migrações, etc.) estão dentro da janela de controlo das contagens reais                                                                                         | Sim                               |
...
| ↳ `check:doc-links`            | As ligações markdown internas na documentação correspondem a ficheiros reais (formato `[text]`/`(path)`)                                                                                                                 | Sim                               |
| ↳ `check:fabricated-docs`      | As rotas, variáveis de ambiente, comandos da CLI, nomes de hooks e caminhos de ficheiros mencionados na documentação existem na base de código. Validação rígida através de `--strict`; falha não bloqueante sem a flag. | Sim (através de `--strict` no CI) |
| `check:cli-i18n`               | As strings dos comandos da CLI estão presentes em todos os ficheiros de localização i18n                                                                                                                                 | Sim                               |
| `check:openapi-coverage`       | A especificação OpenAPI cobre, no mínimo, um limiar progressivo das rotas reais                                                                                                                                          | Sim                               |
...
| `i18n translation drift`       | Chaves não traduzidas nos ficheiros de localização i18n — apenas aviso                                                                                                                                                   | **Consultivo**                    |
...
| `check-new-key-coverage` (inline) | Uma **nova** chave em inglês chega a todas as localizações                                                                                                                                                       | Sim            |
| `check-translation-ratio`         | O rácio de traduções reais por localização (folhas idênticas ao inglês / marcadores de posição / em falta fora da lista de permissões) não pode exceder `config/quality/i18n-translation-baseline.json` + margem | **Consultivo** |
...
os utilizadores que não usam inglês continuam a ler texto formulado de forma convincente, mas agora incorreto.

Isto chegou efetivamente a produção. `oauthModal.googleOAuthWarning` foi reescrito quando o auxiliar de início
de sessão Antigravity foi introduzido (#5203); **39 de 43 localizações** mantiveram texto que instruía os operadores a «copiar o
URL completo e colá-lo abaixo» — um fluxo que não pode ser concluído para esse fornecedor. Passou
despercebido até ao #8463 porque:

- `sync-ui-keys` apenas preenche chaves que estão **ausentes**, nunca as que estão **desatualizadas**;
- `check-ui-keys-coverage` conta a _presença_ da chave, pelo que uma tradução desatualizada é contabilizada como coberta;
- `check-translation-drift` acompanha os espelhos da documentação em `docs/i18n/<locale>/**.md` —
  nunca lê `src/i18n/messages/*.json`. Bloqueante no job `docs-sync-strict` desde a
  resincronização de 2026-09: edite um documento principal → `npm run i18n:run -- --files=<doc>` (ao nível da secção, pouco dispendioso).

**Sensível às diferenças, sem depender de uma referência.** Compara `en.json` na base de integração com a
árvore de trabalho; para cada chave cujo valor em inglês tenha mudado, qualquer localização que ainda contenha uma
tradução inalterada está desatualizada. Isto **congela deliberadamente a dívida preexistente** — uma diferença
não consegue revelar de que versão antiga do inglês proveio uma tradução existente há muito tempo, pelo que a verificação avalia
apenas aquilo em que a alteração atual toca. A alternativa (uma referência de hashes por chave) exigiria
um ficheiro gerado de ~600 KB, 3× maior do que a maior referência existente, com alterações em cada PR de i18n.

Há duas formas de satisfazer esta verificação:

1. atualizar as traduções afetadas; ou
2. defini-las como `__MISSING__:<new english>` — o ambiente de execução apresenta então o inglês corrigido
...
npm run i18n:check-value-drift:warn     # apenas comunicar
BASE_REF=origin/release/vX.Y.Z npm run i18n:check-value-drift
```

Termina com o código 0 e `SKIP reason=base-unresolved` quando o catálogo de base não pode ser lido (clone
superficial sem a referência de base), tal como `check-openapi-breaking`.

### Job: `i18n`

Matriz completa de validação de i18n (um job por localização). O job completo é consultivo.

| Script                          | Valida                                 | Bloqueante                                               |
| ------------------------------- | -------------------------------------- | -------------------------------------------------------- |
| `validate_translation.py quick` | Completude da tradução por localização | **Consultivo** (`continue-on-error: true` em todo o job) |
...
| Script                 | Valida                                                                                                                                                              | Bloqueante |
| ---------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------- |
| `check:pr-test-policy` | Os PRs que alteram código de produção em `src/`, `open-sse/`, `electron/` ou `bin/` têm de incluir ou atualizar testes (Regra Rígida n.º 8)                         | Sim        |
...
| `check:pr-evidence`    | O corpo do PR menciona evidências de testes/VPS para a alteração (automatiza a Regra Rígida n.º 18 através da pesquisa do texto do PR — frágil, consulte o Backlog) | Sim        |

### Job: `test-vitest`

É executado após `build`. Bloqueia a integração em caso de falha.

| Conjunto         | Valida                                                           | Bloqueante                                                                                                                   |
| ---------------- | ---------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------- |
| `test:vitest`    | Servidor MCP (110 ferramentas), autoCombo, cache — runner vitest | Sim                                                                                                                          |
| `test:vitest:ui` | Testes de componentes da IU — runner vitest                      | **Bloqueante** — as falhas preexistentes são explicitamente excluídas em `vitest.config.ts`; novas falhas fazem o job falhar |

### Workflows noturnos (agendados, consultivos)

São executados segundo um agendamento cron (e `workflow_dispatch`), nunca em PRs. Todos são consultivos.

| Workflow               | Valida                                                                                                                                                                            | Bloqueante     |
| ---------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------- |
| `nightly-property`     | Testes baseados em propriedades com fast-check, com uma seed aleatória e um elevado número de execuções                                                                           | **Consultivo** |
| `nightly-resilience`   | Limite de crescimento do heap, injeção caótica de falhas, testes de carga/prolongados com k6                                                                                      | **Consultivo** |
| `nightly-llm-security` | Proteção contra injeção com promptfoo (modo de bloqueio) + sondas garak (ignoradas sem um segredo de fornecedor)                                                                  | **Consultivo** |
| `nightly-schemathesis` | Fuzzing de contratos OpenAPI (schemathesis) numa instância OmniRoute ativa utilizando `docs/openapi.yaml` — revela violações da especificação/erros 500 não tratados (Fase 8 B.4) | **Consultivo** |
| `nightly-mutation`     | Pontuação dos testes de mutação Stryker na via rápida de testes unitários — mutantes sobreviventes revelam asserções fracas                                                       | **Consultivo** |
| `nightly-compat`       | Matriz de compatibilidade do motor Node nos intervalos `engines.node` suportados                                                                                                  | **Consultivo** |
...
#### `check-key-completeness` — barreira de paridade do conjunto de chaves
...
perante qualquer chave terminal ausente ou extra, independentemente de quando a chave foi adicionada. Os marcadores de posição
`__MISSING__:` contam como presentes (o respetivo conteúdo é da responsabilidade da barreira de proporção). É o complemento absoluto
das duas barreiras baseadas em diferenças/percentagens: `check-ui-keys-coverage` impõe um mínimo de 80 % por
idioma (43 chaves ausentes em ~13 000 continuam a resultar em 99,7 %) e `check-new-key-coverage` avalia
...
qualquer chave, pelo que ambas as barreiras relacionadas permaneceram silenciosas quando o lote 1 (#13044) foi integrado com 43 chaves em falta em nove
idiomas e o lote 2 (#13660) com 10 chaves em falta em oito (2026-09-15). Corrija uma falha com
`node scripts/i18n/sync-ui-keys.mjs --locale=<codes> --translate-markers`; uma chave terminal `extra`
significa que a origem a removeu — elimine-a do idioma. `--warn` comunica o problema sem falhar.
`--catalog=cli` executa a mesma comparação em `bin/cli/locales` (`npm run i18n:check-keys:cli`);
ambos os passos pertencem à tarefa `i18n-ui-coverage`.

#### `check-new-key-coverage` — barreira de i18n para novas chaves

Barreira relacionada com `check-ui-value-drift`. Esta deteta um valor em inglês que foi **reescrito**
enquanto as respetivas traduções ficaram desatualizadas; aquela deteta uma chave em inglês que foi **adicionada**
sem que alguns idiomas a tenham recebido.

`check-ui-keys-coverage` não consegue detetar esta categoria: impõe um mínimo percentual por idioma, e
onze chaves ausentes em ~13 000 mantêm a cobertura nos 99,9 %. Uma percentagem por idioma não consegue
expressar «esta funcionalidade foi disponibilizada sem tradução» — uma funcionalidade inteira pode ser integrada num novo idioma sem
qualquer texto e sem que o valor se altere.

O incidente que esta barreira codifica: a Fase 3 do Orchestration Canvas traduziu as suas onze chaves para
...
as receberam. `deepMergeFallback` substitui uma chave ausente pelo inglês, pelo que o modo de falha foi
uma IU não traduzida, em vez de uma IU vazia — um problema real e, por conceção, silencioso.

Tal como a barreira relacionada, é **sensível às diferenças**, comparando o inglês na base de integração com a árvore de
trabalho, pelo que as lacunas preexistentes permanecem inalteradas e a ativação da barreira não exigiu qualquer migração. Solução de recurso:
`__MISSING__:<english>` adia uma tradução, mantendo simultaneamente o comportamento correto em tempo de execução. `vi` proíbe
marcadores de posição (`tests/unit/i18n-vi-completeness.test.ts`) e exige uma tradução real.

#### `check-vitest-exclusions` — barreira de testes suspensos

Um ficheiro na lista `exclude` de `vitest.config.ts` é um teste que não é executado, parecendo
cobertura para quem consulta a árvore. Sessenta e dois ficheiros acumularam-se sob o comentário
`// #8618 — pre-existing failure; remove this exclusion when fixed`. A ocorrência #8618 foi encerrada em
2026-08-11, enquanto a lista que acompanhava cresceu de 45 entradas para 62, herdando cada nova entrada um comentário
que apontava para uma ocorrência encerrada. Quando a lista foi finalmente avaliada ficheiro a ficheiro (#13204), **51 dos 62
passaram na árvore atual sem qualquer alteração ao código-fonte**.

A barreira exige que cada exclusão que corresponda a um ficheiro real (a) indique uma ocorrência de acompanhamento e
(b) conste de `config/quality/vitest-exclusions.json` com o respetivo estado medido, para que adicionar uma seja uma
diferença passível de revisão num ficheiro dedicado, em vez de apenas mais uma linha numa matriz de 60 entradas. Deliberadamente,
não volta a executar os testes excluídos — isso demora ~10 minutos e deve pertencer a uma tarefa periódica; o
