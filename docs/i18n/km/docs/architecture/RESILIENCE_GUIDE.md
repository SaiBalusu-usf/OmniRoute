...
**គោលបំណង:** រំលងសោដែលមានបញ្ហាមួយ ខណៈពេលដែលការតភ្ជាប់ផ្សេងទៀតសម្រាប់អ្នកផ្តល់សេវាដូចគ្នានៅតែបន្តបម្រើសំណើ។
...
- `backoffLevel` — កម្មវិធីរាប់ការពន្យារពេលតាមអិចស្ប៉ូណង់ស្យែល
...
- ការពន្យារពេល៖ `baseCooldownMs * 2 ** failureIndex`

**យន្តការការពារការផ្ទុកសំណើសម្រុកក្នុងពេលតែមួយ:** រារាំងការបរាជ័យដែលកើតឡើងដំណាលគ្នាមិនឱ្យពន្យាររយៈពេលរង់ចាំហួសកម្រិត ឬបង្កើន `backoffLevel` ពីរដង។

**ស្ថានភាពបញ្ចប់ (មិនមែនជារយៈពេលរង់ចាំទេ):**

- `banned` — កំណត់ដោយការរកឃើញពាក្យគន្លឹះដែលត្រូវបានហាមឃាត់ / ការហាមឃាត់គណនី (សូមមើល [BAN_DETECTION](../security/BAN_DETECTION.md))
- `expired` (ប្តូរទៅស្ថានភាពបញ្ចប់បន្ទាប់ពីការព្យាយាមឡើងវិញដែលមានដែនកំណត់ — `EXPIRED_RETRY_MAX = 3` ជាមួយនឹងការពន្យារពេលតាមអិចស្ប៉ូណង់ស្យែល — ដូច្នេះកំហុស OAuth បណ្តោះអាសន្នអាចស្ដារដោយខ្លួនឯង មុនពេលគណនីត្រូវបានបិទដំណើរការជាអចិន្ត្រៃយ៍)
- `credits_exhausted`

ស្ថានភាពទាំងនេះនៅតែបន្តរហូតដល់ព័ត៌មានសម្គាល់អត្តសញ្ញាណផ្លាស់ប្តូរ ឬប្រតិបត្តិករកំណត់វាឡើងវិញ។ កុំសរសេរជាន់លើស្ថានភាពបញ្ចប់ដោយស្ថានភាពរង់ចាំបណ្តោះអាសន្ន។

**ការស្ដារឡើងវិញពេលចាំបាច់:** នៅពេល `rateLimitedUntil` បានកន្លងផុត ការតភ្ជាប់នឹងអាចត្រូវបានជ្រើសរើសម្តងទៀត។ នៅពេលប្រើប្រាស់ដោយជោគជ័យ `clearAccountError()` នឹងសម្អាតវាលកំហុសទាំងអស់។

### ភាពជាប់ទាក់ទងនៃសម័យ (#7274)

**វិសាលភាព:** សម័យម៉ាស៊ីនភ្ញៀវមួយ (`X-Session-Id` / `x-codex-session-id` / `x-omniroute-session` header) ដែលត្រូវបានភ្ជាប់ថេរទៅនឹងការតភ្ជាប់មួយ សម្រាប់អ្នកផ្តល់សេវា**ណាមួយ**។

**គោលបំណង:** រក្សាភ្នាក់ងារសន្ទនាច្រើនជុំ (Claude Code, aider, ភ្នាក់ងារផ្ទាល់ខ្លួន) ឱ្យប្រើគណនីដូចគ្នានៅគ្រប់សំណើ ដើម្បីកាត់បន្ថយការបាត់បង់បរិបទរវាងគណនី និងកំហុស 429 ពីការចាប់ផ្ដើមត្រជាក់ម្តងហើយម្តងទៀត លើអ្នកផ្តល់សេវាដែលមានស្ថានភាពសម័យក្នុងគណនីនីមួយៗ។

**ការអនុវត្ត:**

- ការកំណត់ TTL៖ `src/sse/services/sessionAffinityPin.ts::resolveSessionAffinityTtlMs()`
- ការជ្រើសរើស/បង្កើតការភ្ជាប់ថេរ៖ `src/sse/services/sessionAffinityPin.ts::selectSessionAffinityConnection()`
- ការស្រង់យកបឋមកថា (ទូទៅ សម្រាប់អ្នកផ្តល់សេវាណាមួយ)៖ `src/sse/services/auth.ts::extractSessionAffinityKey()`
- តារាងការភ្ជាប់ថេរដែលត្រូវបានរក្សាទុក៖ `sessionAccountAffinity` (`src/lib/db/sessionAccountAffinity.ts`)
- ការកំណត់៖ `sessionAffinityTtlMs` (TTL សកលគិតជា ms, `0` បិទមុខងារ) — `src/lib/db/settings.ts`។ ត្រូវបានប្តូរឈ្មោះពី `codexSessionAffinityTtlMs` ដែលសម្រាប់តែ Codex ដោយការធ្វើចំណាកស្រុក `124_generic_session_affinity_ttl.sql` ដែលផ្ទេរ TTL របស់ Codex ដែលបានកំណត់ពីមុនណាមួយ ឱ្យក្លាយជាលំនាំដើមថ្មី។

មុន #7274 `resolveSessionAffinityTtlMs()` បានបញ្ចប់ភ្លាមៗដោយប្រគល់ `0` សម្រាប់អ្នកផ្តល់សេវាទាំងអស់ លើកលែងតែ `codex` ដូច្នេះការកំណត់ TTL (និងបឋមកថាសម័យ) មិនមានឥទ្ធិពលនៅកន្លែងផ្សេងទៀតទេ ទោះបីជាយន្តការភ្ជាប់ថេរ និងការស្រង់យកបឋមកថា មិនអាស្រ័យលើអ្នកផ្តល់សេវារួចហើយក៏ដោយ។ ការកែតម្រូវបានដកការប្រគល់តម្លៃមុនពេលកំណត់នោះចេញ។ ឥឡូវនេះ TTL អនុវត្តដូចគ្នាចំពោះអ្នកផ្តល់សេវាទាំងអស់ នៅពេលវាត្រូវបានកំណត់ជាសកលឱ្យលើសពី `0`។

បឋមកថាភាពជាប់ទាក់ទងនៃសម័យទាំងបី មិនត្រូវបានបញ្ជូនទៅប្រភពខាងលើឡើយ — កម្មវិធីប្រតិបត្តិបង្កើតបឋមកថាប្រភពខាងលើដោយខ្លួនឯងពីដំបូង ជំនួសឱ្យការបញ្ជូនបឋមកថារបស់ម៉ាស៊ីនភ្ញៀវបន្ត ដូច្នេះវានៅត្រឹមតែជាលេខសម្គាល់ទំនាក់ទំនងខាងក្នុងប៉ុណ្ណោះ។

### ការជួលការតភ្ជាប់សម័យដែលបានគ្រប់គ្រងផ្តាច់មុខ

**វិសាលភាព:** ម៉ាស៊ីនភ្ញៀវ/សម័យ HTTP ដែលបានគ្រប់គ្រង និងកំពុងសកម្មមួយ កាន់កាប់ការតភ្ជាប់ OmniRoute ដែលមានសិទ្ធិប្រើប្រាស់មួយ។

**គោលបំណង:** ផ្តល់ភាពជាម្ចាស់ការតភ្ជាប់ផ្តាច់មុខដែលមាននិរន្តរភាព សម្រាប់ម៉ាស៊ីនភ្ញៀវដែលត្រូវការរបាំងកំណត់ផ្លូវយ៉ាងតឹងរឹង
នៅទូទាំងសំណើ។ វាខុសពីភាពជាប់ទាក់ទងនៃសម័យ ដែលជាចំណូលចិត្តស្រាលសម្រាប់ភាពបន្ត៖
ការជួលផ្តាច់មុខរក្សាស្ថានភាពវដ្តជីវិតនៅក្នុង SQLite អនុវត្តភាពឯកតារបស់ម្ចាស់សកម្ម និង
ការតភ្ជាប់សកម្មជាសកល ហើយបដិសេធជំនាន់ហួសសម័យ មុនពេលបញ្ជូនទៅអ្នកផ្តល់សេវា។

មុខងារនេះត្រូវបានជ្រើសរើសបើកប្រើសម្រាប់ API key នីមួយៗ។ សោដែលបានគ្រប់គ្រងត្រូវតែមានវិសាលភាព `lease:exclusive` និង
បញ្ជី `allowedConnections` ជាក់លាក់ដែលមិនទទេ។ ម៉ាស៊ីនភ្ញៀវ HTTP ណាមួយអាចប្រើចំណុចចុងនៃវដ្តជីវិតបាន ដោយមិនទាមទារ
ឈ្មោះម៉ាស៊ីនភ្ញៀវ user-agent អ្នកផ្តល់សេវា វិធីសាស្ត្រ OAuth ឬម៉ូដែលឡើយ។ ការជួលកាន់កាប់ការតភ្ជាប់មួយ
មិនមែនម៉ូដែលទេ ដូច្នេះការផ្លាស់ប្តូរម៉ូដែលនៅតែរក្សាការចងភ្ជាប់ ខណៈពេលដែលការតភ្ជាប់នៅតែមានសិទ្ធិប្រើប្រាស់
តាមធម្មតា។ ច្បាប់ម៉ូដែល កូតា សុខភាព រយៈពេលរង់ចាំ និងបញ្ជីអនុញ្ញាតធម្មតា នៅតែមានអំណាចកំណត់ ហើយអាច
ផ្ទេរជំនាន់ដូចគ្នាទៅការតភ្ជាប់ឥតគិតថ្លៃផ្សេងទៀតដែលមានសិទ្ធិប្រើប្រាស់។

វដ្តជីវិតគឺ `POST /api/v1/session-leases` ជាមួយសកម្មភាព JSON `acquire`, `renew`, និង `release`។
សំណើ inference ដែលបានគ្រប់គ្រងបង្ហាញតម្លៃមិនច្បាស់ `X-OmniRoute-Lease-Owner` និង
`X-OmniRoute-Lease-Generation` ពិតប្រាកដ។ តម្លៃម្ចាស់ប្រើ `vlo_` បន្តដោយតួអក្សរ base64url ចំនួន 43។ មានតែ
ហាស SHA-256 របស់វាប៉ុណ្ណោះដែលត្រូវបានរក្សាទុក។ របាំងបញ្ជូនចុងក្រោយនីមួយៗក៏ចងភ្ជាប់ API key ID ដែលបានផ្ទៀងផ្ទាត់ និង
ID ការតភ្ជាប់សកម្មផងដែរ។ បឋមកថាគ្រប់គ្រងការជួលត្រូវបានដកចេញពីកំណត់ហេតុ សំណៅសំណើដែលបានរក្សាទុក និង
បឋមកថារបស់កម្មវិធីប្រតិបត្តិប្រភពខាងលើ។

ប្រសិនបើការកំណត់ផ្លូវធម្មតាមានបេក្ខភាពដែលបានគ្រប់គ្រង និងមានសិទ្ធិប្រើប្រាស់ ប៉ុន្តែបេក្ខភាពឥតគិតថ្លៃទាំងអស់ត្រូវបានកាន់កាប់ដោយ
ការជួលសកម្មរបស់ភាគីផ្សេង OmniRoute នឹងត្រឡប់ HTTP `429` កូដ lease-capacity-unavailable
ស្ថានភាពរង់ចាំសមត្ថភាព និង `Retry-After` ដែលមានដែនកំណត់ និងគណនាពីពេលផុតកំណត់ពាក់ព័ន្ធដែលឆាប់បំផុត។
ការគ្មានសិទ្ធិប្រើប្រាស់ធម្មតាមិនមែនជាការប្រជែងការជួលទេ ហើយរក្សាអត្ថន័យកំហុសកំណត់ផ្លូវដែលមានស្រាប់របស់វា។

យន្តការដែលពាក់ព័ន្ធនៅតែដាច់ដោយឡែកពីគ្នា៖

- ការកាន់កាប់សម័យ OAuth គឺជាការចែកចាយស្រាលក្នុងមូលដ្ឋានរបស់ដំណើរការ សម្រាប់គណនី OAuth។
- សេម៉ាហ្វ័រគណនីផ្តល់សិទ្ធិការអនុញ្ញាតសម្រាប់សំណើដំណាលគ្នា និងបញ្ចប់នៅពេលសំណើបញ្ចប់។
- ការជួលការតភ្ជាប់សម័យដែលបានគ្រប់គ្រងផ្តាច់មុខ គឺជាភាពជាម្ចាស់វដ្តជីវិតដែលមាននិរន្តរភាព ជាមួយរបាំងជំនាន់។

---

## 3. ការចាក់សោម៉ូដែល

**វិសាលភាព:** បន្សំនៃ provider + connection + model។

**គោលបំណង:** ជៀសវាងការបិទ connection ទាំងមូល នៅពេលដែលមានតែ model មួយប៉ុណ្ណោះដែលមិនអាចប្រើបាន ឬត្រូវបានកំណត់កូតា។

**ឧទាហរណ៍:**

- Provider ដែលមានកូតាតាម model ហើយត្រឡប់ 429
- Provider មូលដ្ឋានដែលត្រឡប់ 404 សម្រាប់ model មួយដែលបាត់
- ការបរាជ័យសិទ្ធិប្រើប្រាស់ mode/model ជាក់លាក់របស់ provider (ឧ., Grok modes)

**ការអនុវត្ត:** `open-sse/services/accountFallback.ts` — `lockModel()`, `clearModelLock()`, `getAllModelLockouts()`។

### ផ្ទាំងគ្រប់គ្រងរយៈពេលសម្រាករបស់ម៉ូដែល (v3.8.0)

UI: Settings → Model Cooldowns (`src/app/(dashboard)/dashboard/settings/components/ModelCooldownsCard.tsx`)

បង្ហាញបញ្ជីការចាក់សោដែលកំពុងសកម្ម រួមមាន៖ provider, connection, model, reason, expiresAt។ អ្នកប្រតិបត្តិការអាចបើកឱ្យ model មួយដំណើរការឡើងវិញដោយដៃពី card នេះ។

**REST API:**

- `GET /api/resilience/model-cooldowns` — បង្ហាញបញ្ជីការចាក់សោដែលកំពុងសកម្ម
- `DELETE /api/resilience/model-cooldowns` — បើកឱ្យដំណើរការឡើងវិញដោយដៃ។ Body: `{provider, connection, model}`។ Auth: management។

### UI កំណត់ការចាក់សោ + ការស្ដារឡើងវិញដោយបន្ថយតាមភាពជោគជ័យ (v3.8.23)

ការចាក់សោ model បានផ្លាស់ប្ដូរពីឥរិយាបថដែលសរសេរថេរជាមុន និងបើកជានិច្ច ទៅជាមុខងារដែលអាចកំណត់រចនាសម្ព័ន្ធបានពេញលេញ ត្រូវបើកប្រើដោយចេតនា និងមានមាគ៌ាស្ដារឡើងវិញដោយស្វ័យប្រវត្តិ។

**Card កំណត់:** Settings → Model Lockout
(`src/app/(dashboard)/dashboard/settings/components/ModelLockoutCard.tsx`)។
វា**ខុសគ្នា**ពី `ModelCooldownsCard` ដែលបានរៀបរាប់ខាងលើ និងអាចបានតែអាន (ដែលគ្រាន់តែ
_បង្ហាញបញ្ជី_ការចាក់សោដែលកំពុងសកម្ម) — card ថ្មីនេះ_កំណត់ប៉ារ៉ាម៉ែត្រ_។ តម្លៃលំនាំដើម
ស្ថិតនៅក្នុង `DEFAULT_MODEL_LOCKOUT_SETTINGS`
(`src/lib/resilience/modelLockoutSettings.ts`):

| ការកំណត់                | តម្លៃលំនាំដើម                    | អត្ថន័យ                                                           |
| ----------------------- | -------------------------------- | ----------------------------------------------------------------- |
| `enabled`               | `false`                          | កុងតាក់មេ — ការចាក់សោ model **ត្រូវបានបិទតាមលំនាំដើម**។           |
| `errorCodes`            | `[403, 404, 429, 502, 503, 504]` | ស្ថានភាព upstream ដែលត្រូវបានរាប់ជាការបរាជ័យក្នុងវិសាលភាព model។  |
| `baseCooldownMs`        | `120_000` (120 វិនាទី)           | រយៈពេលចាក់សោដំបូងសម្រាប់ការបរាជ័យលើកទីមួយ។                        |
| `maxCooldownMs`         | `1_800_000` (30 នាទី)            | កម្រិតអតិបរមានៃរយៈពេលសម្រាកដែលបានបង្កើន។                          |
| `maxBackoffSteps`       | `10`                             | ចំនួនជំហានអតិបរមាសម្រាប់ការបង្កើន exponential-backoff។            |
| `useExponentialBackoff` | `true`                           | ថាតើការបរាជ័យដដែលៗត្រូវបង្កើនរយៈពេលសម្រាកតាមអិចស្ប៉ូណង់ស្យែលឬអត់។ |

ការកំណត់ត្រូវបានរក្សាទុកតាមរយៈ settings store ធម្មតា និងផ្ទៀងផ្ទាត់តាម
resilience settings schema; card នេះកំណត់ដែនតម្លៃ `baseCooldownMs`/`maxCooldownMs`
(ដោយមាន `maxCooldownMs ≥ baseCooldownMs`) និង `maxBackoffSteps`។

**ការស្ដារឡើងវិញដោយបន្ថយតាមភាពជោគជ័យ:** ការស្ដារឡើងវិញ**មិន**មែនពឹងផ្អែកទាំងស្រុងលើការផុតកំណត់របស់ timer នោះទេ។ Response ដែលមានសុខភាពល្អ
បន្ថយចំនួនការបរាជ័យរបស់ model ដូច្នេះ model ដែលបានស្ដារឡើងវិញ
នៅពាក់កណ្ដាល window នឹងឈប់បង្កើនកម្រិត (និងសម្អាតការចាក់សោ) មុនពេល timer របស់វាផុតកំណត់។ នៅលើ
combo target ដែលជោគជ័យ `open-sse/services/combo.ts` ហៅ `decayModelFailureCount()`
(`open-sse/services/accountFallback.ts`) ដែល**កាត់ពាក់កណ្ដាល**តម្លៃ
`failureCount` ដែលបានរក្សាទុក (`Math.floor(failureCount / 2)`); នៅពេលវាឈានដល់ `0` ធាតុ lockout
ត្រូវបានលុបចោលទាំងស្រុង។ មុខងារដែលដំណើរការផ្ទុយគ្នាគឺ `recordModelLockoutFailure()`
នឹងបង្កើនចំនួន (និងបង្កើនរយៈពេលសម្រាក) នៅពេលមានការបរាជ័យក្នុង
escalation window។ ការបន្ថយតាមភាពជោគជ័យនេះធ្វើការបន្ថែមពីលើការផុតកំណត់ធម្មតារបស់ timer —
មាគ៌ាណាមួយក៏អាចបើកឱ្យ model ដំណើរការឡើងវិញបានដែរ។

**ស្ថានភាព:** ការចាក់សោត្រូវបានរក្សាទុក**ក្នុងអង្គចងចាំ** (`Map` ក្នុងមួយ process របស់
`ModelLockoutEntry` ដែលកំណត់ key ដោយ `provider:connectionId:model`) មិនត្រូវបានរក្សាទុកក្នុង
DB ទេ — ពួកវានឹងបាត់បង់នៅពេល restart។ _ការកំណត់_ត្រូវបានរក្សាទុក; ចំណែកឯ
_ស្ថានភាព_ការចាក់សោដែលកំពុងសកម្មគឺបណ្ដោះអាសន្ន។
...
- រំលងសោទាំងអស់សម្រាប់អ្នកផ្តល់សេវាមួយ → ពិនិត្យទាំងស្ថានភាព circuit breaker និង `rateLimitedUntil`/`testStatus` របស់ការតភ្ជាប់នីមួយៗ។
- អ្នកផ្តល់សេវាត្រូវបានដកចេញជាអចិន្ត្រៃយ៍បន្ទាប់ពីចន្លោះពេលកំណត់ឡើងវិញ → កូដកំពុងអាន `state` ឆៅ ជំនួសឱ្យ `getStatus()`/`canExecute()`។
- សោមួយបរាជ័យ ប៉ុន្តែសោផ្សេងទៀតគួរតែដំណើរការ → គួរប្រើរយៈពេលរង់ចាំរបស់ការតភ្ជាប់ ជាជាង circuit breaker។
- មានតែម៉ូដែលមួយប៉ុណ្ណោះដែលបរាជ័យ → គួរប្រើការចាក់សោម៉ូដែល ជាជាងរយៈពេលរង់ចាំរបស់ការតភ្ជាប់។
- ស្ថានភាពគួរតែស្ដារឡើងវិញដោយខ្លួនឯង ប៉ុន្តែមិនកើតឡើង → ពិនិត្យរកត្រាពេលវេលានាពេលអនាគត + ផ្លូវអានដែលធ្វើឱ្យស្ថានភាពផុតកំណត់ស្រស់ឡើងវិញ។ ស្ថានភាពអចិន្ត្រៃយ៍តម្រូវឱ្យកែប្រែដោយដៃ។
