...
L-iscripts jinsabu taħt `scripts/check/` (gates tal-politika) u `scripts/quality/` (magna tar-ratchet).
Is-sors awtorevoli tas-CI huwa `.github/workflows/ci.yml`.

### Rotta rapida għall-PRs tar-release (`quality.yml`)

`.github/workflows/quality.yml` jitħaddem fuq PRs immirati lejn `release/**`. Dan iżomm il-branches
tal-kontributuri mexjin permezz ta’ gates rapidi ffiltrati skont il-path, flimkien ma’ sinjal konsultattiv wieħed
tal-build tal-produzzjoni għall-bidliet fil-code:

| Job                                              | Ambitu                                                                                                                                                                                                                                | Imblukkar                                                                                             |
| ------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------- |
| `Build (advisory)`                               | PRs tal-code li mhumiex drafts u branches tal-kju ta’ Mergify; Node 24, `npm-ci-retry`, `check:node-runtime`, `npm run build` b’`OMNIROUTE_USE_TURBOPACK=1`; ebda upload ta’ artifact għax l-ebda job tal-kwalità downstream ma jużah | **Konsultattiv** (`continue-on-error: true`; neħħih wara ġimgħa ta’ runs stabbli tal-PRs tar-release) |
| `Docs Gates (fast-path)`                         | PRs tad-dokumentazzjoni/tal-code; referenzi tad-dokumentazzjoni tal-API u docs-all                                                                                                                                                    | Iva                                                                                                   |
| `Fast Quality Gates`                             | PRs tal-code; kontrolli statiċi, typecheck, typecheck tad-dashboard, tests tal-unitajiet affettwati                                                                                                                                   | Iva                                                                                                   |
| `Forgotten sibling tests`                        | PRs tal-code; moduli mibdula traċċati sal-konsumaturi statiċi u tests sibling kandidati; paths ta’ barrel u dynamic-import jiġu rrappurtati bħala dijanjostika konsultattiva, b’eċċezzjonijiet tal-allowlist referenzjati             | **Konsultattiv**                                                                                      |
| `Vitest (fast-path)`                             | PRs tal-code; suite rapida ta’ vitest                                                                                                                                                                                                 | Iva                                                                                                   |
| `Unit Tests fast-path`                           | PRs tal-code; suite ta’ tests tal-unitajiet b’4 shards                                                                                                                                                                                | Iva                                                                                                   |
| `No new ESLint warnings`                         | PRs tal-code; kontroll tal-lint konxju tas-soppressjonijiet                                                                                                                                                                           | Iva għall-istess oriġini, konsultattiv għall-forks                                                    |
| `Merge integrity (changelog + generated skills)` | PRs li mhumiex drafts; sinkronizzazzjoni tal-changelog u tal-ħiliet iġġenerati                                                                                                                                                        | Iva għall-istess oriġini, konsultattiv għall-forks                                                    |

#### Rapport tat-tests sibling minsija

`npm run check:forgotten-sibling-tests` jerġa’ juża r-resolver tal-imports wara l-mappa tal-impatt tat-tests.
Għal kull modulu tal-produzzjoni mibdul, jirrapporta ktajjen deterministiċi
`modulu/simbolu mibdul -> konsumatur statiku -> test sibling kandidat` meta t-test kandidat
ma jkunx preżenti fid-diff tal-pull request. Is-sommarju Markdown u r-riżultat JSON jinżammu bħala
l-artifact tal-workflow `forgotten-sibling-tests` għall-kalibrazzjoni qabel kwalunkwe introduzzjoni bħala gate li jimblokka.

Ir-re-exports tal-barrel u l-imports dinamiċi huma biss dijanjostika tar-riżoluzzjoni; qatt ma joħolqu
...
kandidat, tagħti raġuni speċifika, u tinkludi link għal issue jew pull request ta’ GitHub. Entrati fformattjati
ħażin iwasslu għal falliment magħluq. L-eċċezzjonijiet ma jistgħux jissopprimu test kandidat imħassar jew diff li jżid `.skip`/`.todo`;
id-dgħufija tal-assertions u forom oħra ta’ masking jibqgħu taħt ir-responsabbiltà tal-gate
...
| Skritt (`npm run ...`)            | Jivvalida                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | Imblukkar                                    |
| --------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------- |
...
| `check:any-budget:t11`            | L-għadd ta’ `@ts-expect-error // any` ma jaqbiżx il-limitu (xatba tal-Livell 11)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | Iva                                          |
| `check:provider-consistency`      | Kull fornitur f’`providers.ts` għandu entrata korrispondenti f’`providerRegistry.ts` (u viċi versa, fil-limiti tal-lista permessa)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | Iva                                          |
| `check:model-lifecycle`           | It-tliet tabelli tar-routing miżmuma manwalment jibqgħu konsistenti mal-istampa taċ-ċiklu tal-ħajja inkluża fir-repo (#11503): `FITNESS_TABLE` (`taskFitness.ts`) ma tagħti punteġġ lil ebda id irtirat li `REGISTRY` jista’ jirrottja; kull mira ta’ `BUILT_IN_ALIASES` tinsab f’`REGISTRY` u hija assenti mill-istampa tal-id irtirati; kull id irtirat li għadu f’`REGISTRY` jiġi ridirezzjonat jew elenkat f’`allowedRetiredInCatalog`; u l-ebda sors jew mira ta’ `DEFAULT_DEGRADATION_MAP` ma jidher bħala rtirat f’dik l-istampa. Dan ma jagħtix prova li mudell bħalissa qed jiġi servut minn upstream attiv. Offline — iqabbel ma’ `config/quality/model-lifecycle.json`, aġġornat manwalment permezz ta’ `npm run quality:refresh-model-lifecycle` (jeħtieġ in-network; mhux integrat fis-CI). `allowedRetiredInCatalog` huwa mekkaniżmu progressiv ta’ tnaqqis: żid entrata biss flimkien ma’ issue ta’ traċċar. | Iva                                          |
| `check:fetch-targets`             | Kull `fetch("/api/...")` f’`src/` fuq in-naħa tal-klijent jirriżolvi għal `route.ts` reali                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Iva                                          |
| `check:deps`                      | Id-dipendenzi kollha li jistgħu jiġu installati b’`npm install` fil-fajls `package.json` kollha fir-repo jinsabu f’`dependency-allowlist.json`; pakketti ġodda mingħajr verżjoni fissa jew b’ismijiet simili qarrieqa jiġu mmarkati                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | Iva                                          |
| `audit:deps`                      | `npm audit` (root + electron) — ebda twissijiet ta’ severità għolja/kritika (jikkoinċidi ma’ osv `check:vuln-ratchet`; ara l-Backlog tar-Razzjonalizzazzjoni)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | Iva                                          |
...
| `check:tracked-artifacts`         | Ebda artefatti tal-build / symlinks ta' `node_modules` ikkommettjati (jaħdem ukoll fil-pre-commit ta' husky; il-pre-push huwa intenzjonalment ħafif — #6716)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | Iva                                          |
...
| `check:error-helper`              | It-tweġibiet ta' żball fl-executors/handlers jużaw `buildErrorBody()` / `sanitizeErrorMessage()` (Regola Stretta #12)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | Iva                                          |
| `check:migration-numbering`       | Il-fajls SQL tal-migrazzjoni huma nnumerati sekwenzjalment, mingħajr lakuni jew duplikati                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | Iva                                          |
| `check:public-creds`              | L-ebda `client_id`/`client_secret` OAuth litterali jew ċwievet Web ta’ Firebase barra minn `publicCreds.ts` (Regola Stretta #11)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | Iva                                          |
| `check:db-rules`                  | L-ebda SQL mhux ipproċessat barra mill-moduli `src/lib/db/`; l-ebda importazzjoni aggregata minn `localDb.ts` (Regoli Stretti #2/#5)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | Iva                                          |
...
| `check:test-discovery`            | Kull fajl `*.test.ts` / `*.spec.ts` fir-repo jinġabar minn mill-inqas test runner wieħed (ratchet: il-lista tal-fajls orfni f’`test-discovery-baseline.json` tista’ tiċkien biss)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | Iva                                          |
| `check:agent-skills-sync`         | L-artefatti ġġenerati tal-ħiliet tal-aġent jaqblu mal-katalgu tas-sors tagħhom (mingħajr devjazzjoni)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `check:provider-asset-provenance` | Il-logos/l-assi tal-fornituri għandhom entrata rreġistrata tal-provenjenza                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| `typecheck:noimplicit:core`       | `noImplicitAny` strett — maħsub għall-futur; ħafna postijiet ta’ sejħa eżistenti għadhom jeħtieġu annotazzjonijiet                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | **Konsultattiv** (`continue-on-error: true`) |
| `check:dashboard-typecheck`       | `tsc` limitat għal `src/app/(dashboard)/**` (#7033) — il-lista kkurata ta’ 27 fajl ta’ `typecheck:core` ma tinkludi ebda TSX tad-dashboard, u `next build` lanqas qatt ma jwettaq kontroll tat-tipi fuqu (`next.config.mjs` jistabbilixxi `ignoreBuildErrors: true`), għalhekk rigressjonijiet ta’ identifikaturi orfni hemmhekk (#6625/#6909) ma kinux jidhru fis-CI. Jqabbel ma’ linja bażi ffriżata tal-għadd għal kull fajl/kodiċi TS (`config/quality/dashboard-typecheck-baseline.json`, bl-istess mudell ta’ infurzar kontra informazzjoni skaduta bħal `check:known-symbols`) — żbalji ĠODDA biss lil hinn mill-għadd fil-linja bażi jikkawżaw il-falliment tal-kontroll; naqqas il-linja bażi b’`--update` meta jiġi rranġat żball eżistenti.                                                                                                                                                                      | Iva                                          |

### Biċċa xogħol: `quality-gate`

Titħaddem wara `test-coverage`. Timblokka l-inkorporazzjoni tal-bidliet jekk tfalli.

| Skript                       | Jivvalida                                                                                                                                                                                                          | Imblokkanti            |
| ---------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------- |
| `quality:collect`            | Jiġġenera `quality-metrics.json` (għadd tat-twissijiet ESLint, kopertura mir-rapport ikkombinat tax-shards)                                                                                                        | Iva (qabel ir-ratchet) |
...
| `check:compression-budget`   | Baġit tal-parametru referenzjarju tal-kompressjoni — il-limiti minimi tal-iffrankar tat-tokens għal kull magna ma jistgħux imorru lura                                                                             | Iva                    |
| `check:type-coverage`        | Ir-ratchet tal-perċentwal b’tipi (`type-coverage`) ma jmurx lura; fil-biċċa l-kbira jinkludi l-funzjonalità ta’ `typecheck:noimplicit:core`                                                                        | Iva                    |
| `check:codeql-ratchet`       | L-għadd ta’ twissijiet CodeQL miftuħa ma jmurx lura (jinqara permezz ta’ `gh api`; jinqabeż b’mod sikur mingħajr token) — għall-frekwenza tal-aġġornament u l-attivazzjoni manwali: ara "CodeQL ratchet" hawn taħt | Iva                    |

### Biċċa xogħol: `quality-extended`

Ix-xogħol kollu huwa konsultattiv (`continue-on-error: true`). Il-kontrolli ratchet ibbażati fuq npm jitħaddmu
...
| ------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------- |
| `check:circular-deps`    | Ma hemmx dipendenzi ċirkolari (dpdm)                                                                                                                                                                                                 | **Konsultattiv** |
...
| `check:workflows`        | Verifika tal-workflows (actionlint + zizmor) — tinqabeż jekk il-binarji jkunu neqsin                                                                                                                                                 | **Konsultattiv** |
| `check:openapi-breaking` | Bidliet inkompatibbli fil-kuntratt tal-API pubblika (`openapi.yaml`) meta mqabbel mal-fergħa bażi (oasdiff) — joħroġ `openapiBreaking=N`; jinqabeż jekk oasdiff ikun nieqes jew l-ispeċifikazzjoni bażi ma tkunx tista’ tiġi riżolta | **Konsultattiv** |
...
| ↳ `check:docs-counts`          | L-għadd imsemmi fit-test (għadd ta’ providers, għadd ta’ migrazzjonijiet, eċċ.) jinsab fit-tieqa ratchet tal-għadd reali                                                                                                          | Iva                                 |
| ↳ `check:env-doc-sync`         | Kull varjabbli tal-ambjent f’`.env.example` hija ddokumentata f’tabella tad-dokumentazzjoni, u viċeversa                                                                                                                          | Iva                                 |
| ↳ `check:deprecated-versions`  | Ma hemmx strings ta’ verżjonijiet deprekati fid-dokumentazzjoni                                                                                                                                                                   | Iva                                 |
| ↳ `check:doc-links`            | Il-links interni tal-markdown fid-dokumentazzjoni jirriżolvu għal fajls reali (forma `[text]`/`(path)`)                                                                                                                           | Iva                                 |
| ↳ `check:fabricated-docs`      | Ir-routes, il-varjabbli tal-ambjent, il-kmandi CLI, l-ismijiet tal-hooks, u l-paths tal-fajls iċċitati fid-dokumentazzjoni jeżistu fil-codebase. Gate strett permezz ta’ `--strict`; falliment mhux imblokkanti mingħajr il-flag. | Iva (permezz ta’ `--strict` fis-CI) |
| `check:cli-i18n`               | L-strings tal-kmandi CLI huma preżenti fil-fajls kollha tal-locale i18n                                                                                                                                                           | Iva                                 |
| `check:openapi-coverage`       | L-ispeċifikazzjoni OpenAPI tkopri mill-inqas limitu minimu ratcheted tar-routes reali                                                                                                                                             | Iva                                 |
...
| `i18n translation drift`       | Keys mhux tradotti fil-fajls tal-locale i18n — twissija biss                                                                                                                                                                      | **Konsultattiv**                    |
...
| `check-ui-value-drift` (inline)   | **Valur** bl-Ingliż miktub mill-ġdid ma jħalli warajh ebda traduzzjoni skaduta                                                                                                                                   | Iva              |
| `check-new-key-coverage` (inline) | Ċavetta **ġdida** bl-Ingliż tasal f’kull locale                                                                                                                                                                  | Iva              |
| `check-translation-ratio`         | Il-proporzjon ta’ traduzzjonijiet reali għal kull locale (valuri identiċi għall-Ingliż / placeholders / neqsin li mhumiex fl-allowlist) ma jridx jaqbeż `config/quality/i18n-translation-baseline.json` + marġni | **Konsultattiv** |

Jeħtieġ `fetch-depth: 0` — il-kontroll tad-drift tal-valuri jqabbel `en.json` mal-bażi tal-merge.

#### `check-ui-value-drift` — kontroll għal traduzzjonijiet skaduti

Jidentifika r-rigressjoni waħda tal-i18n li l-kontrolli l-oħra strutturalment ma jistgħux jaraw: valur bl-Ingliż
jinkiteb mill-ġdid u t-traduzzjonijiet derivati mill-Ingliż _preċedenti_ jibqgħu hemm, u għalhekk
utenti li ma jużawx l-Ingliż jibqgħu jaqraw test miktub b’ċertezza iżda li issa huwa żbaljat.

Dan fil-fatt wasal fil-produzzjoni. `oauthModal.googleOAuthWarning` inkiteb mill-ġdid meta ġie introdott
l-helper tal-login Antigravity (#5203); **39 minn 43 locale** żammew test li kien jgħid lill-operaturi biex "jikkopjaw
il-URL sħiħ u jwaħħluh hawn taħt" — fluss li ma jistax jitlesta għal dak il-fornitur. Dan
ma ġiex innutat qabel #8463 għaliex:

- `sync-ui-keys` jimla biss ċwievet li huma **neqsin**, qatt dawk li huma **skaduti**;
- `check-ui-keys-coverage` jgħodd il-_preżenza_ taċ-ċwievet, għalhekk traduzzjoni skaduta titqies bħala koperta;
- `check-translation-drift` jimmonitorja l-mirja tad-dokumentazzjoni `docs/i18n/<locale>/**.md` —
  qatt ma jaqra `src/i18n/messages/*.json`. Jimblokka fil-job `docs-sync-strict` mir-risinkronizzazzjoni
  tal-2026-09: editja dokument ewlieni → `npm run i18n:run -- --files=<doc>` (fil-livell tat-taqsima, irħis).

**Konxju mid-diff, mhux ibbażat fuq baseline.** Iqabbel `en.json` fil-bażi tal-merge mas-siġra
tax-xogħol; għal kull ċavetta li l-valur tagħha bl-Ingliż inbidel, kwalunkwe locale li jkun għad għandu
traduzzjoni mhux mibdula huwa skadut. Dan intenzjonalment **jiffriża d-dejn eżistenti** — diff
ma jistax jiżvela minn liema verżjoni qadima tal-Ingliż oriġinat traduzzjoni li ilha teżisti, għalhekk il-kontroll jevalwa
...
Jekk it-**tifsira** tas-string inbidlet, ippreferi li **tibdel l-isem taċ-ċavetta**: ċavetta ġdida ma tistax tiret
traduzzjoni skaduta. Dan huwa l-mudell li uża #8463.

```bash
npm run i18n:check-value-drift          # strett (dak li jħaddem is-CI)
npm run i18n:check-value-drift:warn     # irrapporta biss
BASE_REF=origin/release/vX.Y.Z npm run i18n:check-value-drift
```

Joħroġ b’0 flimkien ma’ `SKIP reason=base-unresolved` meta l-katalgu bażi ma jkunx jista’ jinqara (clone baxx
...
Jitħaddem biss fuq pull requests.

| Skritt                 | Jivvalida                                                                                                                                                | Imblukkar |
| ---------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- | --------- |
| `check:pr-test-policy` | PRs li jibdlu kodiċi tal-produzzjoni f’`src/`, `open-sse/`, `electron/`, jew `bin/` iridu jinkludu jew jaġġornaw it-testijiet (Regola Stretta #8)        | Iva       |
| `check:test-masking`   | Fajls tat-test mibdula ma jnaqqsux l-għadd nett ta’ assertions u ma jżidux tautoloġiji `assert.ok(true)`                                                 | Iva       |
| `check:pr-evidence`    | Il-korp tal-PR jiċċita evidenza minn testijiet/VPS għall-bidla (jawtomatizza r-Regola Stretta #18 billi jfittex fit-test tal-PR — fraġli, ara l-Backlog) | Iva       |

### Job: `test-vitest`

Jitħaddem wara `build`. Jimblokka l-merge jekk ifalli.

| Sett             | Jivvalida                                                     | Imblokkanti                                                                                                                |
| ---------------- | ------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| `test:vitest`    | Server MCP (110 għodda), autoCombo, cache — runner ta’ vitest | Iva                                                                                                                        |
| `test:vitest:ui` | Testijiet tal-komponenti tal-UI — runner ta’ vitest           | **Imblokkanti** — fallimenti preeżistenti huma esklużi b’mod espliċitu f’`vitest.config.ts`; fallimenti ġodda jfallu l-job |
...
| Fluss tax-xogħol       | Jivvalida                                                                                                                                                                        | Imblokkanti      |
| ---------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------- |
| `nightly-property`     | Testijiet tal-proprjetajiet fast-check b’seed aleatorju + għadd għoli ta’ eżekuzzjonijiet                                                                                        | **Konsultattiv** |
| `nightly-resilience`   | Limitu fuq it-tkabbir tal-heap, injezzjoni kaotika ta’ ħsarat, testijiet tat-tagħbija/soak b’k6                                                                                  | **Konsultattiv** |
| `nightly-llm-security` | Protezzjoni kontra l-injezzjoni ta’ promptfoo (modalità ta’ mblukkar) + probes ta’ garak (maqbuża mingħajr sigriet tal-fornitur)                                                 | **Konsultattiv** |
...
| `nightly-mutation`     | Punteġġ tal-ittestjar tal-mutazzjonijiet ta’ Stryker fuq il-kors rapidu tat-testijiet tal-unità — mutanti li jibqgħu ħajjin jikxfu asserts dgħajfa                               | **Konsultattiv** |
| `nightly-compat`       | Matriċi tal-kompatibbiltà tal-magna Node tul il-firxiet appoġġjati ta’ `engines.node`                                                                                            | **Konsultattiv** |
...
#### `check-key-completeness` — kontroll tal-parità tas-settijiet taċ-ċwievet
...
jekk ikun hemm xi ċavetta terminali nieqsa jew żejda, irrispettivament minn meta tkun ġiet miżjuda ċ-ċavetta. Il-markaturi
`__MISSING__:` jgħoddu bħala preżenti (il-kontenut tagħhom huwa r-responsabbiltà tal-kontroll tal-proporzjon). Dan huwa l-komplement assolut
taż-żewġ kontrolli bbażati fuq id-differenzi/il-perċentwali: `check-ui-keys-coverage` jinforza limitu minimu ta’ 80 % għal kull
lokalizzazzjoni (43 ċavetta nieqsa minn ~13,000 xorta juru 99.7 %) u `check-new-key-coverage` jevalwa
biss iċ-ċwievet li PR iżid ma’ `en.json`. Lott ta’ lokalizzazzjonijiet jiġi ġġenerat mill-`en.json` tal-jum
li fih tinħoloq il-fergħa tiegħu u jibqa’ jiġi tradott għal diversi jiem waqt li l-bażi tkompli żżid iċ-ċwievet; il-PR tal-lott innifsu
ma jżid l-ebda ċavetta, għalhekk iż-żewġ kontrolli relatati baqgħu siekta meta l-lott 1 (#13044) ġie integrat b’43 ċavetta nieqsa f’disa’
lokalizzazzjonijiet u l-lott 2 (#13660) b’10 ċwievet neqsin fi tmienja (2026-09-15). Irranġa kontroll aħmar permezz ta’
`node scripts/i18n/sync-ui-keys.mjs --locale=<codes> --translate-markers`; ċavetta terminali `extra`
tfisser li s-sors neħħieha — ħassarha mil-lokalizzazzjoni. `--warn` jirrapporta mingħajr ma jfalli.
...
filwaqt li t-traduzzjonijiet tiegħu tħallew kif kienu; dan jaqbad ċavetta bl-Ingliż li tkun ġiet **miżjuda**
filwaqt li xi lokalizzazzjonijiet qatt ma rċevewha.

`check-ui-keys-coverage` ma jistax jinduna b’din il-kategorija: huwa jinforza limitu perċentwali minimu għal kull lokalizzazzjoni, u
ħdax-il ċavetta nieqsa minn ~13,000 iħallu l-kopertura f’99.9%. Perċentwal għal kull lingwa ma jistax
jesprimi “din il-funzjonalità ħarġet mingħajr traduzzjoni” — funzjonalità sħiħa tista’ tasal f’lokalizzazzjoni ġdida mingħajr ebda
test u qatt ma tbiddel in-numru.

L-inċident li jirreġistra: il-Fażi 3 tal-Orchestration Canvas ittraduċiet il-ħdax-il ċavetta tagħha fit-
42 lokalizzazzjoni li kienu jeżistu dak iż-żmien. Ftit sigħat wara, il-lott tal-lingwi tal-UE (#13044) wassal ir-repożitorju
għal 51 lokalizzazzjoni, u d-disa’ lokalizzazzjonijiet ġodda (`el`, `et`, `ga`, `hr`, `lt`, `lv`, `mt`, `sl`, `sr`) qatt ma
rċevewhom. `deepMergeFallback` juża l-Ingliż minflok ċavetta nieqsa, għalhekk ir-riżultat tal-falliment kien
interfaċċa tal-utent mhux tradotta aktar milli vojta — problema reali u, min-natura tagħha, siekta.

Bħall-kontroll relatat miegħu, huwa **konxju tad-differenzi**, billi jqabbel l-Ingliż fil-bażi tal-integrazzjoni mas-siġra tax-xogħol,
għalhekk il-lakuni li kienu jeżistu minn qabel jibqgħu ffriżati u ma kienet meħtieġa l-ebda migrazzjoni biex il-kontroll jiġi attivat. Mod ta’ eċċezzjoni:
`__MISSING__:<english>` jipposponi traduzzjoni filwaqt li jżomm l-eżekuzzjoni korretta. `vi` jipprojbixxi
l-markaturi (`tests/unit/i18n-vi-completeness.test.ts`) u jeħtieġ traduzzjoni reali.
...
kopertura għal min jaqra s-siġra. Tnejn u sittin fajl inġabru wara l-kumment
`// #8618 — falliment li kien jeżisti minn qabel; neħħi din l-esklużjoni meta jiġi rranġat`. Il-kwistjoni #8618 ingħalqet fil-
2026-08-11 filwaqt li l-lista li kienet issegwi kibret minn 45 entrata għal 62, b’kull waħda ġdida tiret kumment
li jirreferi għal kwistjoni magħluqa. Meta fl-aħħar il-lista ġiet imkejla fajl b’fajl (#13204), **51 mit-62
għaddew mas-siġra attwali mingħajr ebda bidla fis-sors**.

Il-kontroll jeħtieġ li kull esklużjoni li tirriżolvi għal fajl reali (a) issemmi kwistjoni ta’ traċċar u
(b) tidher f’`config/quality/vitest-exclusions.json` bl-istatus imkejjel tagħha, sabiex iż-żieda ta’ waħda tkun
differenza li tista’ tiġi rieżaminata f’fajl apposta minflok sempliċement linja oħra f’array ta’ 60 entrata. Intenzjonalment
ma jerġax iħaddem it-testijiet esklużi — dan jieħu ~10 minuti u għandu jsir f’xogħol perjodiku; l-
inventarju jirreġistra meta kull wieħed ġie mkejjel l-aħħar.
