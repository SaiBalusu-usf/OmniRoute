...
`.github/workflows/quality.yml`은 `release/**`를 대상으로 하는 PR에서 실행됩니다. 경로별로
필터링된 빠른 게이트를 통해 기여자 브랜치의 진행을 유지하며, 코드 변경에는 프로덕션 빌드에
대한 권고 신호 하나를 추가로 제공합니다.

| 작업                                             | 범위                                                                                                                                                                                                                | 차단 여부                                                                                    |
| ------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------- |
| `Build (advisory)`                               | 초안이 아닌 코드 PR 및 Mergify 큐 브랜치; Node 24, `npm-ci-retry`, `check:node-runtime`, `OMNIROUTE_USE_TURBOPACK=1`을 사용하는 `npm run build`; 이를 사용하는 후속 품질 작업이 없으므로 아티팩트를 업로드하지 않음 | **권고** (`continue-on-error: true`; 릴리스 PR 실행이 일주일 동안 안정적으로 유지된 후 제거) |
| `Docs Gates (fast-path)`                         | 문서/코드 PR; API 문서 참조 및 전체 문서                                                                                                                                                                            | 예                                                                                           |
| `Fast Quality Gates`                             | 코드 PR; 정적 검사, 타입 검사, 대시보드 타입 검사, 영향받는 단위 테스트                                                                                                                                             | 예                                                                                           |
| `Forgotten sibling tests`                        | 코드 PR; 변경된 모듈에서 정적 소비자 및 후보 형제 테스트까지 추적; 배럴 및 동적 임포트 경로는 참조된 허용 목록 예외와 함께 권고 진단으로 보고됨                                                                     | **권고**                                                                                     |
| `Vitest (fast-path)`                             | 코드 PR; 빠른 vitest 스위트                                                                                                                                                                                         | 예                                                                                           |
| `Unit Tests fast-path`                           | 코드 PR; 4개 샤드 단위 테스트 스위트                                                                                                                                                                                | 예                                                                                           |
| `No new ESLint warnings`                         | 코드 PR; 억제 규칙을 인식하는 린트 가드                                                                                                                                                                             | 동일 출처는 예, 포크는 권고                                                                  |
| `Merge integrity (changelog + generated skills)` | 초안이 아닌 PR; 변경 로그 및 생성된 스킬 동기화                                                                                                                                                                     | 동일 출처는 예, 포크는 권고                                                                  |

#### 누락된 형제 테스트 보고서

`npm run check:forgotten-sibling-tests`는 테스트 영향도 맵에서 사용하는 임포트 해석기를 재사용합니다.
변경된 각 프로덕션 모듈에 대해 후보 테스트가 풀 리퀘스트 diff에 없으면 결정론적인
`변경된 모듈/심볼 -> 정적 소비자 -> 후보 형제 테스트` 체인을 보고합니다. 차단 방식으로 전환하기
전의 보정을 위해 Markdown 요약 및 JSON 결과는 `forgotten-sibling-tests` 워크플로 아티팩트로
보존됩니다.

배럴 재내보내기와 동적 임포트는 해석 진단에만 사용되며, 차단 결과를 생성하지 않습니다.
검토된 예외는 `config/quality/forgotten-sibling-allowlist.json`에 있습니다. 각 항목에는 소비자와
후보 테스트를 명시하고, 구체적인 근거를 제시하며, GitHub 이슈 또는 풀 리퀘스트를 링크해야
합니다. 잘못된 형식의 항목은 차단 방식으로 실패합니다. 예외는 삭제된 후보 테스트나
`.skip`/`.todo`를 추가하는 diff를 억제할 수 없습니다. 어설션 약화 및 기타 은폐 행위는 별도로
차단되는 `check:test-masking` 게이트에서 계속 담당합니다.
...
| `check:cycles`                    | 모든 `src/` + `open-sse/` 모듈의 순환 import                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | 예                                   |
...
| `check:provider-consistency`      | `providers.ts`의 모든 공급자에 `providerRegistry.ts`의 일치하는 항목이 있으며, 그 반대도 마찬가지입니다(허용 목록 내에서).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | 예                                   |
| `check:model-lifecycle`           | 수동으로 유지 관리되는 세 개의 라우팅 테이블이 체크인된 수명 주기 스냅샷(#11503)과 일관되게 유지됩니다. `FITNESS_TABLE`(`taskFitness.ts`)은 `REGISTRY`가 라우팅할 수 있는 폐기된 id에 점수를 부여하지 않습니다. 모든 `BUILT_IN_ALIASES` 대상은 `REGISTRY`에 존재하며 폐기된 id 스냅샷에는 없습니다. `REGISTRY`에 아직 남아 있는 모든 폐기된 id는 전달되거나 `allowedRetiredInCatalog`에 나열됩니다. 또한 `DEFAULT_DEGRADATION_MAP`의 소스나 대상은 해당 스냅샷에서 폐기된 것으로 표시되지 않습니다. 이는 모델이 현재 실제 업스트림에서 제공되고 있음을 보장하지 않습니다. 오프라인 — `config/quality/model-lifecycle.json`과 비교하며, `npm run quality:refresh-model-lifecycle`을 사용하여 수동으로 갱신합니다(네트워크 필요, CI에 연결되지 않음). `allowedRetiredInCatalog`는 점진적으로 줄여 나가는 래칫입니다. 추적 이슈가 있는 경우에만 항목을 추가하세요. | 예                                   |
| `check:fetch-targets`             | 클라이언트 측 `src/`의 모든 `fetch("/api/...")`가 실제 `route.ts`로 해석됩니다.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | 예                                   |
| `check:deps`                      | 저장소의 모든 `package.json`에 있는, `npm install`로 설치 가능한 모든 의존성이 `dependency-allowlist.json`에 포함되어 있습니다. 새로 추가된 버전이 고정되지 않은 패키지나 슬롭스쿼팅 패키지는 플래그로 표시됩니다.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | 예                                   |
| `audit:deps`                      | `npm audit`(루트 + electron) — 높음/심각 수준의 보안 권고가 없습니다(osv `check:vuln-ratchet`과 중복됨. 합리화 백로그 참조).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | 예                                   |
| `check:lockfile`                  | `package-lock.json` 무결성 — https 레지스트리, 무결성 해시, 호스트 재정의 없음                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | 예                                   |
| `check:licenses`                  | 프로덕션 종속성을 위한 SPDX 라이선스 허용 목록                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | 예                                   |
| `check:tracked-artifacts`         | 빌드 아티팩트 / 커밋된 `node_modules` 심볼릭 링크 없음(husky pre-commit에서도 실행됨; pre-push는 의도적으로 가볍게 유지 — #6716)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | 예                                   |
| `check:vitest-exclusions`         | 모든 Vitest 제외 항목은 추적 이슈를 명시하며 `config/quality/vitest-exclusions.json`에 포함됨(#13204)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | 예                                   |
...
| `check:migration-numbering`       | 마이그레이션 SQL 파일에 누락되거나 중복된 번호 없이 순차적으로 번호가 지정됨                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | 예                                   |
| `check:public-creds`              | `publicCreds.ts` 외부에 OAuth `client_id`/`client_secret` 또는 Firebase Web 키 리터럴이 없음(Hard Rule #11)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | 예                                   |
| `check:db-rules`                  | `src/lib/db/` 모듈 외부에 원시 SQL이 없으며, `localDb.ts`에서 배럴 가져오기를 사용하지 않음(Hard Rules #2/#5)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | 예                                   |
| `check:known-symbols`             | 디스패치 테이블에 등록된 공급자 실행기, 라우팅 전략 및 변환기가 디스크의 파일과 일치하며, 고립되거나 선언되지 않은 심볼이 없음                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | 예                                   |
| `check:route-guard-membership`    | 하위 프로세스를 생성하는 모든 라우트가 `isLocalOnlyPath()`에 의해 분류됨(Hard Rules #15/#17)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | 예                                   |
...
| `check:dashboard-typecheck`       | `src/app/(dashboard)/**`로 범위가 제한된 `tsc` (#7033) — `typecheck:core`의 선별된 27개 파일 허용 목록에는 대시보드 TSX가 전혀 포함되지 않고, `next build` 역시 이를 타입 검사하지 않으므로(`next.config.mjs`에서 `ignoreBuildErrors: true` 설정), 해당 영역의 고립된 식별자 회귀(#6625/#6909)가 CI에서 감지되지 않았음. 고정된 파일별/TS 코드별 개수 기준선(`config/quality/dashboard-typecheck-baseline.json`, `check:known-symbols`와 동일한 오래된 항목 강제 적용 패턴)과 비교함 — 기준선 개수를 초과하는 새로운 오류만 게이트를 실패시키며, 기존 오류가 수정되면 `--update`를 사용해 기준선을 낮춤.                                                                                                                                                                                                                                                        | 예                                   |
...
| 스크립트                     | 검증 항목                                                                                                                                             | 차단 여부           |
| ---------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------- |
| `quality:collect`            | `quality-metrics.json`을 생성함(ESLint 경고 수, 병합된 샤드 보고서의 커버리지)                                                                        | 예(래칫의 업스트림) |
| `quality:ratchet`            | `quality-baseline.json`의 각 지표가 회귀하지 않았는지 확인함(ESLint 경고 ≤ 기준선, 커버리지 ≥ 기준선)                                                 | 예                  |
| `check:duplication`          | 코드 중복(jscpd@4)이 `quality-baseline.json`의 기준선을 초과하지 않음                                                                                 | 예                  |
| `check:complexity`           | 파일 수준의 순환 복잡도가 상한을 초과하지 않음(핵심 ESLint `complexity` + `max-lines-per-function`)                                                   | 예                  |
| `check:cognitive-complexity` | 인지 복잡도 래칫(`eslint-plugin-sonarjs`) — 별도의 ESLint 패스이며, CI에서는 두 검사를 단일 `check:complexity-ratchets` 단계로 병합하여 실행함        | 예                  |
| `check:dead-code`            | 사용되지 않는 내보내기/파일 래칫(knip)이 기준선 대비 회귀하지 않음                                                                                    | 예                  |
| `check:compression-budget`   | 압축 벤치마크 예산 — 엔진별 토큰 절감률 하한이 회귀해서는 안 됨                                                                                       | 예                  |
| `check:type-coverage`        | 타입 지정 비율 래칫(`type-coverage`)이 회귀하지 않음. `typecheck:noimplicit:core`를 대부분 포괄함                                                     | 예                  |
| `check:codeql-ratchet`       | 미해결 CodeQL 경고 수가 회귀하지 않음(`gh api`를 통해 읽으며, 토큰이 없으면 정상적으로 건너뜀) — 갱신 주기 및 수동 트리거는 아래의 "CodeQL 래칫" 참조 | 예                  |

### 작업: `quality-extended`

전체 작업은 권고 수준입니다(`continue-on-error: true`). npm 기반 래칫은 실제로 실행되며,
외부 스캐너는 `gh release download`를 통해 설치되고 바이너리가 여전히 없으면 자체적으로
건너뜁니다(exit 0).

| 스크립트                 | 검증 항목                                                                                                                                                             | 차단 여부 |
| ------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------- |
| `check:circular-deps`    | 순환 종속성 없음(dpdm)                                                                                                                                                | **권고**  |
...
| `check:openapi-breaking` | 기본 브랜치와 비교한 공개 API 계약(`openapi.yaml`)의 호환성을 깨는 변경(oasdiff) — `openapiBreaking=N`을 출력함. oasdiff가 없거나 기본 스펙을 확인할 수 없으면 건너뜀 | **권고**  |
...
| 스크립트                       | 검증 항목                                                                                                                                                    | 차단 여부                  |
| ------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------- |
| `check:docs-all`               | 아래 6개 하위 게이트를 순차적으로 실행하는 메타 게이트                                                                                                       | 예                         |
| ↳ `check:docs-sync`            | CHANGELOG / OpenAPI / llm.txt 버전 일관성                                                                                                                    | 예                         |
| ↳ `check:docs-counts`          | 설명문에 명시된 개수(프로바이더 수, 마이그레이션 수 등)가 실제 개수의 래칫 범위 내에 있음                                                                    | 예                         |
| ↳ `check:env-doc-sync`         | `.env.example`의 모든 환경 변수가 문서 표에 기록되어 있으며, 그 반대도 성립함                                                                                | 예                         |
| ↳ `check:deprecated-versions`  | 문서에 더 이상 사용되지 않는 버전 문자열이 없음                                                                                                              | 예                         |
| ↳ `check:doc-links`            | 문서의 내부 마크다운 링크가 실제 파일로 연결됨(`[text]`/`(path)` 형식)                                                                                       | 예                         |
| ↳ `check:fabricated-docs`      | 문서에 인용된 라우트, 환경 변수, CLI 명령어, 훅 이름 및 파일 경로가 코드베이스에 존재함. `--strict`를 사용하면 하드 게이트이며, 플래그가 없으면 소프트 실패. | 예(CI에서 `--strict` 사용) |
| `check:cli-i18n`               | CLI 명령 문자열이 모든 i18n 로케일 파일에 존재함                                                                                                             | 예                         |
| `check:openapi-coverage`       | OpenAPI 스펙이 실제 라우트에 대해 래칫으로 설정된 최소 기준 이상을 포함함                                                                                    | 예                         |
| `check:openapi-security-tiers` | `openapi.yaml`의 보안 계층 주석이 `routeGuard.ts` 분류와 일치함                                                                                              | **권고**                   |
| `check:openapi-routes`         | `openapi.yaml`의 모든 경로가 실제 `route.ts`로 연결됨(환각 방지)                                                                                             | 예                         |
| `check:docs-symbols`           | `docs/**/*.md`의 모든 `/api/...` 참조가 실제 `route.ts`로 연결됨(환각 방지)                                                                                  | 예                         |
...
| `check-new-key-coverage` (인라인) | **새로운** 영어 키가 모든 로케일에 반영되는지                                                                                                                         | 예        |
| `check-translation-ratio`         | 로케일별 실제 번역 비율(허용 목록 외에서 영어와 동일한 값 / 자리표시자 / 누락된 리프)이 `config/quality/i18n-translation-baseline.json` + 여유분을 초과하지 않아야 함 | **권고**  |

`fetch-depth: 0`이 필요합니다. 값 드리프트 게이트가 병합 기준점을 기준으로 `en.json`의 diff를 확인하기 때문입니다.

#### `check-ui-value-drift` — 오래된 번역 게이트

다른 게이트가 구조적으로 감지할 수 없는 한 가지 i18n 회귀를 포착합니다. 영어 값이
재작성되었는데 _이전_ 영어에서 파생된 번역이 그대로 남아, 영어 이외 언어 사용자가
확신에 찬 어조로 작성되었지만 이제는 잘못된 문구를 계속 읽게 되는 경우입니다.

이 문제는 실제로 릴리스되었습니다. Antigravity 로그인 도우미가 도입되었을 때(#5203)
`oauthModal.googleOAuthWarning`이 재작성되었지만, **43개 로케일 중 39개**에는 운영자에게
"전체 URL을 복사하여 아래에 붙여 넣으라"고 안내하는 텍스트가 그대로 남아 있었습니다.
해당 제공자에서는 완료할 수 없는 흐름입니다. 다음 이유로 #8463까지 발견되지 않았습니다.

- `sync-ui-keys`는 **없는** 키만 보충하며, **오래된** 키는 절대 갱신하지 않습니다.
...
- `check-translation-drift`는 `docs/i18n/<locale>/**.md` 문서 미러를 추적하며
  `src/i18n/messages/*.json`은 전혀 읽지 않습니다. 2026-09 재동기화 이후 `docs-sync-strict`
  작업에서 차단됩니다. 핵심 문서 편집 → `npm run i18n:run -- --files=<doc>`(섹션 단위로 실행되어 비용이 낮음).

**diff를 인식하며, 기준선에 의존하지 않습니다.** 병합 기준점의 `en.json`과 작업 트리를
비교합니다. 영어 값이 변경된 모든 키에 대해, 어떤 로케일에든 수정되지 않은 번역이
그대로 남아 있으면 오래된 것으로 간주합니다. 이는 의도적으로 **기존 부채를 동결**합니다.
diff만으로는 오래전부터 존재한 번역이 어떤 이전 영어에서 파생되었는지 알 수 없으므로,
게이트는 현재 변경 사항이 건드린 부분만 판단합니다. 대안인 키별 해시 기준선은 약 600 KB의
생성 파일이 필요하고, 이는 기존 최대 기준선의 3배이며 모든 i18n PR에서 변동을 일으킵니다.
...
문자열의 **의미**가 변경된 경우에는 **키 이름을 변경하는 것**이 좋습니다. 새 키는
오래된 번역을 상속할 수 없습니다. #8463에서 사용한 방식입니다.
...
기본 카탈로그를 읽을 수 없는 경우(기본 ref가 없는 얕은 클론)에는
...
| 스크립트               | 검증 항목                                                                                                                              | 차단 여부 |
| ---------------------- | -------------------------------------------------------------------------------------------------------------------------------------- | --------- |
| `check:pr-test-policy` | `src/`, `open-sse/`, `electron/` 또는 `bin/`의 프로덕션 코드를 변경하는 PR에는 테스트가 포함되거나 업데이트되어야 함(엄격 규칙 #8)     | 예        |
| `check:test-masking`   | 변경된 테스트 파일이 순 어서션 수를 줄이거나 `assert.ok(true)` 동어반복을 추가하지 않는지                                              | 예        |
| `check:pr-evidence`    | PR 본문에 변경 사항에 대한 테스트/VPS 증거가 명시되어 있는지(PR 본문을 grep하여 엄격 규칙 #18을 기계적으로 적용 — 취약함, 백로그 참조) | 예        |
...
| `test:vitest:ui` | UI 컴포넌트 테스트 — vitest 러너                    | **차단** — 기존 실패는 `vitest.config.ts`에서 명시적으로 제외되며, 새로운 실패가 발생하면 작업이 실패합니다 |

### 야간 워크플로(예약 실행, 권고용)

이 워크플로는 cron 일정에 따라(그리고 `workflow_dispatch`를 통해) 실행되며, PR에서는 절대 실행되지 않습니다. 모두 권고용입니다.

| 워크플로               | 검증 항목                                                                                                                                                      | 차단 여부  |
| ---------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------- |
| `nightly-property`     | 무작위 시드와 많은 실행 횟수를 사용하는 fast-check 속성 테스트                                                                                                 | **권고용** |
| `nightly-resilience`   | 힙 증가 게이트, 카오스 장애 주입, k6 부하/지속 테스트                                                                                                          | **권고용** |
| `nightly-llm-security` | promptfoo 인젝션 방어(차단 모드) + garak 프로브(제공자 시크릿이 없으면 건너뜀)                                                                                 | **권고용** |
| `nightly-schemathesis` | `docs/openapi.yaml`을 사용하여 실행 중인 OmniRoute를 대상으로 수행하는 OpenAPI 계약 퍼징(schemathesis) — 명세 위반/처리되지 않은 500 오류를 드러냄(Fase 8 B.4) | **권고용** |
| `nightly-mutation`     | 빠른 단위 테스트 레인에 대한 Stryker 변이 테스트 점수 — 살아남은 변이는 취약한 어설션을 드러냄                                                                 | **권고용** |
| `nightly-compat`       | 지원되는 `engines.node` 범위 전반의 Node 엔진 호환성 매트릭스                                                                                                  | **권고용** |
...
모든 `src/i18n/messages/<locale>.json`의 리프 키 집합을 `en.json`과 비교하며, 키가 추가된 시점과
관계없이 누락되거나 불필요한 리프가 하나라도 있으면 실패합니다. `__MISSING__:` 플레이스홀더는
존재하는 것으로 간주됩니다(그 내용은 비율 게이트가 다룰 사안입니다). 이는 두 가지
차이 기반/백분율 게이트를 완전히 보완합니다. `check-ui-keys-coverage`는 로케일별 80% 하한을
적용하지만(약 13,000개 중 43개 키가 누락되어도 여전히 99.7%로 표시됨),
`check-new-key-coverage`는 PR이 `en.json`에 추가하는 키만 판정합니다. 로케일 배치는 브랜치가
분기된 날의 `en.json`을 바탕으로 생성되어 며칠 동안 번역되지만, 그사이 베이스에는 키가 계속
추가됩니다. 배치 PR 자체는 키를 추가하지 않으므로, 배치 1(#13044)이 9개 로케일에서 43개 키가
누락된 상태로 병합되고 배치 2(#13660)가 8개 로케일에서 10개 키가 누락된 상태로 병합되었을 때
두 형제 게이트 모두 아무 경고도 하지 않았습니다(2026-09-15). 실패를 해결하려면
`node scripts/i18n/sync-ui-keys.mjs --locale=<codes> --translate-markers`를 실행하세요. `extra` 리프는
소스에서 해당 키가 제거되었다는 뜻이므로 로케일에서도 삭제하세요. `--warn`은 실패 처리 없이
보고만 합니다. `--catalog=cli`는 `bin/cli/locales`에 동일한 비교를 수행합니다
(`npm run i18n:check-keys:cli`). 두 단계 모두 `i18n-ui-coverage` 작업에 포함됩니다.

#### `check-new-key-coverage` — 새 키 i18n 게이트

`check-ui-value-drift`의 형제 게이트입니다. 후자는 영어 값이 **다시 작성**되었지만 해당 번역은
갱신되지 않은 경우를 감지하고, 이 게이트는 영어 키가 **추가**되었지만 일부 로케일에 전혀
반영되지 않은 경우를 감지합니다.

`check-ui-keys-coverage`는 이런 종류의 문제를 감지할 수 없습니다. 이 게이트는 로케일별
백분율 하한을 적용하므로 약 13,000개의 리프 중 11개 키가 누락되어도 커버리지는 99.9%로
유지됩니다. 언어별 백분율로는 "이 기능이 번역되지 않은 채 출시되었다"는 사실을 표현할 수
없습니다. 기능 전체가 텍스트 없이 새 로케일에 추가되더라도 수치가 전혀 달라지지 않을 수
있습니다.

이 게이트가 반영하는 사건은 다음과 같습니다. Orchestration Canvas의 Phase 3에서는 당시
존재하던 42개 로케일 전체에 11개 키를 번역했습니다. 몇 시간 후 EU 언어 배치(#13044)로
저장소의 로케일 수가 51개가 되었지만, 새로 추가된 9개 로케일(`el`, `et`, `ga`, `hr`, `lt`,
`lv`, `mt`, `sl`, `sr`)에는 해당 키가 전혀 반영되지 않았습니다. `deepMergeFallback`은 누락된
키를 영어로 대체하므로, 빈 UI 대신 번역되지 않은 UI가 나타났습니다. 이는 실제 문제였으며
구조상 아무런 경고도 발생하지 않았습니다.

형제 게이트와 마찬가지로 이 게이트도 **차이를 인식**합니다. 병합 베이스의 영어와 작업 트리를
비교하므로 기존 누락은 그대로 동결되며, 게이트를 활성화하기 위한 마이그레이션이 필요하지
않았습니다. 우회 수단으로 `__MISSING__:<english>`를 사용하면 런타임의 올바른 동작을 유지하면서
번역을 미룰 수 있습니다. `vi`에서는 플레이스홀더가 금지되며
(`tests/unit/i18n-vi-completeness.test.ts`) 실제 번역이 필요합니다.

#### `check-vitest-exclusions` — 보류된 테스트 게이트

`vitest.config.ts`의 `exclude` 목록에 있는 파일은 실행되지 않는 테스트이지만, 트리를 읽는
사람에게는 커버리지처럼 보입니다. 62개 파일이
`// #8618 — pre-existing failure; remove this exclusion when fixed`라는 주석 뒤에 누적되었습니다.
이 목록을 추적하던 이슈 #8618은 2026-08-11에 종료되었지만, 목록은 45개 항목에서 62개 항목으로
늘어났고 새 항목마다 이미 종료된 이슈를 가리키는 주석이 그대로 붙었습니다. 마침내 파일별로
목록을 측정했을 때(#13204), **62개 중 51개가 소스 변경 없이 현재 트리에서 통과했습니다**.

이 게이트는 실제 파일로 해석되는 모든 제외 항목이 (a) 추적 이슈를 명시하고
(b) 측정된 상태와 함께 `config/quality/vitest-exclusions.json`에 나타나도록 요구합니다. 따라서
제외 항목을 추가하면 60개 항목으로 된 배열에 줄 하나를 더하는 대신 전용 파일에 검토 가능한
차이가 생깁니다. 이 게이트는 제외된 테스트를 의도적으로 다시 실행하지 않습니다. 그렇게 하면
약 10분이 걸리며 주기적 작업에서 수행하는 것이 적절합니다. 인벤토리에는 각 테스트를 마지막으로
측정한 시점이 기록됩니다.
