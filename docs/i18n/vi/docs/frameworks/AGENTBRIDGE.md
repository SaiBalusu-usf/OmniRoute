...
AgentBridge là proxy MITM (Man-in-the-Middle, xen giữa) của OmniRoute, có chức năng chặn lưu lượng HTTPS từ các tác nhân AI trong IDE và định tuyến lại thông qua công cụ định tuyến hợp nhất của OmniRoute. Công cụ này hỗ trợ **10 tác nhân IDE** — Antigravity, Kiro, GitHub Copilot, GHE Copilot, OpenAI Codex, Cursor, Zed, Claude Code, Open Code và Trae (đang được nghiên cứu) — giúp OmniRoute trở thành proxy MITM có phạm vi hỗ trợ rộng nhất trên thị trường dành cho các trợ lý lập trình AI.

**Vị trí bảng điều khiển:** `/dashboard/tools/agent-bridge`
**Nhóm thanh bên:** Công cụ (sau Tác nhân đám mây)
**Xem thêm:** [`TRAFFIC_INSPECTOR.md`](./TRAFFIC_INSPECTOR.md) — giám sát toàn bộ lưu lượng bị chặn theo thời gian thực; `docs/security/MITM-TPROXY-DECRYPT.md` (git; không được biên dịch vào `/docs`) — chế độ thu thập giải mã trong suốt TPROXY trên Linux, được điều khiển bởi route `/api/tools/agent-bridge/tproxy`.
...
Khi một tác nhân IDE (ví dụ: GitHub Copilot, Cursor, Claude Code) thực hiện lệnh gọi API, tác nhân đó sẽ kết nối trực tiếp với nhà cung cấp AI thượng nguồn (OpenAI, Anthropic, v.v.). AgentBridge chặn kết nối đó một cách trong suốt ở cấp độ TLS — mà không yêu cầu thay đổi bất kỳ cấu hình nào của tác nhân — và ghi lại yêu cầu để chuyển qua OmniRoute.
...
- **Áp dụng ánh xạ mô hình**: `gemini-3-flash` → `claude-sonnet-4.7` một cách trong suốt ở cấp độ trình xử lý.
- **Quan sát toàn bộ lưu lượng của tác nhân**: mọi yêu cầu bị chặn đều được công bố đến [Trình kiểm tra lưu lượng](./TRAFFIC_INSPECTOR.md).
- **Áp dụng khả năng phục hồi của OmniRoute**: định tuyến kết hợp, bộ ngắt mạch, phương án dự phòng và theo dõi chi phí cũng hoạt động với lưu lượng từ tác nhân IDE.
...
src/mitm/server.cjs  (cổng 443, tiến trình con CJS)
    ...
    │               ├── maskSecrets() trên nội dung/tiêu đề yêu cầu
    │               ├── TrafficBuffer.push() — xuất bản tới Trình kiểm tra lưu lượng
...
Máy chủ MITM cốt lõi chạy dưới dạng tiến trình con CJS của Node.js (để tránh phải viết lại cơ sở mã CJS hiện có). Máy chủ này:
...
- Phân giải tác nhân đích theo tiêu đề Host thông qua registry `targets/index.ts`
- Điều phối tới tầng trình xử lý TypeScript qua HTTP tại `http://127.0.0.1:20128`
...
> là mô hình CA gốc có tính bền vững được bổ sung trong #6684 (`src/mitm/cert/rootCa.ts` +
...
> chỉ giới hạn cho các máy chủ antigravity) mà sự hiện diện của cặp
> `server.crt`/`server.key` đơn thuần trên đĩa biểu thị. **Hành vi di chuyển**: một bản cài đặt mới (không có
> `server.crt` trước đó) sẽ tự động sử dụng mô hình CA gốc; một bản cài đặt đã
...
> **bất kỳ** máy chủ nào có quyền hạn lớn hơn đáng kể so với chứng chỉ lá có SAN cố định cũ, vì vậy
> quá trình chuyển đổi không bao giờ diễn ra âm thầm đối với bản cài đặt đã được tin cậy). Chứng chỉ CA được cài đặt
> vào cùng vị trí `omniroute-mitm.crt` trong kho tin cậy mà chứng chỉ lá cũ từng sử dụng
...
export abstract class MitmHandlerBase {
  ...
  // Các hàm trợ giúp được bảo vệ: fetchRouter, pipeSSE, hookBufferStart, hookBufferUpdate
}
```

Mỗi trình xử lý gọi `hookBufferStart()` trước khi chuyển tiếp proxy và gọi `hookBufferUpdate()` khi hoàn tất. Các hàm này đẩy những mục `InterceptedRequest` vào `globalTrafficBuffer` (xem [Trình kiểm tra lưu lượng](./TRAFFIC_INSPECTOR.md) §4).

### 2.4 Registry đích (`src/mitm/targets/`)
...
Registry (`targets/index.ts`) xuất `ALL_TARGETS` và tạo `DATA_DIR/mitm/targets.json` khi khởi động.

### 2.5 Chuyển tiếp và danh sách bỏ qua (`src/mitm/passthrough.ts`)

**Danh sách bỏ qua** (được kiểm tra trước, có độ ưu tiên cao hơn so với khớp đích):
...
- Các máy chủ bị bỏ qua nhận một đường hầm TCP trong suốt — TLS **không bao giờ bị giải mã**
...
Khi được đặt, biến này cấu hình bộ điều phối toàn cục của `undici` với chứng chỉ CA bổ sung, cho phép AgentBridge kết nối tới các nhà cung cấp thượng nguồn thông qua proxy kết thúc TLS của doanh nghiệp.

### 2.7 Che giấu bí mật (`src/mitm/maskSecrets.ts`)

Trình quét độc lập được triển khai theo phương pháp phòng sạch được áp dụng cho nội dung yêu cầu và các tiêu đề chứa thông tin xác thực
...
- Các token có tiền tố `sk-` / `ak-` / `pk-` (kiểu OpenAI/Anthropic)
- Thông tin xác thực `Authorization: Bearer <token>` theo RFC 6750, với độ ưu tiên dành cho toàn bộ token
- Các token mờ dài thông dụng (≥40 ký tự), bao gồm cả dạng có dấu chấm và dạng có phần đệm

`sanitizeHeaders()` chuyển tên được giữ lại thành chữ thường, nối các giá trị mảng theo cách xác định, loại bỏ
danh sách từ chối dùng chung cho các tiêu đề theo từng chặng/định khung (bao gồm xác thực proxy), che hoàn toàn `cookie` và
`set-cookie`, đồng thời chuyển các giá trị thông tin xác thực cho trình quét xử lý.
...
| Tạo lại chứng chỉ     | Tạo cặp khóa CA mới (mọi chứng chỉ hiện có của từng agent sẽ mất hiệu lực) |
...
Một số IDE — đặc biệt là **Antigravity IDE** và các ứng dụng khác bắt nguồn từ Electron / VS Code — đóng gói
môi trường thời gian chạy Node.js riêng, vốn **không tham chiếu kho tin cậy của hệ điều hành** cho các yêu cầu
...
sẽ gặp lỗi TLS — biểu hiện là ứng dụng ở trạng thái _đã đăng xuất_ hoặc hiển thị _"lỗi kết nối"_,
dù nhật ký MITM cho thấy các lệnh gọi khởi tạo của backend trả về `200`. Cần thực hiện hai bước
và cả hai đều quan trọng:

1. Trỏ rõ môi trường thời gian chạy đến CA:
   ```bash
   export NODE_EXTRA_CA_CERTS=/path/to/omniroute-agentbridge-ca.crt
   ```
2. **Khởi chạy IDE từ shell đó.** Việc khởi động từ biểu tượng trên màn hình nền / Dock / menu Start
   **không** kế thừa các biến được xuất từ shell, còn `~/.config/environment.d/*.conf` chỉ được áp dụng sau
   khi đăng nhập mới vào giao diện đồ họa. Trước tiên, hãy thoát hoàn toàn khỏi IDE — khóa đơn thể của Electron
   khiến lần khởi chạy thứ hai chỉ chuyển tiêu điểm sang tiến trình hiện có và môi trường mới bị bỏ qua.

Bước thiết lập độ tin cậy ở cấp hệ điều hành + NSS nêu trên vẫn cần thiết (ngăn xếp mạng Chromium được một số
luồng xác thực sử dụng sẽ đọc kho NSS của từng người dùng và có các pin tĩnh riêng cho `*.googleapis.com` mà
...
Ví dụ về các mục nhập `/etc/hosts` dành cho GitHub Copilot:
...
Sử dụng Bảng ánh xạ mô hình trong từng thẻ agent để xác định các ánh xạ nguồn → đích:
...
Ký tự đại diện `*` ánh xạ mọi mô hình không được nhận dạng tới đích đã chỉ định. Dữ liệu được lưu bền vững trong bảng `agent_bridge_mappings`.

> **Mẹo — khám phá ID mô hình thực của agent.** Một IDE có thể gửi tên mô hình khác với
> nhãn trên giao diện người dùng và những tên này có thể thay đổi giữa các phiên bản chính. Ví dụ: **Antigravity 2** gửi
> `gemini-3.1-pro-low`, `gemini-pro-agent` và `gemini-3.1-flash-lite` qua đường truyền — chứ không phải
> `gemini-2.5-pro` được hiển thị trong tài liệu cũ. Hãy gửi một cuộc trò chuyện khi chưa có ánh xạ phù hợp: MITM
> sẽ ghi vào nhật ký chính xác giá trị `model:` nhận được và chuyển tiếp yêu cầu. Ánh xạ giá trị nguyên văn đó, sau đó
> yêu cầu tiếp theo sẽ được chặn bắt và định tuyến đến đích của bạn.

### 3.5 Thông báo rủi ro

AgentBridge chặn bắt thông tin xác thực (token OAuth, khóa API) mà IDE sử dụng để xác thực với các nhà cung cấp thượng nguồn. Những thông tin này được **che trước khi ghi nhật ký** (xem §2.7) nhưng vẫn hiển thị với lớp MITM của OmniRoute. Lần kích hoạt đầu tiên của mỗi agent sẽ hiển thị một hộp thoại thông báo rủi ro có thể đóng.

### 3.6 Bảo trì & Chẩn đoán

Bảng điều khiển cung cấp thẻ **Bảo trì & Chẩn đoán** (`AgentBridgeMaintenanceCard`, trong `src/app/(dashboard)/dashboard/tools/agent-bridge/components/`) để hiển thị các tuyến MITM vận hành mà trước đây không có giao diện người dùng. Phụ đề của thẻ: _"Tự kiểm tra quy trình chặn bắt, hoàn tác trạng thái hệ thống còn sót lại và di chuyển cấu hình của bạn giữa các máy."_ Các hàm trợ giúp phía máy khách của thẻ nằm trong `src/lib/inspector/agentBridgeMaintenanceApi.ts`.
...
| **Sửa chữa**      | `POST /api/tools/agent-bridge/repair`  | Hoàn tác trạng thái hệ thống MITM mồ côi (các mục giả mạo DNS, CA gốc, proxy hệ thống) còn sót lại do sự cố hoặc SIGKILL. Có tính lũy đẳng — báo cáo "Không có gì cần sửa chữa" khi trạng thái sạch. |
...
**Các mục kiểm tra chẩn đoán** (`summarizeDiagnostics()` trong `src/mitm/inspector/diagnostics.ts`). Route chạy phép thăm dò có hiệu ứng cho từng mục và đưa các giá trị boolean vào bộ tóm tắt thuần túy; kết quả `healthy` duy nhất cùng với gợi ý cho từng lỗi sẽ được trả về:

| Tên mục kiểm tra   | Nội dung xác minh                                             | Gợi ý khi thất bại                                                                                                                                         |
| ------------------ | ------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `server-running`   | Tiến trình máy chủ MITM đang hoạt động                        | "Máy chủ MITM không chạy. Hãy khởi động máy chủ từ thẻ AgentBridge."                                                                                       |
...
| `cert-exists`      | Chứng chỉ MITM đã được tạo trên đĩa                           | "Chưa có chứng chỉ MITM nào được tạo. Hãy tạo một chứng chỉ từ thẻ AgentBridge."                                                                           |
| `cert-trusted`     | CA gốc MITM có trong kho tin cậy của HĐH                      | "CA gốc MITM không được kho của HĐH tin cậy, vì vậy việc chặn bắt TLS sẽ thất bại. Hãy tin cậy chứng chỉ từ thẻ AgentBridge."                              |
...
**Biểu ngữ trạng thái mồ côi:** khi trang phát hiện trạng thái còn sót lại do sự cố (giả mạo DNS / CA / proxy hệ thống), thẻ sẽ hiển thị một biểu ngữ màu hổ phách — _"Một phiên trước đã để lại trạng thái hệ thống (giả mạo DNS, CA hoặc proxy hệ thống). Hãy chạy Sửa chữa để dọn dẹp."_ — và làm nổi bật nút **Sửa chữa**. `Repair` là thành phần tương đương ở tầng ứng dụng với cờ `--cleanup` của ProxyBridge (nó ủy quyền cho `repairMitm()` trong `src/mitm/manager.ts`).

> CA gốc MITM được giữ nguyên trạng thái cài đặt qua các lần dừng/khởi động để tránh
> các lời nhắc sudo lặp lại (hành vi tương tự như mitmproxy/Charles), vì vậy việc xóa CA là một
> hành động **Xóa CA** tường minh thay vì tự động diễn ra khi dừng.

### 3.7 Nhập/xuất cấu hình có thể di chuyển

AgentBridge có thể tuần tự hóa trạng thái **mà người vận hành có thể điều chỉnh** thành một khối JSON có phiên bản để có thể sao chép thiết lập giữa các máy. Bộ tuần tự hóa là `src/lib/inspector/configPortability.ts` (`exportConfig()` / `importConfig()`), được xác thực bởi `AgentBridgeConfigSchema`.

Dữ liệu xuất bao gồm chính xác ba phần (các giá trị mặc định tích hợp sẵn được chủ ý **KHÔNG** xuất, do đó việc nhập sẽ không bao giờ sao chép trùng hoặc xung đột với chúng):
...
**Hành vi nhập** (`POST /api/tools/agent-bridge/config`): các mẫu bỏ qua và ánh xạ theo từng agent được **thay thế toàn bộ**; các máy chủ tùy chỉnh được thêm vào theo cách **có tính lũy đẳng** (`INSERT OR IGNORE`). Phản hồi báo cáo số lượng mục thuộc từng loại đã được áp dụng:

```jsonc
{ "ok": true, "bypassPatterns": 1, "customHosts": 1, "agents": 1 }
```

Những nội dung **KHÔNG** có trong cấu hình: trạng thái chạy của máy chủ, đường dẫn chứng chỉ, trạng thái DNS theo từng agent, đường dẫn CA thượng nguồn và các thiết lập TPROXY — đó là trạng thái máy chủ/thời gian chạy, không phải các tùy chọn có thể di chuyển.
...
Đối với các tác nhân 1–8, AgentBridge cố gắng tự động phát hiện việc cài đặt IDE:
...
Việc phát hiện sử dụng các đường dẫn dành riêng cho hệ điều hành và kiểm tra tệp nhị phân (ví dụ: `code --list-extensions | grep github.copilot` cho Copilot, `~/.config/antigravity/` cho Antigravity).
...
| ---------------------------------------- | ---------------------------------------------------------------------------------------------------- |
| **#12** `sanitizeErrorMessage`           | Mọi lỗi của trình xử lý đều được làm sạch trước khi phản hồi hoặc đưa vào bộ đệm                     |
| **#13** Truyền biến môi trường cho shell | Việc chỉnh sửa `/etc/hosts` sử dụng tùy chọn `env` — không nội suy chuỗi đối với đường dẫn           |
...
Danh sách bỏ qua đảm bảo rằng các tổ chức tài chính, nhà cung cấp OAuth/SSO và những máy chủ nhạy cảm khác **không bao giờ bị giải mã**. Lưu lượng TLS của chúng đi qua dưới dạng đường hầm TCP trong suốt — OmniRoute không bao giờ nhìn thấy văn bản thuần.
...
- Trên mọi tiêu đề trước khi ghi nhật ký hoặc phát quảng bá

Các mẫu: token có tiền tố `sk-`/`ak-`/`pk-`, token `Bearer` và token chung có độ dài ≥40 ký tự.
...
- **Cổng 443 yêu cầu đặc quyền**: Trên Linux, AgentBridge cần `setcap 'cap_net_bind_service=+ep'` trên tệp nhị phân Node hoặc chạy thông qua `authbind`. Trình hướng dẫn thiết lập hiển thị hướng dẫn dành riêng cho hệ điều hành.
- **Cần khởi động lại IDE**: Sau khi chuyển hướng DNS, IDE phải được khởi động lại để cơ chế phân giải máy chủ mới có hiệu lực.
- **Token OAuth được mã hóa cứng**: Một số tác nhân (Kiro, Antigravity) lưu trữ cục bộ token làm mới OAuth. Các token này trong suốt đối với AgentBridge — AgentBridge nhìn thấy token Bearer trong mỗi yêu cầu và token đó được che giấu trước khi ghi nhật ký.
- **Giao diện Electron cần `NODE_EXTRA_CA_CERTS`**: Các IDE có giao diện chạy trên môi trường thực thi Node/Electron đóng gói sẵn sẽ bỏ qua kho tin cậy của hệ điều hành/NSS và phải được khởi chạy từ shell với `NODE_EXTRA_CA_CERTS` đã được đặt (xem §3.2). Triệu chứng khi thiếu: phần backend của IDE xác thực thành công (MITM hiển thị các mã `200`) nhưng giao diện người dùng vẫn ở trạng thái chưa đăng nhập.
- **Nhiều bản cài đặt của cùng một IDE hoạt động độc lập**: một bản cài đặt hệ thống (ví dụ: `/usr/share/antigravity/antigravity`) và một bản cài đặt "Full" cục bộ của người dùng (ví dụ: `~/AntigravityIDE_Full/antigravity-ide`) là các tiến trình riêng biệt với môi trường thực thi riêng — mỗi tiến trình phải được khởi chạy lại với CA đã được chèn. Hãy xác định bản nào đang chạy bằng đường dẫn tệp nhị phân trước khi khởi chạy lại.
- **Danh tính do lời nhắc hệ thống của tác nhân thiết lập, không phải mô hình được định tuyến**: khi bạn ánh xạ lại mô hình của tác nhân sang một nhà cung cấp khác, phản hồi vẫn tự nhận danh tính gốc của tác nhân (ví dụ: Antigravity trả lời "Tôi được vận hành bởi Gemini") vì IDE chèn thông tin đó vào lời nhắc hệ thống. Hãy xác nhận backend thực tế trong `call_logs` / `proxy_logs` (`provider`, `model`, `target_format`), thay vì hỏi mô hình đó là ai.
...
sudo fuser -k 443/tcp  # buộc dừng (hãy thận trọng khi sử dụng)
```
...
2. Một số ứng dụng duy trì kho tin cậy riêng (Firefox, Chrome trên Linux). Chạy lại "Tin cậy chứng chỉ" và kiểm tra kho chứng chỉ dành riêng cho NSS/Firefox.
3. Khởi động lại IDE sau khi thiết lập tin cậy — các phiên TLS đang diễn ra vẫn sử dụng trạng thái tin cậy cũ.
...
mở ra ở trạng thái **đã đăng xuất** hoặc hiển thị lỗi xác thực/kết nối, mặc dù nhật ký MITM cho thấy các
lệnh gọi khởi động (`loadCodeAssist`, `fetchAvailableModels`, …) trả về `200`.

Nguyên nhân: **môi trường chạy Node/Electron đi kèm của IDE bỏ qua kho tin cậy của hệ điều hành**. Phần phụ trợ
gốc (một máy chủ ngôn ngữ Go) tin cậy CA của hệ điều hành và xác thực thành công, nhưng giao diện Electron
thì không — vì vậy giao diện người dùng cho rằng nó đang ngoại tuyến.

Cách khắc phục (cả hai bước): xuất `NODE_EXTRA_CA_CERTS=<ca.crt>` **và khởi chạy lại IDE từ chính
shell đó**, không phải từ biểu tượng trên màn hình nền. Trước tiên, hãy thoát hoàn toàn khỏi IDE — khóa đơn thể của Electron khiến
lần khởi chạy thứ hai chỉ đưa tiến trình hiện có ra phía trước và môi trường mới bị bỏ qua. Xem §3.2.
Điều này tương ứng với một báo cáo thượng nguồn đang mở, trong đó một agent độc lập hoạt động qua MITM nhưng biến thể
...
Tính năng tự động phát hiện sử dụng các đường dẫn cài đặt phổ biến. Nếu việc phát hiện thất bại nhưng IDE đã được cài đặt:

- Kiểm tra xem tệp nhị phân IDE có nằm ở vị trí không chuẩn hay không
- Trình hướng dẫn thiết lập vẫn hoạt động — việc phát hiện thất bại chỉ có nghĩa là huy hiệu sẽ không hiển thị đường dẫn cài đặt

### Lỗi trình xử lý (không thể truy xuất từ thượng nguồn)

Nếu AgentBridge chặn được lưu lượng nhưng tất cả yêu cầu đều thất bại:

1. Xác minh ít nhất một nhà cung cấp đã được kết nối tại `/dashboard/providers`
2. Kiểm tra nhật ký máy chủ OmniRoute: `APP_LOG_LEVEL=debug` trong `.env`
3. Xác minh `OMNIROUTE_BASE_URL` trỏ đến đúng điểm cuối của bộ định tuyến (mặc định: `http://127.0.0.1:20128`)

---

## §7 Tham chiếu API

Tất cả các tuyến đều là `LOCAL_ONLY` (chỉ dành cho loopback, được thực thi trước khi xác thực) và `SPAWN_CAPABLE`. Xem `src/server/authz/routeGuard.ts`.

Đường dẫn cơ sở: `/api/tools/agent-bridge/`

| Phương thức         | Đường dẫn                                      | Mô tả                                                                                                                          |
| ------------------- | ---------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ |
| GET                 | `/api/tools/agent-bridge/state`                | Trạng thái máy chủ toàn cục + kết quả phát hiện/trạng thái của từng agent                                                      |
| GET                 | `/api/tools/agent-bridge/agents`               | Liệt kê các agent đã đăng ký (id, tên, host, tính khả dụng, trạng thái)                                                        |
...
| POST                | `/api/tools/agent-bridge/repair`               | Hoàn tác trạng thái hệ thống MITM còn sót lại (xem §3.6)                                                                       |
| GET                 | `/api/tools/agent-bridge/config`               | Xuất cấu hình JSON có thể di chuyển (xem §3.7)                                                                                 |
| POST                | `/api/tools/agent-bridge/config`               | Nhập cấu hình JSON có thể di chuyển (xem §3.7)                                                                                 |
| GET                 | `/api/tools/agent-bridge/upstream-ca`          | Lấy đường dẫn CA ngược dòng đã cấu hình                                                                                        |
| POST                | `/api/tools/agent-bridge/upstream-ca`          | Xác thực + lưu đường dẫn CA ngược dòng                                                                                         |
| POST                | `/api/tools/agent-bridge/upstream-ca/test`     | Chỉ xác thực (chạy thử) đường dẫn CA ngược dòng — không lưu                                                                    |
...
Trae là một trợ lý lập trình AI tương đối mới. Trước khi triển khai trình xử lý:

1. Xác định tệp nhị phân/tiện ích mở rộng trên các marketplace của VS Code / JetBrains hoặc dưới dạng ứng dụng độc lập
2. Ghi lại lưu lượng bằng mitmproxy để khám phá các máy chủ API và cấu trúc endpoint
3. Xác định cơ chế xác thực
4. Đánh giá có nên tiếp tục hay không dựa trên TOS và khả năng khám phá API

Cho đến khi quá trình điều tra hoàn tất, thẻ Trae trên dashboard sẽ hiển thị huy hiệu "Đang điều tra" cùng liên kết "Báo cáo tính khả thi". Phần khung trình xử lý tại `src/mitm/handlers/trae.ts` sẽ đưa ra lỗi có cấu trúc `Chưa được triển khai`.

### Các agent tồn đọng (yêu cầu MITM — không hỗ trợ URL cơ sở tùy chỉnh)

Các công cụ sau không hỗ trợ URL cơ sở tùy chỉnh trong phiên bản hiện tại, khiến MITM trở thành phương thức chặn duy nhất. Việc đánh giá tính khả thi vẫn đang chờ xử lý:
