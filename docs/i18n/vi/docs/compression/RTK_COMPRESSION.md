...
Nén RTK là công cụ nén nhận biết lệnh của OmniRoute dành cho đầu ra terminal và công cụ. Công cụ này
được thiết kế cho các phiên tác nhân lập trình, nơi phần lớn mức tăng ngữ cảnh đến từ nhật ký kiểm thử, đầu ra bản dựng,
thông tin dư thừa từ trình quản lý gói, bản ghi shell, đầu ra Docker, đầu ra git và dấu vết ngăn xếp.

RTK có thể chạy trực tiếp với `defaultMode: "rtk"` hoặc làm bước đầu tiên trong một pipeline xếp chồng, thường là:

```txt
rtk -> caveman
```

Thứ tự đó nén đầu ra máy nhiều nhiễu trước, sau đó cho phép Caveman cô đọng phần văn bản còn lại.

RTK thượng nguồn báo cáo mức tiết kiệm `60-90%` đối với đầu ra lệnh. Phiên mẫu trong README của nó giảm từ
`~118,000` token tiêu chuẩn xuống còn `~23,900` token RTK, tức là tiết kiệm `79.7%` (`~80%`). OmniRoute sử dụng
mức trung bình thượng nguồn đó để tính mức tiết kiệm xếp chồng với tính năng nén đầu vào của Caveman:
...
trước khi chọn bộ lọc. Các bộ lọc cũng có thể khớp theo mẫu lệnh hoặc biểu thức chính quy của đầu ra khi một lớp lệnh
không đủ.
...
Trong cùng một phạm vi, các bộ lọc RTK TOML schema v1 được ưu tiên hơn các bộ lọc JSON của OmniRoute. Các biểu thức TOML
`match_command` được kiểm tra trước khi khớp loại lệnh để một bộ lọc dành riêng cho lệnh đã nhập có thể ghi đè
một bộ lọc rộng hơn trong phạm vi đó. Phạm vi dự án vẫn được ưu tiên hơn phạm vi toàn cục,
bất kể định dạng tệp.

Các bộ lọc dự án được cố ý kiểm soát bằng mức độ tin cậy vì bộ lọc biểu thức chính quy có thể thay đổi cách đầu ra công cụ
được hiển thị cho các tác nhân. Một tệp bộ lọc dự án được chấp nhận khi một trong các điều kiện sau đúng:
...
Các hàm băm là riêng biệt: `filtersSha256` xác nhận độ tin cậy của `.rtk/filters.json`, trong khi `filtersTomlSha256`
xác nhận độ tin cậy của `.rtk/filters.toml`. Việc chỉnh sửa một trong hai tệp chỉ làm mất hiệu lực mục tin cậy tương ứng của chính tệp đó. Các tệp toàn cục
do quản trị viên cài đặt và sử dụng cơ chế tin cậy bộ lọc toàn cục hiện có.
...
Các trường được hỗ trợ là `description`, `match_command`, `strip_ansi`, `filter_stderr`,
...
Các trường không xác định, biểu thức chính quy không hợp lệ hoặc không an toàn, quy tắc loại bỏ/giữ lại đồng thời, tệp lớn hơn
1 MiB và tham chiếu đến các bộ lọc không xác định đều bị từ chối. Một tệp có kiểm thử nội tuyến không đạt có thể được
xác thực để kiểm tra nhưng không thể được cài đặt hoặc tải. Lỗi tải tệp tùy chỉnh vẫn hoạt động theo cơ chế
mở khi lỗi: tệp không hợp lệ sẽ bị bỏ qua và các bộ lọc còn lại tiếp tục hoạt động.

OmniRoute nhận đầu ra của công cụ sau khi máy khách đã thu thập đầu ra đó, vì vậy `filter_stderr = true`
không thể thay đổi quá trình thu thập tiến trình. Trường này được chấp nhận dưới dạng không thực hiện thao tác nào và quá trình xác thực trả về cảnh báo.
Điều này được mô tả có chủ đích là **khả năng tương thích với RTK TOML schema v1**, không phải khả năng tương thích đầy đủ
với tệp thực thi RTK, các hook shell, các triển khai lệnh Rust hoặc bố cục kho lưu trữ tin cậy của RTK.

Chế độ xem RTK nâng cao của bảng điều khiển chấp nhận TOML được dán hoặc tải lên. Việc xác thực chỉ đọc.
Quá trình cài đặt ghi `DATA_DIR/rtk/filters.toml` theo cách nguyên tử với các quyền hạn chế và làm mới
danh mục bộ lọc đang hoạt động mà không cần khởi động lại. Việc thay thế một tệp hiện có yêu cầu xác nhận `overwrite`
rõ ràng và trước tiên sẽ tạo `DATA_DIR/rtk/filters.toml.bak`.

## DSL bộ lọc

Các bộ lọc sử dụng JSON schema được mô tả trong [Định dạng quy tắc nén](./COMPRESSION_RULES_FORMAT.md).
Môi trường thời gian chạy áp dụng các giai đoạn này theo thứ tự:
...
| Trường                       | Mục đích                                                              |
| ---------------------------- | --------------------------------------------------------------------- |
| `rules.stripAnsi`            | Loại bỏ các chuỗi màu/điều khiển của terminal trước khi so khớp       |
| `rules.filterStderr`         | Chuẩn hóa các tiền tố stderr phổ biến trước khi so khớp/lọc           |
| `rules.replace`              | Áp dụng các phép thay thế biểu thức chính quy theo thứ tự             |
| `rules.matchOutput`          | Trả về bản tóm tắt ngắn gọn khi đầu ra khớp với một điều kiện đã biết |
...
Các bộ lọc tích hợp sẵn được yêu cầu phải bao gồm các mẫu `tests[]` nội tuyến. Các bộ lọc tùy chỉnh cũng nên bao gồm
chúng, đặc biệt khi được chia sẻ giữa các dự án.

## Loại bỏ dòng trùng lặp (hai lớp)
...
1. **`deduplicate` theo từng bộ lọc (tùy chọn bật, mặc định là `false`).** Một bộ lọc có thể đặt `rules.deduplicate: true`
   để thu gọn các dòng trùng lặp liên tiếp _trong phần đầu ra khớp của bộ lọc đó_, trước khi cắt bớt.
   Quá trình này chạy bên trong `lineFilter.ts`. Đối với các bộ lọc cũ, tính năng này được tự động bật khi bộ lọc định nghĩa
   `collapsePatterns`. Schema: `deduplicate: z.boolean().default(false)` trong
   `open-sse/services/compression/engines/rtk/filterSchema.ts`.
2. **`deduplicateThreshold` trên toàn engine (mặc định là `3`).** Sau khi tất cả các bộ lọc chạy xong, engine sẽ thu gọn
...
Lượt xử lý theo từng bộ lọc chạy trước (bên trong bộ lọc), còn lượt xử lý trên toàn engine chạy sau cùng (trên đầu ra đã được nối),
do đó hai lớp phối hợp với nhau mà không tính trùng.

## Nhóm dòng (`enableGrouping`)

Khi `rtkConfig.enableGrouping` là `true` (mặc định là `false`), RTK chạy thêm một lượt `groupSimilarLines`
trên kết quả sau khi loại bỏ trùng lặp để thu gọn các chuỗi dòng liên tiếp _gần tương đương_ (không giống hệt nhau ở cấp byte).
`rtkConfig.groupingThreshold` (mặc định là `3`) là độ dài chuỗi tối thiểu để kích hoạt
việc nhóm. Đây là đối tác về mặt cấu trúc của `deduplicateThreshold`: loại bỏ trùng lặp xử lý các dòng lặp lại chính xác,
còn nhóm xử lý các dòng "có cùng hình dạng với một vài khác biệt nhỏ". Cả hai cờ đều là một phần của JSON `rtkConfig`
được lưu trong bảng `key_value` (xem phần Cấu hình ở trên), vì vậy thiết lập này được duy trì sau khi khởi động lại.
...
- `stripCodeComments` (mặc định là `false`) — tùy chọn bật. Khi là `true`, RTK loại bỏ chú thích khỏi các khối JavaScript
  và TypeScript có hàng rào. Trước đây, cờ này được đọc nhưng chưa từng được áp dụng, vì vậy giá trị mặc định vẫn là
  "giữ nguyên" để tránh một thay đổi ngầm trong môi trường sản xuất.
- `preserveDocstrings` (mặc định là `true`) — khi loại bỏ chú thích, các chú thích khối JSDoc/`/** … */` được
  giữ lại (chúng chứa tài liệu API có giá trị lớn hơn số byte mà chúng chiếm). Đặt thành `false` để loại bỏ
  cả những chú thích này.

Việc loại bỏ chú thích được triển khai trong `open-sse/services/compression/engines/rtk/codeStripper.ts`. Nó sử dụng
**trình phân tích cú pháp TypeScript** (không phải biểu thức chính quy) để các literal chuỗi, template và regex không bao giờ bị nhầm
là chú thích, đồng thời hủy toàn bộ thao tác khi phát hiện JSX (để các chú thích trong vùng chứa biểu thức JSX không bao giờ
bị làm hỏng). Hiện tại, việc loại bỏ chú thích chỉ áp dụng cho **JavaScript và TypeScript** — các
ngôn ngữ khác trong tập `CodeLanguage` của trình loại bỏ (Python, Rust, Go, Ruby, Java) có chức năng thu gọn dòng trống và
khoảng trắng nhưng không loại bỏ chú thích. Lượt xử lý khối đã loại bỏ chú thích được gắn thẻ `rtk:code-strip` trong
`rulesApplied`.

> **Lưu ý — GCF / mã hóa dạng bảng là một engine riêng biệt.** RTK **không** chứa bộ mã hóa JSON dạng bảng/cột
> "GCF" (Graph Compact Format). Bộ mã hóa đó — vốn thay thế bộ mã hóa `omni-tabular`
> cũ hơn — nằm trong engine **headroom**
> (`open-sse/services/compression/engines/headroom/`, với codec được tích hợp sẵn bên dưới
> `headroom/gcf/`). Nó không liên quan đến pipeline bộ lọc RTK được mô tả tại đây.

## Cấu hình

Các thiết lập toàn cục có sẵn thông qua `/api/settings/compression`. Các thiết lập dành riêng cho RTK cũng
...
`enabledFilters` và `disabledFilters` sử dụng ID bộ lọc, ví dụ như `test-vitest` hoặc `git-diff`.

Cấu trúc đầy đủ của `rtkConfig` được định nghĩa bởi `RtkConfig` / `DEFAULT_RTK_CONFIG` trong
`open-sse/services/compression/types.ts`. Toàn bộ đối tượng được lưu trữ bền vững dưới dạng một giá trị JSON duy nhất trong
...
đều được ghi và đọc lại thông qua cùng một kho lưu trữ và vẫn tồn tại sau khi khởi động lại.

| Khóa                   | Mặc định | Mục đích                                                                                  |
| ---------------------- | -------- | ----------------------------------------------------------------------------------------- |
| `deduplicateThreshold` | `3`      | Toàn bộ engine: số dòng liên tiếp giống hệt nhau tối thiểu để thu gọn (giới hạn từ 2–100) |
...
| `/api/context/rtk/import`          | POST        | Xác thực hoặc cài đặt tệp schema RTK TOML v1        |
| `/api/context/rtk/test`            | POST        | Xem trước khả năng nén RTK cho một nội dung văn bản |
...
Payload xem trước quá trình nén:
...
Các tuyến quản lý yêu cầu xác thực quản lý bảng điều khiển hoặc chính sách khóa API tương ứng.

Payload xác thực RTK TOML:
...
Sử dụng `"action": "install"` để cài đặt tệp đã xác thực trên toàn hệ thống. Chỉ thêm `"overwrite": true`
sau khi xem xét và xác nhận việc thay thế một tệp toàn cục hiện có.

## Khôi phục đầu ra thô

RTK thường chỉ trả về văn bản đã nén. Để gỡ lỗi, `rawOutputRetention` có thể giữ lại đầu ra thô
đã được che giấu:
...
| `always`   | Giữ lại mọi đầu ra thô đã nén của RTK, sau khi được che giấu |
...
Thông tin bí mật được che giấu trước khi lưu trữ lâu dài, bao gồm các bearer token phổ biến, khóa API, token Slack,
khóa truy cập AWS và các giá trị kiểu phép gán `token=...`, `secret=...`, `password=...`. Dữ liệu phân tích
chỉ lưu id con trỏ, kích thước và siêu dữ liệu hàm băm.

## Cổng xác minh

Cổng xác minh tập trung chạy các kiểm thử bộ lọc nội tuyến tích hợp sẵn mà không gọi các lệnh shell bên ngoài:
...
Chạy cổng nén toàn diện trước khi phát hành:
...
4. Bổ sung phạm vi kiểm thử phát hiện lệnh khi giới thiệu một lớp đầu ra mới.
5. Chạy cổng xác minh và cổng RTK toàn diện.
6. Nếu bộ lọc chỉ dành riêng cho dự án, hãy commit `.rtk/filters.json` và chỉ làm mới `.rtk/trust.json` sau khi xem xét.

---

## Các mức cường độ (v3.8.16+)

RTK hỗ trợ **3 mức cường độ** cân bằng giữa **độ mạnh của quá trình nén** và **độ an toàn**. Mức này được thiết lập qua `config.intensity` trong cấu hình engine.

### 3 mức

| Mức                   | Ngưỡng cắt ngắn  | Tiết kiệm token | Rủi ro     | Phù hợp nhất với                            |
| --------------------- | ---------------- | --------------- | ---------- | ------------------------------------------- |
| `minimal`             | 24 dòng mỗi phần | ~20-40%         | Rất thấp   | Môi trường production có ngữ cảnh trọng yếu |
| `standard` (mặc định) | 24 dòng mỗi phần | ~50-70%         | Thấp       | Các phiên lập trình hằng ngày               |
| `aggressive`          | 16 dòng mỗi phần | ~70-90%         | Trung bình | Phiên dài, mức tiết kiệm tối đa             |

### Vị trí diễn ra việc cắt ngắn

Ngưỡng cắt ngắn ảnh hưởng đến `lineFilter.ts`:
...
Cả **phần đầu** và **phần cuối** của mỗi mục đều được giữ lại; nội dung ở giữa bị loại bỏ khi việc cắt ngắn được kích hoạt.

### Nội dung được giữ lại so với nội dung bị cắt
...
| Đầu ra thông thường (nhật ký info) | 🟡 được thu gọn | 🟡 được thu gọn | ❌ bị loại bỏ   |
...
`CompressionEngine` và không có phương thức `updateConfig`. Thay vào đó, hãy cập nhật cấu hình của engine
thông qua helper registry:
...
if (!result.passed) {
  console.error("Bộ lọc không vượt qua kiểm tra ở cường độ aggressive");
}
```
...
Thư mục `engines/rtk/filters/` chứa **hơn 49 tệp JSON bộ lọc tích hợp sẵn**. Bạn có thể thêm bộ lọc riêng để nén đầu ra từ các công cụ tùy chỉnh không được hỗ trợ theo mặc định.

### Lược đồ bộ lọc (Zod)

```ts
{
  "id": "string",                      // Bắt buộc. Mã định danh bộ lọc (kebab-case, ví dụ: "python-traceback")
  ...
  "match": {
    ...
    "patterns": ["string"],            // Các biểu thức chính quy để khớp với đầu ra
    "outputTypes": ["string"]          // Các lớp đầu ra đã phát hiện (ví dụ: "test-failure")
  },
  "rules": {
    ...
    "matchOutput": [                   // Dừng sớm khi mẫu khớp (mặc định: [])
      {
        "pattern": "regex",
        "message": "bản tóm tắt ngắn",
  ...
  "preserve": {
    "errorPatterns": ["string"],       // Các mẫu luôn phải được giữ lại (mặc định: [])
  ...
  "tests": [                           // Các kiểm thử nội tuyến để xác minh (mặc định: [])
    {
      ...
      "input": "đầu ra mẫu",           // Bắt buộc. Văn bản đầu vào mẫu
      "expected": "đầu ra mong đợi",   // Bắt buộc. Đầu ra nén mong đợi
      "command": "lệnh tùy chọn"       // Tùy chọn. Ngữ cảnh lệnh
    }
  ]
}
```
...
{
  "id": "python-traceback",
  "label": "Bộ lọc traceback của Python",
  "description": "Nén traceback của Python để chỉ giữ lại các vị trí tệp/dòng thiết yếu và loại lỗi",
  "category": "test",
  "priority": 60,
  "match": {
...
Đặt tệp vào một vị trí được nhận diện:
...
Các bộ lọc được tải tự động khi khởi động thông qua `loadRtkFilters()` trong `open-sse/services/compression/engines/rtk/filterLoader.ts`. Trình tải tìm các bộ lọc từ:
...
// Tùy chọn: customFiltersEnabled (tải bộ lọc của người dùng/dự án, mặc định bật),
...
Các bộ lọc được xác thực theo lược đồ Zod khi tải. Bộ lọc có cấu trúc không hợp lệ sẽ không tải được và một lỗi sẽ được ghi lại:

```
RTK_FILTER_LOADER: bộ lọc "my-filter" không vượt qua bước xác thực:
  - rules.replace.0.pattern: Biểu thức chính quy không hợp lệ
  - match.commands: không được để trống
```

Để xác thực tất cả các bộ lọc đã cài đặt, hãy gọi `runRtkFilterTests()`, được xuất từ `open-sse/services/compression/engines/rtk/verify.ts`.

### Các phương pháp hay nhất

1. **Luôn bao gồm `tests[]`** — chúng chứng minh bộ lọc của bạn hoạt động và ngăn lỗi tái diễn
2. **Sử dụng `matchOutput` cho các trường hợp dừng sớm** — nếu một dòng duy nhất đã truyền tải đầy đủ nội dung, hãy thay thế toàn bộ khối
3. **Ưu tiên `keep` hơn `strip`** — các quy tắc rõ ràng kiểu "luôn giữ lại" an toàn hơn "luôn loại bỏ"
4. **Kiểm thử ở cả 3 mức cường độ** — `minimal` không nên thực hiện thay đổi nào, còn `aggressive` vẫn phải giữ lại các lỗi
5. **Sử dụng trường `unless`** — bảo vệ các trường hợp dừng sớm bằng điều kiện "không kích hoạt nếu có X"

---

## Khôi phục đầu ra thô & Cổng xác minh

Khi RTK nén đầu ra ở mức cao, bạn có thể **khôi phục văn bản gốc** để gỡ lỗi, kiểm tra hoặc phát lại.

### Cách hoạt động của việc khôi phục đầu ra thô
...
| Đầu ra thô được lưu   | ~50-500KB     | ~500KB-5MB     |
...
const raw = readRtkRawOutput(pointerId); // pointerId từ thống kê nén
if (raw) {
...
1. Mọi bộ lọc đều tải được và vượt qua xác thực schema
2. Mọi mục trong `tests[]` đều tạo ra đầu ra như mong đợi
3. Cường độ `minimal` không thực hiện thay đổi nào (giữ nguyên bản gốc, chỉ áp dụng các bộ lọc cấu trúc)
4. Cường độ `aggressive` giữ nguyên lỗi, các kiểm thử thất bại và dấu vết ngăn xếp
5. Đầu ra đã nén không bao giờ lớn hơn đầu vào gốc

- Nguồn: `open-sse/services/compression/engines/rtk/` (63 tệp, ~70KB)

- **Trước khi hợp nhất thay đổi đối với bộ lọc** — luôn đảm bảo các kiểm thử vượt qua
- **Sau khi nâng cấp engine RTK** — schema có thể đã thay đổi
...
- [COMPRESSION_ENGINES.md](./COMPRESSION_ENGINES.md) — Registry engine và các engine tích hợp sẵn
