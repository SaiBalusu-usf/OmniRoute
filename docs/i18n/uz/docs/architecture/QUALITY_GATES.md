...
## Darvozalar roʻyxati (~90 ta skript)

Skriptlar `scripts/check/` (siyosat darvozalari) va `scripts/quality/` (ratchet mexanizmi)
ichida joylashgan. CI uchun asosiy haqiqat manbai — `.github/workflows/ci.yml`.

### Release PR tezkor yoʻli (`quality.yml`)

`.github/workflows/quality.yml` `release/**` ga yoʻnaltirilgan PRlarda ishlaydi. U yoʻllar
boʻyicha filtrlangan tezkor darvozalar va kod oʻzgarishlari uchun bitta tavsiyaviy ishlab chiqarish
tuzilishi signali yordamida hissa qoʻshuvchilar shoxlarining oldinga siljishini taʼminlaydi:

| Topshiriq                                        | Qamrov                                                                                                                                                                                                                                             | Bloklash                                                                                             |
| ------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------- |
| `Build (advisory)`                               | Qoralama boʻlmagan kod PRlari va Mergify navbat shoxlari; Node 24, `npm-ci-retry`, `check:node-runtime`, `OMNIROUTE_USE_TURBOPACK=1` bilan `npm run build`; undan foydalanadigan quyi sifat topshirigʻi boʻlmagani sababli artefakt yuklanmaydi    | **Tavsiyaviy** (`continue-on-error: true`; release PRlar bir hafta barqaror ishlagach olib tashlang) |
| `Docs Gates (fast-path)`                         | Hujjatlar/kod PRlari; API hujjatlari havolalari va barcha hujjatlar                                                                                                                                                                                | Ha                                                                                                   |
| `Fast Quality Gates`                             | Kod PRlari; statik tekshiruvlar, tur tekshiruvi, boshqaruv paneli tur tekshiruvi, taʼsirlangan modul testlari                                                                                                                                      | Ha                                                                                                   |
| `Forgotten sibling tests`                        | Kod PRlari; oʻzgartirilgan modullardan statik isteʼmolchilar va nomzod qardosh testlargacha kuzatuv; barrel va dinamik import yoʻllari havola qilingan ruxsat etilganlar roʻyxati istisnolari bilan tavsiyaviy diagnostika sifatida xabar qilinadi | **Tavsiyaviy**                                                                                       |
| `Vitest (fast-path)`                             | Kod PRlari; tezkor vitest toʻplami                                                                                                                                                                                                                 | Ha                                                                                                   |
| `Unit Tests fast-path`                           | Kod PRlari; 4 qismli modul testlari toʻplami                                                                                                                                                                                                       | Ha                                                                                                   |
| `No new ESLint warnings`                         | Kod PRlari; bostirishlardan xabardor lint himoyasi                                                                                                                                                                                                 | Oʻz manbasi uchun ha, forklarda tavsiyaviy                                                           |
| `Merge integrity (changelog + generated skills)` | Qoralama boʻlmagan PRlar; oʻzgarishlar jurnali va yaratilgan koʻnikmalar sinxronligi                                                                                                                                                               | Oʻz manbasi uchun ha, forklarda tavsiyaviy                                                           |

#### Unutilgan qardosh testlar hisoboti

`npm run check:forgotten-sibling-tests` test taʼsiri xaritasi ortidagi import aniqlagichidan
qayta foydalanadi. Har bir oʻzgartirilgan ishlab chiqarish moduli uchun, agar nomzod test
pull-request farqlarida mavjud boʻlmasa, u deterministik
`oʻzgartirilgan modul/belgi -> statik isteʼmolchi -> nomzod qardosh test` zanjirlarini xabar qiladi.
Markdown xulosasi va JSON natijasi har qanday bloklovchi joriy etishdan oldin kalibrlash uchun
`forgotten-sibling-tests` ish jarayoni artefakti sifatida saqlanadi.

Barrel qayta eksportlari va dinamik importlar faqat aniqlash diagnostikasi hisoblanadi; ular hech
qachon bloklovchi topilma yaratmaydi. Koʻrib chiqilgan istisnolar
`config/quality/forgotten-sibling-allowlist.json` faylida joylashgan. Har bir yozuv isteʼmolchi va
nomzod testni nomlashi, aniq asos keltirishi hamda GitHub muammosi yoki pull requestga havola
berishi kerak. Notoʻgʻri shakllangan yozuvlar yopiq tarzda muvaffaqiyatsiz tugaydi. Istisnolar
oʻchirilgan nomzod testni yoki `.skip`/`.todo` qoʻshadigan farqni bostira olmaydi; tasdiqlarni
zaiflashtirish va boshqa niqoblashlar mustaqil ravishda bloklovchi `check:test-masking`
darvozasi zimmasida qoladi.

### Topshiriq: `lint`

`main` ga yuborilgan har bir PRda ishlaydi. Xatolik yuz bersa, birlashtirishni bloklaydi.

| Skript (`npm run ...`)            | Tekshiradi                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | Bloklovchi                                 |
| --------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------ |
| `check:node-runtime`              | Node.js versiyasi qoʻllab-quvvatlanadigan diapazonda ekanligini                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | Ha                                         |
...
| `check:model-lifecycle`           | Qoʻlda yuritiladigan uchta marshrutlash jadvali repozitoriyga kiritilgan hayotiy sikl snapshoti (#11503) bilan mosligini saqlaydi: `FITNESS_TABLE` (`taskFitness.ts`) `REGISTRY` marshrutlay oladigan, foydalanishdan chiqarilgan hech qanday id uchun ball bermaydi; har bir `BUILT_IN_ALIASES` nishoni `REGISTRY` ichida mavjud va foydalanishdan chiqarilgan idlar snapshotida yoʻq; `REGISTRY` ichida hali ham mavjud boʻlgan har bir foydalanishdan chiqarilgan id boshqa manzilga yoʻnaltirilgan yoki `allowedRetiredInCatalog` ichida koʻrsatilgan; shuningdek, `DEFAULT_DEGRADATION_MAP` ichidagi hech bir manba yoki nishon ushbu snapshotda foydalanishdan chiqarilgan deb belgilanmagan. Bu model ayni paytda faol yuqori oqim tomonidan taqdim etilayotganini isbotlamaydi. Oflayn — qoʻlda `npm run quality:refresh-model-lifecycle` (tarmoq talab qiladi; CI tizimiga ulanmagan) orqali yangilanadigan `config/quality/model-lifecycle.json` bilan taqqoslaydi. `allowedRetiredInCatalog` — bosqichma-bosqich qisqartirish mexanizmi: yozuvni faqat kuzatuv masalasi mavjud boʻlganda qoʻshing. | Ha                                         |
| `check:fetch-targets`             | Mijoz tomonidagi `src/` ichidagi har bir `fetch("/api/...")` haqiqiy `route.ts` fayliga yechiladi                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | Ha                                         |
| `check:deps`                      | Repozitoriydagi har bir `package.json` boʻylab `npm install` orqali oʻrnatiladigan barcha bogʻliqliklar `dependency-allowlist.json` ichida mavjud; yangi versiyasi mahkamlanmagan yoki slopsquatting orqali qalbakilashtirilgan paketlar belgilanadi                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | Ha                                         |
| `audit:deps`                      | `npm audit` (ildiz + electron) — yuqori/kritik darajadagi tavsiyalar yoʻq (osv `check:vuln-ratchet` bilan qisman ustma-ust tushadi; Rationalization Backlog boʻlimiga qarang)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | Ha                                         |
| `check:lockfile`                  | `package-lock.json` yaxlitligi — https registri, yaxlitlik xeshlari, host almashtirishlari yoʻq                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | Ha                                         |
| `check:licenses`                  | Ishlab chiqarish bogʻliqliklari uchun SPDX litsenziyalarining ruxsat etilgan roʻyxati                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | Ha                                         |
| `check:tracked-artifacts`         | Build artefaktlari / commit qilingan `node_modules` ramziy havolalari yoʻq (husky pre-commit jarayonida ham ishga tushadi; pre-push ataylab yengil qoldirilgan — #6716)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | Ha                                         |
| `check:vitest-exclusions`         | Har bir Vitest istisnosi kuzatuv masalasini koʻrsatadi va `config/quality/vitest-exclusions.json` faylida mavjud (#13204)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | Ha                                         |
...
| `check:error-helper`              | Ijrochi/ishlov beruvchilardagi xato javoblari `buildErrorBody()` / `sanitizeErrorMessage()` dan foydalanadi (Qatʼiy qoida #12)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | Ha                                         |
| `check:migration-numbering`       | Migratsiya SQL fayllari ketma-ket raqamlangan, oraliqlar yoki takrorlar yoʻq                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Ha                                         |
| `check:public-creds`              | `publicCreds.ts` faylidan tashqarida literal OAuth `client_id`/`client_secret` yoki Firebase Web kalitlari mavjud emas (Qatʼiy qoida #11)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | Ha                                         |
| `check:db-rules`                  | `src/lib/db/` modullaridan tashqarida bevosita SQL mavjud emas; `localDb.ts` faylidan barrel-importlar mavjud emas (Qatʼiy qoidalar #2/#5)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | Ha                                         |
| `check:known-symbols`             | Dispetcher jadvallarida roʻyxatdan oʻtkazilgan provayder ijrochilari, marshrutlash strategiyalari va tarjimonlar diskdagi fayllarga mos keladi — ajratilgan yoki eʼlon qilinmagan belgilar mavjud emas                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | Ha                                         |
| `check:route-guard-membership`    | Bola jarayonini ishga tushiradigan har bir marshrut `isLocalOnlyPath()` tomonidan tasniflangan (Qatʼiy qoidalar #15/#17)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | Ha                                         |
| `check:test-discovery`            | Repodagi har bir `*.test.ts` / `*.spec.ts` fayli kamida bitta test bajaruvchisi tomonidan aniqlanadi (ratchet: `test-discovery-baseline.json` faylidagi ajratilgan fayllar roʻyxati faqat qisqarishi mumkin)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Ha                                         |
...
| `typecheck:noimplicit:core`       | Qatʼiy `noImplicitAny` — istiqbolga yoʻnaltirilgan; avvaldan mavjud koʻplab chaqiruv joylari hali ham annotatsiyalarni talab qiladi                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | **Tavsiyaviy** (`continue-on-error: true`) |
| `check:dashboard-typecheck`       | `src/app/(dashboard)/**` doirasidagi `tsc` (#7033) — `typecheck:core`ning maxsus tanlangan 27 faylli ruxsat roʻyxati hech qanday dashboard TSX faylini oʻz ichiga olmaydi va `next build` ham uni hech qachon tur tekshiruvidan oʻtkazmaydi (`next.config.mjs` faylida `ignoreBuildErrors: true` oʻrnatilgan), shu sababli u yerdagi yetim identifikator regressiyalari (#6625/#6909) CI uchun koʻrinmas edi. Har bir fayl/TS-kod boʻyicha muzlatilgan xatolar soni bazaviy koʻrsatkichi (`config/quality/dashboard-typecheck-baseline.json`, `check:known-symbols` bilan bir xil eskirganlikni nazorat qilish andozasi) bilan farqlarni solishtiradi — faqat bazaviy miqdordan ortiq YANGI xatolar tekshiruvdan oʻtolmaydi; avvaldan mavjud xato tuzatilganda `--update` yordamida bazaviy koʻrsatkichni pasaytiring.                                                                                                                                                                                                                                                                                        | Ha                                         |

### Vazifa: `quality-gate`

`test-coverage`dan keyin ishlaydi. Xatolik yuz bersa, birlashtirishni bloklaydi.
...
| `quality:ratchet`            | `quality-baseline.json` faylidagi har bir koʻrsatkich yomonlashmagan (ESLint ogohlantirishlari ≤ bazaviy koʻrsatkich; qamrov ≥ bazaviy koʻrsatkich)                                                                        | Ha                    |
| `check:duplication`          | Kod takrorlanishi (jscpd@4) `quality-baseline.json` faylidagi bazaviy koʻrsatkichdan oshmaydi                                                                                                                              | Ha                    |
...
| `check:cognitive-complexity` | Kognitiv murakkablik ratcheti (`eslint-plugin-sonarjs`) — alohida ESLint tekshiruvi; CI ikkalasini yagona `check:complexity-ratchets` bosqichi sifatida ishga tushiradi                                                    | Ha                    |
| `check:dead-code`            | Ishlatilmaydigan eksportlar/fayllar ratcheti (knip) bazaviy koʻrsatkichga nisbatan yomonlashmaydi                                                                                                                          | Ha                    |
| `check:compression-budget`   | Siqish mezoni budjeti — har bir mexanizm uchun token tejalishining minimal chegaralari yomonlashmasligi kerak                                                                                                              | Ha                    |
| `check:type-coverage`        | Turlashtirilgan foiz ratcheti (`type-coverage`) yomonlashmaydi; asosan `typecheck:noimplicit:core`ni qamrab oladi                                                                                                          | Ha                    |
| `check:codeql-ratchet`       | Ochiq CodeQL ogohlantirishlari soni yomonlashmaydi (`gh api` orqali oʻqiydi; tokensiz holatda xavfsiz tarzda oʻtkazib yuboradi) — yangilash davriyligi va qoʻlda ishga tushirish uchun quyidagi "CodeQL ratcheti"ga qarang | Ha                    |

### Vazifa: `quality-extended`

Butun job tavsiyaviy (`continue-on-error: true`). npm asosidagi ratchet tekshiruvlari amalda
bajariladi; tashqi skanerlar `gh release download` orqali oʻrnatiladi va ikkilik fayl
hali ham mavjud boʻlmasa, oʻzini oʻzi oʻtkazib yuboradi (exit 0).

| Skript                   | Tekshiradi                                                                                                                                                                                                                       | Bloklash       |
| ------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------- |
| `check:circular-deps`    | Siklik bogʻliqliklar yoʻqligi (dpdm)                                                                                                                                                                                             | **Tavsiyaviy** |
| `check:bundle-size`      | Bundle hajmi belgilangan chegaradan oshmasligi                                                                                                                                                                                   | **Tavsiyaviy** |
| `check:secrets`          | Maxfiy maʼlumotlarni skanerlash (gitleaks) — ikkilik fayl mavjud boʻlmasa, oʻtkazib yuboradi                                                                                                                                     | **Tavsiyaviy** |
| `check:vuln-ratchet`     | Bogʻliqliklardagi zaifliklar (osv-scanner) ortib ketmasligi — ikkilik fayl mavjud boʻlmasa, oʻtkazib yuboradi                                                                                                                    | **Tavsiyaviy** |
| `check:workflows`        | Workflow lint tekshiruvi (actionlint + zizmor) — ikkilik fayllar mavjud boʻlmasa, oʻtkazib yuboradi                                                                                                                              | **Tavsiyaviy** |
| `check:openapi-breaking` | Ommaviy API shartnomasidagi (`openapi.yaml`) asosiy branchga nisbatan buzuvchi oʻzgarishlar (oasdiff) — `openapiBreaking=N` chiqaradi; oasdiff mavjud boʻlmasa yoki asosiy spetsifikatsiyani aniqlab boʻlmasa, oʻtkazib yuboradi | **Tavsiyaviy** |

### Job: `docs-sync-strict`

`main`ga yuborilgan har bir PRda ishga tushadi. Xatolik yuz bersa, birlashtirishni bloklaydi.

| Skript                         | Tekshiradi                                                                                                                                                                               | Bloklash                     |
| ------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------- |
| `check:docs-all`               | Quyidagi 6 ta quyi gateʼni ketma-ket ishga tushiradigan meta-gate                                                                                                                        | Ha                           |
| ↳ `check:docs-sync`            | CHANGELOG / OpenAPI / llm.txt versiyalarining izchilligi                                                                                                                                 | Ha                           |
...
| ↳ `check:doc-links`            | Hujjatlardagi ichki markdown havolalari haqiqiy fayllarga olib borishi (`[text]`/`(path)` shakli)                                                                                        | Ha                           |
| ↳ `check:fabricated-docs`      | Hujjatlarda keltirilgan routeʼlar, muhit oʻzgaruvchilari, CLI buyruqlari, hook nomlari va fayl yoʻllari kod bazasida mavjudligi. `--strict` orqali qatʼiy gate; flagsiz yumshoq xatolik. | Ha (CIʼda `--strict` orqali) |
| `check:cli-i18n`               | CLI buyruq satrlari barcha i18n locale fayllarida mavjudligi                                                                                                                             | Ha                           |
| `check:openapi-coverage`       | OpenAPI spetsifikatsiyasi haqiqiy routeʼlarning kamida ratchet bilan belgilangan minimal miqdorini qamrab olishi                                                                         | Ha                           |
...
| `check:openapi-routes`         | `openapi.yaml` ichidagi har bir path haqiqiy `route.ts` fayliga olib borishi (uydirmalarga qarshi)                                                                                       | Ha                           |
| `check:docs-symbols`           | `docs/**/*.md` ichidagi har bir `/api/...` havolasi haqiqiy `route.ts` fayliga olib borishi (uydirmalarga qarshi)                                                                        | Ha                           |
| `i18n translation drift`       | i18n locale fayllaridagi tarjima qilinmagan keyʼlar — faqat ogohlantirish                                                                                                                | **Tavsiyaviy**               |
...
| `check-new-key-coverage` (ichki) | **Yangi** inglizcha kalit har bir lokalga yetib borishini                                                                                                                                                                                 | Ha             |
| `check-translation-ratio`        | Har bir lokal uchun haqiqiy tarjima nisbati (ruxsat roʻyxatidan tashqaridagi inglizcha bilan bir xil / toʻldiruvchi / yetishmayotgan barglar) `config/quality/i18n-translation-baseline.json` + ruxsat etilgan ogʻishdan oshmasligi kerak | **Tavsiyaviy** |

`fetch-depth: 0` talab qilinadi — qiymat ogʻishi toʻsigʻi `en.json` faylini birlashtirish bazasi bilan solishtiradi.
...
qayta yoziladi, ammo _avvalgi_ inglizcha qiymatdan olingan tarjimalar oʻzgarishsiz qoladi, natijada
ingliz tilidan boshqa tillardagi foydalanuvchilar ishonchli ohangda yozilgan, ammo endi notoʻgʻri matnni oʻqishda davom etadilar.

Bu amalda relizga chiqib ketgan. Antigravity tizimga kirish yordamchisi qoʻshilganda (#5203)
`oauthModal.googleOAuthWarning` qayta yozilgan; **43 lokaldan 39 tasida** operatorlarga «toʻliq URL manzilini
nusxalab, quyiga joylashtirish» aytilgan matn saqlanib qolgan — bu provayder uchun yakunlab boʻlmaydigan jarayon. Bu
#8463 gacha sezilmagan, chunki:

- `sync-ui-keys` faqat **mavjud boʻlmagan** kalitlarni toʻldiradi, **eskirgan** kalitlarni esa hech qachon yangilamaydi;
...
- `check-translation-drift` `docs/i18n/<locale>/**.md` hujjat koʻzgularini kuzatadi —
  u `src/i18n/messages/*.json` fayllarini umuman oʻqimaydi. 2026-09 qayta sinxronlashdan beri `docs-sync-strict`
  vazifasida bloklovchi: asosiy hujjatni tahrirlang → `npm run i18n:run -- --files=<doc>` (boʻlim darajasida, arzon).

**Diff asosida ishlaydi, tayanch koʻrsatkichga tayanmaydi.** U birlashtirish bazasidagi `en.json` faylini
ishchi daraxt bilan solishtiradi; inglizcha qiymati oʻzgargan har bir kalit uchun hali ham
oʻzgartirilmagan tarjimani saqlayotgan har qanday lokal eskirgan hisoblanadi. Bu ataylab **avvaldan mavjud qarzni muzlatadi** — diff
uzoq vaqtdan beri mavjud tarjima qaysi eski inglizcha matndan kelib chiqqanini koʻrsata olmaydi, shu sababli toʻsiq
faqat joriy oʻzgarish tegadigan qismlarni baholaydi. Muqobil yechim (har bir kalit uchun xesh tayanch koʻrsatkichi)
taxminan 600 KB hajmdagi generatsiya qilingan faylni talab qilgan boʻlardi, bu mavjud eng katta tayanch fayldan 3 baravar katta
va har bir i18n PRʼda oʻzgarib turardi.

Talabni qondirishning ikki usuli mavjud:

1. taʼsirlangan tarjimalarni yangilash yoki
2. ularni `__MISSING__:<new english>` qiymatiga oʻrnatish — shunda bajarilish muhiti tuzatilgan inglizcha matnni
   taqdim etadi (`src/i18n/request.ts::deepMergeFallback`, #7258) va kalit tarjima navbatiga qoʻshiladi.

Agar satrning **maʼnosi** oʻzgargan boʻlsa, **kalit nomini oʻzgartirishni** afzal koʻring: yangi kalit
eskirgan tarjimani meros qilib ololmaydi. #8463 aynan shu yondashuvdan foydalangan.
...
Asosiy katalogni oʻqib boʻlmaganda (asosiy ref mavjud boʻlmagan sayoz
klon) `SKIP reason=base-unresolved` bilan 0 kodini qaytaradi, bu `check-openapi-breaking` xatti-harakatiga mos keladi.
...
Faqat pull requestʼlarda ishga tushadi.

| Skript                 | Tekshiradi                                                                                                                                                            | Bloklovchi |
| ---------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------- |
| `check:pr-test-policy` | `src/`, `open-sse/`, `electron/` yoki `bin/` ichidagi ishlab chiqarish kodini oʻzgartiruvchi PRʼlar testlarni qoʻshishi yoki yangilashi kerak (Qatʼiy qoida #8)       | Ha         |
| `check:test-masking`   | Oʻzgartirilgan test fayllari tasdiqlar sof sonini kamaytirmasligi yoki `assert.ok(true)` tavtologiyalarini qoʻshmasligi kerak                                         | Ha         |
| `check:pr-evidence`    | PR tavsifi oʻzgarish uchun test/VPS dalillarini keltiradi (PR matnini grep orqali tekshirib, Qatʼiy qoida #18 ni avtomatlashtiradi — moʻrt, Backlog boʻlimiga qarang) | Ha         |

### Vazifa: `test-vitest`

`build` vazifasidan keyin ishga tushadi. Xatolik yuz bersa, birlashtirishni bloklaydi.

| Toʻplam          | Tekshiradi                                                         | Bloklovchi                                                                                                                                  |
| ---------------- | ------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------- |
| `test:vitest`    | MCP serveri (110 ta vosita), autoCombo, kesh — vitest bajaruvchisi | Ha                                                                                                                                          |
| `test:vitest:ui` | UI komponentlari testlari — vitest bajaruvchisi                    | **Bloklovchi** — avvaldan mavjud xatolar `vitest.config.ts` faylida aniq chiqarib tashlangan; yangi xatolar vazifani muvaffaqiyatsiz qiladi |

### Tungi ish jarayonlari (rejalashtirilgan, tavsiyaviy)

Ular cron jadvali asosida (va `workflow_dispatch` orqali) ishga tushadi, PRlarda esa hech qachon ishga tushmaydi. Ularning barchasi tavsiyaviydir.
...
| `nightly-resilience`   | Heap o‘sishi nazorati, nosozliklarni chaos usulida kiritish, k6 yuklama/uzoq muddatli testlari                                                                                                                     | **Tavsiyaviy** |
| `nightly-llm-security` | promptfoo inyeksiya himoyasi (bloklash rejimi) + garak tekshiruvlari (provayder siri bo‘lmasa o‘tkazib yuboriladi)                                                                                                 | **Tavsiyaviy** |
| `nightly-schemathesis` | `docs/openapi.yaml` yordamida ishlayotgan OmniRoute tizimiga qarshi OpenAPI shartnomalarini fuzzing qilish (schemathesis) — spetsifikatsiya buzilishlari / qayta ishlanmagan 500 xatolarini aniqlaydi (Fase 8 B.4) | **Tavsiyaviy** |
| `nightly-mutation`     | Tezkor unit-test yo‘lagi bo‘yicha Stryker mutatsion testlash bahosi — omon qolgan mutantlar zaif assertlarni ko‘rsatadi                                                                                            | **Tavsiyaviy** |
| `nightly-compat`       | Qo‘llab-quvvatlanadigan `engines.node` diapazonlari bo‘yicha Node dvigateli moslik matritsasi                                                                                                                      | **Tavsiyaviy** |
...
- Taʼminot zanjiri (kelib chiqish, SBOM, Trivy, Scorecard): [`docs/security/SUPPLY_CHAIN.md`](../security/SUPPLY_CHAIN.md)
...
Har bir `src/i18n/messages/<locale>.json` faylining yakuniy kalitlar toʻplamini `en.json` bilan
taqqoslaydi va kalit qachon qoʻshilganidan qatʼi nazar, birorta yakuniy kalit yoʻq yoki ortiqcha
boʻlsa, tekshiruvni muvaffaqiyatsiz yakunlaydi. `__MISSING__:` toʻldirgichlari mavjud deb
hisoblanadi (ularning mazmuni nisbat darvozasining vazifasi). Bu diff asosidagi/foizli ikki
darvozaning mutlaq toʻldiruvchisidir: `check-ui-keys-coverage` har bir lokal uchun 80 % lik quyi
chegarani talab qiladi (taxminan 13 000 ta kalitdan 43 tasi yoʻq boʻlsa ham, koʻrsatkich 99,7 %
boʻlib qoladi), `check-new-key-coverage` esa faqat PR `en.json` fayliga qoʻshadigan kalitlarni
baholaydi. Lokal toʻplami uning tarmogʻi ajratilgan kundagi `en.json` asosida yaratiladi va asosiy
tarmoqda yangi kalitlar qoʻshilishda davom etayotgan bir paytda tarjima bir necha kun davom etadi;
toʻplam PRʼining oʻzi hech qanday kalit qoʻshmaydi, shu sababli 1-toʻplam (#13044) toʻqqizta
lokalda 43 ta kalit yetishmagan holda va 2-toʻplam (#13660) sakkizta lokalda 10 ta kalit
yetishmagan holda qoʻshilganda (2026-09-15), ikkala qardosh tekshiruv ham jim qoldi. Qizil holatni
`node scripts/i18n/sync-ui-keys.mjs --locale=<codes> --translate-markers` yordamida tuzating;
`extra` yakuniy kalit manbadan olib tashlanganini anglatadi — uni lokaldan oʻchiring. `--warn`
muvaffaqiyatsiz yakunlamasdan hisobot beradi. `--catalog=cli` xuddi shu taqqoslashni
`bin/cli/locales` uchun bajaradi (`npm run i18n:check-keys:cli`); har ikkala qadam
`i18n-ui-coverage` vazifasida joylashgan.

#### `check-new-key-coverage` — yangi kalitlar i18n darvozasi

`check-ui-value-drift` tekshiruvining qardoshi. U inglizcha qiymat tarjimalari yangilanmasdan
**qayta yozilgan** holatni aniqlaydi; bu tekshiruv esa inglizcha kalit **qoʻshilgan**, ammo ayrim
lokallar uni umuman olmagan holatni aniqlaydi.

`check-ui-keys-coverage` bu toifani koʻra olmaydi: u har bir lokal uchun foizli quyi chegarani
talab qiladi, taxminan 13 000 ta yakuniy kalitdan oʻn bittasi yoʻq boʻlsa, qamrov 99,9 % boʻlib
qoladi. Har bir til uchun foiz «bu funksiya tarjimasiz chiqarildi» degan holatni ifodalay olmaydi
— butun bir funksiya yangi lokalga hech qanday matnsiz qoʻshilishi va koʻrsatkichni umuman
oʻzgartirmasligi mumkin.

U qayd etadigan hodisa: Orchestration Canvasʼning 3-bosqichi oʻzining oʻn bitta kalitini oʻsha
paytda mavjud boʻlgan 42 ta lokalning barchasiga tarjima qildi. Bir necha soatdan keyin YI tillari
toʻplami (#13044) repozitoriydagi lokallar sonini 51 taga yetkazdi va toʻqqizta yangi lokal
(`el`, `et`, `ga`, `hr`, `lt`, `lv`, `mt`, `sl`, `sr`) bu kalitlarni hech qachon olmadi.
`deepMergeFallback` yoʻq kalit oʻrniga inglizcha matnni qoʻyadi, shu sababli nosozlik boʻsh UI
emas, tarjima qilinmagan UI koʻrinishida namoyon boʻldi — bu haqiqiy va tuzilishi sababli jim
qoladigan muammo edi.

Qardoshi singari, u ham **diff-aware** boʻlib, birlashtirish bazasidagi inglizcha matnni ishchi
daraxt bilan taqqoslaydi; shu sababli avvaldan mavjud boʻlgan boʻshliqlar muzlatib qoʻyiladi va
darvozani yoqish uchun migratsiya talab qilinmaydi. Istisno yoʻli:
`__MISSING__:<english>` tarjimani keyinga qoldiradi, ayni paytda ish vaqtida toʻgʻri ishlashni
saqlaydi. `vi` toʻldirgichlarni taqiqlaydi (`tests/unit/i18n-vi-completeness.test.ts`) va haqiqiy
tarjimani talab qiladi.

#### `check-vitest-exclusions` — chetga surilgan testlar darvozasi

`vitest.config.ts` faylining `exclude` roʻyxatidagi fayl — bajarilmaydigan testdir, ammo daraxtni
koʻrayotgan kishi uchun u qamrov bordek koʻrinadi. Oltmish ikkita fayl
`// #8618 — pre-existing failure; remove this exclusion when fixed` izohi ortida toʻplanib qoldi.
#8618 muammosi 2026-08-11 kuni yopilgan boʻlsa-da, u kuzatib borgan roʻyxat 45 ta yozuvdan 62
tagacha oʻsdi va har bir yangi yozuv yopilgan muammoga ishora qiluvchi izohni meros qilib oldi.
Nihoyat roʻyxat faylma-fayl oʻlchanganda (#13204), **62 ta testdan 51 tasi manba kodiga hech
qanday oʻzgartirish kiritmasdan joriy daraxtda muvaffaqiyatli oʻtdi**.

Darvoza haqiqiy faylga mos keladigan har bir istisnodan (a) kuzatuv muammosini koʻrsatishni va
(b) oʻlchangan holati bilan birga `config/quality/vitest-exclusions.json` faylida mavjud boʻlishni
talab qiladi; shu tariqa yangi istisno qoʻshish 60 ta yozuvli massivdagi navbatdagi satr emas,
maxsus fayldagi koʻrib chiqilishi mumkin boʻlgan diffga aylanadi. U ataylab istisno qilingan
testlarni qayta bajarmaydi — bu taxminan 10 daqiqa oladi va davriy vazifaga tegishli; inventar
roʻyxatida har bir test oxirgi marta qachon oʻlchangani qayd etiladi.
