...
Scripturile se află în `scripts/check/` (porți de politici) și `scripts/quality/` (motorul ratchet).
Sursa de adevăr pentru CI este `.github/workflows/ci.yml`.

### Calea rapidă pentru PR-uri de lansare (`quality.yml`)

`.github/workflows/quality.yml` rulează pentru PR-urile care vizează `release/**`. Aceasta permite
continuarea activității pe ramurile contribuitorilor prin porți rapide filtrate în funcție de căi, plus un semnal consultativ privind compilarea pentru producție în cazul modificărilor
de cod:

| Job                                              | Domeniu de aplicare                                                                                                                                                                                                                                                      | Blocant                                                                                                              |
| ------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------- |
| `Build (advisory)`                               | PR-uri de cod care nu sunt ciorne și ramuri din coada Mergify; Node 24, `npm-ci-retry`, `check:node-runtime`, `npm run build` cu `OMNIROUTE_USE_TURBOPACK=1`; fără încărcarea artefactelor, deoarece niciun job de calitate ulterior nu le utilizează                    | **Consultativ** (`continue-on-error: true`; eliminați după o săptămână de rulări stabile pentru PR-urile de lansare) |
| `Docs Gates (fast-path)`                         | PR-uri de documentație/cod; referințe pentru documentația API și docs-all                                                                                                                                                                                                | Da                                                                                                                   |
| `Fast Quality Gates`                             | PR-uri de cod; verificări statice, verificarea tipurilor, verificarea tipurilor pentru tabloul de bord, teste unitare afectate                                                                                                                                           | Da                                                                                                                   |
| `Forgotten sibling tests`                        | PR-uri de cod; modulele modificate sunt urmărite până la consumatorii statici și testele înrudite candidate; căile prin module-baril și importuri dinamice sunt raportate drept diagnostice consultative, împreună cu excepțiile corespunzătoare din lista de permisiuni | **Consultativ**                                                                                                      |
| `Vitest (fast-path)`                             | PR-uri de cod; suita rapidă vitest                                                                                                                                                                                                                                       | Da                                                                                                                   |
| `Unit Tests fast-path`                           | PR-uri de cod; suita de teste unitare împărțită în 4 fragmente                                                                                                                                                                                                           | Da                                                                                                                   |
| `No new ESLint warnings`                         | PR-uri de cod; protecție lint care ține cont de suprimări                                                                                                                                                                                                                | Da pentru PR-uri din același depozit, consultativ pentru fork-uri                                                    |
| `Merge integrity (changelog + generated skills)` | PR-uri care nu sunt ciorne; sincronizarea jurnalului de modificări și a abilităților generate                                                                                                                                                                            | Da pentru PR-uri din același depozit, consultativ pentru fork-uri                                                    |

#### Raportul testelor înrudite uitate

`npm run check:forgotten-sibling-tests` reutilizează mecanismul de rezolvare a importurilor pe care se bazează harta impactului asupra testelor.
...
lipsește din diferențele cererii de integrare. Rezumatul Markdown și rezultatul JSON sunt păstrate ca
artefactul de flux de lucru `forgotten-sibling-tests`, pentru calibrare înainte de orice implementare cu caracter blocant.

Reexporturile prin module-baril și importurile dinamice reprezintă doar diagnostice de rezolvare; acestea nu generează niciodată
o constatare blocantă. Excepțiile revizuite se află în
`config/quality/forgotten-sibling-allowlist.json`. Fiecare intrare trebuie să specifice consumatorul și testul
candidat, să ofere o justificare concretă și să includă un link către un tichet GitHub sau o cerere de integrare. Intrările incorecte
sunt respinse implicit. Excepțiile nu pot suprima un test candidat șters sau o diferență care adaugă `.skip`/`.todo`;
...
| `check:any-budget:t11`            | Numărul de `@ts-expect-error // any` nu depășește limita (catraca Nivelului 11)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Da                                          |
...
| `check:tracked-artifacts`         | Fără artefacte de compilare / legături simbolice `node_modules` incluse în versionare (rulează și în pre-commit husky; pre-push este intenționat simplificat — #6716)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | Da                                          |
...
| `check:file-size`                 | Niciun fișier sursă nu depășește limita per extensie (mecanism incremental: fișierele mari sunt fixate în lista `frozen`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | Da                                          |
...
| `check:known-symbols`             | Executorii furnizorilor, strategiile de rutare și adaptoarele de traducere înregistrate în tabelele lor de expediere corespund fișierelor de pe disc — fără simboluri orfane sau nedeclarate                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | Da                                          |
...
| `check:agent-skills-sync`         | Artefactele generate pentru abilitățile agenților corespund catalogului lor sursă (fără discrepanțe)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `check:provider-asset-provenance` | Siglele/resursele furnizorilor au asociată o înregistrare documentată privind proveniența                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `lint:json`                       | Fișierele de configurare JSON sunt analizate cu succes și respectă regulile de linting ale repo-ului                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `typecheck:core`                  | Compilare TypeScript fără erori (doar avertismente consultative)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | Da                                          |
| `typecheck:noimplicit:core`       | `noImplicitAny` strict — orientat spre viitor; multe locuri de apel preexistente încă necesită adnotări                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | **Consultativ** (`continue-on-error: true`) |
| `check:dashboard-typecheck`       | `tsc` limitat la `src/app/(dashboard)/**` (#7033) — lista atent selectată de 27 de fișiere din `typecheck:core` nu include niciun fișier TSX de tablou de bord, iar `next build` nu le verifică niciodată tipurile (`next.config.mjs` setează `ignoreBuildErrors: true`), astfel încât regresiile privind identificatorii rămași fără referință de acolo (#6625/#6909) nu erau vizibile în CI. Compară cu o bază de referință înghețată a numărului de erori per fișier/per cod TS (`config/quality/dashboard-typecheck-baseline.json`, același model de impunere împotriva învechirii ca `check:known-symbols`) — doar erorile NOI care depășesc numărul din baza de referință fac ca verificarea să eșueze; reduceți treptat valorile cu `--update` atunci când este remediată o eroare preexistentă.                                                                                                                                          | Da                                          |
...
| ---------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------- |
| `quality:collect`            | Generează `quality-metrics.json` (numărul de avertismente ESLint, acoperirea din raportul combinat al partițiilor)                                                                                                           | Da (în amonte de mecanismul de ajustare progresivă) |
| `quality:ratchet`            | Nicio metrică din `quality-baseline.json` nu a regresat (avertismente ESLint ≤ baza de referință; acoperire ≥ baza de referință)                                                                                             | Da                                                  |
| `check:duplication`          | Duplicarea codului (jscpd@4) nu depășește baza de referință din `quality-baseline.json`                                                                                                                                      | Da                                                  |
...
| `check:cognitive-complexity` | Ajustare progresivă a complexității cognitive (`eslint-plugin-sonarjs`) — execuție ESLint separată; CI le rulează pe ambele, reunite într-un singur pas `check:complexity-ratchets`                                          | Da                                                  |
| `check:dead-code`            | Ajustarea progresivă pentru exporturile/fișierele neutilizate (knip) nu regresează față de baza de referință                                                                                                                 | Da                                                  |
| `check:compression-budget`   | Bugetul etalonului de compresie — pragurile minime ale economiilor de tokenuri pentru fiecare motor nu trebuie să regreseze                                                                                                  | Da                                                  |
| `check:type-coverage`        | Ajustarea progresivă a procentului de cod tipizat (`type-coverage`) nu regresează; în mare parte, înlocuiește `typecheck:noimplicit:core`                                                                                    | Da                                                  |
| `check:codeql-ratchet`       | Numărul alertelor CodeQL deschise nu regresează (citit prin `gh api`; omitere controlată în lipsa unui token) — pentru frecvența reîmprospătării și declanșarea manuală, consultați „Ajustarea progresivă CodeQL” de mai jos | Da                                                  |

### Sarcină: `quality-extended`

Întregul job este consultativ (`continue-on-error: true`). Verificările incrementale bazate pe npm rulează
...
| `check:secrets`          | Scanarea secretelor (gitleaks) — se omite dacă binarul este absent                                                                                                                                                   | **Consultativ** |
| `check:vuln-ratchet`     | Vulnerabilitățile dependențelor (osv-scanner) nu regresează — se omite dacă binarul este absent                                                                                                                      | **Consultativ** |
| `check:workflows`        | Verificarea workflow-urilor (actionlint + zizmor) — se omite dacă binarele sunt absente                                                                                                                              | **Consultativ** |
| `check:openapi-breaking` | Modificări incompatibile ale contractului API public (`openapi.yaml`) față de ramura de bază (oasdiff) — emite `openapiBreaking=N`; se omite dacă oasdiff este absent sau specificația de bază nu poate fi rezolvată | **Consultativ** |
...
| Script                         | Validează                                                                                                                                                                                          | Blochează                  |
| ------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------- |
| `check:docs-all`               | Poartă de control agregată care rulează secvențial cele 6 porți secundare de mai jos                                                                                                               | Da                         |
| ↳ `check:docs-sync`            | Consistența versiunilor dintre CHANGELOG / OpenAPI / llm.txt                                                                                                                                       | Da                         |
| ↳ `check:docs-counts`          | Numerele din text (numărul de furnizori, numărul de migrări etc.) se încadrează în intervalul incremental al valorilor reale                                                                       | Da                         |
...
| ↳ `check:deprecated-versions`  | Nu există șiruri de versiune învechite în documentație                                                                                                                                             | Da                         |
...
| `check:openapi-coverage`       | Specificația OpenAPI acoperă cel puțin un prag incremental al rutelor reale                                                                                                                        | Da                         |
...
| `check:openapi-routes`         | Fiecare cale din `openapi.yaml` corespunde unui fișier `route.ts` real (protecție împotriva halucinațiilor)                                                                                        | Da                         |
| `check:docs-symbols`           | Fiecare referință `/api/...` din `docs/**/*.md` corespunde unui fișier `route.ts` real (protecție împotriva halucinațiilor)                                                                        | Da                         |
...
| `check-new-key-coverage` (inline) | O cheie **nouă** în limba engleză ajunge în fiecare localizare                                                                                                                                                        | Da              |
| `check-translation-ratio`         | Raportul traducerilor reale per localizare (intrări identice cu engleza / substituenți / intrări lipsă din afara listei de excepții) nu trebuie să depășească `config/quality/i18n-translation-baseline.json` + marja | **Consultativ** |

Necesită `fetch-depth: 0` — verificarea derivei valorilor compară `en.json` cu baza de îmbinare.

#### `check-ui-value-drift` — verificarea traducerilor învechite

Detectează singura regresie i18n pe care celelalte verificări nu o pot identifica structural: o valoare în limba engleză
...
Acest lucru a ajuns efectiv în producție. `oauthModal.googleOAuthWarning` a fost rescris când a fost introdus utilitarul
de autentificare Antigravity (#5203); **39 din 43 de localizări** au păstrat textul care le spunea operatorilor să „copieze
URL-ul complet și să-l lipească mai jos” — un flux care nu poate fi finalizat pentru furnizorul respectiv. Problema a rămas
...
- `check-translation-drift` urmărește copiile în oglindă ale documentației din `docs/i18n/<locale>/**.md` —
  nu citește niciodată `src/i18n/messages/*.json`. Este blocant în jobul `docs-sync-strict` începând cu resincronizarea
  din 2026-09: modificați un document de bază → `npm run i18n:run -- --files=<doc>` (la nivel de secțiune, puțin costisitor).

**Conștientă de diferențe, fără referință de bază.** Compară `en.json` de la baza de îmbinare cu arborele
de lucru; pentru fiecare cheie a cărei valoare în limba engleză s-a schimbat, orice localizare care încă păstrează o
traducere nemodificată este învechită. Această abordare **îngheață intenționat datoria preexistentă** — o diferență
nu poate dezvălui din ce versiune veche în limba engleză provine o traducere existentă de mult timp, astfel încât verificarea evaluează
doar ceea ce este afectat de modificarea curentă. Alternativa (o referință de bază cu hash per cheie) ar necesita
un fișier generat de aproximativ 600 KB, de 3 ori mai mare decât cea mai mare referință de bază existentă, care s-ar modifica la fiecare PR i18n.

Există două modalități de a trece verificarea:

1. actualizați traducerile afectate sau
2. setați-le la `__MISSING__:<new english>` — la rulare va fi furnizat atunci textul corectat în limba engleză
...
| Script                 | Validează                                                                                                                                                          | Blocant |
| ---------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------- |
| `check:pr-test-policy` | PR-urile care modifică codul de producție din `src/`, `open-sse/`, `electron/` sau `bin/` trebuie să includă ori să actualizeze teste (Regula strictă nr. 8)       | Da      |
...
| `check:pr-evidence`    | Corpul PR-ului menționează dovezi de testare/VPS pentru modificare (automatizează Regula strictă nr. 18 căutând în textul PR-ului — fragil, consultați Restanțele) | Da      |
...
| `test:vitest:ui` | Testele componentelor UI — runner vitest                         | **Blocant** — erorile preexistente sunt excluse explicit în `vitest.config.ts`; erorile noi duc la eșecul jobului |

### Fluxuri de lucru nocturne (programate, cu rol consultativ)

Acestea rulează conform unui program cron (și prin `workflow_dispatch`), niciodată pentru PR-uri. Toate au rol consultativ.

| Flux de lucru          | Validează                                                                                                                                                                                   | Blocant         |
| ---------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------- |
| `nightly-property`     | Teste bazate pe proprietăți cu fast-check, cu un seed aleatoriu și un număr mare de rulări                                                                                                  | **Consultativ** |
| `nightly-resilience`   | Pragul de creștere a heap-ului, injectarea haotică a defecțiunilor, teste k6 de încărcare/anduranță                                                                                         | **Consultativ** |
| `nightly-llm-security` | Protecția promptfoo împotriva injectării (mod de blocare) + sonde garak (omise fără secretul unui furnizor)                                                                                 | **Consultativ** |
...
| `nightly-mutation`     | Scorul testării prin mutații Stryker pentru suita rapidă de teste unitare — mutanții supraviețuitori evidențiază aserțiunile slabe                                                          | **Consultativ** |
...
- Lanțul de aprovizionare (proveniență, SBOM, Trivy, Scorecard): [`docs/security/SUPPLY_CHAIN.md`](../security/SUPPLY_CHAIN.md)

#### `check-key-completeness` — poarta pentru paritatea seturilor de chei

`scripts/i18n/check-key-completeness.mjs` (`npm run i18n:check-keys`, jobul `i18n-ui-coverage`).
Compară setul de chei terminale din fiecare `src/i18n/messages/<locale>.json` cu `en.json` și eșuează
la orice cheie terminală absentă sau suplimentară, indiferent când a fost adăugată cheia. Substituenții
`__MISSING__:` sunt considerați prezenți (conținutul lor ține de poarta bazată pe raport). Este complementul
absolut al celor două porți bazate pe diferențe/procente: `check-ui-keys-coverage` impune un prag minim de 80 % pentru
fiecare localizare (43 de chei absente din ~13.000 înseamnă tot 99,7 %), iar `check-new-key-coverage` evaluează
doar cheile pe care un PR le adaugă în `en.json`. Un lot de localizări este generat din versiunea `en.json` din ziua
în care este creată ramura sa și este tradus timp de mai multe zile, în timp ce baza continuă să primească chei; PR-ul lotului nu adaugă el însuși
nicio cheie, astfel încât ambele porți înrudite au rămas tăcute când lotul 1 (#13044) a fost integrat cu 43 de chei lipsă în nouă
localizări, iar lotul 2 (#13660), cu 10 chei lipsă în opt (2026-09-15). Remediați o verificare eșuată cu
`node scripts/i18n/sync-ui-keys.mjs --locale=<codes> --translate-markers`; o cheie terminală `extra`
înseamnă că sursa a eliminat-o — ștergeți-o din localizare. `--warn` raportează fără a eșua.
...
#### `check-new-key-coverage` — poarta i18n pentru chei noi

Poarta înrudită cu `check-ui-value-drift`. Aceasta detectează o valoare în limba engleză care a fost **rescrisă**
în timp ce traducerile sale au rămas neschimbate; poarta de față detectează o cheie în limba engleză care a fost **adăugată**
fără să fie inclusă vreodată în unele localizări.
...
exprima „această funcționalitate a fost livrată netradusă” — o funcționalitate întreagă poate ajunge într-o localizare nouă fără
niciun text și fără să modifice vreodată valoarea.

Incidentul pe care îl codifică: faza 3 a Orchestration Canvas și-a tradus cele unsprezece chei în
cele 42 de localizări care existau la acel moment. Câteva ore mai târziu, lotul de limbi UE (#13044) a adus depozitul
la 51 de localizări, iar cele nouă nou-venite (`el`, `et`, `ga`, `hr`, `lt`, `lv`, `mt`, `sl`, `sr`) nu
le-au primit niciodată. `deepMergeFallback` înlocuiește o cheie absentă cu textul în limba engleză, astfel încât modul de eșec a fost
o interfață netradusă, nu una fără text — o problemă reală și implicit silențioasă.

La fel ca poarta înrudită, aceasta este **conștientă de diferențe**, comparând versiunea în limba engleză de la baza fuziunii cu arborele
de lucru, astfel încât lacunele preexistente rămân înghețate, iar activarea porții nu a necesitat nicio migrare. Mecanism de evitare:
`__MISSING__:<english>` amână o traducere, menținând în același timp funcționarea corectă la rulare. `vi` interzice
substituenții (`tests/unit/i18n-vi-completeness.test.ts`) și necesită o traducere reală.

#### `check-vitest-exclusions` — poarta pentru testele suspendate

Un fișier din lista `exclude` a `vitest.config.ts` este un test care nu rulează și pare să ofere
acoperire pentru oricine consultă arborele. Șaizeci și două de fișiere s-au acumulat în spatele comentariului
`// #8618 — eșec preexistent; eliminați această excludere după remediere`. Problema #8618 a fost închisă la
2026-08-11, în timp ce lista pe care o urmărea a crescut de la 45 la 62 de intrări, fiecare nouă intrare moștenind un comentariu
care indica o problemă închisă. Când lista a fost în cele din urmă evaluată fișier cu fișier (#13204), **51 dintre cele 62
au trecut în raport cu arborele curent fără nicio modificare a sursei**.

Poarta impune ca fiecare excludere care se rezolvă la un fișier real (a) să numească o problemă de urmărire și
(b) să apară în `config/quality/vitest-exclusions.json` împreună cu starea sa măsurată, astfel încât adăugarea uneia să constituie o
diferență verificabilă într-un fișier dedicat, nu încă o linie într-un tablou cu 60 de intrări. În mod intenționat,
aceasta nu rulează din nou testele excluse — acest lucru durează ~10 minute și aparține unui job periodic; inventarul
înregistrează când a fost măsurat ultima dată fiecare test.
