...
**दायरा:** एकल प्रोवाइडर कनेक्शन/अकाउंट/कुंजी।

**उद्देश्य:** एक खराब कुंजी को छोड़ना, जबकि उसी प्रोवाइडर के अन्य कनेक्शन सेवा देना जारी रखें।
...
**थंडरिंग-हर्ड-रोधी सुरक्षा:** समवर्ती विफलताओं को कूलडाउन अत्यधिक बढ़ाने या `backoffLevel` को दो बार बढ़ाने से रोकती है।

**टर्मिनल स्थितियाँ (कूलडाउन नहीं):**

- `banned` — प्रतिबंधित-कीवर्ड / अकाउंट-प्रतिबंध पहचान द्वारा सेट की जाती है ([BAN_DETECTION](../security/BAN_DETECTION.md) देखें)
- `expired` (सीमित पुनःप्रयासों के बाद टर्मिनल स्थिति में परिवर्तित होती है — एक्सपोनेंशियल बैकऑफ़ के साथ `EXPIRED_RETRY_MAX = 3` — ताकि अस्थायी OAuth त्रुटियाँ अकाउंट के स्थायी रूप से निष्क्रिय होने से पहले स्वतः ठीक हो सकें)
- `credits_exhausted`

ये तब तक बनी रहती हैं जब तक क्रेडेंशियल नहीं बदलते या कोई ऑपरेटर इन्हें रीसेट नहीं करता। टर्मिनल स्थितियों को अस्थायी कूलडाउन स्थिति से अधिलेखित न करें।
...
**दायरा:** एक क्लाइंट सेशन (`X-Session-Id` / `x-codex-session-id` / `x-omniroute-session` हेडर), **किसी भी** प्रोवाइडर के लिए, एक कनेक्शन से पिन किया गया।

**उद्देश्य:** किसी मल्टी-टर्न एजेंट (Claude Code, aider, कस्टम एजेंट) को अनुरोधों के दौरान उसी अकाउंट पर बनाए रखना, जिससे क्रॉस-अकाउंट संदर्भ हानि और प्रति-अकाउंट सेशन स्थिति वाले प्रोवाइडरों पर बार-बार होने वाले कोल्ड-स्टार्ट 429 कम हों।
...
- हेडर निष्कर्षण (सामान्य, किसी भी प्रोवाइडर के लिए): `src/sse/services/auth.ts::extractSessionAffinityKey()`
- स्थायी पिन तालिका: `sessionAccountAffinity` (`src/lib/db/sessionAccountAffinity.ts`)
- सेटिंग: `sessionAffinityTtlMs` (ms में वैश्विक TTL, `0` इसे अक्षम करता है) — `src/lib/db/settings.ts`। माइग्रेशन `124_generic_session_affinity_ttl.sql` द्वारा इसका नाम केवल-Codex वाले `codexSessionAffinityTtlMs` से बदला गया, जो पहले से कॉन्फ़िगर किए गए किसी भी Codex TTL को नए डिफ़ॉल्ट के रूप में आगे ले जाता है।

#7274 से पहले, `resolveSessionAffinityTtlMs()` `codex` को छोड़कर हर प्रोवाइडर के लिए तुरंत `0` लौटाता था, इसलिए TTL सेटिंग (और सेशन हेडर) का अन्यत्र कोई प्रभाव नहीं था, भले ही पिनिंग तंत्र और हेडर निष्कर्षण पहले से ही प्रोवाइडर-अज्ञेय थे। समाधान ने उस शुरुआती रिटर्न को हटा दिया; अब TTL, वैश्विक रूप से `0` से अधिक सेट होने पर, प्रत्येक प्रोवाइडर पर समान रूप से लागू होता है।

तीन सेशन-एफ़िनिटी हेडर कभी भी अपस्ट्रीम अग्रेषित नहीं किए जाते — एक्ज़ीक्यूटर क्लाइंट हेडर को आगे भेजने के बजाय अपने अपस्ट्रीम हेडर शुरू से बनाते हैं, इसलिए यह केवल एक आंतरिक सहसंबंध ID बना रहता है।

### विशिष्ट प्रबंधित सेशन कनेक्शन लीज़

**दायरा:** एक सक्रिय प्रबंधित HTTP क्लाइंट/सेशन एक पात्र OmniRoute कनेक्शन का स्वामी होता है।

**उद्देश्य:** उन क्लाइंट के लिए टिकाऊ, विशिष्ट कनेक्शन स्वामित्व प्रदान करना जिन्हें अनुरोधों के बीच एक कठोर रूटिंग
सीमा की आवश्यकता होती है। यह सेशन एफ़िनिटी से भिन्न है, जो एक सहज निरंतरता प्राथमिकता है:
एक विशिष्ट लीज़ SQLite में लाइफ़साइकल स्थिति को बनाए रखती है, वैश्विक सक्रिय-स्वामी और
सक्रिय-कनेक्शन विशिष्टता लागू करती है, और प्रोवाइडर डिस्पैच से पहले किसी पुराने जेनरेशन को अस्वीकार करती है।

यह सुविधा प्रति API कुंजी ऑप्ट-इन है। किसी प्रबंधित कुंजी के पास `lease:exclusive` स्कोप और एक
स्पष्ट गैर-रिक्त `allowedConnections` सूची होनी चाहिए। कोई भी HTTP क्लाइंट लाइफ़साइकल एंडपॉइंट का उपयोग कर सकता है; किसी
क्लाइंट नाम, यूज़र-एजेंट, प्रोवाइडर, OAuth विधि या मॉडल की आवश्यकता नहीं है। लीज़ किसी कनेक्शन का स्वामित्व रखती है,
मॉडल का नहीं, इसलिए मॉडल परिवर्तन के बाद भी बाइंडिंग बनी रहती है, बशर्ते कनेक्शन सामान्य रूप से
पात्र बना रहे। सामान्य मॉडल, कोटा, स्वास्थ्य, कूलडाउन और अनुमति-सूची नियम प्रभावी बने रहते हैं और
उसी जेनरेशन को किसी अन्य मुक्त पात्र कनेक्शन में परिवर्तित कर सकते हैं।

लाइफ़साइकल JSON क्रियाओं `acquire`, `renew`, और `release` के साथ `POST /api/v1/session-leases` है।
...
उसका SHA-256 हैश संग्रहीत किया जाता है। प्रत्येक अंतिम डिस्पैच फ़ेंस प्रमाणीकृत API कुंजी ID और
सक्रिय कनेक्शन ID को भी बाइंड करता है। लीज़ नियंत्रण हेडर लॉग, बनाए रखे गए अनुरोध स्नैपशॉट और
...
अन्य सक्रिय लीज़ का कब्ज़ा है, तो OmniRoute HTTP `429`, लीज़-क्षमता-अनुपलब्ध कोड, एक
क्षमता-की-प्रतीक्षा स्थिति, और निकटतम प्रासंगिक समाप्ति से प्राप्त सीमित `Retry-After` लौटाता है।
सामान्य रिक्त पात्रता लीज़ विवाद नहीं है और अपने मौजूदा रूटिंग त्रुटि अर्थ-विज्ञान को बनाए रखती है।

संबंधित तंत्र अलग बने रहते हैं:

- OAuth सेशन अधिभोग, OAuth अकाउंट के लिए प्रोसेस-स्थानीय सहज वितरण है।
- अकाउंट सेमाफ़ोर अनुरोध-समवर्तीता परमिट प्रदान करते हैं और अनुरोध पूरा होने पर समाप्त हो जाते हैं।
- विशिष्ट प्रबंधित सेशन लीज़, जेनरेशन फ़ेंस के साथ टिकाऊ लाइफ़साइकल स्वामित्व हैं।

---

## 3. मॉडल लॉकआउट

**दायरा:** provider + connection + model ट्रिपल।

**उद्देश्य:** जब केवल एक model अनुपलब्ध हो या उसकी quota सीमित हो, तब पूरे connection को अक्षम करने से बचना।

**उदाहरण:**

- प्रति-model quota वाले providers द्वारा 429 लौटाना
- किसी एक अनुपलब्ध model के लिए local providers द्वारा 404 लौटाना
...
यह सक्रिय lockouts को इन विवरणों के साथ सूचीबद्ध करता है: provider, connection, model, reason, expiresAt। ऑपरेटर कार्ड से किसी model को मैन्युअल रूप से फिर से सक्षम कर सकते हैं।

**REST API:**

- `GET /api/resilience/model-cooldowns` — सक्रिय lockouts की सूची
- `DELETE /api/resilience/model-cooldowns` — मैन्युअल रूप से फिर से सक्षम करना। Body: `{provider, connection, model}`। Auth: management।

### लॉकआउट सेटिंग्स UI + सफलता-क्षय पुनर्प्राप्ति (v3.8.23)

मॉडल लॉकआउट हमेशा चालू रहने वाले हार्डकोडेड व्यवहार से बदलकर पूर्णतः कॉन्फ़िगर करने योग्य,
ऑप्ट-इन सुविधा बन गया है, जिसका अपना सेटिंग्स कार्ड और स्व-उपचार पुनर्प्राप्ति पथ है।

**सेटिंग्स कार्ड:** Settings → Model Lockout
(`src/app/(dashboard)/dashboard/settings/components/ModelLockoutCard.tsx`)।
यह ऊपर दिए गए केवल-पढ़ने योग्य `ModelCooldownsCard` से **अलग** है (जो केवल
सक्रिय lockouts को _सूचीबद्ध_ करता है) — नया कार्ड _पैरामीटर कॉन्फ़िगर करता है_। डिफ़ॉल्ट
`DEFAULT_MODEL_LOCKOUT_SETTINGS`
(`src/lib/resilience/modelLockoutSettings.ts`) में मौजूद हैं:

| सेटिंग                  | डिफ़ॉल्ट                         | अर्थ                                                               |
| ----------------------- | -------------------------------- | ------------------------------------------------------------------ |
| `enabled`               | `false`                          | मुख्य टॉगल — मॉडल लॉकआउट **डिफ़ॉल्ट रूप से बंद** है।               |
| `errorCodes`            | `[403, 404, 429, 502, 503, 504]` | वे upstream statuses जिन्हें model-स्कोप वाली विफलता माना जाता है। |
| `baseCooldownMs`        | `120_000` (120 सेकंड)            | पहली विफलता के लिए प्रारंभिक लॉकआउट अवधि।                          |
| `maxCooldownMs`         | `1_800_000` (30 मिनट)            | बढ़ाए गए कूलडाउन की अधिकतम सीमा।                                   |
| `maxBackoffSteps`       | `10`                             | exponential-backoff वृद्धि के अधिकतम चरण।                          |
| `useExponentialBackoff` | `true`                           | क्या बार-बार होने वाली विफलताएँ कूलडाउन को तेजी से बढ़ाएँगी।       |

सेटिंग्स सामान्य settings store के माध्यम से बनी रहती हैं और
resilience settings schema के जरिए सत्यापित होती हैं; कार्ड `baseCooldownMs`/`maxCooldownMs`
(`maxCooldownMs ≥ baseCooldownMs` के साथ) और `maxBackoffSteps` को अनुमत सीमा में रखता है।

**सफलता-क्षय पुनर्प्राप्ति:** पुनर्प्राप्ति **केवल** टाइमर समाप्ति पर निर्भर नहीं है। एक स्वस्थ
response model की विफलता संख्या को क्रमशः कम करता है, ताकि विंडो के बीच में पुनर्प्राप्त हुआ model
अपने टाइमर से पहले वृद्धि रोक दे (और लॉकआउट साफ़ कर दे)। किसी सफल
combo target पर, `open-sse/services/combo.ts` द्वारा `decayModelFailureCount()`
(`open-sse/services/accountFallback.ts`) कॉल किया जाता है, जो संग्रहीत
`failureCount` को **आधा** करता है (`Math.floor(failureCount / 2)`); जब यह `0` तक पहुँचता है, तो lockout
entry पूरी तरह हटा दी जाती है। इसका समकक्ष `recordModelLockoutFailure()`
escalation window के भीतर होने वाली विफलताओं पर संख्या बढ़ाता है (और cooldown को बढ़ाता है)।
यह सफलता-क्षय सामान्य टाइमर समाप्ति के अतिरिक्त है —
दोनों में से कोई भी पथ model को फिर से सक्षम कर सकता है।

**स्थिति:** lockouts को **मेमोरी में** रखा जाता है (`provider:connectionId:model` द्वारा कुंजीबद्ध
`ModelLockoutEntry` के प्रति-process `Map`), DB में स्थायी रूप से संग्रहीत नहीं किया जाता
— restart होने पर वे खो जाते हैं। _सेटिंग्स_ स्थायी रहती हैं; सक्रिय
...
- रीसेट विंडो के बाद प्रोवाइडर स्थायी रूप से बाहर रखा गया → कोड `getStatus()`/`canExecute()` के बजाय सीधे `state` पढ़ रहा है।
- एक कुंजी विफल होती है, अन्य को काम करना चाहिए → सर्किट ब्रेकर के बजाय कनेक्शन कूलडाउन को प्राथमिकता दें।
- केवल एक मॉडल विफल होता है → कनेक्शन कूलडाउन के बजाय मॉडल लॉकआउट को प्राथमिकता दें।
- स्थिति को स्वतः पुनर्प्राप्त होना चाहिए, लेकिन ऐसा नहीं होता → भविष्य के टाइमस्टैम्प और समाप्त हो चुकी स्थिति को रीफ़्रेश करने वाले रीड पाथ की जाँच करें। स्थायी स्थितियों में मैन्युअल बदलाव आवश्यक हैं।
