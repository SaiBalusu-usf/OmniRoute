...
स्क्रिप्ट `scripts/check/` (नीति गेट्स) और `scripts/quality/` (रैचेट इंजन) के अंतर्गत स्थित हैं।
CI के लिए सत्य का स्रोत `.github/workflows/ci.yml` है।

### रिलीज़ PR फ़ास्ट-पाथ (`quality.yml`)

`.github/workflows/quality.yml`, `release/**` को लक्षित करने वाले PRs पर चलता है। यह पाथ-फ़िल्टर किए गए
त्वरित गेट्स के साथ योगदानकर्ता ब्रांचों पर काम जारी रखता है, और कोड परिवर्तनों के लिए एक
परामर्शात्मक प्रोडक्शन-बिल्ड संकेत भी प्रदान करता है:

| जॉब                                              | दायरा                                                                                                                                                                                                                                   | अवरोधक                                                                                  |
| ------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------- |
| `Build (advisory)`                               | गैर-ड्राफ़्ट कोड PRs और Mergify क्यू ब्रांचें; Node 24, `npm-ci-retry`, `check:node-runtime`, `OMNIROUTE_USE_TURBOPACK=1` के साथ `npm run build`; कोई आर्टिफ़ैक्ट अपलोड नहीं, क्योंकि कोई डाउनस्ट्रीम गुणवत्ता जॉब इसका उपयोग नहीं करता | **परामर्शात्मक** (`continue-on-error: true`; स्थिर रिलीज़-PR रन के एक सप्ताह बाद हटाएँ) |
| `Docs Gates (fast-path)`                         | दस्तावेज़/कोड PRs; API दस्तावेज़ संदर्भ और docs-all                                                                                                                                                                                     | हाँ                                                                                     |
| `Fast Quality Gates`                             | कोड PRs; स्थैतिक जाँच, टाइपचेक, डैशबोर्ड टाइपचेक, प्रभावित यूनिट टेस्ट                                                                                                                                                                  | हाँ                                                                                     |
| `Forgotten sibling tests`                        | कोड PRs; बदले गए मॉड्यूलों से स्थैतिक उपभोक्ताओं और संभावित सिबलिंग टेस्ट तक ट्रेसिंग; बैरल और डायनेमिक-इम्पोर्ट पाथ को संदर्भित अनुमतिसूची अपवादों सहित परामर्शात्मक निदान के रूप में रिपोर्ट किया जाता है                             | **परामर्शात्मक**                                                                        |
| `Vitest (fast-path)`                             | कोड PRs; त्वरित vitest सुइट                                                                                                                                                                                                             | हाँ                                                                                     |
| `Unit Tests fast-path`                           | कोड PRs; 4-शार्ड यूनिट सुइट                                                                                                                                                                                                             | हाँ                                                                                     |
| `No new ESLint warnings`                         | कोड PRs; सप्रेशन-जागरूक lint गार्ड                                                                                                                                                                                                      | अपने मूल स्रोत के लिए हाँ, forks के लिए परामर्शात्मक                                    |
| `Merge integrity (changelog + generated skills)` | गैर-ड्राफ़्ट PRs; changelog और जनरेट किए गए skill का सिंक                                                                                                                                                                               | अपने मूल स्रोत के लिए हाँ, forks के लिए परामर्शात्मक                                    |

#### भूले हुए सिबलिंग टेस्ट की रिपोर्ट

`npm run check:forgotten-sibling-tests`, टेस्ट-प्रभाव मैप के पीछे मौजूद इम्पोर्ट रिज़ॉल्वर का पुनः उपयोग करता है।
प्रत्येक बदले गए प्रोडक्शन मॉड्यूल के लिए, जब संभावित टेस्ट
पुल-रिक्वेस्ट diff में अनुपस्थित होता है, तो यह नियतात्मक
`बदला गया मॉड्यूल/सिंबल -> स्थैतिक उपभोक्ता -> संभावित सिबलिंग टेस्ट` शृंखलाएँ रिपोर्ट करता है। Markdown सारांश और JSON परिणाम को
किसी भी अवरोधक रोलआउट से पहले अंशांकन के लिए `forgotten-sibling-tests` वर्कफ़्लो आर्टिफ़ैक्ट के रूप में रखा जाता है।

बैरल री-एक्सपोर्ट और डायनेमिक इम्पोर्ट केवल रिज़ॉल्यूशन निदान हैं; वे कभी भी
अवरोधक निष्कर्ष उत्पन्न नहीं करते। समीक्षा किए गए अपवाद
`config/quality/forgotten-sibling-allowlist.json` में स्थित हैं। प्रत्येक प्रविष्टि में उपभोक्ता और संभावित
टेस्ट का नाम, एक विशिष्ट औचित्य और GitHub issue या pull request का लिंक होना आवश्यक है। विकृत प्रविष्टियाँ
सुरक्षित रूप से विफल होती हैं। अपवाद हटाए गए संभावित टेस्ट या `.skip`/`.todo` जोड़ने वाले diff को दबा नहीं सकते;
अभिकथन को कमज़ोर करना और अन्य छिपाव स्वतंत्र रूप से अवरोधक
`check:test-masking` गेट के अधीन रहते हैं।

### जॉब: `lint`

`main` पर आने वाले प्रत्येक PR पर चलता है। विफलता की स्थिति में मर्ज को अवरुद्ध करता है।
...
| `check:any-budget:t11`            | `@ts-expect-error // any` की संख्या बजट से अधिक नहीं है (टियर 11 catraca)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | हाँ                                          |
| `check:provider-consistency`      | `providers.ts` में प्रत्येक प्रदाता की `providerRegistry.ts` में एक मेल खाती प्रविष्टि है (और इसके विपरीत, अनुमतिसूची के भीतर)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | हाँ                                          |
| `check:model-lifecycle`           | हाथ से अनुरक्षित तीन रूटिंग तालिकाएँ चेक-इन किए गए जीवनचक्र स्नैपशॉट (#11503) के अनुरूप रहती हैं: `FITNESS_TABLE` (`taskFitness.ts`) ऐसे किसी सेवानिवृत्त id को स्कोर नहीं करती जिसे `REGISTRY` रूट कर सकती है; प्रत्येक `BUILT_IN_ALIASES` लक्ष्य `REGISTRY` में मौजूद और सेवानिवृत्त-id स्नैपशॉट में अनुपस्थित है; `REGISTRY` में अब भी मौजूद प्रत्येक सेवानिवृत्त id को अग्रेषित किया गया है या `allowedRetiredInCatalog` में सूचीबद्ध किया गया है; और उस स्नैपशॉट में कोई भी `DEFAULT_DEGRADATION_MAP` स्रोत या लक्ष्य सेवानिवृत्त नहीं दिखता। यह सिद्ध नहीं करता कि कोई मॉडल वर्तमान में किसी सक्रिय अपस्ट्रीम द्वारा उपलब्ध कराया जा रहा है। ऑफ़लाइन — `config/quality/model-lifecycle.json` के साथ तुलना करता है, जिसे `npm run quality:refresh-model-lifecycle` (नेटवर्क; CI में संबद्ध नहीं) द्वारा हाथ से रीफ़्रेश किया जाता है। `allowedRetiredInCatalog` एक क्रमिक कमी वाला रैचेट है: कोई प्रविष्टि केवल ट्रैकिंग समस्या के साथ जोड़ें। | हाँ                                          |
| `check:fetch-targets`             | क्लाइंट-साइड `src/` में प्रत्येक `fetch("/api/...")` वास्तविक `route.ts` से संबद्ध होता है                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | हाँ                                          |
| `check:deps`                      | रिपॉज़िटरी में प्रत्येक `package.json` की सभी `npm install`-योग्य निर्भरताएँ `dependency-allowlist.json` में हैं; नए अनपिन किए गए या स्लॉपस्क्वैट किए गए पैकेज चिह्नित किए जाते हैं                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | हाँ                                          |
| `audit:deps`                      | `npm audit` (रूट + electron) — कोई उच्च/गंभीर परामर्श नहीं (`check:vuln-ratchet` osv के साथ ओवरलैप करता है; युक्तिसंगतीकरण बैकलॉग देखें)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | हाँ                                          |
| `check:lockfile`                  | `package-lock.json` की अखंडता — https रजिस्ट्री, अखंडता हैश, कोई होस्ट ओवरराइड नहीं                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | हाँ                                          |
| `check:licenses`                  | प्रोडक्शन डिपेंडेंसीज़ के लिए SPDX लाइसेंस अनुमति-सूची                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | हाँ                                          |
...
| `check:vitest-exclusions`         | प्रत्येक Vitest अपवर्जन में एक ट्रैकिंग इश्यू का उल्लेख होता है और वह `config/quality/vitest-exclusions.json` में मौजूद होता है (#13204)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | हाँ                                          |
| `check:file-size`                 | कोई भी सोर्स फ़ाइल प्रति-एक्सटेंशन सीमा से अधिक नहीं होती (रैचेट: बड़ी फ़ाइलें `frozen` सूची में फ़्रीज़ की गई हैं)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | हाँ                                          |
...
| `check:migration-numbering`       | माइग्रेशन SQL फ़ाइलें क्रमिक रूप से क्रमांकित हैं; कोई अंतराल या डुप्लिकेट नहीं है                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | हाँ                                          |
| `check:public-creds`              | `publicCreds.ts` के बाहर कोई शाब्दिक OAuth `client_id`/`client_secret` या Firebase Web कुंजियाँ नहीं (कठोर नियम #11)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | हाँ                                          |
| `check:db-rules`                  | `src/lib/db/` मॉड्यूल के बाहर कोई रॉ SQL नहीं; `localDb.ts` से कोई बैरल-इम्पोर्ट नहीं (कठोर नियम #2/#5)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | हाँ                                          |
...
| `check:test-discovery`            | रेपो में प्रत्येक `*.test.ts` / `*.spec.ts` फ़ाइल कम-से-कम एक टेस्ट रनर द्वारा एकत्रित की जाती है (रैचेट: `test-discovery-baseline.json` में अनाथ सूची केवल छोटी हो सकती है)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | हाँ                                          |
| `check:agent-skills-sync`         | जनरेट किए गए एजेंट-स्किल्स आर्टिफ़ैक्ट अपने स्रोत कैटलॉग से मेल खाते हैं (कोई विचलन नहीं)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `check:provider-asset-provenance` | प्रोवाइडर लोगो/एसेट के लिए रिकॉर्ड की गई उत्पत्ति प्रविष्टि मौजूद है                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `lint:json`                       | JSON कॉन्फ़िगरेशन फ़ाइलें पार्स होती हैं और रिपॉज़िटरी के लिंट नियमों को पूरा करती हैं                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| `typecheck:core`                  | त्रुटियों के बिना TypeScript संकलन (केवल परामर्शात्मक चेतावनियाँ)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | हाँ                                          |
| `typecheck:noimplicit:core`       | सख़्त `noImplicitAny` — भविष्य-उन्मुख; पहले से मौजूद कई कॉल साइटों को अभी भी एनोटेशन की आवश्यकता है                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | **परामर्शात्मक** (`continue-on-error: true`) |
| `check:dashboard-typecheck`       | `tsc`, जिसका दायरा `src/app/(dashboard)/**` (#7033) तक सीमित है — `typecheck:core` की चयनित 27-फ़ाइल अनुमति-सूची में कोई भी डैशबोर्ड TSX शामिल नहीं है, और `next build` भी कभी इसका टाइप-चेक नहीं करता (`next.config.mjs` में `ignoreBuildErrors: true` सेट है), इसलिए वहाँ के ऑर्फ़न आइडेंटिफ़ायर रिग्रेशन (#6625/#6909) CI को दिखाई नहीं देते थे। एक स्थिर प्रति-फ़ाइल/प्रति-TS-कोड गणना बेसलाइन (`config/quality/dashboard-typecheck-baseline.json`, `check:known-symbols` के समान स्टेल-एनफ़ोर्समेंट पैटर्न) के विरुद्ध अंतर जाँचा जाता है — बेसलाइन की गई गणना से अधिक केवल नई त्रुटियाँ ही गेट को विफल करती हैं; पहले से मौजूद त्रुटि ठीक होने पर `--update` के साथ बेसलाइन को क्रमशः घटाएँ।                                                                                                                                                                                                                                                  | हाँ                                          |
...
| ---------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------- |
| `quality:collect`            | `quality-metrics.json` उत्पन्न करता है (ESLint चेतावनी गणना, मर्ज की गई शार्ड रिपोर्ट से कवरेज)                                                                                                   | हाँ (रैचेट से पहले) |
| `quality:ratchet`            | `quality-baseline.json` में प्रत्येक मेट्रिक रिग्रेस नहीं हुआ है (ESLint चेतावनियाँ ≤ बेसलाइन; कवरेज ≥ बेसलाइन)                                                                                   | हाँ                 |
| `check:duplication`          | कोड दोहराव (jscpd@4), `quality-baseline.json` की बेसलाइन से अधिक नहीं है                                                                                                                          | हाँ                 |
...
| `check:dead-code`            | अप्रयुक्त एक्सपोर्ट / फ़ाइलों का रैचेट (knip), बेसलाइन की तुलना में रिग्रेस नहीं हुआ है                                                                                                           | हाँ                 |
| `check:compression-budget`   | संपीड़न बेंचमार्क बजट — प्रति-इंजन टोकन-बचत की न्यूनतम सीमाएँ रिग्रेस नहीं होनी चाहिए                                                                                                             | हाँ                 |
| `check:type-coverage`        | टाइप किए गए प्रतिशत का रैचेट (`type-coverage`) रिग्रेस नहीं हुआ है; यह काफ़ी हद तक `typecheck:noimplicit:core` को समाहित करता है                                                                  | हाँ                 |
| `check:codeql-ratchet`       | खुले CodeQL अलर्ट की संख्या रिग्रेस नहीं हुई है (`gh api` के माध्यम से पढ़ता है; टोकन के बिना सुचारु रूप से छोड़ देता है) — रीफ़्रेश आवृत्ति और मैन्युअल ट्रिगर के लिए: नीचे "CodeQL रैचेट" देखें | हाँ                 |
...
अब भी अनुपस्थित होने पर स्वयं स्किप हो जाते हैं (exit 0)।
...
| `check:secrets`          | गोपनीय जानकारी की स्कैनिंग (gitleaks) — बाइनरी अनुपस्थित होने पर स्किप करता है                                                                                                                           | **परामर्शात्मक** |
| `check:vuln-ratchet`     | निर्भरता संबंधी कमजोरियाँ (osv-scanner) बदतर नहीं होतीं — बाइनरी अनुपस्थित होने पर स्किप करता है                                                                                                         | **परामर्शात्मक** |
| `check:workflows`        | वर्कफ़्लो लिंट (actionlint + zizmor) — बाइनरियाँ अनुपस्थित होने पर स्किप करता है                                                                                                                         | **परामर्शात्मक** |
| `check:openapi-breaking` | बेस ब्रांच की तुलना में सार्वजनिक API अनुबंध (`openapi.yaml`) में ब्रेकिंग बदलाव (oasdiff) — `openapiBreaking=N` उत्सर्जित करता है; oasdiff अनुपस्थित होने या बेस स्पेक को हल न कर पाने पर स्किप करता है | **परामर्शात्मक** |

### जॉब: `docs-sync-strict`

`main` के प्रत्येक PR पर चलता है। विफलता होने पर मर्ज को रोकता है।

| स्क्रिप्ट                      | सत्यापित करता है                                                                                                                                        | अवरोधक                               |
| ------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------ |
| `check:docs-all`               | मेटा-गेट, जो नीचे दिए गए 6 सब-गेट को क्रमिक रूप से चलाता है                                                                                             | हाँ                                  |
| ↳ `check:docs-sync`            | CHANGELOG / OpenAPI / llm.txt संस्करण संगतता                                                                                                            | हाँ                                  |
...
| ↳ `check:env-doc-sync`         | `.env.example` का प्रत्येक env var किसी दस्तावेज़ तालिका में प्रलेखित है, और इसका विपरीत भी सत्य है                                                     | हाँ                                  |
| ↳ `check:deprecated-versions`  | दस्तावेज़ों में कोई पदावनत संस्करण स्ट्रिंग नहीं है                                                                                                     | हाँ                                  |
| ↳ `check:doc-links`            | दस्तावेज़ों में आंतरिक markdown लिंक वास्तविक फ़ाइलों तक पहुँचते हैं (`[text]`/`(path)` रूप)                                                            | हाँ                                  |
| ↳ `check:fabricated-docs`      | दस्तावेज़ों में उद्धृत रूट, env vars, CLI कमांड, हुक नाम और फ़ाइल पथ कोडबेस में मौजूद हैं। `--strict` के माध्यम से हार्ड गेट; फ़्लैग के बिना सॉफ्ट-फेल। | हाँ (CI में `--strict` के माध्यम से) |
| `check:cli-i18n`               | CLI कमांड स्ट्रिंग सभी i18n लोकेल फ़ाइलों में मौजूद हैं                                                                                                 | हाँ                                  |
| `check:openapi-coverage`       | OpenAPI स्पेक वास्तविक रूट के कम-से-कम रैचेट द्वारा निर्धारित न्यूनतम स्तर को कवर करता है                                                               | हाँ                                  |
| `check:openapi-security-tiers` | `openapi.yaml` में सुरक्षा टियर एनोटेशन `routeGuard.ts` वर्गीकरणों के अनुरूप हैं                                                                        | **परामर्शात्मक**                     |
| `check:openapi-routes`         | `openapi.yaml` का प्रत्येक पथ वास्तविक `route.ts` तक पहुँचता है (मतिभ्रम-रोधी)                                                                          | हाँ                                  |
| `check:docs-symbols`           | `docs/**/*.md` में प्रत्येक `/api/...` संदर्भ वास्तविक `route.ts` तक पहुँचता है (मतिभ्रम-रोधी)                                                          | हाँ                                  |
| `i18n translation drift`       | i18n लोकेल फ़ाइलों में अनूदित न की गई कुंजियाँ — केवल चेतावनी                                                                                           | **परामर्शात्मक**                     |

### जॉब: `i18n-ui-coverage`

| स्क्रिप्ट                         | क्या सत्यापित करती है                                                                                                                                                                  | अवरोधक           |
| --------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------- |
| `check-ui-keys-coverage` (इनलाइन) | UI i18n कुंजी कवरेज ≥ 65% है                                                                                                                                                           | हाँ              |
| `check-ui-value-drift` (इनलाइन)   | दोबारा लिखा गया अंग्रेज़ी **मान** किसी पुराने अनुवाद को पीछे नहीं छोड़ता                                                                                                               | हाँ              |
| `check-new-key-coverage` (इनलाइन) | एक **नई** अंग्रेज़ी कुंजी प्रत्येक लोकेल तक पहुँचती है                                                                                                                                 | हाँ              |
| `check-translation-ratio`         | प्रति लोकेल वास्तविक-अनुवाद अनुपात (अंग्रेज़ी के समान / प्लेसहोल्डर / अनुमति-सूची से बाहर अनुपलब्ध लीफ़) `config/quality/i18n-translation-baseline.json` + छूट से अधिक नहीं होना चाहिए | **परामर्शात्मक** |

`fetch-depth: 0` आवश्यक है — मान-ड्रिफ़्ट गेट मर्ज बेस के विरुद्ध `en.json` का अंतर देखता है।
...
दोबारा लिखा जाता है और _पिछले_ अंग्रेज़ी मान से बने अनुवाद वहीं रह जाते हैं, इसलिए
गैर-अंग्रेज़ी उपयोगकर्ता आत्मविश्वास से लिखी गई, पर अब गलत सामग्री पढ़ते रहते हैं।

यह वास्तव में रिलीज़ हुआ था। Antigravity लॉगिन सहायक आने पर (#5203)
`oauthModal.googleOAuthWarning` को दोबारा लिखा गया था; **43 में से 39 लोकेल** में ऐसा पाठ बना रहा जो ऑपरेटरों से कहता था कि वे "पूरा
URL कॉपी करके नीचे पेस्ट करें" — एक ऐसा प्रवाह जो उस प्रदाता के लिए पूरा ही नहीं हो सकता। इस पर
#8463 तक ध्यान नहीं गया, क्योंकि:
...
- `check-translation-drift` `docs/i18n/<locale>/**.md` दस्तावेज़ीकरण मिरर ट्रैक करता है —
  यह कभी `src/i18n/messages/*.json` नहीं पढ़ता। 2026-09 री-सिंक से जॉब `docs-sync-strict` में अवरोधक:
  कोई मुख्य दस्तावेज़ संपादित करें → `npm run i18n:run -- --files=<doc>` (सेक्शन-स्तरीय, कम लागत वाला)।

**अंतर-जागरूक, बेसलाइन-समर्थित नहीं।** यह मर्ज बेस पर मौजूद `en.json` की तुलना
वर्किंग ट्री से करता है; प्रत्येक ऐसी कुंजी के लिए, जिसका अंग्रेज़ी मान बदला है, कोई भी लोकेल जिसमें अभी भी
अछूता अनुवाद मौजूद है, पुराना माना जाता है। यह जानबूझकर **पहले से मौजूद तकनीकी ऋण को स्थिर रखता है** — कोई अंतर
यह नहीं बता सकता कि लंबे समय से मौजूद अनुवाद किस पुराने अंग्रेज़ी पाठ से आया था, इसलिए गेट
केवल उन चीज़ों का आकलन करता है जिन्हें वर्तमान परिवर्तन छूता है। विकल्प (प्रति-कुंजी हैश बेसलाइन) के लिए
...
2. उन्हें `__MISSING__:<new english>` पर सेट करें — इसके बाद रनटाइम संशोधित अंग्रेज़ी पाठ प्रस्तुत करता है
   (`src/i18n/request.ts::deepMergeFallback`, #7258) और कुंजी अनुवाद के लिए कतार में चली जाती है।

यदि स्ट्रिंग का **अर्थ** बदल गया है, तो **कुंजी का नाम बदलना** बेहतर है: कोई नई कुंजी
...
जब बेस कैटलॉग पढ़ा नहीं जा सकता (बेस ref के बिना शैलो
क्लोन), तो `check-openapi-breaking` के अनुरूप यह `SKIP reason=base-unresolved` के साथ 0 पर समाप्त होता है।

### जॉब: `i18n`

पूर्ण i18n सत्यापन मैट्रिक्स (प्रति लोकेल एक जॉब)। पूरी जॉब परामर्शात्मक है।

| स्क्रिप्ट                       | क्या सत्यापित करती है         | अवरोधक                                                   |
| ------------------------------- | ----------------------------- | -------------------------------------------------------- |
| `validate_translation.py quick` | प्रति लोकेल अनुवाद की पूर्णता | **परामर्शात्मक** (पूरी जॉब पर `continue-on-error: true`) |
...
| स्क्रिप्ट              | क्या सत्यापित करती है                                                                                                                               | अवरोधक |
| ---------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- | ------ |
| `check:pr-test-policy` | `src/`, `open-sse/`, `electron/`, या `bin/` में प्रोडक्शन कोड बदलने वाले PR में परीक्षण शामिल या अपडेट किए जाने चाहिए (कठोर नियम #8)                | हाँ    |
| `check:test-masking`   | बदली गई परीक्षण फ़ाइलें शुद्ध अभिकथन संख्या कम नहीं करतीं और न ही `assert.ok(true)` जैसे पुनरुक्तिपूर्ण अभिकथन जोड़ती हैं                           | हाँ    |
| `check:pr-evidence`    | PR विवरण में परिवर्तन के लिए परीक्षण/VPS साक्ष्य का उल्लेख होता है (PR गद्य में खोज करके कठोर नियम #18 को स्वचालित बनाता है — नाज़ुक, बैकलॉग देखें) | हाँ    |
...
| `test:vitest:ui` | UI कंपोनेंट परीक्षण — vitest रनर                 | **अवरोधक** — पहले से मौजूद विफलताओं को `vitest.config.ts` में स्पष्ट रूप से बाहर रखा गया है; नई विफलताएँ जॉब को विफल कर देती हैं |

### रात्रिकालीन वर्कफ़्लो (शेड्यूल किए गए, परामर्शात्मक)

ये cron शेड्यूल (और `workflow_dispatch`) पर चलते हैं, PR पर कभी नहीं। ये सभी परामर्शात्मक हैं।

| वर्कफ़्लो              | सत्यापित करता है                                                                                                                                                                | अवरोधक           |
| ---------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------- |
| `nightly-property`     | रैंडम सीड + उच्च रन संख्या के साथ fast-check प्रॉपर्टी परीक्षण                                                                                                                  | **परामर्शात्मक** |
| `nightly-resilience`   | हीप-वृद्धि गेट, केऑस फ़ॉल्ट-इंजेक्शन, k6 लोड/सोक                                                                                                                                | **परामर्शात्मक** |
| `nightly-llm-security` | promptfoo इंजेक्शन गार्ड (ब्लॉक मोड) + garak प्रोब (प्रदाता सीक्रेट के बिना छोड़े जाते हैं)                                                                                     | **परामर्शात्मक** |
| `nightly-schemathesis` | `docs/openapi.yaml` का उपयोग करके लाइव OmniRoute के विरुद्ध OpenAPI कॉन्ट्रैक्ट फ़ज़िंग (schemathesis) — स्पेक उल्लंघनों / अनहैंडल्ड 500 त्रुटियों को उजागर करता है (चरण 8 B.4) | **परामर्शात्मक** |
| `nightly-mutation`     | तेज़ यूनिट लेन पर Stryker म्यूटेशन-परीक्षण स्कोर — जीवित रहने वाले म्यूटेंट कमज़ोर असर्शन उजागर करते हैं                                                                        | **परामर्शात्मक** |
...
किसी भी अनुपस्थित या अतिरिक्त लीफ़ पर विफल हो जाता है, भले ही कुंजी कब जोड़ी गई हो। `__MISSING__:`
प्लेसहोल्डर उपस्थित माने जाते हैं (उनकी सामग्री अनुपात गेट का विषय है)। यह दो
डिफ़-आधारित/प्रतिशत गेटों का पूर्ण पूरक है: `check-ui-keys-coverage` प्रत्येक लोकेल के लिए 80 % की
न्यूनतम सीमा लागू करता है (लगभग 13,000 में से 43 अनुपस्थित कुंजियाँ भी 99.7 % दर्शाती हैं) और
`check-new-key-coverage` केवल उन कुंजियों का मूल्यांकन करता है जिन्हें कोई PR `en.json` में जोड़ता है।
किसी लोकेल बैच को उस दिन के `en.json` से जनरेट किया जाता है जिस दिन उसकी ब्रांच बनाई जाती है और
वह कई दिनों तक अनुवाद करता रहता है, जबकि बेस में कुंजियाँ जुड़ती रहती हैं; बैच PR स्वयं कोई कुंजी
नहीं जोड़ता, इसलिए जब बैच 1 (#13044) नौ लोकेल में 43 कुंजियों की कमी के साथ और बैच 2 (#13660)
आठ लोकेल में 10 कुंजियों की कमी के साथ मर्ज हुआ (2026-09-15), तब दोनों संबंधित गेट मौन रहे।
लाल स्थिति को ठीक करने के लिए
`node scripts/i18n/sync-ui-keys.mjs --locale=<codes> --translate-markers` चलाएँ; किसी `extra` लीफ़
का अर्थ है कि स्रोत ने उसे हटा दिया है — उसे लोकेल से हटाएँ। `--warn` विफल हुए बिना रिपोर्ट करता है।
`--catalog=cli` यही तुलना `bin/cli/locales` पर चलाता है (`npm run i18n:check-keys:cli`);
दोनों चरण जॉब `i18n-ui-coverage` में मौजूद हैं।

#### `check-new-key-coverage` — नई-कुंजी i18n गेट

`check-ui-value-drift` का सहोदर। वह ऐसे अंग्रेज़ी मान को पकड़ता है जिसे **फिर से लिखा गया**
हो, जबकि उसके अनुवाद पीछे छूट गए हों; यह ऐसी अंग्रेज़ी कुंजी को पकड़ता है जिसे **जोड़ा गया**
हो, जबकि कुछ लोकेल को वह कभी मिली ही न हो।

`check-ui-keys-coverage` इस श्रेणी को नहीं देख सकता: यह प्रत्येक लोकेल के लिए एक प्रतिशत न्यूनतम
सीमा लागू करता है, और लगभग 13,000 में से ग्यारह अनुपस्थित कुंजियाँ कवरेज को 99.9% पर बनाए रखती हैं।
प्रति भाषा प्रतिशत यह व्यक्त नहीं कर सकता कि "यह फ़ीचर बिना अनुवाद के जारी हुआ" — कोई संपूर्ण
फ़ीचर किसी नए लोकेल में बिना किसी टेक्स्ट के आ सकता है और संख्या में कोई बदलाव नहीं कर सकता।

वह घटना जिसे यह निरूपित करता है: Orchestration Canvas के Phase 3 ने उस समय मौजूद 42 लोकेल में
अपनी ग्यारह कुंजियों का अनुवाद किया था। कुछ घंटों बाद EU-भाषा बैच (#13044) ने रिपॉज़िटरी को
51 लोकेल तक पहुँचा दिया, और नौ नए लोकेल (`el`, `et`, `ga`, `hr`, `lt`, `lv`, `mt`, `sl`, `sr`) को
वे कुंजियाँ कभी नहीं मिलीं। किसी अनुपस्थित कुंजी के लिए `deepMergeFallback` अंग्रेज़ी टेक्स्ट
प्रतिस्थापित करता है, इसलिए विफलता का रूप खाली UI के बजाय अनुवाद-रहित UI था — वास्तविक, और
संरचना के कारण मौन।

अपने सहोदर की तरह यह भी **डिफ़-जागरूक** है, जो मर्ज बेस पर मौजूद अंग्रेज़ी की तुलना वर्किंग
ट्री से करता है, इसलिए पहले से मौजूद कमियाँ स्थिर रहती हैं और गेट को चालू करने के लिए किसी
माइग्रेशन की आवश्यकता नहीं पड़ी। बचाव का रास्ता:
`__MISSING__:<english>` रनटाइम को सही रखते हुए अनुवाद को स्थगित करता है। `vi` प्लेसहोल्डर
प्रतिबंधित करता है (`tests/unit/i18n-vi-completeness.test.ts`) और उसे वास्तविक अनुवाद चाहिए।

#### `check-vitest-exclusions` — स्थगित-परीक्षण गेट

`vitest.config.ts` की `exclude` सूची में मौजूद फ़ाइल ऐसा परीक्षण है जो चलता नहीं है, और ट्री
पढ़ने वाले व्यक्ति को वह कवरेज जैसा दिखाई देता है। बासठ फ़ाइलें इस टिप्पणी के पीछे जमा हो गईं:
`// #8618 — pre-existing failure; remove this exclusion when fixed`। Issue #8618 को
2026-08-11 को बंद कर दिया गया था, जबकि उसके द्वारा ट्रैक की जाने वाली सूची 45 प्रविष्टियों से
बढ़कर 62 हो गई थी और प्रत्येक नई प्रविष्टि को एक बंद issue की ओर संकेत करने वाली टिप्पणी
विरासत में मिली थी। जब अंततः सूची को फ़ाइल-दर-फ़ाइल मापा गया (#13204), तो **62 में से 51
वर्तमान ट्री पर स्रोत में किसी बदलाव के बिना पास हो गईं**।

गेट यह आवश्यक बनाता है कि वास्तविक फ़ाइल में रिज़ॉल्व होने वाला प्रत्येक एक्सक्लूज़न (a) किसी
ट्रैकिंग issue का नाम दे और (b) अपनी मापी गई स्थिति के साथ
`config/quality/vitest-exclusions.json` में दिखाई दे, ताकि किसी एक्सक्लूज़न को जोड़ना 60-प्रविष्टि
वाले ऐरे में एक और पंक्ति जोड़ने के बजाय किसी समर्पित फ़ाइल में समीक्षा-योग्य डिफ़ हो। यह जानबूझकर
एक्सक्लूड किए गए परीक्षणों को दोबारा नहीं चलाता — इसमें लगभग 10 मिनट लगते हैं और यह किसी आवधिक
जॉब का कार्य है; इन्वेंट्री दर्ज करती है कि प्रत्येक को पिछली बार कब मापा गया था।
