...
at kalde administrationsendpoints, uden at brugeren skal angive en JWT eller
...
3. CLI'en sender kun tokenet som `x-omniroute-cli-token`, når den fundne
   ...
   omdirigering ikke kan videresende det til en anden origin. Fjernkontekster bruger afgrænsede
   adgangstokens i stedet. Hvis afledningen ikke er tilgængelig, udelader CLI'en headeren,
...
| Egenskab                                   | Detaljer                                                                                                                                                                                             |
| ------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Kun loopback**                           | Accepteres kun, når serverens betroede peer-lokalitetsstempel (afledt af den faktiske TCP-peeradresse) angiver loopback. Den klientstyrede `Host`-header betros aldrig til bestemmelse af lokalitet. |
...
| **Kan ikke eksporteres**                   | Tokenet skrives aldrig til disk eller logges.                                                                                                                                                        |

## Rotation af salt

Angiv `OMNIROUTE_CLI_SALT` for at rotere det afledte token uden kodeændringer.
Efter rotationen vil alle CLI-processer på denne maskine automatisk bruge det nye token.
Dette er nyttigt efter en lækage af proceslisten, som kan have afsløret den
tidligere afledte værdi.

```bash
# Permanent rotation (føj til shell-profilen)
export OMNIROUTE_CLI_SALT="my-secret-salt-2026"

# Bekræft, at det nye token er i brug
omniroute status
```
...
indgående header med hvert token ved hjælp af `timingSafeEqual`
(`src/server/authz/policies/management.ts` og `src/lib/middleware/cliTokenAuth.ts`).
Et token er således gyldigt, hvis det matcher **enten** HMAC-digestet på 64 tegn eller det
ældre SHA-256-præfiks på 32 tegn.

**Fravalg:** Angiv `OMNIROUTE_DISABLE_CLI_TOKEN=true` (i miljøet eller `.env`) for at deaktivere
CLI-tokenmekanismen helt. Al adgang kræver derefter en eksplicit API-nøgle. På værter med flere brugere
anbefales dette, da `machine-id` gælder pr. enhed (ikke pr. bruger), og en anden
bruger på samme vært kan beregne det samme token.
