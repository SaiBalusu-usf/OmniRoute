...
`.github/workflows/quality.yml` körs för PR:er som riktar sig mot `release/**`. Det håller
bidragsgivares grenar i rörelse med sökvägsfiltrerade snabbgrindar samt en rådgivande signal från produktionsbygget för
kodändringar:
...
| `Fast Quality Gates`                             | Kod-PR:er; statiska kontroller, typkontroll, typkontroll av instrumentpanelen, berörda enhetstester                                                                                                                                | Ja                                                                                                         |
| `Forgotten sibling tests`                        | Kod-PR:er; ändrade moduler spåras till statiska konsumenter och möjliga syskontester; barrel- och dynamiska importsökvägar rapporteras som rådgivande diagnostik, med refererade undantag i tillåtelselistan                       | **Rådgivande**                                                                                             |
| `Vitest (fast-path)`                             | Kod-PR:er; snabb Vitest-svit                                                                                                                                                                                                       | Ja                                                                                                         |
| `Unit Tests fast-path`                           | Kod-PR:er; enhetstestsvit med 4 shards                                                                                                                                                                                             | Ja                                                                                                         |
| `No new ESLint warnings`                         | Kod-PR:er; undertryckningsmedvetet lint-skydd                                                                                                                                                                                      | Ja för PR:er från det egna arkivet, rådgivande för förgreningar                                            |
| `Merge integrity (changelog + generated skills)` | Icke-utkast till PR:er; synkronisering av ändringslogg och genererade färdigheter                                                                                                                                                  | Ja för PR:er från det egna arkivet, rådgivande för förgreningar                                            |
...
`ändrad modul/symbol -> statisk konsument -> möjligt syskontest` när det möjliga
testet saknas i pull request-diffen. Markdown-sammanfattningen och JSON-resultatet bevaras som
workflow-artefakten `forgotten-sibling-tests` för kalibrering innan en blockerande lansering genomförs.
...
testet, ge en specifik motivering och länka till ett GitHub-ärende eller en pull request. Felaktigt utformade poster
nekas som standard. Undantag kan inte undertrycka ett borttaget möjligt test eller en diff som lägger till `.skip`/`.todo`;
försvagning av assertioner och annan maskering hanteras fortsatt av den separat blockerande
grinden `check:test-masking`.
...
| `check:model-lifecycle`           | De tre manuellt underhållna routningstabellerna förblir konsekventa med den incheckade livscykelögonblicksbilden (#11503): `FITNESS_TABLE` (`taskFitness.ts`) poängsätter inget avvecklat id som `REGISTRY` kan routa; varje mål för `BUILT_IN_ALIASES` finns i `REGISTRY` och saknas i ögonblicksbilden över avvecklade id:n; varje avvecklat id som fortfarande finns i `REGISTRY` vidarebefordras eller listas i `allowedRetiredInCatalog`; och ingen källa eller inget mål i `DEFAULT_DEGRADATION_MAP` förekommer som avvecklat i den ögonblicksbilden. Detta bevisar inte att en modell för närvarande tillhandahålls av en aktiv uppströmsleverantör. Offline — jämför mot `config/quality/model-lifecycle.json`, som uppdateras manuellt med `npm run quality:refresh-model-lifecycle` (nätverk; inte integrerat i CI). `allowedRetiredInCatalog` är en stegvis avvecklingsspärr: lägg bara till en post tillsammans med ett spårningsärende. | Ja                                         |
| `check:fetch-targets`             | Varje `fetch("/api/...")` i `src/` på klientsidan matchar en faktisk `route.ts`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | Ja                                         |
| `check:deps`                      | Alla beroenden som kan installeras med `npm install` i varje `package.json` i repositoriet finns i `dependency-allowlist.json`; nya paket utan låst version eller paket som misstänks för slopsquatting flaggas                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | Ja                                         |
...
| `check:tracked-artifacts`         | Inga byggartefakter eller incheckade `node_modules`-symboliska länkar (körs även i Husky före commit; före push är avsiktligt lättviktig — #6716)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | Ja                                         |
...
| `check:migration-numbering`       | SQL-filer för migreringar är numrerade i ordningsföljd, utan luckor eller dubbletter                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | Ja                                         |
| `check:public-creds`              | Inga literala OAuth-värden för `client_id`/`client_secret` eller Firebase-webbnycklar utanför `publicCreds.ts` (hård regel nr 11)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | Ja                                         |
| `check:db-rules`                  | Ingen rå SQL utanför modulerna i `src/lib/db/`; inga barrel-importer från `localDb.ts` (hårda regler nr 2/nr 5)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | Ja                                         |
| `check:known-symbols`             | Provider-exekverare, routningsstrategier och översättare som registrerats i sina dispatch-tabeller överensstämmer med filerna på disken – inga övergivna eller odeklarerade symboler                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | Ja                                         |
| `check:route-guard-membership`    | Varje rutt som startar en underordnad process klassificeras av `isLocalOnlyPath()` (hårda regler nr 15/nr 17)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | Ja                                         |
| `check:test-discovery`            | Varje `*.test.ts`- eller `*.spec.ts`-fil i kodförrådet samlas in av minst en testkörare (spärrmekanism: listan över övergivna filer i `test-discovery-baseline.json` får endast minska)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | Ja                                         |
| `check:agent-skills-sync`         | Genererade agent-skills-artefakter överensstämmer med sin källkatalog (ingen avvikelse)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| `check:provider-asset-provenance` | Providerlogotyper/-resurser har en registrerad provenienspost                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| `typecheck:noimplicit:core`       | Strikt `noImplicitAny` — framåtblickande; många befintliga anropsplatser behöver fortfarande annoteringar                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | **Rådgivande** (`continue-on-error: true`) |
| `check:dashboard-typecheck`       | `tsc` begränsad till `src/app/(dashboard)/**` (#7033) — den handplockade listan med 27 filer i `typecheck:core` innehåller ingen TSX för kontrollpanelen, och `next build` typkontrollerar den inte heller (`next.config.mjs` anger `ignoreBuildErrors: true`), så regressioner med fristående identifierare där (#6625/#6909) var osynliga för CI. Jämför mot en fryst baslinje för antal per fil/per TS-kod (`config/quality/dashboard-typecheck-baseline.json`, samma mönster för kontroll av inaktuella poster som i `check:known-symbols`) — endast NYA fel utöver det baslinjeförda antalet gör att kontrollen misslyckas; sänk stegvis med `--update` när ett befintligt fel har åtgärdats.                                                                                                                                                                                                                                                   | Ja                                         |
...
| ---------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------- |
| `quality:collect`            | Skapar `quality-metrics.json` (antal ESLint-varningar, täckning från den sammanslagna fragmentrapporten)                                                                               | Ja (föregår spärrmekanismen) |
...
| `check:complexity`           | Cyklomatisk komplexitet på filnivå överskrider inte gränsen (grundläggande ESLint-reglerna `complexity` + `max-lines-per-function`)                                                    | Ja                           |
...
| `check:codeql-ratchet`       | Antalet öppna CodeQL-varningar försämras inte (läses via `gh api`; hoppas över utan fel om token saknas) — uppdateringsfrekvens och manuell utlösning: se "CodeQL-spärrmekanism" nedan | Ja                           |

### Jobb: `quality-extended`

Hela jobbet är rådgivande (`continue-on-error: true`). De npm-baserade spärrkontrollerna körs
...
| `check:openapi-breaking` | Brytande ändringar i det offentliga API-kontraktet (`openapi.yaml`) jämfört med basgrenen (oasdiff) — matar ut `openapiBreaking=N`; hoppas över om oasdiff saknas eller basspecifikationen inte kan hittas | **Rådgivande** |
...
| Skript                         | Validerar                                                                                                                                                         | Blockerande              |
| ------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------ |
| `check:docs-all`               | Metagrind som kör de 6 undergrindarna nedan sekventiellt                                                                                                          | Ja                       |
| ↳ `check:docs-sync`            | Versionsöverensstämmelse mellan CHANGELOG / OpenAPI / llm.txt                                                                                                     | Ja                       |
| ↳ `check:docs-counts`          | Antal i löptext (antal leverantörer, antal migreringar osv.) ligger inom spärrkontrollens intervall för de verkliga antalen                                       | Ja                       |
| ↳ `check:env-doc-sync`         | Varje miljövariabel i `.env.example` dokumenteras i en dokumentationstabell och vice versa                                                                        | Ja                       |
...
| `check:openapi-coverage`       | OpenAPI-specifikationen täcker minst en spärrstyrd lägstanivå av verkliga rutter                                                                                  | Ja                       |
...
| Skript                            | Validerar                                                                                                                                                                                                     | Blockerande    |
| --------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------- |
| `check-ui-keys-coverage` (inline) | Täckningen för UI:ns i18n-nycklar är ≥ 65 %                                                                                                                                                                   | Ja             |
| `check-ui-value-drift` (inline)   | Ett omskrivet engelskt **värde** lämnar inte kvar någon inaktuell översättning                                                                                                                                | Ja             |
| `check-new-key-coverage` (inline) | En **ny** engelsk nyckel når alla språkversioner                                                                                                                                                              | Ja             |
| `check-translation-ratio`         | Andelen riktiga översättningar per språkversion (oförändrade engelska värden/platshållare/saknade löv utanför tillåtelselistan) får inte överstiga `config/quality/i18n-translation-baseline.json` + marginal | **Rådgivande** |

Kräver `fetch-depth: 0` — kontrollen för värdeavvikelse jämför `en.json` med sammanslagningsbasen.

#### `check-ui-value-drift` — kontroll för inaktuella översättningar

Fångar den enda i18n-regression som de andra kontrollerna strukturellt inte kan upptäcka: ett engelskt värde
skrivs om medan översättningarna som baserades på den _föregående_ engelska texten ligger kvar, så att
icke-engelskspråkiga användare fortsätter läsa självsäkert formulerad men numera felaktig text.

Detta nådde faktiskt produktion. `oauthModal.googleOAuthWarning` skrevs om när inloggningshjälpen
Antigravity lades till (#5203); **39 av 43 språkversioner** behöll text som uppmanade operatörer att ”kopiera
hela URL:en och klistra in den nedan” — ett flöde som inte kan slutföras för den leverantören. Det
...
arbetskopian; för varje nyckel vars engelska värde har ändrats är varje språkversion som fortfarande har en
orörd översättning inaktuell. Detta **fryser avsiktligt befintlig skuld** — en diff
kan inte avslöja vilken äldre engelsk text en sedan länge befintlig översättning baserades på, så kontrollen bedömer
endast det som den aktuella ändringen berör. Alternativet (en hashbaslinje per nyckel) skulle kräva
en genererad fil på cirka 600 kB, tre gånger större än den största befintliga baslinjen, med ändringar i varje i18n-PR.
...
Om strängens **betydelse** har ändrats bör du helst **byta namn på nyckeln**: en ny nyckel kan inte ärva
...
Fullständig valideringsmatris för i18n (ett jobb per språkversion). Hela jobbet är rådgivande.
...
| ---------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------- |
| `nightly-property`     | fast-check-egenskapstester med ett slumpmässigt startvärde + ett högt antal körningar                                                                                 | **Rådgivande** |
| `nightly-resilience`   | gränskontroll för heap-tillväxt, felinjektion med kaostestning, k6-belastnings-/uthållighetstest                                                                      | **Rådgivande** |
...
| `nightly-mutation`     | Poäng från Stryker-mutationstestning för det snabba enhetstestspåret — överlevande mutanter synliggör svaga assertioner                                               | **Rådgivande** |
...
- Leveranskedja (proveniens, SBOM, Trivy, Scorecard): [`docs/security/SUPPLY_CHAIN.md`](../security/SUPPLY_CHAIN.md)
...
misslyckas vid varje saknad eller extra lövnyckel, oavsett när nyckeln lades till. Platshållare av
typen `__MISSING__:` räknas som närvarande (deras innehåll hanteras av kvotspärren). Den utgör
det absoluta komplementet till de två diffbaserade/procentuella spärrarna: `check-ui-keys-coverage`
kräver minst 80 % per språkversion (43 saknade nycklar av cirka 13 000 visas fortfarande som
99,7 %) och `check-new-key-coverage` bedömer endast de nycklar som en PR lägger till i `en.json`.
En språkversionsbatch genereras från den `en.json` som finns den dag dess gren skapas och
översätts under flera dagar medan basen fortsätter att få nya nycklar; batchens PR lägger inte själv
till någon nyckel, så båda syskonkontrollerna förblev tysta när batch 1 (#13044) slogs samman med
43 saknade nycklar i nio språkversioner och batch 2 (#13660) med 10 saknade nycklar i åtta
(2026-09-15). Åtgärda ett rött resultat med
...
Syskon till `check-ui-value-drift`. Den kontrollen upptäcker när ett engelskt värde har
**skrivits om** utan att dess översättningar har uppdaterats; den här upptäcker när en engelsk
nyckel har **lagts till** utan att vissa språkversioner har fått den.

`check-ui-keys-coverage` kan inte upptäcka den här typen av fel: den kräver en lägsta procentsats
per språk, och elva saknade nycklar av cirka 13 000 ger fortfarande en täckning på 99,9 %. En
procentsats per språk kan inte uttrycka ”den här funktionen levererades oöversatt” — en hel
funktion kan införas i en ny språkversion utan någon text alls utan att siffran förändras.

Incidenten som kontrollen kodifierar: Fas 3 av Orchestration Canvas fick sina elva nycklar
översatta i de 42 språkversioner som fanns vid den tidpunkten. Några timmar senare utökade
EU-språksbatchen (#13044) kodbasen till 51 språkversioner, och de nio nytillkomna (`el`, `et`,
`ga`, `hr`, `lt`, `lv`, `mt`, `sl`, `sr`) fick dem aldrig. `deepMergeFallback` ersätter en saknad
nyckel med engelska, så felet yttrade sig som ett oöversatt användargränssnitt snarare än ett
tomt användargränssnitt — ett verkligt fel som genom sin konstruktion förblev tyst.

Precis som sitt syskon är den **diffmedveten** och jämför engelskan vid sammanslagningsbasen
med arbetsträdet, så befintliga luckor förblir frysta och spärren behövde ingen migrering för att
aktiveras. Undantagsmekanism: `__MISSING__:<english>` skjuter upp en översättning samtidigt
som korrekt beteende vid körning bibehålls. `vi` förbjuder platshållare
(`tests/unit/i18n-vi-completeness.test.ts`) och kräver en riktig översättning.

#### `check-vitest-exclusions` — spärr för parkerade tester

En fil i `exclude`-listan i `vitest.config.ts` är ett test som inte körs, och för den som läser
trädet ser den ut som täckning. Sextiotvå filer samlades bakom kommentaren
`// #8618 — befintligt fel; ta bort detta undantag när det är åtgärdat`. Ärende #8618 stängdes
2026-08-11 medan listan som ärendet följde växte från 45 till 62 poster, där varje ny post ärvde
en kommentar som pekade på ett avslutat ärende. När listan slutligen mättes fil för fil (#13204)
**klarade sig 51 av de 62 mot det aktuella trädet utan någon ändring i källkoden**.

Spärren kräver att varje undantag som kan lösas till en verklig fil (a) anger ett uppföljningsärende
och (b) finns i `config/quality/vitest-exclusions.json` med sin uppmätta status, så att tillägget av
ett undantag blir en granskningsbar diff i en särskild fil i stället för ännu en rad i en array med
60 poster. Den kör avsiktligt inte om de undantagna testerna — det tar cirka 10 minuter och hör
