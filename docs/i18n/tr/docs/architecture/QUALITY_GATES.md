...
## Kapı Envanteri (~90 betik)

Betikler `scripts/check/` (politika kapıları) ve `scripts/quality/` (ratchet motoru)
...
dallarındaki ilerlemeyi, yol filtreli hızlı kapıların yanı sıra kod değişiklikleri için danışman
niteliğinde tek bir üretim derlemesi sinyaliyle sürdürür:

| İş                                               | Kapsam                                                                                                                                                                                                                               | Engelleyici                                                                                              |
| ------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------- |
| `Build (advisory)`                               | Taslak olmayan kod PR'ları ve Mergify kuyruk dalları; Node 24, `npm-ci-retry`, `check:node-runtime`, `OMNIROUTE_USE_TURBOPACK=1` ile `npm run build`; bunu kullanan aşağı akış kalite işi olmadığından yapıt yüklemesi yok           | **Danışman** (`continue-on-error: true`; kararlı sürüm PR'ı çalıştırmalarından bir hafta sonra kaldırın) |
| `Docs Gates (fast-path)`                         | Dokümantasyon/kod PR'ları; API dokümantasyonu referansları ve docs-all                                                                                                                                                               | Evet                                                                                                     |
| `Fast Quality Gates`                             | Kod PR'ları; statik kontroller, tür kontrolü, pano tür kontrolü, etkilenen birim testleri                                                                                                                                            | Evet                                                                                                     |
| `Forgotten sibling tests`                        | Kod PR'ları; değiştirilen modüllerden statik tüketicilere ve aday kardeş testlere kadar izleme; barrel ve dinamik içe aktarma yolları, referans verilen izin listesi istisnalarıyla birlikte danışman tanılamaları olarak raporlanır | **Danışman**                                                                                             |
| `Vitest (fast-path)`                             | Kod PR'ları; hızlı vitest paketi                                                                                                                                                                                                     | Evet                                                                                                     |
| `Unit Tests fast-path`                           | Kod PR'ları; 4 parçalı birim testi paketi                                                                                                                                                                                            | Evet                                                                                                     |
| `No new ESLint warnings`                         | Kod PR'ları; bastırmaları dikkate alan lint koruması                                                                                                                                                                                 | Kendi kaynağından gelenler için evet, çatallar için danışman                                             |
| `Merge integrity (changelog + generated skills)` | Taslak olmayan PR'lar; değişiklik günlüğü ve oluşturulan becerilerin eşitlenmesi                                                                                                                                                     | Kendi kaynağından gelenler için evet, çatallar için danışman                                             |

#### Unutulmuş kardeş testler raporu

`npm run check:forgotten-sibling-tests`, test etki haritasının arkasındaki içe aktarma çözümleyicisini yeniden kullanır.
Değiştirilen her üretim modülü için, aday test pull request farkında bulunmadığında deterministik
`değiştirilen modül/sembol -> statik tüketici -> aday kardeş test` zincirlerini raporlar. Markdown özeti ve JSON sonucu,
herhangi bir engelleyici kullanıma geçilmeden önce kalibrasyon amacıyla
`forgotten-sibling-tests` iş akışı yapıtı olarak saklanır.

Barrel yeniden dışa aktarımları ve dinamik içe aktarmalar yalnızca çözümleme tanılamalarıdır; hiçbir zaman
engelleyici bir bulgu oluşturmazlar. İncelenmiş istisnalar
`config/quality/forgotten-sibling-allowlist.json` içinde bulunur. Her kayıt, tüketiciyi ve aday
testi belirtmeli, belirli bir gerekçe sunmalı ve bir GitHub issue'suna veya pull request'ine bağlantı vermelidir. Hatalı biçimlendirilmiş kayıtlar
kapalı durumda hata verir. İstisnalar, silinmiş bir aday testi veya `.skip`/`.todo` ekleyen bir farkı bastıramaz;
assertion zayıflatma ve diğer maskeleme işlemleri, bağımsız olarak engelleyici olan
`check:test-masking` kapısının sorumluluğunda kalır.
...
| `check:cycles`                    | Döngüsel içe aktarımlar — tüm `src/` + `open-sse/` modülleri                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | Evet                                                 |
...
| `check:model-lifecycle`           | Elle bakımı yapılan üç yönlendirme tablosu, depoya kaydedilmiş yaşam döngüsü anlık görüntüsüyle (#11503) tutarlı kalır: `FITNESS_TABLE` (`taskFitness.ts`), `REGISTRY` tarafından yönlendirilebilen kullanımdan kaldırılmış hiçbir kimliği puanlamaz; her `BUILT_IN_ALIASES` hedefi `REGISTRY` içinde bulunur ve kullanımdan kaldırılmış kimliklerin anlık görüntüsünde bulunmaz; hâlâ `REGISTRY` içinde olan kullanımdan kaldırılmış her kimlik başka yere yönlendirilir veya `allowedRetiredInCatalog` içinde listelenir; ayrıca hiçbir `DEFAULT_DEGRADATION_MAP` kaynağı veya hedefi bu anlık görüntüde kullanımdan kaldırılmış olarak görünmez. Bu, bir modelin şu anda canlı bir yukarı akış hizmeti tarafından sunulduğunu kanıtlamaz. Çevrimdışı — elle `npm run quality:refresh-model-lifecycle` (ağ gerektirir; CI'a bağlı değildir) komutuyla yenilenen `config/quality/model-lifecycle.json` ile karşılaştırır. `allowedRetiredInCatalog`, kademeli azaltma mandalıdır: yalnızca bir takip kaydıyla birlikte girdi ekleyin. | Evet                                                 |
| `check:fetch-targets`             | İstemci tarafındaki `src/` içinde bulunan her `fetch("/api/...")`, gerçek bir `route.ts` dosyasına çözümlenir                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | Evet                                                 |
| `check:deps`                      | Depodaki her `package.json` genelinde `npm install` ile kurulabilen tüm bağımlılıklar `dependency-allowlist.json` içinde yer alır; yeni sabitlenmemiş veya ad benzerliğiyle ele geçirilmiş paketler işaretlenir                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | Evet                                                 |
| `audit:deps`                      | `npm audit` (kök + electron) — yüksek/kritik düzeyde güvenlik bildirimi yoktur (osv `check:vuln-ratchet` ile örtüşür; bkz. Gerekçelendirme İş Listesi)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | Evet                                                 |
...
| `check:tracked-artifacts`         | Derleme çıktıları / commit edilmiş `node_modules` sembolik bağlantıları yoktur (husky pre-commit sırasında da çalışır; pre-push kasıtlı olarak hafiftir — #6716)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | Evet                                                 |
| `check:vitest-exclusions`         | Her Vitest hariç tutma kaydı bir takip sorununu belirtir ve `config/quality/vitest-exclusions.json` içinde yer alır (#13204)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | Evet                                                 |
| `check:file-size`                 | Hiçbir kaynak dosya, uzantı başına sınırı aşmaz (mandal: büyük dosyalar `frozen` listesinde sabitlenmiştir)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | Evet                                                 |
...
| `check:migration-numbering`       | Geçiş SQL dosyaları ardışık olarak numaralandırılmıştır; boşluk veya yinelenen numara yoktur                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | Evet                                                 |
| `check:public-creds`              | `publicCreds.ts` dışında sabit kodlanmış OAuth `client_id`/`client_secret` veya Firebase Web anahtarı yok (Kesin Kural #11)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | Evet                                                 |
| `check:db-rules`                  | `src/lib/db/` modülleri dışında ham SQL yok; `localDb.ts` dosyasından barrel import yok (Kesin Kurallar #2/#5)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | Evet                                                 |
| `check:known-symbols`             | Gönderim tablolarına kaydedilen sağlayıcı yürütücüleri, yönlendirme stratejileri ve dönüştürücüler diskteki dosyalarla eşleşiyor — sahipsiz veya bildirilmemiş sembol yok                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | Evet                                                 |
| `check:route-guard-membership`    | Alt süreç başlatan her rota `isLocalOnlyPath()` tarafından sınıflandırılıyor (Kesin Kurallar #15/#17)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Evet                                                 |
| `check:test-discovery`            | Depodaki her `*.test.ts` / `*.spec.ts` dosyası en az bir test çalıştırıcısı tarafından toplanıyor (mandal: `test-discovery-baseline.json` içindeki sahipsizler listesi yalnızca küçülebilir)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | Evet                                                 |
| `check:agent-skills-sync`         | Oluşturulan agent-skills yapıtları kaynak kataloglarıyla eşleşiyor (sapma yok)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `check:provider-asset-provenance` | Sağlayıcı logoları/varlıkları kayıtlı bir köken girdisine sahip                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `lint:json`                       | JSON yapılandırma dosyaları ayrıştırılır ve deponun lint kurallarını karşılar                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `typecheck:core`                  | Hatasız TypeScript derlemesi (yalnızca bilgilendirme amaçlı uyarılar)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Evet                                                 |
| `typecheck:noimplicit:core`       | Katı `noImplicitAny` — ileriye dönük; önceden var olan birçok çağrı noktası hâlâ ek açıklamalara ihtiyaç duyuyor                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | **Bilgilendirme amaçlı** (`continue-on-error: true`) |
| `check:dashboard-typecheck`       | `src/app/(dashboard)/**` ile sınırlandırılmış `tsc` (#7033) — `typecheck:core` komutunun özenle seçilmiş 27 dosyalık izin listesi hiçbir dashboard TSX dosyasını içermiyor ve `next build` de bunlar için hiçbir zaman tür denetimi yapmıyor (`next.config.mjs`, `ignoreBuildErrors: true` olarak ayarlıyor); bu nedenle buradaki sahipsiz tanımlayıcı regresyonları (#6625/#6909) CI tarafından görülemiyordu. Sabitlenmiş dosya başına/TS kodu başına sayı temeliyle (`config/quality/dashboard-typecheck-baseline.json`, `check:known-symbols` ile aynı eski kayıt uygulama modeli) karşılaştırma yapar — yalnızca temel sayıların üzerindeki YENİ hatalar geçidin başarısız olmasına neden olur; önceden var olan bir hata düzeltildiğinde `--update` ile eşiği aşağı çekin.                                                                                                                                                                                                                                                       | Evet                                                 |
...
| ---------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------- |
| `quality:collect`            | `quality-metrics.json` dosyasını oluşturur (ESLint uyarı sayısı, birleştirilmiş parça raporundan kapsam)                                                                                                   | Evet (kademeli eşiğin üst akışında) |
...
| `check:cognitive-complexity` | Bilişsel karmaşıklık kademeli eşiği (`eslint-plugin-sonarjs`) — ayrı bir ESLint geçişi; CI, her ikisini tek bir `check:complexity-ratchets` adımı olarak birleştirip çalıştırır                            | Evet                                |
| `check:dead-code`            | Kullanılmayan dışa aktarımlar / dosyalar için kademeli eşik (knip), temel değere kıyasla gerilemez                                                                                                         | Evet                                |
| `check:compression-budget`   | Sıkıştırma kıyaslama bütçesi — motor başına token tasarrufu alt sınırları gerilememelidir                                                                                                                  | Evet                                |
| `check:type-coverage`        | Türlendirilmiş kod yüzdesi için kademeli eşik (`type-coverage`) gerilemez; `typecheck:noimplicit:core` denetimini büyük ölçüde kapsar                                                                      | Evet                                |
| `check:codeql-ratchet`       | Açık CodeQL uyarılarının sayısı gerilemez (`gh api` aracılığıyla okur; token olmadan kontrollü biçimde atlar) — yenileme sıklığı ve manuel tetikleme için aşağıdaki "CodeQL kademeli eşiği" bölümüne bakın | Evet                                |

### İş: `quality-extended`

İşin tamamı tavsiye niteliğindedir (`continue-on-error: true`). npm tabanlı eşik kontrolleri
gerçekten çalışır; harici tarayıcılar `gh release download` aracılığıyla kurulur ve
ikili dosya hâlâ mevcut değilse kendilerini atlar (çıkış kodu 0).

| Betik                    | Doğruladığı                                                                                                                                                                                              | Engelleyici             |
| ------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------- |
| `check:circular-deps`    | Döngüsel bağımlılık bulunmaması (dpdm)                                                                                                                                                                   | **Tavsiye niteliğinde** |
| `check:bundle-size`      | Paket boyutunun üst sınırı aşmaması                                                                                                                                                                      | **Tavsiye niteliğinde** |
| `check:secrets`          | Gizli bilgi taraması (gitleaks) — ikili dosya yoksa atlanır                                                                                                                                              | **Tavsiye niteliğinde** |
| `check:vuln-ratchet`     | Bağımlılık güvenlik açıklarının (osv-scanner) gerilememesi — ikili dosya yoksa atlanır                                                                                                                   | **Tavsiye niteliğinde** |
| `check:workflows`        | İş akışı lint kontrolü (actionlint + zizmor) — ikili dosyalar yoksa atlanır                                                                                                                              | **Tavsiye niteliğinde** |
| `check:openapi-breaking` | Genel API sözleşmesindeki (`openapi.yaml`) temel dala göre geriye dönük uyumsuz değişiklikler (oasdiff) — `openapiBreaking=N` çıktısı üretir; oasdiff yoksa veya temel belirtim çözümlenemiyorsa atlanır | **Tavsiye niteliğinde** |
...
| ↳ `check:docs-counts`          | Düzyazıdaki sayıların (sağlayıcı sayısı, geçiş sayısı vb.) gerçek sayıların eşik penceresi içinde olması                                                                                                         | Evet                                 |
| ↳ `check:env-doc-sync`         | `.env.example` içindeki her ortam değişkeninin bir dokümantasyon tablosunda belgelenmesi ve bunun tersinin de geçerli olması                                                                                     | Evet                                 |
| ↳ `check:deprecated-versions`  | Dokümantasyonda kullanım dışı bırakılmış sürüm dizelerinin bulunmaması                                                                                                                                           | Evet                                 |
| ↳ `check:doc-links`            | Dokümantasyondaki dahili markdown bağlantılarının gerçek dosyalara çözümlenmesi (`[metin]`/`(yol)` biçimi)                                                                                                       | Evet                                 |
| ↳ `check:fabricated-docs`      | Dokümantasyonda belirtilen rotaların, ortam değişkenlerinin, CLI komutlarının, kanca adlarının ve dosya yollarının kod tabanında bulunması. `--strict` ile zorunlu geçit; bayrak olmadan kontrollü başarısızlık. | Evet (CI'da `--strict` aracılığıyla) |
| `check:cli-i18n`               | CLI komut dizelerinin tüm i18n yerel ayar dosyalarında bulunması                                                                                                                                                 | Evet                                 |
| `check:openapi-coverage`       | OpenAPI belirtiminin gerçek rotaların en az eşiklerle belirlenmiş alt sınırını kapsaması                                                                                                                         | Evet                                 |
| `check:openapi-security-tiers` | `openapi.yaml` içindeki güvenlik katmanı ek açıklamalarının `routeGuard.ts` sınıflandırmalarıyla tutarlı olması                                                                                                  | **Tavsiye niteliğinde**              |
| `check:openapi-routes`         | `openapi.yaml` içindeki her yolun gerçek bir `route.ts` dosyasına çözümlenmesi (halüsinasyon önleme)                                                                                                             | Evet                                 |
| `check:docs-symbols`           | `docs/**/*.md` içindeki her `/api/...` referansının gerçek bir `route.ts` dosyasına çözümlenmesi (halüsinasyon önleme)                                                                                           | Evet                                 |
| `i18n çeviri sapması`          | i18n yerel ayar dosyalarındaki çevrilmemiş anahtarlar — yalnızca uyarı                                                                                                                                           | **Tavsiye niteliğinde**              |
...
| `check-ui-value-drift` (satır içi)   | Yeniden yazılan bir İngilizce **değer**, geride güncelliğini yitirmiş çeviri bırakmaz                                                                                                                   | Evet                    |
| `check-new-key-coverage` (satır içi) | **Yeni** bir İngilizce anahtar her yerel ayara ulaşır                                                                                                                                                   | Evet                    |
| `check-translation-ratio`            | Yerel ayar başına gerçek çeviri oranı (izin listesinin dışında kalan İngilizceyle aynı / yer tutucu / eksik yapraklar), `config/quality/i18n-translation-baseline.json` + tolerans değerini aşmamalıdır | **Tavsiye niteliğinde** |
...
Diğer kapıların yapısal olarak göremediği i18n gerilemesini yakalar: İngilizce bir değer
yeniden yazılırken _önceki_ İngilizceden türetilen çeviriler geride kalır; böylece
İngilizce dışındaki dilleri kullanan kullanıcılar kendinden emin bir dille yazılmış ancak artık yanlış olan metni okumaya devam eder.

Bu durum gerçekten üretime çıktı. Antigravity oturum açma yardımcısı eklendiğinde (#5203)
`oauthModal.googleOAuthWarning` yeniden yazıldı; **43 yerel ayarın 39'unda**, operatörlere
"tam URL'yi kopyalayıp aşağıya yapıştırmalarını" söyleyen metin kaldı — bu sağlayıcı için
tamamlanması mümkün olmayan bir akış. Şu nedenlerle #8463'e kadar fark edilmedi:

- `sync-ui-keys` yalnızca **bulunmayan** anahtarları tamamlar, **güncelliğini yitirmiş** olanları asla güncellemez;
- `check-ui-keys-coverage` anahtar _varlığını_ sayar; dolayısıyla güncelliğini yitirmiş bir çeviri kapsanmış olarak puanlanır;
- `check-translation-drift`, `docs/i18n/<locale>/**.md` dokümantasyon yansımalarını izler —
  `src/i18n/messages/*.json` dosyalarını asla okumaz. 2026-09 yeniden senkronizasyonundan beri `docs-sync-strict` işinde engelleyicidir: temel bir belgeyi düzenleyin → `npm run i18n:run -- --files=<doc>` (bölüm düzeyinde, düşük maliyetli).

**Fark değişikliklerinden haberdardır, taban çizgisine dayanmaz.** Birleştirme tabanındaki `en.json`
ile çalışma ağacını karşılaştırır; İngilizce değeri değişen her anahtar için, hâlâ
değiştirilmemiş bir çeviri içeren tüm yerel ayarlar güncelliğini yitirmiş sayılır. Bu yaklaşım, önceden var olan teknik borcu kasıtlı olarak **dondurur** — bir fark, uzun süredir mevcut olan bir çevirinin hangi eski İngilizce metinden geldiğini gösteremez; bu nedenle kapı yalnızca geçerli değişikliğin dokunduğu öğeleri değerlendirir. Alternatif yaklaşım (anahtar başına karma taban çizgisi), mevcut en büyük taban çizgisinin 3 katı olan yaklaşık 600 KB'lık oluşturulmuş bir dosya gerektirir ve her i18n PR'ında değişiklik gürültüsü yaratır.

Bunu karşılamanın iki yolu vardır:

1. etkilenen çevirileri güncelleyin veya
2. bunları `__MISSING__:<new english>` olarak ayarlayın — çalışma zamanı daha sonra düzeltilmiş İngilizceyi sunar
   (`src/i18n/request.ts::deepMergeFallback`, #7258) ve anahtar çeviri kuyruğuna alınır.

Dizgenin **anlamı** değiştiyse **anahtarı yeniden adlandırmayı** tercih edin: yeni bir anahtar,
...
Temel katalog okunamadığında (temel ref'in bulunmadığı sığ klon), `check-openapi-breaking`
...
| Betik                           | Doğruladığı                      | Engelleyici                                                        |
| ------------------------------- | -------------------------------- | ------------------------------------------------------------------ |
| `validate_translation.py quick` | Yerel ayar başına çeviri tamlığı | **Tavsiye niteliğinde** (işin tamamında `continue-on-error: true`) |
...
| Betik                  | Doğruladığı                                                                                                                                               | Engelleyici |
| ---------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------- |
| `check:pr-test-policy` | `src/`, `open-sse/`, `electron/` veya `bin/` içindeki üretim kodunu değiştiren PR'lar test eklemeli veya mevcut testleri güncellemelidir (Katı Kural #8)  | Evet        |
...
| `check:pr-evidence`    | PR gövdesi, değişiklik için test/VPS kanıtlarına atıfta bulunur (PR metninde arama yaparak Katı Kural #18'i otomatikleştirir — kırılgandır, bkz. Backlog) | Evet        |

### İş: `test-vitest`

`build` sonrasında çalışır. Başarısız olduğunda birleştirmeyi engeller.
...
### Gecelik iş akışları (zamanlanmış, bilgilendirme amaçlı)

Bunlar cron zamanlamasına göre (ve `workflow_dispatch` ile) çalışır; PR'larda hiçbir zaman çalışmaz. Tümü bilgilendirme amaçlıdır.

| İş Akışı               | Doğruladıkları                                                                                                                                                                          | Engelleyici              |
| ---------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------ |
| `nightly-property`     | Rastgele tohum ve yüksek çalıştırma sayısıyla fast-check özellik testleri                                                                                                               | **Bilgilendirme amaçlı** |
| `nightly-resilience`   | Yığın büyüme eşiği, kaos hata enjeksiyonu, k6 yük/uzun süreli yük testi                                                                                                                 | **Bilgilendirme amaçlı** |
| `nightly-llm-security` | promptfoo enjeksiyon koruması (engelleme modu) + garak probları (sağlayıcı gizli anahtarı olmadan atlanır)                                                                              | **Bilgilendirme amaçlı** |
| `nightly-schemathesis` | `docs/openapi.yaml` kullanan canlı bir OmniRoute'a karşı OpenAPI sözleşme fuzz testi (schemathesis) — spesifikasyon ihlallerini / işlenmemiş 500 hatalarını ortaya çıkarır (Fase 8 B.4) | **Bilgilendirme amaçlı** |
| `nightly-mutation`     | Hızlı birim test hattında Stryker mutasyon testi puanı — hayatta kalan mutantlar zayıf doğrulamaları ortaya çıkarır                                                                     | **Bilgilendirme amaçlı** |
| `nightly-compat`       | Desteklenen `engines.node` aralıkları genelinde Node motoru uyumluluk matrisi                                                                                                           | **Bilgilendirme amaçlı** |
...
- Tedarik zinciri (köken, SBOM, Trivy, Scorecard): [`docs/security/SUPPLY_CHAIN.md`](../security/SUPPLY_CHAIN.md)

#### `check-key-completeness` — anahtar kümesi eşliği geçidi

`scripts/i18n/check-key-completeness.mjs` (`npm run i18n:check-keys`, iş `i18n-ui-coverage`).
Her `src/i18n/messages/<locale>.json` dosyasının yaprak anahtar kümesini `en.json` ile karşılaştırır
ve anahtarın ne zaman eklendiğine bakılmaksızın eksik veya fazladan herhangi bir yaprak olduğunda
başarısız olur. `__MISSING__:` yer tutucuları mevcut sayılır (içerikleri oran geçidinin konusudur).
Bu, fark tabanlı/yüzdelik diğer iki geçidin mutlak tamamlayıcısıdır: `check-ui-keys-coverage`, yerel
başına %80'lik bir alt sınır uygular (~13.000 anahtardan 43'ünün eksik olması hâlâ %99,7 olarak
görünür) ve `check-new-key-coverage` yalnızca bir PR'ın `en.json` dosyasına eklediği anahtarları
değerlendirir. Bir yerel grubu, dalının oluşturulduğu günkü `en.json` dosyasından üretilir ve taban
yeni anahtarlar eklemeye devam ederken günlerce çevrilir; grup PR'ı kendisi hiçbir anahtar eklemez,
bu nedenle grup 1 (#13044) dokuz yerelde 43 eksik anahtarla ve grup 2 (#13660) sekiz yerelde 10
eksik anahtarla birleştirildiğinde iki kardeş geçit de sessiz kaldı (2026-09-15). Kırmızı bir sonucu
`node scripts/i18n/sync-ui-keys.mjs --locale=<codes> --translate-markers` ile düzeltin; bir `extra`
yaprağı, kaynağın onu kaldırdığı anlamına gelir — yerelden silin. `--warn`, başarısız olmadan raporlar.
`--catalog=cli`, aynı karşılaştırmayı `bin/cli/locales` üzerinde çalıştırır
(`npm run i18n:check-keys:cli`); her iki adım da `i18n-ui-coverage` işinde yer alır.

#### `check-new-key-coverage` — yeni anahtar i18n geçidi

`check-ui-value-drift` geçidinin kardeşidir. O geçit, çevirileri geride bırakılmışken **yeniden
yazılan** bir İngilizce değeri yakalar; bu geçit ise bazı yerellerin hiç almadığı, **eklenmiş** bir
İngilizce anahtarı yakalar.

`check-ui-keys-coverage` bu sınıfı göremez: yerel başına bir yüzdelik alt sınır uygular ve ~13.000
yaprak içinden on bir anahtarın eksik olması kapsamı %99,9'da bırakır. Dil başına bir yüzde,
"bu özellik çevrilmeden yayımlandı" durumunu ifade edemez — yeni bir yereldeki özelliğin tamamı
hiçbir metin olmadan eklenebilir ve sayı hiç değişmeyebilir.

Kodladığı olay şudur: Orchestration Canvas'ın 3. Aşaması, on bir anahtarını o sırada mevcut olan
42 yerelin tamamına çevirdi. Saatler sonra AB dilleri grubu (#13044), depodaki yerel sayısını
51'e çıkardı ve yeni gelen dokuz yerel (`el`, `et`, `ga`, `hr`, `lt`, `lv`, `mt`, `sl`, `sr`) bu
anahtarları hiçbir zaman almadı. `deepMergeFallback`, eksik bir anahtarın yerine İngilizceyi
koyduğundan, hata biçimi boş bir kullanıcı arayüzü değil, çevrilmemiş bir kullanıcı arayüzüydü —
gerçekti ve tasarım gereği sessizdi.

Kardeşi gibi bu da **fark duyarlıdır**; birleştirme tabanındaki İngilizceyi çalışma ağacıyla
karşılaştırır, böylece önceden mevcut olan boşluklar sabit kalır ve geçidin etkinleştirilmesi için
herhangi bir geçiş gerekmez. Kaçış yolu: `__MISSING__:<english>`, çalışma zamanını doğru tutarken
bir çeviriyi erteler. `vi`, yer tutucuları yasaklar
(`tests/unit/i18n-vi-completeness.test.ts`) ve gerçek bir çeviri gerektirir.

#### `check-vitest-exclusions` — beklemeye alınmış test geçidi

`vitest.config.ts` dosyasının `exclude` listesindeki bir dosya, çalışmayan bir testtir ve ağacı
okuyan kişiye kapsam varmış gibi görünür. Altmış iki dosya,
`// #8618 — pre-existing failure; remove this exclusion when fixed` yorumunun arkasında birikti.
#8618 numaralı sorun 2026-08-11 tarihinde kapatılırken izlediği liste 45 girdiden 62 girdiye çıktı;
her yeni girdi, artık geçerli olmayan bir soruna işaret eden yorumu devraldı. Liste sonunda dosya
dosya ölçüldüğünde (#13204), **62 dosyanın 51'i hiçbir kaynak değişikliği olmadan mevcut ağaçta
başarılı oldu**.

Geçit, gerçek bir dosyaya çözümlenen her hariç tutmanın (a) bir izleme sorunu belirtmesini ve
(b) ölçülen durumuyla birlikte `config/quality/vitest-exclusions.json` dosyasında yer almasını
gerektirir; böylece yeni bir hariç tutma eklemek, 60 girdili bir diziye bir satır daha eklemek
yerine özel bir dosyada incelenebilir bir fark oluşturur. Hariç tutulan testleri kasıtlı olarak
yeniden çalıştırmaz — bu işlem yaklaşık 10 dakika sürer ve periyodik bir işe aittir; envanter,
her birinin en son ne zaman ölçüldüğünü kaydeder.
