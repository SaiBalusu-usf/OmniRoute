...
## Перелік шлюзів (~90 скриптів)

Скрипти розміщено в `scripts/check/` (шлюзи політик) і `scripts/quality/` (рушій храпового механізму).
Першоджерелом конфігурації CI є `.github/workflows/ci.yml`.

### Швидкий шлях для PR релізу (`quality.yml`)

`.github/workflows/quality.yml` запускається для PR, націлених на `release/**`. Він забезпечує
безперервну роботу гілок учасників завдяки швидким шлюзам із фільтрацією за шляхами, а також одному рекомендаційному сигналу виробничої збірки для змін
коду:

| Завдання                                         | Область                                                                                                                                                                                                                                              | Блокування                                                                                                |
| ------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------- |
| `Build (advisory)`                               | PR коду, що не є чернетками, і гілки черги Mergify; Node 24, `npm-ci-retry`, `check:node-runtime`, `npm run build` з `OMNIROUTE_USE_TURBOPACK=1`; артефакт не завантажується, оскільки жодне подальше завдання перевірки якості його не використовує | **Рекомендаційне** (`continue-on-error: true`; видалити після одного тижня стабільних запусків PR релізу) |
| `Docs Gates (fast-path)`                         | PR документації/коду; посилання в документації API та повна перевірка документації                                                                                                                                                                   | Так                                                                                                       |
| `Fast Quality Gates`                             | PR коду; статичні перевірки, перевірка типів, перевірка типів панелі керування, відповідні модульні тести                                                                                                                                            | Так                                                                                                       |
| `Forgotten sibling tests`                        | PR коду; відстеження змінених модулів до статичних споживачів і тестів споріднених кандидатів; шляхи через модулі-агрегатори та динамічні імпорти повідомляються як рекомендаційна діагностика з посиланнями на винятки зі списку дозволів           | **Рекомендаційне**                                                                                        |
| `Vitest (fast-path)`                             | PR коду; швидкий набір тестів vitest                                                                                                                                                                                                                 | Так                                                                                                       |
| `Unit Tests fast-path`                           | PR коду; набір модульних тестів у 4 сегментах                                                                                                                                                                                                        | Так                                                                                                       |
| `No new ESLint warnings`                         | PR коду; перевірка lint з урахуванням приглушень                                                                                                                                                                                                     | Так для власного джерела, рекомендаційне для форків                                                       |
| `Merge integrity (changelog + generated skills)` | PR, що не є чернетками; синхронізація журналу змін і згенерованих навичок                                                                                                                                                                            | Так для власного джерела, рекомендаційне для форків                                                       |
...
Для кожного зміненого виробничого модуля він повідомляє детерміновані ланцюжки
`змінений модуль/символ -> статичний споживач -> тест спорідненого кандидата`, якщо тест-кандидат
відсутній у різниці pull request. Зведення Markdown і результат JSON зберігаються як
артефакт робочого процесу `forgotten-sibling-tests` для калібрування перед будь-яким упровадженням блокування.

Повторні експорти через модулі-агрегатори та динамічні імпорти є лише діагностикою розв’язання; вони ніколи не створюють
блокувального результату. Перевірені винятки містяться у
`config/quality/forgotten-sibling-allowlist.json`. Кожен запис має вказувати споживача й тест-кандидат,
містити конкретне обґрунтування та посилання на issue або pull request GitHub. Некоректно сформовані записи
спричиняють відмову. Винятки не можуть приглушити видалений тест-кандидат або різницю, яка додає `.skip`/`.todo`;
послаблення тверджень та інші способи маскування залишаються сферою відповідальності незалежного блокувального
шлюзу `check:test-masking`.

### Завдання: `lint`

Запускається для кожного PR до `main`. У разі невдачі блокує злиття.

| Скрипт (`npm run ...`)            | Перевіряє                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | Блокувальна перевірка                           |
| --------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------- |
...
| `check:model-lifecycle`           | Три таблиці маршрутизації, що підтримуються вручну, залишаються узгодженими із зафіксованим у репозиторії знімком життєвого циклу (#11503): `FITNESS_TABLE` (`taskFitness.ts`) не оцінює жодного виведеного з експлуатації ідентифікатора, який може маршрутизувати `REGISTRY`; кожна ціль `BUILT_IN_ALIASES` присутня в `REGISTRY` і відсутня у знімку виведених з експлуатації ідентифікаторів; кожен виведений з експлуатації ідентифікатор, що досі є в `REGISTRY`, перенаправляється або зазначений у `allowedRetiredInCatalog`; і жодне джерело чи ціль `DEFAULT_DEGRADATION_MAP` не позначені як виведені з експлуатації в цьому знімку. Це не доводить, що модель наразі обслуговується активним зовнішнім сервісом. Перевірка виконується офлайн — шляхом порівняння з `config/quality/model-lifecycle.json`, який оновлюється вручну за допомогою `npm run quality:refresh-model-lifecycle` (потребує мережі; не підключено до CI). `allowedRetiredInCatalog` — це храповий механізм поступового скорочення: додавайте запис лише за наявності задачі для відстеження. | Так                                             |
| `check:fetch-targets`             | Кожен `fetch("/api/...")` у клієнтській частині `src/` веде до реального `route.ts`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | Так                                             |
| `check:deps`                      | Усі залежності, які можна встановити за допомогою `npm install`, у кожному `package.json` репозиторію присутні в `dependency-allowlist.json`; нові залежності без зафіксованої версії або пакети з назвами, що імітують інші пакети, позначаються                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | Так                                             |
| `audit:deps`                      | `npm audit` (корінь + electron) — немає рекомендацій із високим/критичним рівнем небезпеки (частково збігається з osv `check:vuln-ratchet`; див. перелік завдань із раціоналізації)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | Так                                             |
| `check:lockfile`                  | Цілісність `package-lock.json` — реєстр https, хеші цілісності, без перевизначень хоста                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | Так                                             |
| `check:licenses`                  | Список дозволених ліцензій SPDX для виробничих залежностей                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | Так                                             |
| `check:tracked-artifacts`         | Відсутні артефакти збірки / закомічені символічні посилання `node_modules` (також запускається в husky pre-commit; pre-push навмисно полегшений — #6716)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | Так                                             |
| `check:vitest-exclusions`         | Кожен виняток Vitest містить посилання на задачу для відстеження та наявний у `config/quality/vitest-exclusions.json` (#13204)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | Так                                             |
| `check:file-size`                 | Жоден файл вихідного коду не перевищує обмеження для відповідного розширення (храповий механізм: зафіксовані великі файли у списку `frozen`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | Так                                             |
...
| `check:public-creds`              | Відсутні літеральні OAuth `client_id`/`client_secret` або вебключі Firebase поза `publicCreds.ts` (жорстке правило №11)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | Так                                             |
| `check:db-rules`                  | Відсутній необроблений SQL поза модулями `src/lib/db/`; відсутні імпорти з барельних файлів із `localDb.ts` (жорсткі правила №2/№5)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | Так                                             |
...
| `typecheck:noimplicit:core`       | Суворий `noImplicitAny` — орієнтований на майбутнє; багато наявних місць виклику все ще потребують анотацій                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | **Рекомендаційний** (`continue-on-error: true`) |
| `check:dashboard-typecheck`       | `tsc`, обмежений областю `src/app/(dashboard)/**` (#7033) — ретельно сформований список дозволених 27 файлів у `typecheck:core` не містить жодного dashboard TSX, а `next build` також ніколи не перевіряє їхні типи (`next.config.mjs` задає `ignoreBuildErrors: true`), тому регресії з осиротілими ідентифікаторами в них (#6625/#6909) були невидимими для CI. Порівнює із зафіксованою базовою лінією кількості помилок для кожного файлу й коду TS (`config/quality/dashboard-typecheck-baseline.json`, та сама схема контролю застарілості, що й у `check:known-symbols`) — перевірка завершується невдало лише для НОВИХ помилок понад базову кількість; зменшуйте базовий рівень за допомогою `--update`, коли наявну помилку виправлено.                                                                                                                                                                                                                                                                                                                               | Так                                             |
...
| ---------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------- |
| `quality:collect`            | Створює `quality-metrics.json` (кількість попереджень ESLint, покриття з об’єднаного звіту сегментів)                                                                                        | Так (передує механізму храповика) |
...
| `check:complexity`           | Цикломатична складність на рівні файлу не перевищує граничне значення (основні правила ESLint `complexity` + `max-lines-per-function`)                                                       | Так                               |
| `check:cognitive-complexity` | Храповик когнітивної складності (`eslint-plugin-sonarjs`) — окремий прохід ESLint; CI запускає обидві перевірки разом як єдиний крок `check:complexity-ratchets`                             | Так                               |
| `check:dead-code`            | Храповик невикористаних експортів / файлів (knip) не погіршується порівняно з базовим рівнем                                                                                                 | Так                               |
| `check:compression-budget`   | Бюджет тесту продуктивності стиснення — мінімальні пороги економії токенів для кожного рушія не повинні погіршуватися                                                                        | Так                               |
| `check:type-coverage`        | Храповик відсотка типізованого коду (`type-coverage`) не погіршується; значною мірою охоплює `typecheck:noimplicit:core`                                                                     | Так                               |
| `check:codeql-ratchet`       | Кількість відкритих сповіщень CodeQL не збільшується (зчитується через `gh api`; коректно пропускається без токена) — періодичність оновлення та ручний запуск: див. «Храповик CodeQL» нижче | Так                               |
...
по-справжньому; зовнішні сканери встановлюються через `gh release download` і самостійно пропускають виконання (exit 0),
якщо бінарний файл усе ще відсутній.

| Скрипт                   | Перевіряє                                                                                                                                                                                                         | Блокування |
| ------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------- |
| `check:circular-deps`    | Відсутність циклічних залежностей (dpdm)                                                                                                                                                                          | **Ні**     |
| `check:bundle-size`      | Розмір бандла не перевищує встановлену межу                                                                                                                                                                       | **Ні**     |
| `check:secrets`          | Сканування секретів (gitleaks) — пропускається, якщо бінарний файл відсутній                                                                                                                                      | **Ні**     |
| `check:vuln-ratchet`     | Відсутність регресій щодо вразливостей залежностей (osv-scanner) — пропускається, якщо бінарний файл відсутній                                                                                                    | **Ні**     |
| `check:workflows`        | Лінтинг робочих процесів (actionlint + zizmor) — пропускається, якщо бінарні файли відсутні                                                                                                                       | **Ні**     |
| `check:openapi-breaking` | Критичні зміни в публічному контракті API (`openapi.yaml`) порівняно з базовою гілкою (oasdiff) — виводить `openapiBreaking=N`; пропускається, якщо oasdiff відсутній або базову специфікацію неможливо визначити | **Ні**     |
...
| ↳ `check:docs-counts`          | Кількісні значення в тексті (кількість провайдерів, міграцій тощо) перебувають у межах порогового вікна відносно фактичних значень                                                                     | Так                         |
...
| ↳ `check:doc-links`            | Внутрішні markdown-посилання в документації ведуть до наявних файлів (у форматі `[text]`/`(path)`)                                                                                                     | Так                         |
| ↳ `check:fabricated-docs`      | Маршрути, змінні середовища, команди CLI, назви хуків і шляхи до файлів, згадані в документації, існують у кодовій базі. Жорстка перевірка через `--strict`; без прапорця помилка не блокує виконання. | Так (через `--strict` у CI) |
| `check:cli-i18n`               | Рядки команд CLI наявні в усіх файлах локалей i18n                                                                                                                                                     | Так                         |
| `check:openapi-coverage`       | Специфікація OpenAPI охоплює щонайменше визначений пороговий мінімум реальних маршрутів                                                                                                                | Так                         |
| `check:openapi-security-tiers` | Анотації рівнів безпеки в `openapi.yaml` узгоджені з класифікаціями в `routeGuard.ts`                                                                                                                  | **Ні**                      |
...
| `i18n translation drift`       | Неперекладені ключі у файлах локалей i18n — лише попередження                                                                                                                                          | **Ні**                      |

### Завдання: `i18n-ui-coverage`

| Скрипт                            | Що перевіряє                                                                                                                                                                                                         | Блокувальний        |
| --------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------- |
| `check-ui-keys-coverage` (inline) | Покриття ключів UI i18n становить ≥ 65%                                                                                                                                                                              | Так                 |
| `check-ui-value-drift` (inline)   | Перезаписане англійське **значення** не залишає застарілих перекладів                                                                                                                                                | Так                 |
| `check-new-key-coverage` (inline) | **Новий** англійський ключ наявний у кожній локалі                                                                                                                                                                   | Так                 |
| `check-translation-ratio`         | Частка справжніх перекладів для кожної локалі (ідентичні англійським / заповнювачі / відсутні листки поза списком дозволених) не має перевищувати `config/quality/i18n-translation-baseline.json` + допустимий запас | **Рекомендаційний** |
...
Виявляє єдину регресію i18n, яку інші перевірки структурно не здатні побачити: англійське значення
перезаписано, а переклади, створені на основі _попереднього_ англійського тексту, залишилися, тож
неангломовні користувачі продовжують читати впевнено сформульований, але вже неправильний текст.

Це справді потрапило у випуск. `oauthModal.googleOAuthWarning` було перезаписано після додавання
помічника входу Antigravity (#5203); **39 із 43 локалей** зберегли текст, який пропонував операторам
«скопіювати повну URL-адресу та вставити її нижче» — цей процес неможливо завершити для цього
провайдера. Проблема залишалася непоміченою до #8463, оскільки:

- `sync-ui-keys` заповнює лише **відсутні** ключі, але ніколи не оновлює **застарілі**;
...
- `check-translation-drift` відстежує дзеркальні копії документації `docs/i18n/<locale>/**.md` —
  він ніколи не читає `src/i18n/messages/*.json`. Блокує у завданні `docs-sync-strict` після
  повторної синхронізації 2026-09: змініть основний документ → `npm run i18n:run -- --files=<doc>` (на рівні розділу, недорого).

**Враховує різницю, а не спирається на базовий рівень.** Він порівнює `en.json` у базі злиття
з робочим деревом; для кожного ключа, англійське значення якого змінилося, будь-яка локаль, що
все ще містить незмінений переклад, вважається застарілою. Це навмисно **заморожує наявний борг** —
за різницею неможливо визначити, з якого старого англійського тексту походить давній переклад,
тому перевірка оцінює лише те, чого торкається поточна зміна. Альтернатива (базовий рівень хешів
для кожного ключа) потребувала б згенерованого файлу розміром ~600 КБ, утричі більшого за найбільший
наявний базовий файл, який змінювався б у кожному PR i18n.

Задовольнити перевірку можна двома способами:

1. оновити відповідні переклади або
2. установити для них `__MISSING__:<new english>` — тоді середовище виконання віддаватиме виправлений
   англійський текст (`src/i18n/request.ts::deepMergeFallback`, #7258), а ключ буде поставлено в чергу на переклад.
...
```bash
npm run i18n:check-value-drift          # суворий режим (виконується в CI)
...
Повна матриця перевірки i18n (одне завдання для кожної локалі). Усе завдання є рекомендаційним.

| Скрипт                          | Що перевіряє                        | Блокувальний                                                        |
| ------------------------------- | ----------------------------------- | ------------------------------------------------------------------- |
| `validate_translation.py quick` | Повноту перекладу для кожної локалі | **Рекомендаційний** (`continue-on-error: true` для всього завдання) |
...
| Скрипт                 | Що перевіряє                                                                                                                                            | Блокувальний |
| ---------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------ |
| `check:pr-test-policy` | PR, що змінюють виробничий код у `src/`, `open-sse/`, `electron/` або `bin/`, мають додавати або оновлювати тести (суворе правило №8)                   | Так          |
| `check:test-masking`   | Змінені файли тестів не зменшують загальну кількість перевірок і не додають тавтологій `assert.ok(true)`                                                | Так          |
| `check:pr-evidence`    | Текст PR містить посилання на підтвердження тестування/VPS для зміни (автоматизує суворе правило №18 через пошук у тексті PR — ненадійно, див. Backlog) | Так          |
...
| `test:vitest:ui` | Тести компонентів UI — засіб запуску vitest                          | **Блокує** — наявні раніше збої явно виключено у `vitest.config.ts`; нові збої спричиняють невиконання завдання |
...
| `nightly-resilience`   | Перевірка зростання купи, ін’єкція хаотичних збоїв, навантажувальне/тривале тестування k6                                                                                    | **Рекомендаційне** |
| `nightly-llm-security` | Захист від ін’єкцій promptfoo (режим блокування) + перевірки garak (пропускаються без секрету постачальника)                                                                 | **Рекомендаційне** |
| `nightly-schemathesis` | Фаззинг контракту OpenAPI (schemathesis) на запущеному OmniRoute з використанням `docs/openapi.yaml` — виявляє порушення специфікації / необроблені помилки 500 (Фаза 8 B.4) | **Рекомендаційне** |
| `nightly-mutation`     | Оцінка мутаційного тестування Stryker для швидкого набору модульних тестів — мутанти, що вижили, виявляють слабкі перевірки                                                  | **Рекомендаційне** |
| `nightly-compat`       | Матриця сумісності рушія Node для підтримуваних діапазонів `engines.node`                                                                                                    | **Рекомендаційне** |
...
його було додано. Заповнювачі `__MISSING__:` вважаються наявними (їхній вміст належить до сфери
відповідальності шлюзу співвідношення). Це абсолютне доповнення до двох шлюзів на основі різниці
та відсоткового показника: `check-ui-keys-coverage` вимагає щонайменше 80 % для кожної локалі
(43 відсутні ключі з ~13 000 усе одно дають 99,7 %), а `check-new-key-coverage` оцінює лише ключі,
які PR додає до `en.json`. Пакет локалей генерується з актуального на день створення його гілки
`en.json` і перекладається протягом кількох днів, поки до базової гілки продовжують додавати ключі;
сам пакетний PR не додає жодного ключа, тому обидва споріднені шлюзи промовчали, коли пакет 1
(#13044) потрапив до репозиторію з нестачею 43 ключів у дев’яти локалях, а пакет 2 (#13660) —
з нестачею 10 ключів у восьми (2026-09-15). Щоб виправити помилку, виконайте
`node scripts/i18n/sync-ui-keys.mjs --locale=<codes> --translate-markers`; кінцевий ключ `extra`
означає, що його видалено з джерела, — видаліть його з локалі. `--warn` лише повідомляє про
проблему, не спричиняючи помилки. `--catalog=cli` виконує таке саме порівняння для
...
Споріднений із `check-ui-value-drift`. Той виявляє випадки, коли англійське значення було
**переписано**, а його переклади залишилися незмінними; цей виявляє випадки, коли англійський
ключ було **додано**, але деякі локалі так його й не отримали.

`check-ui-keys-coverage` не може виявити цей клас проблем: він установлює мінімальний відсоток
для кожної локалі, а за відсутності одинадцяти ключів із ~13 000 покриття залишається на рівні
99,9 %. Відсотковий показник для окремої мови не здатен виразити твердження «цю функцію випущено
без перекладу» — ціла функція може потрапити до нової локалі без жодного тексту, а показник при
цьому не зміниться.

Інцидент, який він ураховує: на етапі 3 Orchestration Canvas усі одинадцять ключів було перекладено
для 42 локалей, що існували на той час. За кілька годин пакет мов ЄС (#13044) збільшив кількість
локалей у репозиторії до 51, але дев’ять нових локалей (`el`, `et`, `ga`, `hr`, `lt`, `lv`, `mt`,
`sl`, `sr`) так і не отримали цих ключів. `deepMergeFallback` підставляє англійський текст замість
відсутнього ключа, тому проблема проявилася як неперекладений, а не порожній інтерфейс — реальний
і за своєю природою непомітний збій.

Як і споріднений шлюз, він **враховує різницю**, порівнюючи англійську версію в базі злиття з
робочим деревом, тому наявні раніше прогалини залишаються зафіксованими, а для ввімкнення шлюзу
не знадобилася міграція. Аварійний обхід: `__MISSING__:<english>` дає змогу відкласти переклад,
зберігаючи правильну роботу середовища виконання. `vi` забороняє заповнювачі
(`tests/unit/i18n-vi-completeness.test.ts`) і потребує справжнього перекладу.

#### `check-vitest-exclusions` — шлюз відкладених тестів

Файл у списку `exclude` з `vitest.config.ts` — це тест, який не виконується, хоча для того, хто
переглядає дерево, він виглядає як частина покриття. Шістдесят два файли накопичилися за коментарем
`// #8618 — pre-existing failure; remove this exclusion when fixed`. Проблему #8618 було закрито
2026-08-11, тоді як відстежуваний нею список зріс із 45 до 62 записів, і кожен новий запис
успадковував коментар із посиланням на вже закриту проблему. Коли список нарешті перевірили
пофайлово (#13204), **51 із 62 файлів успішно пройшов перевірку на поточному дереві без жодних
змін у вихідному коді**.

Шлюз вимагає, щоб кожен виняток, який відповідає реальному файлу, (a) містив номер проблеми для
відстеження та (b) був зазначений у `config/quality/vitest-exclusions.json` із виміряним статусом.
Завдяки цьому додавання винятку стає видимою під час перевірки зміною в окремому файлі, а не ще
одним рядком у масиві з 60 записів. Шлюз навмисно не запускає виключені тести повторно — це займає
~10 хвилин і має виконуватися в періодичному завданні; в інвентарі зазначається дата останнього
вимірювання для кожного тесту.
