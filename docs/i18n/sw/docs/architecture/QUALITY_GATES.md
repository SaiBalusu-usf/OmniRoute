...
## Orodha ya Vizuizi (~90 scripts)

Scripts zipo chini ya `scripts/check/` (vizuizi vya sera) na `scripts/quality/` (injini ya ratchet).
Chanzo rasmi cha CI ni `.github/workflows/ci.yml`.

### Njia ya haraka ya PR ya toleo (`quality.yml`)

`.github/workflows/quality.yml` huendeshwa kwenye PR zinazolenga `release/**`. Huwezesha matawi ya
wachangiaji kuendelea kupitia vizuizi vya haraka vilivyochujwa kwa njia, pamoja na ishara moja ya
ushauri ya uundaji wa uzalishaji kwa mabadiliko ya msimbo:

| Kazi                                             | Upeo                                                                                                                                                                                                                                            | Uzuiaji                                                                                                    |
| ------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------- |
| `Build (advisory)`                               | PR za msimbo zisizo rasimu na matawi ya foleni ya Mergify; Node 24, `npm-ci-retry`, `check:node-runtime`, `npm run build` yenye `OMNIROUTE_USE_TURBOPACK=1`; hakuna upakiaji wa artifact kwa sababu hakuna kazi ya ubora ya baadaye inayoitumia | **Ya ushauri** (`continue-on-error: true`; iondoe baada ya wiki moja ya uendeshaji thabiti wa PR za toleo) |
...
| `Forgotten sibling tests`                        | PR za msimbo; modules zilizobadilishwa zinazofuatiliwa hadi kwa watumiaji tuli na candidate sibling tests; njia za barrel na dynamic-import huripotiwa kama uchunguzi wa ushauri, pamoja na vizuizi vinavyorejelewa vya orodha ya kuruhusu      | **Ya ushauri**                                                                                             |
| `Vitest (fast-path)`                             | PR za msimbo; mkusanyiko wa haraka wa vitest                                                                                                                                                                                                    | Ndiyo                                                                                                      |
| `Unit Tests fast-path`                           | PR za msimbo; mkusanyiko wa unit wenye shards 4                                                                                                                                                                                                 | Ndiyo                                                                                                      |
| `No new ESLint warnings`                         | PR za msimbo; ulinzi wa lint unaozingatia suppressions                                                                                                                                                                                          | Ndiyo kwa zenye asili yake, ya ushauri kwa forks                                                           |
| `Merge integrity (changelog + generated skills)` | PR zisizo rasimu; usawazishaji wa changelog na skills zilizozalishwa                                                                                                                                                                            | Ndiyo kwa zenye asili yake, ya ushauri kwa forks                                                           |

#### Ripoti ya forgotten sibling tests

`npm run check:forgotten-sibling-tests` hutumia tena import resolver iliyo nyuma ya ramani ya athari
za tests. Kwa kila module ya uzalishaji iliyobadilishwa, huripoti misururu thabiti ya
`changed module/symbol -> static consumer -> candidate sibling test` wakati candidate
test haipo kwenye diff ya pull request. Muhtasari wa Markdown na matokeo ya JSON huhifadhiwa kama
artifact ya workflow ya `forgotten-sibling-tests` kwa ajili ya urekebishaji kabla ya utekelezaji wowote wa kuzuia.

Barrel re-exports na dynamic imports ni uchunguzi wa resolution pekee; kamwe hazisababishi
finding inayozuia. Vizuizi vilivyokaguliwa vinapatikana katika
`config/quality/forgotten-sibling-allowlist.json`. Kila ingizo lazima litaje consumer na candidate
test, litoe sababu mahususi, na liunganishe na issue au pull request ya GitHub. Maingizo yenye
muundo usio sahihi husababisha kufeli kwa njia salama. Vizuizi haviwezi kuficha candidate test iliyofutwa au diff
inayoongeza `.skip`/`.todo`; kudhoofishwa kwa assertions na aina nyingine za ufichaji husalia kuwa
jukumu la kizuizi cha `check:test-masking` kinachozuia kwa kujitegemea.

### Kazi: `lint`

Huendeshwa kwenye kila PR kuelekea `main`. Huzuia kuunganisha inapofeli.
...
| `check:any-budget:t11`            | Idadi ya `@ts-expect-error // any` haizidi kikomo (catraca ya Kiwango cha 11)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | Ndiyo                                   |
| `check:provider-consistency`      | Kila mtoa huduma katika `providers.ts` ana ingizo linalolingana katika `providerRegistry.ts` (na kinyume chake, ndani ya orodha ya yanayoruhusiwa)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | Ndiyo                                   |
| `check:model-lifecycle`           | Majedwali matatu ya uelekezaji yanayotunzwa mwenyewe yanaendelea kuwiana na muhtasari wa mzunguko wa maisha uliohifadhiwa (#11503): `FITNESS_TABLE` (`taskFitness.ts`) haitoi alama kwa id yoyote iliyostaafishwa ambayo `REGISTRY` inaweza kuelekeza; kila lengo la `BUILT_IN_ALIASES` lipo katika `REGISTRY` na halipo katika muhtasari wa id zilizostaafishwa; kila id iliyostaafishwa ambayo bado ipo katika `REGISTRY` inaelekezwa kwingine au imeorodheshwa katika `allowedRetiredInCatalog`; na hakuna chanzo au lengo la `DEFAULT_DEGRADATION_MAP` linaloonekana kuwa limestaafishwa katika muhtasari huo. Hii haithibitishi kwamba modeli kwa sasa inahudumiwa na huduma hai ya nje. Nje ya mtandao — hulinganisha dhidi ya `config/quality/model-lifecycle.json`, inayosasishwa mwenyewe kwa `npm run quality:refresh-model-lifecycle` (mtandao; haijaunganishwa kwenye CI). `allowedRetiredInCatalog` ni utaratibu wa kupunguza deni hatua kwa hatua: ongeza ingizo tu pamoja na suala la ufuatiliaji. | Ndiyo                                   |
| `check:fetch-targets`             | Kila `fetch("/api/...")` katika `src/` ya upande wa mteja huelekezwa kwenye `route.ts` halisi                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | Ndiyo                                   |
| `check:deps`                      | Vitegemezi vyote vinavyoweza kusakinishwa kwa `npm install` katika kila `package.json` iliyo kwenye hazina vipo katika `dependency-allowlist.json`; vifurushi vipya visivyofungwa kwenye toleo mahususi au vilivyoigwa kwa majina yanayofanana vinaalamishwa                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | Ndiyo                                   |
| `audit:deps`                      | `npm audit` (mzizi + electron) — hakuna tahadhari za kiwango cha juu/kritikali (inaingiliana na osv `check:vuln-ratchet`; tazama Orodha ya Masuala ya Kurahisisha)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | Ndiyo                                   |
...
| `check:licenses`                  | Orodha ya ruhusa ya leseni za SPDX kwa vitegemezi vya uzalishaji                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Ndiyo                                   |
...
| `check:vitest-exclusions`         | Kila hali ya kutengwa ya Vitest hutaja suala la ufuatiliaji na huonekana katika `config/quality/vitest-exclusions.json` (#13204)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Ndiyo                                   |
| `check:file-size`                 | Hakuna faili ya msimbo chanzo inayozidi kikomo cha kila kiendelezi (ratchet: faili kubwa zisizobadilishwa katika orodha ya `frozen`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | Ndiyo                                   |
...
| `check:migration-numbering`       | Faili za SQL za uhamishaji zimepewa nambari kwa mfuatano, bila mapengo wala nambari zinazojirudia                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | Ndiyo                                   |
...
| `check:known-symbols`             | Vitekelezaji vya watoa huduma, mikakati ya uelekezaji, na vitafsiri vilivyosajiliwa katika majedwali yao ya usambazaji vinalingana na faili zilizo kwenye diski — hakuna alama zisizo na matumizi au ambazo hazijatangazwa                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | Ndiyo                                   |
| `check:route-guard-membership`    | Kila njia inayoanzisha mchakato-toto imeainishwa na `isLocalOnlyPath()` (Kanuni Ngumu #15/#17)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | Ndiyo                                   |
| `check:test-discovery`            | Kila faili ya `*.test.ts` / `*.spec.ts` katika repo inakusanywa na angalau kiendesha majaribio kimoja (ratchet: orodha ya faili zisizokusanywa katika `test-discovery-baseline.json` inaweza kupungua tu)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | Ndiyo                                   |
| `check:agent-skills-sync`         | Vizalia vya agent-skills vilivyozalishwa vinalingana na katalogi yake chanzo (hakuna mkengeuko)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `check:provider-asset-provenance` | Nembo/vipengee vya watoa huduma vina ingizo la asili lililorekodiwa                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `lint:json`                       | Faili za usanidi za JSON zinaweza kuchanganuliwa na zinatimiza sheria za lint za repo                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `typecheck:core`                  | Ukusanyaji wa TypeScript bila hitilafu (maonyo ya ushauri pekee)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Ndiyo                                   |
| `typecheck:noimplicit:core`       | `noImplicitAny` kali — kwa ajili ya siku zijazo; sehemu nyingi za wito zilizokuwepo awali bado zinahitaji ufafanuzi wa aina                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | **Ushauri** (`continue-on-error: true`) |
| `check:dashboard-typecheck`       | `tsc` inayolenga `src/app/(dashboard)/**` (#7033) — orodha iliyoratibiwa ya faili 27 inayoruhusiwa ya `typecheck:core` haijumuishi TSX yoyote ya dashibodi, na `next build` pia kamwe haiikagui aina (`next.config.mjs` huweka `ignoreBuildErrors: true`), kwa hivyo hitilafu za vipengele tambulishi vilivyoachwa huko (#6625/#6909) hazikuonekana kwa CI. Hulinganisha na msingi uliogandishwa wa idadi kwa kila faili/kila msimbo wa TS (`config/quality/dashboard-typecheck-baseline.json`, muundo sawa wa utekelezaji dhidi ya msingi uliopitwa na wakati kama `check:known-symbols`) — ni hitilafu MPYA pekee zinazozidi idadi ya msingi ndizo zinazosababisha lango kushindwa; punguza msingi kwa `--update` hitilafu iliyokuwepo awali inaporekebishwa.                                                                                                                                                                                                                                                   | Ndiyo                                   |

### Kazi: `quality-gate`

Huendeshwa baada ya `test-coverage`. Huzuia kuunganisha mabadiliko inaposhindwa.

| Skripti                      | Huthibitisha                                                                                                                                                                                              | Inazuia                  |
| ---------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------ |
| `quality:collect`            | Hutoa `quality-metrics.json` (idadi ya maonyo ya ESLint, ufunikaji kutoka kwenye ripoti iliyounganishwa ya vipande)                                                                                       | Ndiyo (kabla ya ratchet) |
...
| `check:complexity`           | Uchangamani wa mzunguko katika kiwango cha faili hauzidi kikomo (`complexity` ya msingi ya ESLint + `max-lines-per-function`)                                                                             | Ndiyo                    |
| `check:cognitive-complexity` | Ratchet ya uchangamani wa kiutambuzi (`eslint-plugin-sonarjs`) — upitishaji tofauti wa ESLint; CI huendesha zote zikiwa zimeunganishwa kama hatua moja ya `check:complexity-ratchets`                     | Ndiyo                    |
| `check:dead-code`            | Ratchet ya uhamishaji / faili zisizotumika (knip) hairudi nyuma ikilinganishwa na msingi                                                                                                                  | Ndiyo                    |
| `check:compression-budget`   | Bajeti ya kipimo cha utendaji cha ubanaji — viwango vya chini vya uokoaji wa tokeni kwa kila injini havipaswi kurudi nyuma                                                                                | Ndiyo                    |
...
| `check:codeql-ratchet`       | Idadi ya arifa zilizo wazi za CodeQL hairudi nyuma (husoma kupitia `gh api`; huruka kwa utaratibu bila tokeni) — kwa marudio ya kusasisha na uanzishaji wa mikono: angalia "Ratchet ya CodeQL" hapa chini | Ndiyo                    |

### Kazi: `quality-extended`

Kazi nzima ni ya ushauri (`continue-on-error: true`). Vikagua vya hatua vinavyotegemea npm huendeshwa
kikamilifu; vikagua vya nje husakinishwa kupitia `gh release download` na hujiruka vyenyewe (exit 0)
wakati faili ya kutekelezwa bado haipo.

| Script                   | Huthibitisha                                                                                                                                                                                          | Uzuiaji     |
| ------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------- |
| `check:circular-deps`    | Hakuna vitegemezi vya mzunguko (dpdm)                                                                                                                                                                 | **Ushauri** |
| `check:bundle-size`      | Ukubwa wa kifurushi hauzidi kikomo                                                                                                                                                                    | **Ushauri** |
| `check:secrets`          | Ukaguzi wa siri (gitleaks) — hurukwa ikiwa faili ya kutekelezwa haipo                                                                                                                                 | **Ushauri** |
| `check:vuln-ratchet`     | Udhaifu wa vitegemezi (osv-scanner) haurudi nyuma — hurukwa ikiwa faili ya kutekelezwa haipo                                                                                                          | **Ushauri** |
| `check:workflows`        | Ukaguzi wa mtiririko wa kazi (actionlint + zizmor) — hurukwa ikiwa faili za kutekelezwa hazipo                                                                                                        | **Ushauri** |
| `check:openapi-breaking` | Mabadiliko yanayovunja mkataba wa API ya umma (`openapi.yaml`) yakilinganishwa na tawi msingi (oasdiff) — hutoa `openapiBreaking=N`; hurukwa ikiwa oasdiff haipo au vipimo msingi haviwezi kutatuliwa | **Ushauri** |

### Kazi: `docs-sync-strict`

Huendeshwa kwenye kila PR kwenda `main`. Huzuia uunganishaji inaposhindwa.
...
| ↳ `check:docs-counts`          | Idadi katika maelezo (idadi ya watoa huduma, idadi ya uhamishaji, n.k.) ziko ndani ya dirisha la hatua la idadi halisi                                                                   | Ndiyo                                |
...
| ↳ `check:doc-links`            | Viungo vya ndani vya markdown katika nyaraka huelekeza kwenye faili halisi (muundo wa `[text]`/`(path)`)                                                                                 | Ndiyo                                |
| ↳ `check:fabricated-docs`      | Njia, vigezo vya mazingira, amri za CLI, majina ya hook, na njia za faili zilizotajwa katika nyaraka zipo katika codebase. Lango gumu kupitia `--strict`; kushindwa kwa upole bila flag. | Ndiyo (kupitia `--strict` katika CI) |
| `check:cli-i18n`               | Mifuatano ya amri za CLI ipo katika faili zote za locale za i18n                                                                                                                         | Ndiyo                                |
| `check:openapi-coverage`       | Vipimo vya OpenAPI vinashughulikia angalau kiwango cha chini kilichowekwa kwa hatua cha njia halisi                                                                                      | Ndiyo                                |
...
| `check:openapi-routes`         | Kila path katika `openapi.yaml` huelekeza kwenye `route.ts` halisi (kuzuia ubunifu wa taarifa zisizo halisi)                                                                             | Ndiyo                                |
| `check:docs-symbols`           | Kila rejeleo la `/api/...` katika `docs/**/*.md` huelekeza kwenye `route.ts` halisi (kuzuia ubunifu wa taarifa zisizo halisi)                                                            | Ndiyo                                |
| `i18n translation drift`       | Funguo ambazo hazijatafsiriwa katika faili za locale za i18n — onyo pekee                                                                                                                | **Ushauri**                          |

### Kazi: `i18n-ui-coverage`

| Skripti                          | Huthibitisha                                                                                                                                                                                                                        | Inazuia     |
| -------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------- |
| `check-ui-keys-coverage` (ndani) | Ufunikaji wa funguo za i18n za UI ni ≥ 65%                                                                                                                                                                                          | Ndiyo       |
| `check-ui-value-drift` (ndani)   | **Thamani** ya Kiingereza iliyoandikwa upya haiachi tafsiri yoyote iliyopitwa na wakati                                                                                                                                             | Ndiyo       |
| `check-new-key-coverage` (ndani) | Ufunguo **mpya** wa Kiingereza unafikia kila lugha                                                                                                                                                                                  | Ndiyo       |
| `check-translation-ratio`        | Uwiano wa tafsiri halisi kwa kila lugha (sawa na Kiingereza / kishikilia nafasi / vipengele vya mwisho vinavyokosekana nje ya orodha ya kuruhusu) haupaswi kuzidi `config/quality/i18n-translation-baseline.json` + nafasi ya ziada | **Ushauri** |
...
watumiaji wasiotumia Kiingereza wanaendelea kusoma maandishi yenye kauli ya uhakika lakini ambayo sasa si sahihi.

Hili lilitolewa kwa watumiaji kwa kweli. `oauthModal.googleOAuthWarning` iliandikwa upya wakati kisaidizi cha kuingia cha Antigravity
kilipoanzishwa (#5203); **lugha 39 kati ya 43** zilibaki na maandishi yaliyowaambia waendeshaji "kunakili
...
- `sync-ui-keys` hujaza funguo ambazo **hazipo** pekee, kamwe si zile ambazo **zimepitwa na wakati**;
- `check-ui-keys-coverage` huhesabu _uwepo_ wa ufunguo, hivyo tafsiri iliyopitwa na wakati huhesabiwa kuwa imefunikwa;
- `check-translation-drift` hufuatilia vioo vya nyaraka vya `docs/i18n/<locale>/**.md` —
  ...
  usawazishaji upya wa 2026-09: hariri hati ya msingi → `npm run i18n:run -- --files=<doc>` (kwa kiwango cha sehemu, gharama ndogo).

**Huzingatia tofauti, haitegemei msingi.** Hulinganisha `en.json` katika msingi wa uunganishaji dhidi ya
...
haiwezi kufichua tafsiri ya muda mrefu ilitokana na Kiingereza gani cha zamani, hivyo lango hutathmini
tu kile ambacho badiliko la sasa linagusa. Njia mbadala (msingi wa hashi kwa kila ufunguo) ingegharimu
faili inayozalishwa ya takribani KB 600, mara 3 ya msingi mkubwa zaidi uliopo, na kubadilika katika kila PR ya i18n.

Njia mbili za kulitimiza:

1. sasisha tafsiri zilizoathiriwa, au
2. ziweke kuwa `__MISSING__:<new english>` — wakati wa utekelezaji ndipo Kiingereza kilichosahihishwa hutolewa
   (`src/i18n/request.ts::deepMergeFallback`, #7258) na ufunguo huwekwa kwenye foleni ya kutafsiriwa.

Ikiwa **maana** ya tungo imebadilika, pendelea **kubadilisha jina la ufunguo**: ufunguo mpya hauwezi kurithi
tafsiri iliyopitwa na wakati. Huo ndio mtindo uliotumiwa na #8463.

```bash
npm run i18n:check-value-drift          # kali (kile ambacho CI huendesha)
npm run i18n:check-value-drift:warn     # ripoti pekee
BASE_REF=origin/release/vX.Y.Z npm run i18n:check-value-drift
```

Hutoka kwa msimbo 0 pamoja na `SKIP reason=base-unresolved` wakati katalogi ya msingi haiwezi kusomwa (nakala
finyu isiyo na rejeleo la msingi), sawa na `check-openapi-breaking`.

### Kazi: `i18n`

Mkusanyiko kamili wa uthibitishaji wa i18n (kazi moja kwa kila lugha). Kazi yote ni ya ushauri.

| Skripti                         | Huthibitisha                        | Inazuia                                               |
| ------------------------------- | ----------------------------------- | ----------------------------------------------------- |
| `validate_translation.py quick` | Ukamilifu wa tafsiri kwa kila lugha | **Ushauri** (`continue-on-error: true` kwa kazi yote) |

### Kazi: `pr-test-policy`

Huendeshwa kwenye maombi ya kuvuta pekee.
...
| `check:test-masking`   | Faili za majaribio zilizobadilishwa hazipunguzi jumla halisi ya uthibitisho wala kuongeza kauli daima-kweli za `assert.ok(true)`                                        | Ndiyo   |
| `check:pr-evidence`    | Maelezo ya PR yanataja ushahidi wa jaribio/VPS wa badiliko (huifanya Kanuni Kali #18 kuwa ya kiotomatiki kwa kutafuta maandishi ya PR — dhaifu, angalia Orodha ya Kazi) | Ndiyo   |

### Kazi: `test-vitest`

Huendeshwa baada ya `build`. Huzuia uunganishaji ikishindwa.

| Kifurushi        | Huthibitisha                                                | Kizuizi                                                                                                                            |
| ---------------- | ----------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------- |
| `test:vitest`    | Seva ya MCP (zana 110), autoCombo, kache — kiendesha vitest | Ndiyo                                                                                                                              |
| `test:vitest:ui` | Majaribio ya vipengele vya UI — kiendesha vitest            | **Kizuizi** — hitilafu zilizokuwepo awali zimeondolewa waziwazi katika `vitest.config.ts`; hitilafu mpya husababisha kazi ishindwe |
...
| Mtiririko wa kazi      | Huthibitisha                                                                                                                                                                                 | Kizuizi        |
| ---------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------- |
| `nightly-property`     | Majaribio ya sifa ya fast-check yenye mbegu nasibu + idadi kubwa ya uendeshaji                                                                                                               | **Ya ushauri** |
| `nightly-resilience`   | Lango la ukuaji wa heap, uingizaji wa hitilafu za vurugu, mzigo/udumishaji wa k6                                                                                                             | **Ya ushauri** |
...
| `nightly-schemathesis` | Fuzzing ya mkataba wa OpenAPI (schemathesis) dhidi ya OmniRoute inayotumika kwa kutumia `docs/openapi.yaml` — hufichua ukiukaji wa vipimo / hitilafu 500 zisizoshughulikiwa (Awamu ya 8 B.4) | **Ya ushauri** |
| `nightly-mutation`     | Alama ya majaribio ya mabadiliko ya Stryker kwenye mkondo wa haraka wa majaribio ya kitengo — mutants zinazonusurika hufichua madai dhaifu                                                   | **Ya ushauri** |
...
#### `check-key-completeness` — lango la usawa wa seti za funguo
...
iwapo kuna funguo zozote za mwisho zinazokosekana au zilizozidi, bila kujali funguo hizo ziliongezwa lini. Vishikilia nafasi vya `__MISSING__:`
huhesabiwa kuwa vipo (maudhui yake yanahusu lango la uwiano). Hiki ni kikamilisho kamili
cha malango mawili yanayotegemea tofauti/asilimia: `check-ui-keys-coverage` huweka kiwango cha chini cha 80 % kwa kila
lugha (funguo 43 zinazokosekana kati ya ~13,000 bado huonekana kama 99.7 %) na `check-new-key-coverage` huhukumu
tu funguo ambazo PR huongeza kwenye `en.json`. Kundi la lugha huzalishwa kutoka kwa `en.json` ya siku
tawi lake linapoundwa na hutafsiri kwa siku kadhaa huku msingi ukiendelea kuongeza funguo; PR ya kundi yenyewe haiongezi
ufunguo wowote, kwa hivyo malango yote mawili husika yalikaa kimya wakati kundi la 1 (#13044) lilipounganishwa likiwa na upungufu wa funguo 43 katika
lugha tisa na kundi la 2 (#13660) likiwa na upungufu wa funguo 10 katika lugha nane (2026-09-15). Rekebisha hali nyekundu kwa
`node scripts/i18n/sync-ui-keys.mjs --locale=<codes> --translate-markers`; ufunguo wa mwisho wa `extra`
humaanisha chanzo kiliuondoa — uufute kutoka kwenye lugha hiyo. `--warn` huripoti bila kusababisha kushindwa.
`--catalog=cli` huendesha ulinganisho huohuo kwenye `bin/cli/locales` (`npm run i18n:check-keys:cli`);
hatua zote mbili zipo katika kazi `i18n-ui-coverage`.

#### `check-new-key-coverage` — lango la i18n la funguo mpya

Ni lango linalohusiana na `check-ui-value-drift`. Hilo hunasa thamani ya Kiingereza ambayo **iliandikwa upya**
huku tafsiri zake zikiachwa bila kusasishwa; hili hunasa ufunguo wa Kiingereza ambao **uliongezwa**
huku baadhi ya lugha zikiwa hazijaupokea kamwe.

`check-ui-keys-coverage` haiwezi kuona aina hii ya tatizo: huweka kiwango cha chini cha asilimia kwa kila lugha, na
funguo kumi na moja zinazokosekana kati ya ~13,000 huacha ufunikaji ukiwa 99.9%. Asilimia kwa kila lugha haiwezi
kueleza "kipengele hiki kilitolewa bila kutafsiriwa" — kipengele kizima kinaweza kuongezwa katika lugha mpya bila
maandishi yoyote na bila kubadilisha kamwe namba hiyo.

Tukio linalowakilishwa na lango hili: Awamu ya 3 ya Orchestration Canvas ilitafsiri funguo zake kumi na moja katika
lugha 42 zilizokuwepo wakati huo. Saa kadhaa baadaye, kundi la lugha za EU (#13044) lilifanya hazina
ifikie lugha 51, na lugha tisa mpya (`el`, `et`, `ga`, `hr`, `lt`, `lv`, `mt`, `sl`, `sr`) hazikuwahi
kuzipokea. `deepMergeFallback` hutumia Kiingereza badala ya ufunguo unaokosekana, kwa hivyo hali ya hitilafu ilikuwa
UI isiyotafsiriwa badala ya UI tupu — tatizo halisi, na lililokuwa kimya kutokana na muundo wake.

Kama lango linalohusiana nalo, hili **hutambua tofauti**, kwa kulinganisha Kiingereza katika msingi wa muunganisho na mti wa kazi,
kwa hivyo mapengo yaliyokuwepo awali hubaki bila kubadilishwa na lango halikuhitaji uhamishaji ili kuwashwa. Njia ya muda ya kukwepa:
`__MISSING__:<english>` huahirisha tafsiri huku ikidumisha usahihi wa mazingira ya utekelezaji. `vi` hupiga marufuku
vishikilia nafasi (`tests/unit/i18n-vi-completeness.test.ts`) na huhitaji tafsiri halisi.

#### `check-vitest-exclusions` — lango la majaribio yaliyowekwa kando

Faili iliyo katika orodha ya `exclude` ya `vitest.config.ts` ni jaribio ambalo halitekelezwi, na linaonekana kama
ufunikaji kwa yeyote anayesoma mti. Faili sitini na mbili zilikusanyika nyuma ya maoni
`// #8618 — pre-existing failure; remove this exclusion when fixed`. Suala #8618 lilifungwa tarehe
2026-08-11 huku orodha iliyofuatiliwa nalo ikiongezeka kutoka vipengee 45 hadi 62, kila kipengee kipya kikiritihisha maoni
yanayoelekeza kwenye suala lililofungwa. Hatimaye orodha ilipopimwa faili moja baada ya nyingine (#13204), **faili 51 kati ya 62
zilifaulu dhidi ya mti wa sasa bila mabadiliko yoyote ya msimbo chanzo**.

Lango linahitaji kila kiondolewaji kinacholingana na faili halisi (a) kitaje suala la ufuatiliaji na
(b) kionekane katika `config/quality/vitest-exclusions.json` pamoja na hali yake iliyopimwa, ili kuongeza kiondolewaji
kuwe tofauti inayoweza kukaguliwa katika faili mahususi badala ya kuwa mstari mmoja zaidi katika safu yenye vipengee 60. Kwa makusudi,
halitekelezi tena majaribio yaliyoondolewa — hilo huchukua ~10 dakika na linafaa kufanywa katika kazi ya mara kwa mara; orodha
hurekodi wakati kila jaribio lilipopimwa mara ya mwisho.
