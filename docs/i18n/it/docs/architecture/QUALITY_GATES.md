...
`.github/workflows/quality.yml` viene eseguito sulle PR destinate a `release/**`. Mantiene in
movimento i branch dei collaboratori con gate rapidi filtrati per percorso, oltre a un segnale
consultivo di build di produzione per le modifiche al codice:

| Job                                              | Ambito                                                                                                                                                                                                                                     | Bloccante                                                                                                          |
| ------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------ |
| `Build (advisory)`                               | PR di codice non in bozza e branch della coda Mergify; Node 24, `npm-ci-retry`, `check:node-runtime`, `npm run build` con `OMNIROUTE_USE_TURBOPACK=1`; nessun caricamento di artefatti perché nessun job di qualità downstream li utilizza | **Consultivo** (`continue-on-error: true`; rimuovere dopo una settimana di esecuzioni stabili delle PR di release) |
| `Docs Gates (fast-path)`                         | PR di documentazione/codice; riferimenti alla documentazione API e docs-all                                                                                                                                                                | Sì                                                                                                                 |
| `Fast Quality Gates`                             | PR di codice; controlli statici, verifica dei tipi, verifica dei tipi della dashboard, test unitari interessati                                                                                                                            | Sì                                                                                                                 |
| `Forgotten sibling tests`                        | PR di codice; moduli modificati ricondotti ai consumer statici e ai test sibling candidati; i percorsi tramite barrel e import dinamici vengono segnalati come diagnostica consultiva, con le eccezioni dell'allowlist di riferimento      | **Consultivo**                                                                                                     |
...
| `No new ESLint warnings`                         | PR di codice; protezione lint consapevole delle soppressioni                                                                                                                                                                               | Sì per quelle della stessa origine, consultivo per i fork                                                          |
| `Merge integrity (changelog + generated skills)` | PR non in bozza; sincronizzazione del changelog e delle skill generate                                                                                                                                                                     | Sì per quelle della stessa origine, consultivo per i fork                                                          |
...
è assente dal diff della pull request. Il riepilogo Markdown e il risultato JSON vengono conservati
come artefatto del workflow `forgotten-sibling-tests` per la calibrazione prima di qualsiasi
adozione bloccante.

Le riesportazioni tramite barrel e gli import dinamici sono esclusivamente diagnostiche di
risoluzione; non generano mai un rilievo bloccante. Le eccezioni sottoposte a revisione si trovano in
...
di GitHub. Le voci non valide causano un fallimento preventivo. Le eccezioni non possono sopprimere
un test candidato eliminato o un diff che aggiunge `.skip`/`.todo`; l'indebolimento delle asserzioni
e altri meccanismi di mascheramento rimangono di competenza del gate bloccante indipendente
`check:test-masking`.
...
| `check:route-validation:t06`      | Presenza degli schemi Zod su tutte le route (criterio Tier 6)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | Sì                                         |
| `check:any-budget:t11`            | Il numero di `@ts-expect-error // any` non supera il limite (controllo Tier 11)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | Sì                                         |
...
| `check:lockfile`                  | Integrità di `package-lock.json` — registro HTTPS, hash di integrità, nessun override dell'host                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | Sì                                         |
| `check:licenses`                  | Elenco consentito di licenze SPDX per le dipendenze di produzione                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | Sì                                         |
| `check:tracked-artifacts`         | Nessun artefatto di build / collegamento simbolico a `node_modules` incluso nel controllo di versione (eseguito anche nell'hook pre-commit di husky; il pre-push è intenzionalmente leggero — #6716)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | Sì                                         |
| `check:vitest-exclusions`         | Ogni esclusione Vitest indica un issue di tracciamento ed è presente in `config/quality/vitest-exclusions.json` (#13204)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | Sì                                         |
| `check:file-size`                 | Nessun file sorgente supera il limite per estensione (ratchet: file di grandi dimensioni bloccati nell'elenco `frozen`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | Sì                                         |
...
| `check:migration-numbering`       | I file SQL di migrazione sono numerati in sequenza, senza lacune né duplicati                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | Sì                                         |
...
| `check:db-rules`                  | Nessun SQL grezzo al di fuori dei moduli `src/lib/db/`; nessuna importazione tramite barrel da `localDb.ts` (Regole rigide n. 2/n. 5)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | Sì                                         |
| `check:known-symbols`             | Gli esecutori dei provider, le strategie di routing e i traduttori registrati nelle rispettive tabelle di dispatch corrispondono ai file presenti su disco, senza simboli orfani o non dichiarati                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | Sì                                         |
...
| `check:test-discovery`            | Ogni file `*.test.ts` / `*.spec.ts` nel repository viene rilevato da almeno un test runner (ratchet: l'elenco degli elementi orfani in `test-discovery-baseline.json` può solo ridursi)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | Sì                                         |
...
| `typecheck:noimplicit:core`       | `noImplicitAny` rigoroso — orientato al futuro; molti siti di chiamata preesistenti necessitano ancora di annotazioni                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | **Consultivo** (`continue-on-error: true`) |
| `check:dashboard-typecheck`       | `tsc` limitato a `src/app/(dashboard)/**` (#7033) — l'elenco consentito curato di 27 file di `typecheck:core` non include alcun TSX della dashboard e neppure `next build` ne esegue mai il controllo dei tipi (`next.config.mjs` imposta `ignoreBuildErrors: true`), pertanto le regressioni relative a identificatori orfani presenti lì (#6625/#6909) erano invisibili alla CI. Confronta i risultati con una baseline bloccata del conteggio per file/per codice TS (`config/quality/dashboard-typecheck-baseline.json`, lo stesso modello di applicazione contro l'obsolescenza usato da `check:known-symbols`) — solo i NUOVI errori oltre il conteggio registrato nella baseline causano il fallimento del gate; ridurre progressivamente la baseline con `--update` quando viene corretto un errore preesistente.                                                                                                                                                                     | Sì                                         |
...
| `check:compression-budget`   | Budget del benchmark di compressione — le soglie minime di risparmio dei token per ciascun motore non devono peggiorare                                                                                              | Sì                       |
...
| `check:codeql-ratchet`       | Il numero di avvisi CodeQL aperti non peggiora (lettura tramite `gh api`; omissione controllata in assenza di token) — per la frequenza di aggiornamento e l'attivazione manuale, vedere "Ratchet CodeQL" di seguito | Sì                       |

### Job: `quality-extended`

L'intero job è consultivo (`continue-on-error: true`). I controlli incrementali basati su npm vengono eseguiti
effettivamente; gli scanner esterni vengono installati tramite `gh release download` e si ignorano automaticamente (exit 0)
...
| Script                         | Verifica                                                                                                                                                                                                               | Bloccante                        |
| ------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------- |
| `check:docs-all`               | Meta-gate che esegue in sequenza i 6 controlli secondari riportati di seguito                                                                                                                                          | Sì                               |
| ↳ `check:docs-sync`            | Coerenza delle versioni tra CHANGELOG / OpenAPI / llm.txt                                                                                                                                                              | Sì                               |
| ↳ `check:docs-counts`          | I conteggi nel testo (numero di provider, numero di migrazioni, ecc.) rientrano nella finestra incrementale rispetto ai conteggi reali                                                                                 | Sì                               |
| ↳ `check:env-doc-sync`         | Ogni variabile di ambiente in `.env.example` è documentata in una tabella della documentazione e viceversa                                                                                                             | Sì                               |
| ↳ `check:deprecated-versions`  | Assenza di stringhe di versione deprecate nella documentazione                                                                                                                                                         | Sì                               |
| ↳ `check:doc-links`            | I link markdown interni nella documentazione corrispondono a file reali (formato `[testo]`/`(percorso)`)                                                                                                               | Sì                               |
| ↳ `check:fabricated-docs`      | Le route, le variabili di ambiente, i comandi CLI, i nomi degli hook e i percorsi dei file citati nella documentazione esistono nel codebase. Controllo rigido tramite `--strict`; errore non bloccante senza il flag. | Sì (tramite `--strict` nella CI) |
| `check:cli-i18n`               | Le stringhe dei comandi CLI sono presenti in tutti i file delle impostazioni locali i18n                                                                                                                               | Sì                               |
| `check:openapi-coverage`       | La specifica OpenAPI copre almeno una soglia minima incrementale delle route reali                                                                                                                                     | Sì                               |
...
| `check:openapi-routes`         | Ogni percorso in `openapi.yaml` corrisponde a un file `route.ts` reale (anti-allucinazione)                                                                                                                            | Sì                               |
| `check:docs-symbols`           | Ogni riferimento `/api/...` in `docs/**/*.md` corrisponde a un file `route.ts` reale (anti-allucinazione)                                                                                                              | Sì                               |
...
| `check-new-key-coverage` (inline) | Una **nuova** chiave inglese raggiunge ogni locale                                                                                                                                                      | Sì             |
| `check-translation-ratio`         | Il rapporto di traduzioni reali per locale (valori identici all'inglese / segnaposto / mancanti al di fuori dell'allowlist) non deve superare `config/quality/i18n-translation-baseline.json` + margine | **Consultivo** |

Richiede `fetch-depth: 0`: il gate per la divergenza dei valori confronta `en.json` con la base di merge.

#### `check-ui-value-drift` — gate per le traduzioni obsolete

Individua l'unica regressione i18n che gli altri gate non sono strutturalmente in grado di rilevare: un valore inglese viene riscritto e le traduzioni derivate dall'inglese _precedente_ rimangono invariate, così gli utenti non anglofoni continuano a leggere un testo formulato con sicurezza, ma ormai errato.

Questo problema è arrivato davvero in produzione. `oauthModal.googleOAuthWarning` è stato riscritto quando è stato introdotto l'helper di accesso Antigravity (#5203); **39 locale su 43** hanno mantenuto il testo che indicava agli operatori di "copiare l'URL completo e incollarlo qui sotto", un flusso che non può essere completato per quel provider. Il problema è passato inosservato fino alla #8463 perché:

- `sync-ui-keys` inserisce soltanto le chiavi **assenti**, mai quelle **obsolete**;
- `check-ui-keys-coverage` conteggia la _presenza_ delle chiavi, quindi una traduzione obsoleta risulta coperta;
- `check-translation-drift` tiene traccia dei mirror della documentazione `docs/i18n/<locale>/**.md`: non legge mai `src/i18n/messages/*.json`. Bloccante nel job `docs-sync-strict` dalla risincronizzazione 2026-09: modifica un documento principale → `npm run i18n:run -- --files=<doc>` (a livello di sezione, poco costoso).

**Basato sulle differenze, non su una baseline.** Confronta `en.json` alla base di merge con l'albero di lavoro; per ogni chiave il cui valore inglese è cambiato, qualsiasi locale che conservi ancora una traduzione non modificata è obsoleto. Questo approccio **congela intenzionalmente il debito preesistente**: una differenza non può rivelare da quale vecchio testo inglese derivi una traduzione esistente da tempo, quindi il gate valuta soltanto ciò che viene interessato dalla modifica corrente. L'alternativa (una baseline di hash per chiave) richiederebbe un file generato di circa 600 KB, 3 volte più grande della maggiore baseline esistente, che verrebbe modificato a ogni PR i18n.

Esistono due modi per soddisfarlo:

1. aggiornare le traduzioni interessate oppure
2. impostarle su `__MISSING__:<new english>`: il runtime fornisce quindi il testo inglese corretto (`src/i18n/request.ts::deepMergeFallback`, #7258) e la chiave viene accodata per la traduzione.

Se è cambiato il **significato** della stringa, è preferibile **rinominare la chiave**: una nuova chiave non può ereditare una traduzione obsoleta. Questo è il modello utilizzato dalla #8463.

```bash
npm run i18n:check-value-drift          # rigoroso (quello eseguito dalla CI)
npm run i18n:check-value-drift:warn     # solo report
BASE_REF=origin/release/vX.Y.Z npm run i18n:check-value-drift
```

Termina con codice 0 e `SKIP reason=base-unresolved` quando il catalogo di base non può essere letto (clone superficiale senza il riferimento di base), analogamente a `check-openapi-breaking`.
...
| Script                          | Verifica                                | Bloccante                                                  |
| ------------------------------- | --------------------------------------- | ---------------------------------------------------------- |
| `validate_translation.py quick` | Completezza della traduzione per locale | **Consultivo** (`continue-on-error: true` sull'intero job) |
...
| `check:pr-evidence`    | Il corpo della PR cita evidenze di test/VPS relative alla modifica (automatizza la Regola rigida n. 18 cercando nel testo della PR — fragile, vedere Backlog) | Sì        |
...
Vengono eseguiti secondo una pianificazione cron (e tramite `workflow_dispatch`), mai nelle PR. Sono tutti consultivi.

| Workflow               | Valida                                                                                                                                                                            | Bloccante      |
| ---------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------- |
| `nightly-property`     | Test basati su proprietà con fast-check, con seed casuale e un numero elevato di esecuzioni                                                                                       | **Consultivo** |
| `nightly-resilience`   | Controllo della crescita dell'heap, iniezione caotica di guasti, test di carico/tenuta con k6                                                                                     | **Consultivo** |
| `nightly-llm-security` | Protezione dalle injection con promptfoo (modalità di blocco) + sonde garak (ignorate in assenza del secret di un provider)                                                       | **Consultivo** |
| `nightly-schemathesis` | Fuzzing dei contratti OpenAPI (schemathesis) su un'istanza OmniRoute attiva usando `docs/openapi.yaml` — rileva violazioni delle specifiche / errori 500 non gestiti (Fase 8 B.4) | **Consultivo** |
| `nightly-mutation`     | Punteggio dei test di mutazione Stryker sulla corsia rapida dei test unitari — i mutanti sopravvissuti evidenziano asserzioni deboli                                              | **Consultivo** |
...
Confronta l'insieme delle chiavi foglia di ogni `src/i18n/messages/<locale>.json` con `en.json` e non riesce
in presenza di qualsiasi chiave foglia mancante o aggiuntiva, indipendentemente da quando la chiave è stata aggiunta. I segnaposto
`__MISSING__:` contano come presenti (il loro contenuto riguarda il gate del rapporto). È il complemento assoluto
dei due gate basati su diff/percentuale: `check-ui-keys-coverage` impone una soglia minima dell'80 % per
locale (43 chiavi mancanti su ~13.000 risultano comunque nel 99,7 %) e `check-new-key-coverage` valuta
solo le chiavi che una PR aggiunge a `en.json`. Un batch di locale viene generato dall'`en.json` del giorno
in cui viene creato il relativo branch e continua la traduzione per giorni, mentre la base continua ad aggiungere chiavi; la PR del batch non aggiunge
essa stessa alcuna chiave, perciò entrambi i gate correlati sono rimasti silenti quando il batch 1 (#13044) è stato integrato con 43 chiavi
mancanti in nove locale e il batch 2 (#13660) con 10 chiavi mancanti in otto locale (2026-09-15). Per correggere un errore, usare
`node scripts/i18n/sync-ui-keys.mjs --locale=<codes> --translate-markers`; una chiave foglia `extra`
significa che la sorgente l'ha rimossa: eliminarla dal locale. `--warn` genera una segnalazione senza causare errori.
...
senza aggiornare le relative traduzioni; questo rileva una chiave inglese che è stata **aggiunta**
senza che alcuni locale l'abbiano mai ricevuta.

`check-ui-keys-coverage` non può rilevare questa classe di problemi: impone una soglia percentuale minima per locale e
undici chiavi mancanti su ~13.000 lasciano la copertura al 99,9%. Una percentuale per lingua non può
esprimere «questa funzionalità è stata distribuita senza traduzione»: un'intera funzionalità può essere introdotta in un nuovo locale senza alcun
testo e senza che il valore cambi.

L'incidente che codifica: la fase 3 di Orchestration Canvas ha tradotto le sue undici chiavi nei
42 locale esistenti all'epoca. Ore dopo, il batch delle lingue UE (#13044) ha portato il repository
a 51 locale e i nove nuovi arrivati (`el`, `et`, `ga`, `hr`, `lt`, `lv`, `mt`, `sl`, `sr`) non le hanno mai
ricevute. `deepMergeFallback` sostituisce una chiave mancante con l'inglese, quindi la modalità di errore consisteva in
un'interfaccia utente non tradotta anziché vuota: un problema reale e, per costruzione, silenzioso.

Come il gate correlato, è **consapevole dei diff**: confronta l'inglese alla base del merge con l'albero di lavoro,
quindi le lacune preesistenti restano congelate e il gate non ha richiesto alcuna migrazione per essere attivato. Via d'uscita:
`__MISSING__:<english>` rinvia una traduzione mantenendo corretto il comportamento in fase di esecuzione. `vi` vieta
i segnaposto (`tests/unit/i18n-vi-completeness.test.ts`) e richiede una traduzione effettiva.

#### `check-vitest-exclusions` — gate per i test accantonati

Un file nell'elenco `exclude` di `vitest.config.ts` è un test che non viene eseguito e appare
come copertura a chiunque esamini l'albero. Sessantadue file si sono accumulati dietro il commento
`// #8618 — pre-existing failure; remove this exclusion when fixed`. L'issue #8618 è stata chiusa il
2026-08-11, mentre l'elenco che monitorava è cresciuto da 45 a 62 voci, ognuna delle quali ereditava un commento
che rimandava a un'issue chiusa. Quando l'elenco è stato finalmente misurato file per file (#13204), **51 dei 62
test sono passati rispetto all'albero corrente senza alcuna modifica al codice sorgente**.

Il gate richiede che ogni esclusione che si risolve in un file reale (a) indichi un'issue di monitoraggio e
(b) compaia in `config/quality/vitest-exclusions.json` con il relativo stato misurato, affinché la sua aggiunta costituisca
un diff revisionabile in un file dedicato anziché un'ulteriore riga in un array di 60 voci. Deliberatamente,
non riesegue i test esclusi: ciò richiede ~10 minuti ed è compito di un job periodico; l'inventario
registra quando ciascuno di essi è stato misurato l'ultima volta.
