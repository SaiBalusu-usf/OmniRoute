...
**هدف:** نادیدهگرفتن یک کلید معیوب، درحالیکه سایر اتصالهای همان ارائهدهنده همچنان سرویسدهی میکنند.
...
- کلید API با خطای 429: سرآیندهای `Retry-After`/بازنشانی بالادستی یا متن قابلتجزیه بازنشانی را ترجیح میدهد
- عقبنشینی: `baseCooldownMs * 2 ** failureIndex`

**محافظ ضد ازدحام ناگهانی:** از تمدید بیشازحد دوره انتظار یا افزایش دوباره `backoffLevel` بر اثر خطاهای همزمان جلوگیری میکند.

**وضعیتهای نهایی (دوره انتظار نیستند):**

- `banned` — توسط تشخیص کلیدواژه ممنوعیت / مسدودشدن حساب تنظیم میشود (به [BAN_DETECTION](../security/BAN_DETECTION.md) مراجعه کنید)
...
این وضعیتها تا زمانی که اعتبارنامهها تغییر کنند یا یک اپراتور آنها را بازنشانی کند، باقی میمانند. وضعیتهای نهایی را با وضعیت دوره انتظار موقت بازنویسی نکنید.

**بازیابی تنبل:** هنگامی که زمان `rateLimitedUntil` گذشته باشد، اتصال دوباره واجد شرایط میشود. پس از استفاده موفق، `clearAccountError()` تمام فیلدهای خطا را پاک میکند.

### وابستگی نشست (#7274)

**دامنه:** یک نشست کلاینت (سرآیند `X-Session-Id` / `x-codex-session-id` / `x-omniroute-session`) که برای **هر** ارائهدهنده به یک اتصال ثابت متصل شده است.

**هدف:** نگهداشتن یک عامل چندمرحلهای (Claude Code، aider، عاملهای سفارشی) روی همان حساب در درخواستهای مختلف، بهمنظور کاهش از دست رفتن زمینه میان حسابها و خطاهای 429 مکرر ناشی از شروع سرد در ارائهدهندگانی که وضعیت نشست را برای هر حساب نگه میدارند.

**پیادهسازی:**

- تعیین TTL: `src/sse/services/sessionAffinityPin.ts::resolveSessionAffinityTtlMs()`
- انتخاب/ایجاد اتصال ثابت: `src/sse/services/sessionAffinityPin.ts::selectSessionAffinityConnection()`
- استخراج سرآیند (عمومی، برای هر ارائهدهنده): `src/sse/services/auth.ts::extractSessionAffinityKey()`
- جدول اتصال ثابت ماندگار: `sessionAccountAffinity` (`src/lib/db/sessionAccountAffinity.ts`)
- تنظیم: `sessionAffinityTtlMs` (TTL سراسری برحسب میلیثانیه، `0` آن را غیرفعال میکند) — `src/lib/db/settings.ts`. نام آن از `codexSessionAffinityTtlMs` که فقط مختص Codex بود، بهوسیله مهاجرت `124_generic_session_affinity_ttl.sql` تغییر کرده است؛ این مهاجرت هر TTL از پیش پیکربندیشده Codex را بهعنوان مقدار پیشفرض جدید منتقل میکند.

پیش از #7274، تابع `resolveSessionAffinityTtlMs()` برای همه ارائهدهندگان بهجز `codex` فوراً مقدار `0` را برمیگرداند؛ بنابراین تنظیم TTL (و سرآیندهای نشست) در هیچ جای دیگری اثری نداشتند، با اینکه سازوکار اتصال ثابت و استخراج سرآیند از قبل مستقل از ارائهدهنده بودند. این اصلاح، آن بازگشت زودهنگام را حذف کرد؛ اکنون پس از تنظیم مقدار سراسری به عددی بزرگتر از `0`، TTL بهطور یکسان برای همه ارائهدهندگان اعمال میشود.

سه سرآیند وابستگی نشست هرگز به بالادست ارسال نمیشوند — اجراکنندهها بهجای عبور دادن سرآیندهای کلاینت، سرآیندهای بالادست خود را از ابتدا میسازند؛ بنابراین این مقدار فقط یک شناسه همبستگی داخلی باقی میماند.
...
**هدف:** فراهمکردن مالکیت انحصاری و پایدار اتصال برای کلاینتهایی که به یک مرز سخت مسیریابی
در میان درخواستها نیاز دارند. این با وابستگی نشست که یک ترجیح نرم برای تداوم است تفاوت دارد:
یک اجاره انحصاری، وضعیت چرخه عمر را در SQLite ماندگار میکند، یکتایی سراسری مالک فعال و
اتصال فعال را اعمال میکند و نسل منقضیشده را پیش از ارسال به ارائهدهنده رد میکند.

این قابلیت برای هر کلید API بهصورت اختیاری فعال میشود. یک کلید مدیریتشده باید دامنه `lease:exclusive` و یک
فهرست صریح و غیرخالی `allowedConnections` داشته باشد. هر کلاینت HTTP میتواند از نقطه پایانی چرخه عمر استفاده کند؛ هیچ
نام کلاینت، عامل کاربر، ارائهدهنده، روش OAuth یا مدلی لازم نیست. اجاره مالک یک اتصال است،
نه یک مدل؛ بنابراین تغییر مدل تا زمانی که اتصال طبق روال معمول
واجد شرایط باقی بماند، پیوند را حفظ میکند. قواعد عادی مدل، سهمیه، سلامت، دوره انتظار و فهرست مجاز همچنان
مرجع نهایی هستند و ممکن است همان نسل را به یک اتصال آزاد و واجد شرایط دیگر منتقل کنند.

چرخه عمر، `POST /api/v1/session-leases` با کنشهای JSON شامل `acquire`، `renew` و `release` است.
درخواستهای استنتاج مدیریتشده، مقدار مبهم `X-OmniRoute-Lease-Owner` و مقدار دقیق
`X-OmniRoute-Lease-Generation` را ارائه میکنند. مالک از `vlo_` و سپس 43 نویسه base64url استفاده میکند؛ فقط
هش SHA-256 آن ذخیره میشود. هر مرز نهایی ارسال، شناسه کلید API احراز هویتشده و
شناسه اتصال فعال را نیز مقید میکند. سرآیندهای کنترل اجاره از گزارشها، تصاویر لحظهای نگهداریشده درخواستها و
سرآیندهای اجراکننده بالادست حذف میشوند.

اگر مسیریابی عادی دارای گزینههای مدیریتشده واجد شرایط باشد، اما هر گزینه آزاد توسط یک
اجاره فعال خارجی اشغال شده باشد، OmniRoute وضعیت HTTP `429`، کد lease-capacity-unavailable، یک
وضعیت انتظار برای ظرفیت و یک `Retry-After` محدودشده را که از نزدیکترین زمان انقضای مرتبط محاسبه شده است، برمیگرداند.
خالیبودن عادی مجموعه گزینههای واجد شرایط، رقابت بر سر اجاره محسوب نمیشود و معناشناسی خطای مسیریابی موجود خود را حفظ میکند.

سازوکارهای مرتبط همچنان جدا از یکدیگر باقی میمانند:

- اشغال نشست OAuth، یک توزیع نرم محلی برای فرایند برای حسابهای OAuth است.
- سمافورهای حساب، مجوزهای همزمانی درخواست را اعطا میکنند و با تکمیل درخواست پایان مییابند.
- اجارههای انحصاری اتصال برای نشست مدیریتشده، مالکیت پایدار چرخه عمر با یک مرز نسلی هستند.
...
**هدف:** جلوگیری از غیرفعالکردن کل یک اتصال، زمانی که فقط یک مدل در دسترس نیست یا با محدودیت سهمیه مواجه شده است.

**مثالها:**

- ارائهدهندگان دارای سهمیه بهازای هر مدل که 429 برمیگردانند
- ارائهدهندگان محلی که برای یک مدل موجودنباشد 404 برمیگردانند
- خطاهای مجوز حالت/مدل مختص ارائهدهنده (برای مثال، حالتهای Grok)

**پیادهسازی:** `open-sse/services/accountFallback.ts` — `lockModel()`، `clearModelLock()`، `getAllModelLockouts()`.

### داشبورد زمانهای انتظار مدل (v3.8.0)

رابط کاربری: تنظیمات ← زمانهای انتظار مدل (`src/app/(dashboard)/dashboard/settings/components/ModelCooldownsCard.tsx`)

قفلهای فعال را همراه با این موارد فهرست میکند: ارائهدهنده، اتصال، مدل، دلیل، expiresAt. اپراتورها میتوانند یک مدل را بهصورت دستی از طریق کارت دوباره فعال کنند.

**REST API:**

- `GET /api/resilience/model-cooldowns` — فهرستکردن قفلهای فعال
- `DELETE /api/resilience/model-cooldowns` — فعالسازی مجدد دستی. بدنه: `{provider, connection, model}`. احراز هویت: مدیریتی.

### رابط کاربری تنظیمات قفل + بازیابی با کاهش پس از موفقیت (v3.8.23)

قفل مدل از رفتاری همیشهفعال و هاردکدشده، به قابلیتی کاملاً قابلپیکربندی و
اختیاری، با کارت تنظیمات مخصوص خود و مسیر بازیابی خودترمیمشونده تبدیل شد.

**کارت تنظیمات:** تنظیمات ← قفل مدل
(`src/app/(dashboard)/dashboard/settings/components/ModelLockoutCard.tsx`).
این کارت از `ModelCooldownsCard` فقطخواندنی بالا (که تنها قفلهای فعال را
_فهرست میکند_) **متمایز** است — کارت جدید _پارامترها را پیکربندی میکند_. مقادیر
پیشفرض در `DEFAULT_MODEL_LOCKOUT_SETTINGS`
(`src/lib/resilience/modelLockoutSettings.ts`) قرار دارند:

| تنظیم                   | مقدار پیشفرض                     | مفهوم                                                       |
| ----------------------- | -------------------------------- | ----------------------------------------------------------- |
| `enabled`               | `false`                          | کلید اصلی — قفل مدل **بهصورت پیشفرض خاموش است**.            |
| `errorCodes`            | `[403, 404, 429, 502, 503, 504]` | وضعیتهای بالادستی که بهعنوان خرابی در سطح مدل محسوب میشوند. |
| `baseCooldownMs`        | `120_000` (120 ثانیه)            | مدت قفل اولیه برای نخستین خرابی.                            |
| `maxCooldownMs`         | `1_800_000` (30 دقیقه)           | سقف زمان انتظار افزایشیافته.                                |
| `maxBackoffSteps`       | `10`                             | حداکثر مراحل افزایش پسروی نمایی.                            |
| `useExponentialBackoff` | `true`                           | آیا خرابیهای مکرر زمان انتظار را بهصورت نمایی افزایش دهند.  |

تنظیمات از طریق مخزن عادی تنظیمات ماندگار میشوند و بهوسیله طرحواره تنظیمات
تابآوری اعتبارسنجی میشوند؛ کارت مقادیر `baseCooldownMs`/`maxCooldownMs`
را محدود میکند (با شرط `maxCooldownMs ≥ baseCooldownMs`) و همین کار را برای `maxBackoffSteps` نیز انجام میدهد.

**بازیابی با کاهش پس از موفقیت:** بازیابی **صرفاً** بر اساس انقضای تایمر نیست. یک
پاسخ سالم تعداد خرابیهای مدل را کاهش میدهد تا مدلی که در میانه بازه بازیابی شده است،
پیش از زمان مقرر تایمر دیگر تشدید نشود (و قفل آن پاک شود). هنگام موفقیت یک
هدف ترکیبی، `open-sse/services/combo.ts` تابع `decayModelFailureCount()`
(`open-sse/services/accountFallback.ts`) را فراخوانی میکند که مقدار ذخیرهشده
`failureCount` را **نصف** میکند (`Math.floor(failureCount / 2)`)، و وقتی این مقدار
به `0` برسد، ورودی قفل بهطور کامل حذف میشود. تابع متناظر `recordModelLockoutFailure()`
در صورت وقوع خرابی در بازه تشدید، تعداد را افزایش میدهد (و زمان انتظار را تشدید میکند).
این کاهش پس از موفقیت، افزون بر انقضای عادی تایمر است — هرکدام از این مسیرها
میتواند یک مدل را دوباره فعال کند.

**وضعیت:** قفلها **در حافظه** نگهداری میشوند (`Map`های مختص هر فرایند از
`ModelLockoutEntry` که با `provider:connectionId:model` کلیدگذاری شدهاند) و در
DB ماندگار نمیشوند — با راهاندازی مجدد از بین میروند. _تنظیمات_ ماندگار میشوند؛
_وضعیت_ قفل فعال موقتی است.
...
- همه کلیدهای یک ارائهدهنده نادیده گرفته میشوند → هم وضعیت قطعکننده مدار و هم `rateLimitedUntil`/`testStatus` هر اتصال را بررسی کنید.
- ارائهدهنده پس از پنجره بازنشانی برای همیشه مستثنا میشود → کد بهجای `getStatus()`/`canExecute()` مقدار خام `state` را میخواند.
- یک کلید از کار میافتد، اما بقیه باید کار کنند → وقفه موقت اتصال را به قطعکننده مدار ترجیح دهید.
- فقط یک مدل از کار میافتد → قفلشدن مدل را به وقفه موقت اتصال ترجیح دهید.
- وضعیت باید خودکار بازیابی شود اما نمیشود → وجود مُهر زمانی در آینده و مسیر خواندنی را که وضعیت منقضیشده را تازهسازی میکند بررسی کنید. وضعیتهای دائمی به تغییرات دستی نیاز دارند.
