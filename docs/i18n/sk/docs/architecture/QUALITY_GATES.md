...
Skripty sa nachádzajú v `scripts/check/` (politické brány) a `scripts/quality/` (sprísňovací mechanizmus).
Zdrojom pravdy pre CI je `.github/workflows/ci.yml`.

### Zrýchlený postup pre PR vydania (`quality.yml`)

`.github/workflows/quality.yml` sa spúšťa pri PR smerujúcich na `release/**`. Udržiava plynulý postup
prispievateľských vetiev pomocou rýchlych brán filtrovaných podľa ciest a jedného poradného signálu produkčného zostavenia pre
zmeny kódu:

| Úloha                                            | Rozsah                                                                                                                                                                                                                                          | Blokovanie                                                                                             |
| ------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------ |
| `Build (advisory)`                               | PR s kódom, ktoré nie sú konceptmi, a vetvy frontu Mergify; Node 24, `npm-ci-retry`, `check:node-runtime`, `npm run build` s `OMNIROUTE_USE_TURBOPACK=1`; bez nahrávania artefaktu, pretože ho nepoužíva žiadna nadväzujúca úloha kvality       | **Poradné** (`continue-on-error: true`; odstráňte po jednom týždni stabilných spustení pre PR vydania) |
| `Docs Gates (fast-path)`                         | PR s dokumentáciou/kódom; odkazy v dokumentácii API a docs-all                                                                                                                                                                                  | Áno                                                                                                    |
| `Fast Quality Gates`                             | PR s kódom; statické kontroly, kontrola typov, kontrola typov dashboardu, ovplyvnené jednotkové testy                                                                                                                                           | Áno                                                                                                    |
| `Forgotten sibling tests`                        | PR s kódom; zmenené moduly sledované až k statickým konzumentom a kandidátskym súrodeneckým testom; cesty cez zberné moduly a dynamické importy sa hlásia ako poradná diagnostika spolu s odkazovanými výnimkami zo zoznamu povolených položiek | **Poradné**                                                                                            |
| `Vitest (fast-path)`                             | PR s kódom; rýchla sada Vitest                                                                                                                                                                                                                  | Áno                                                                                                    |
| `Unit Tests fast-path`                           | PR s kódom; jednotková sada rozdelená na 4 časti                                                                                                                                                                                                | Áno                                                                                                    |
| `No new ESLint warnings`                         | PR s kódom; kontrola lintovania zohľadňujúca potlačenia                                                                                                                                                                                         | Áno pre vlastný pôvod, poradné pre forky                                                               |
| `Merge integrity (changelog + generated skills)` | PR, ktoré nie sú konceptmi; synchronizácia záznamu zmien a generovaných zručností                                                                                                                                                               | Áno pre vlastný pôvod, poradné pre forky                                                               |

#### Správa o zabudnutých súrodeneckých testoch

`npm run check:forgotten-sibling-tests` opätovne používa resolver importov, ktorý používa mapa vplyvu na testy.
...
test chýba v rozdiele požiadavky na začlenenie. Súhrn vo formáte Markdown a výsledok JSON sa uchovávajú ako
artefakt workflowu `forgotten-sibling-tests` na kalibráciu pred zavedením blokovania.

Opätovné exporty zo zberných modulov a dynamické importy slúžia iba ako diagnostika rozlíšenia; nikdy nevytvárajú
...
test, obsahovať konkrétne odôvodnenie a odkazovať na problém alebo požiadavku na začlenenie na GitHube. Chybne vytvorené položky
vedú k zamietnutiu. Výnimky nemôžu potlačiť odstránený kandidátsky test ani rozdiel, ktorý pridáva `.skip`/`.todo`;
oslabovanie tvrdení a iné maskovanie naďalej pokrýva nezávisle blokujúca
...
| Skript (`npm run ...`)            | Overuje                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | Blokujúci                                    |
| --------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------- |
...
| `check:model-lifecycle`           | Tri ručne udržiavané smerovacie tabuľky zostávajú konzistentné so snapshotom životného cyklu uloženým v repozitári (#11503): `FITNESS_TABLE` (`taskFitness.ts`) nehodnotí žiadne vyradené id, ktoré dokáže `REGISTRY` smerovať; každý cieľ `BUILT_IN_ALIASES` sa nachádza v `REGISTRY` a nenachádza sa v snapshote vyradených id; každé vyradené id, ktoré je stále v `REGISTRY`, je presmerované alebo uvedené v `allowedRetiredInCatalog`; a žiadny zdroj ani cieľ `DEFAULT_DEGRADATION_MAP` sa v danom snapshote nejaví ako vyradený. Toto nedokazuje, že model je aktuálne poskytovaný aktívnou upstream službou. Offline — porovnáva sa s `config/quality/model-lifecycle.json`, ktorý sa ručne aktualizuje pomocou `npm run quality:refresh-model-lifecycle` (vyžaduje sieť; nie je zapojený do CI). `allowedRetiredInCatalog` je postupne sprísňovaná poistka: záznam pridávajte iba spolu so sledovacou úlohou. | Áno                                          |
| `check:fetch-targets`             | Každé `fetch("/api/...")` v klientskom `src/` sa odkazuje na skutočný súbor `route.ts`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Áno                                          |
| `check:deps`                      | Všetky závislosti inštalovateľné pomocou `npm install` vo všetkých súboroch `package.json` v repozitári sa nachádzajú v `dependency-allowlist.json`; nové závislosti bez pripnutej verzie alebo balíky s preklepom napodobňujúce iné balíky sú označené                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | Áno                                          |
| `audit:deps`                      | `npm audit` (koreňový projekt + Electron) — žiadne upozornenia s vysokou/kritickou závažnosťou (prekrýva sa s OSV `check:vuln-ratchet`; pozrite si zoznam úloh na racionalizáciu)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | Áno                                          |
| `check:lockfile`                  | Integrita súboru `package-lock.json` — register HTTPS, haše integrity, žiadne prepísania hostiteľa                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | Áno                                          |
| `check:licenses`                  | Zoznam povolených licencií SPDX pre produkčné závislosti                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | Áno                                          |
| `check:tracked-artifacts`         | Žiadne artefakty zostavenia ani commitnuté symbolické odkazy na `node_modules` (spúšťa sa aj v husky pre-commit; pre-push je zámerne odľahčený — #6716)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | Áno                                          |
...
| `check:file-size`                 | Žiadny zdrojový súbor neprekračuje limit pre príslušnú príponu (ratchet: nemenný zoznam veľkých súborov v zozname `frozen`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | Áno                                          |
| `check:error-helper`              | Chybové odpovede v executoroch/handleroch používajú `buildErrorBody()` / `sanitizeErrorMessage()` (Pevné pravidlo č. 12)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | Áno                                          |
| `check:migration-numbering`       | Migračné súbory SQL sú číslované postupne, bez medzier alebo duplicít                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | Áno                                          |
| `check:public-creds`              | Žiadne literály OAuth `client_id`/`client_secret` ani webové kľúče Firebase mimo `publicCreds.ts` (pevné pravidlo č. 11)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | Áno                                          |
| `check:db-rules`                  | Žiadne nespracované SQL mimo modulov `src/lib/db/`; žiadne importy zo súhrnného modulu `localDb.ts` (pevné pravidlá č. 2/č. 5)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | Áno                                          |
...
| `check:route-guard-membership`    | Každá trasa, ktorá spúšťa podradený proces, je klasifikovaná pomocou `isLocalOnlyPath()` (pevné pravidlá č. 15/č. 17)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | Áno                                          |
...
| `check:agent-skills-sync`         | Vygenerované artefakty agentových zručností zodpovedajú svojmu zdrojovému katalógu (bez odchýlok)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `check:provider-asset-provenance` | Logá/zdroje poskytovateľov majú zaznamenaný údaj o pôvode                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| `typecheck:noimplicit:core`       | Striktné `noImplicitAny` — zamerané na budúcnosť; mnohé existujúce miesta volania stále potrebujú anotácie                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | **Informatívne** (`continue-on-error: true`) |
| `check:dashboard-typecheck`       | `tsc` obmedzené na `src/app/(dashboard)/**` (#7033) — kurátorsky spravovaný zoznam 27 súborov v `typecheck:core` neobsahuje žiadne súbory TSX dashboardu a ani `next build` ich nikdy typovo nekontroluje (`next.config.mjs` nastavuje `ignoreBuildErrors: true`), takže regresie osirelých identifikátorov v tejto oblasti (#6625/#6909) neboli pre CI viditeľné. Rozdiely sa porovnávajú so zmrazenou východiskovou hodnotou počtu podľa súboru/kódu TS (`config/quality/dashboard-typecheck-baseline.json`, rovnaký vzor vynucovania zastaranosti ako pri `check:known-symbols`) — kontrola zlyhá iba pri NOVÝCH chybách nad rámec východiskového počtu; keď sa opraví existujúca chyba, znížte východiskovú hodnotu pomocou `--update`.                                                                                                                                                                             | Áno                                          |
...
| ---------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------ |
| `quality:collect`            | Vytvorí `quality-metrics.json` (počet upozornení ESLint, pokrytie zo zlúčenej správy segmentov)                                                                                                                        | Áno (predchádza západkovej kontrole) |
...
| `check:cognitive-complexity` | Západková kontrola kognitívnej zložitosti (`eslint-plugin-sonarjs`) — samostatný priechod ESLint; CI spúšťa obe kontroly zlúčené do jediného kroku `check:complexity-ratchets`                                         | Áno                                  |
| `check:dead-code`            | Západková kontrola nepoužívaných exportov/súborov (knip) sa nezhoršila oproti východiskovej hodnote                                                                                                                    | Áno                                  |
...
| `check:type-coverage`        | Západková kontrola percentuálneho podielu typovaného kódu (`type-coverage`) sa nezhoršila; vo veľkej miere nahrádza `typecheck:noimplicit:core`                                                                        | Áno                                  |
| `check:codeql-ratchet`       | Počet otvorených upozornení CodeQL sa nezhoršil (načítava sa cez `gh api`; bez tokenu sa kontrola korektne preskočí) — frekvencia obnovovania a manuálne spustenie: pozrite si časť „Západková kontrola CodeQL“ nižšie | Áno                                  |

### Úloha: `quality-extended`

Celá úloha je poradná (`continue-on-error: true`). Kontroly založené na npm sa spúšťajú
reálne; externé skenery sa inštalujú pomocou `gh release download` a samy sa preskočia (ukončia sa s kódom 0),
keď binárny súbor stále chýba.

| Skript                   | Overuje                                                                                                                                                                                         | Blokujúci   |
| ------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------- |
| `check:circular-deps`    | Žiadne cyklické závislosti (dpdm)                                                                                                                                                               | **Poradný** |
| `check:bundle-size`      | Veľkosť balíka neprekračuje limit                                                                                                                                                               | **Poradný** |
| `check:secrets`          | Skenovanie tajomstiev (gitleaks) — preskočí sa, ak binárny súbor chýba                                                                                                                          | **Poradný** |
| `check:vuln-ratchet`     | Zraniteľnosti závislostí (osv-scanner) sa nezhoršujú — preskočí sa, ak binárny súbor chýba                                                                                                      | **Poradný** |
| `check:workflows`        | Kontrola workflowov (actionlint + zizmor) — preskočí sa, ak binárne súbory chýbajú                                                                                                              | **Poradný** |
| `check:openapi-breaking` | Zásadné zmeny verejného kontraktu API (`openapi.yaml`) oproti základnej vetve (oasdiff) — vypíše `openapiBreaking=N`; preskočí sa, ak oasdiff chýba alebo základnú špecifikáciu nemožno načítať | **Poradný** |
...
| ↳ `check:docs-counts`          | Počty v texte (počet poskytovateľov, počet migrácií atď.) sa nachádzajú v rámci kontrolného rozsahu skutočných počtov                                                                    | Áno                       |
| ↳ `check:env-doc-sync`         | Každá premenná prostredia v `.env.example` je zdokumentovaná v tabuľke dokumentácie a naopak                                                                                             | Áno                       |
| ↳ `check:deprecated-versions`  | V dokumentácii nie sú žiadne reťazce zastaraných verzií                                                                                                                                  | Áno                       |
| ↳ `check:doc-links`            | Interné markdownové odkazy v dokumentácii odkazujú na skutočné súbory (forma `[text]`/`(path)`)                                                                                          | Áno                       |
| ↳ `check:fabricated-docs`      | Trasy, premenné prostredia, príkazy CLI, názvy hookov a cesty k súborom uvedené v dokumentácii existujú v kódovej základni. Tvrdá kontrola cez `--strict`; bez príznaku povolí zlyhanie. | Áno (cez `--strict` v CI) |
| `check:cli-i18n`               | Reťazce príkazov CLI sa nachádzajú vo všetkých súboroch lokalizácií i18n                                                                                                                 | Áno                       |
| `check:openapi-coverage`       | Špecifikácia OpenAPI pokrýva aspoň postupne sprísňovanú minimálnu hranicu skutočných trás                                                                                                | Áno                       |
| `check:openapi-security-tiers` | Anotácie úrovní zabezpečenia v `openapi.yaml` sú konzistentné s klasifikáciami v `routeGuard.ts`                                                                                         | **Poradný**               |
...
| `i18n translation drift`       | Nepreložené kľúče v súboroch lokalizácií i18n — iba upozornenie                                                                                                                          | **Poradný**               |
...
| `check-new-key-coverage` (inline) | **Nový** anglický kľúč sa dostane do každého jazykového variantu                                                                                                                                                  | Áno            |
| `check-translation-ratio`         | Pomer skutočných prekladov pre každý jazykový variant (položky identické s angličtinou / zástupné / chýbajúce mimo zoznamu povolených) nesmie prekročiť `config/quality/i18n-translation-baseline.json` + rezervu | **Informačné** |

Vyžaduje `fetch-depth: 0` — kontrola odchýlky hodnôt porovnáva `en.json` so spoločným predkom vetiev.

#### `check-ui-value-drift` — kontrola zastaraných prekladov

Zachytáva jeden typ regresie i18n, ktorý ostatné kontroly z princípu nedokážu odhaliť: anglická hodnota
...
Tento problém sa skutočne dostal do produkcie. Hodnota `oauthModal.googleOAuthWarning` bola prepísaná pri pridaní
pomocníka na prihlásenie Antigravity (#5203); **39 zo 43 jazykových variantov** si ponechalo text, ktorý operátorom
hovoril, aby „skopírovali celú URL adresu a vložili ju nižšie“ — postup, ktorý pre daného poskytovateľa nemožno
...
- `check-ui-keys-coverage` počíta _prítomnosť_ kľúča, takže zastaraný preklad sa považuje za pokrytý;
- `check-translation-drift` sleduje dokumentačné zrkadlá `docs/i18n/<locale>/**.md` —
  nikdy nečíta `src/i18n/messages/*.json`. Od opätovnej synchronizácie 2026-09 je blokujúci v úlohe
  `docs-sync-strict`: úprava základného dokumentu → `npm run i18n:run -- --files=<doc>` (na úrovni sekcie, nenáročné).

**Zohľadňuje rozdiely, neopiera sa o základnú hodnotu.** Porovnáva `en.json` v spoločnom predkovi vetiev
s pracovným stromom; pre každý kľúč, ktorého anglická hodnota sa zmenila, sa každý jazykový variant, ktorý stále
obsahuje nedotknutý preklad, považuje za zastaraný. Týmto sa zámerne **zmrazuje existujúci dlh** — z rozdielu
nemožno zistiť, z ktorej starej anglickej verzie dlhodobo existujúci preklad pochádza, preto kontrola posudzuje
iba to, čoho sa dotýka aktuálna zmena. Alternatíva (základná hodnota hashov pre jednotlivé kľúče) by vyžadovala
približne 600 KB generovaný súbor, 3× väčší než najväčšia existujúca základná hodnota, ktorý by sa menil pri každom i18n PR.

Kontrolu možno splniť dvoma spôsobmi:

1. aktualizovať dotknuté preklady alebo
2. nastaviť ich na `__MISSING__:<new english>` — aplikácia potom poskytne opravenú anglickú verziu
...
Skončí s kódom 0 a hlásením `SKIP reason=base-unresolved`, keď nie je možné načítať základný katalóg (plytký
klon bez odkazu na základnú vetvu), podobne ako `check-openapi-breaking`.

### Úloha: `i18n`

Úplná matica overovania i18n (jedna úloha pre každý jazykový variant). Celá úloha je informačná.

| Skript                          | Overuje                                     | Blokujúce                                                 |
| ------------------------------- | ------------------------------------------- | --------------------------------------------------------- |
| `validate_translation.py quick` | Úplnosť prekladu pre každý jazykový variant | **Informačné** (`continue-on-error: true` pre celú úlohu) |
...
| `check:test-masking`   | Zmenené testovacie súbory neznižujú celkový počet asercií ani nepridávajú tautológie `assert.ok(true)`                                          | Áno       |
| `check:pr-evidence`    | Telo PR uvádza dôkazy z testov/VPS pre danú zmenu (automatizuje Prísne pravidlo č. 18 vyhľadávaním v texte PR — krehké, pozri Nevybavené úlohy) | Áno       |
...
| ---------------- | -------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------- |
| `test:vitest`    | MCP server (110 nástrojov), autoCombo, vyrovnávacia pamäť — spúšťač vitest | Áno                                                                                                                       |
...
| `nightly-resilience`   | Kontrola rastu haldy, chaotické vkladanie porúch, záťažové/dlhodobé testy k6                                                                                                  | **Informatívne** |
| `nightly-llm-security` | Ochrana promptfoo proti injektáži (režim blokovania) + sondy garak (preskočené bez tajného kľúča poskytovateľa)                                                               | **Informatívne** |
...
| `nightly-mutation`     | Skóre mutačného testovania Stryker nad rýchlou vetvou jednotkových testov — prežívajúce mutanty odhaľujú slabé asercie                                                        | **Informatívne** |
| `nightly-compat`       | Matica kompatibility enginu Node naprieč podporovanými rozsahmi `engines.node`                                                                                                | **Informatívne** |
...
pri každom chýbajúcom alebo nadbytočnom koncovom kľúči bez ohľadu na to, kedy bol kľúč pridaný. Zástupné hodnoty `__MISSING__:`
sa počítajú ako prítomné (ich obsah rieši kontrola pomeru). Ide o absolútny doplnok
dvoch kontrol založených na rozdieloch/percentách: `check-ui-keys-coverage` presadzuje minimálnu hranicu 80 % pre každé
miestne nastavenie (43 chýbajúcich kľúčov z približne 13 000 sa stále javí ako 99,7 %) a `check-new-key-coverage` posudzuje
iba kľúče, ktoré PR pridáva do súboru `en.json`. Dávka miestnych nastavení sa vygeneruje zo súboru `en.json` platného v deň
vytvorenia jej vetvy a prekladá sa celé dni, zatiaľ čo základná vetva naďalej pridáva kľúče; samotný PR dávky nepridáva žiadny
kľúč, takže obe súvisiace kontroly zostali bez odozvy, keď bola dávka 1 (#13044) zlúčená s 43 chýbajúcimi kľúčmi v deviatich
...
znamená, že ho zdroj odstránil — odstráňte ho aj z miestneho nastavenia. `--warn` iba vypíše hlásenie bez zlyhania.
...
Súvisiaca kontrola ku `check-ui-value-drift`. Tá zachytáva anglickú hodnotu, ktorá bola **prepísaná**,
...
`check-ui-keys-coverage` tento typ problému nedokáže odhaliť: presadzuje percentuálnu hranicu pre každé miestne nastavenie a
...
vyjadriť „táto funkcia bola vydaná bez prekladu“ — celá funkcia môže byť pridaná do nového miestneho nastavenia bez akéhokoľvek
textu bez toho, aby sa hodnota vôbec zmenila.

Incident, ktorý táto kontrola zachytáva: Fáza 3 produktu Orchestration Canvas preložila svojich jedenásť kľúčov vo všetkých
42 miestnych nastaveniach, ktoré v tom čase existovali. O niekoľko hodín neskôr dávka jazykov EÚ (#13044) zvýšila počet miestnych
nastavení v repozitári na 51 a deväť nových (`el`, `et`, `ga`, `hr`, `lt`, `lv`, `mt`, `sl`, `sr`) ich nikdy
nedostalo. `deepMergeFallback` nahrádza chýbajúci kľúč angličtinou, takže výsledkom bolo
nepreložené používateľské rozhranie namiesto prázdneho používateľského rozhrania — skutočný problém, ktorý bol zo svojej podstaty neviditeľný.

Rovnako ako súvisiaca kontrola je **zohľadňujúca rozdiely** a porovnáva angličtinu v spoločnom predkovi vetiev s pracovným
stromom, takže už existujúce medzery zostanú zmrazené a zapnutie kontroly si nevyžadovalo žiadnu migráciu. Núdzová výnimka:
`__MISSING__:<english>` odloží preklad a zároveň zachová správne fungovanie za behu. `vi` zakazuje
zástupné hodnoty (`tests/unit/i18n-vi-completeness.test.ts`) a vyžaduje skutočný preklad.

#### `check-vitest-exclusions` — kontrola odložených testov

Súbor v zozname `exclude` v `vitest.config.ts` predstavuje test, ktorý sa nespúšťa, no každému, kto si prezerá strom,
sa javí ako pokrytie. Za komentárom sa nahromadilo šesťdesiatdva súborov
`// #8618 — už existujúce zlyhanie; po oprave túto výnimku odstráňte`. Problém #8618 bol uzavretý
2026-08-11, zatiaľ čo zoznam, ktorý sledoval, narástol zo 45 položiek na 62, pričom každá nová položka zdedila komentár
odkazujúci na uzavretý problém. Keď sa zoznam napokon vyhodnotil súbor po súbore (#13204), **51 zo 62
prešlo oproti aktuálnemu stromu bez akejkoľvek zmeny zdrojového kódu**.

Kontrola vyžaduje, aby každá výnimka odkazujúca na skutočný súbor (a) uvádzala sledovaný problém a
(b) bola uvedená v `config/quality/vitest-exclusions.json` spolu so svojím nameraným stavom, takže jej pridanie
predstavuje kontrolovateľný rozdiel vo vyhradenom súbore namiesto ďalšieho riadka v poli so 60 položkami. Zámerne
znova nespúšťa vylúčené testy — trvá to približne 10 minút a patrí to do pravidelnej úlohy; inventár
zaznamenáva, kedy bol každý z nich naposledy vyhodnotený.
