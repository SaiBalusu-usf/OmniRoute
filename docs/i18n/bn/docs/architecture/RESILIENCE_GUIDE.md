...
**উদ্দেশ্য:** একই প্রোভাইডারের অন্যান্য সংযোগ সেবা দেওয়া চালিয়ে যাওয়ার সময় একটি ত্রুটিপূর্ণ কী এড়িয়ে যাওয়া।

**বাস্তবায়ন:**

- অনুপলব্ধ হিসেবে চিহ্নিত করা: `src/sse/services/auth.ts::markAccountUnavailable()`
...
**থান্ডারিং-হার্ড প্রতিরোধ ব্যবস্থা:** সমসাময়িক ব্যর্থতার কারণে কুলডাউন অতিরিক্ত দীর্ঘ হওয়া বা `backoffLevel` দুবার বৃদ্ধি পাওয়া প্রতিরোধ করে।

**টার্মিনাল অবস্থা (কুলডাউন নয়):**

- `banned` — নিষিদ্ধ-কীওয়ার্ড / অ্যাকাউন্ট-ব্যান শনাক্তকরণের মাধ্যমে সেট হয় ([BAN_DETECTION](../security/BAN_DETECTION.md) দেখুন)
- `expired` (সীমাবদ্ধ সংখ্যক পুনঃচেষ্টার পর টার্মিনাল অবস্থায় রূপান্তরিত হয় — এক্সপোনেনশিয়াল ব্যাকঅফসহ `EXPIRED_RETRY_MAX = 3` — যাতে সাময়িক OAuth ত্রুটি অ্যাকাউন্টটি স্থায়ীভাবে নিষ্ক্রিয় হওয়ার আগে নিজে থেকেই সেরে উঠতে পারে)
- `credits_exhausted`

ক্রেডেনশিয়াল পরিবর্তন না হওয়া বা কোনো অপারেটর এগুলো রিসেট না করা পর্যন্ত এসব অবস্থা বজায় থাকে। সাময়িক কুলডাউন অবস্থা দিয়ে টার্মিনাল অবস্থা ওভাররাইট করবেন না।

**লেজি পুনরুদ্ধার:** `rateLimitedUntil` অতীত হয়ে গেলে সংযোগটি আবার যোগ্য হয়ে ওঠে। সফলভাবে ব্যবহৃত হলে `clearAccountError()` সব ত্রুটি-সংক্রান্ত ফিল্ড মুছে দেয়।

### সেশন অ্যাফিনিটি (#7274)

**পরিধি:** একটি ক্লায়েন্ট সেশন (`X-Session-Id` / `x-codex-session-id` / `x-omniroute-session` হেডার), যা **যেকোনো** প্রোভাইডারের একটি সংযোগে পিন করা থাকে।

**উদ্দেশ্য:** একাধিক অনুরোধজুড়ে একটি মাল্টি-টার্ন এজেন্টকে (Claude Code, aider, কাস্টম এজেন্ট) একই অ্যাকাউন্টে রাখা, যাতে ক্রস-অ্যাকাউন্ট কনটেক্সট হারানো এবং প্রতি-অ্যাকাউন্ট সেশন স্টেট রয়েছে এমন প্রোভাইডারে বারবার কোল্ড-স্টার্ট 429 হওয়া কমে।
...
- হেডার এক্সট্র্যাকশন (সাধারণ, যেকোনো প্রোভাইডার): `src/sse/services/auth.ts::extractSessionAffinityKey()`
- স্থায়ীভাবে সংরক্ষিত পিন টেবিল: `sessionAccountAffinity` (`src/lib/db/sessionAccountAffinity.ts`)
- সেটিং: `sessionAffinityTtlMs` (ms-এ গ্লোবাল TTL, `0` এটি নিষ্ক্রিয় করে) — `src/lib/db/settings.ts`। মাইগ্রেশন `124_generic_session_affinity_ttl.sql` দ্বারা কেবল Codex-এর জন্য থাকা `codexSessionAffinityTtlMs` থেকে এর নাম পরিবর্তন করা হয়েছে; এটি আগে কনফিগার করা যেকোনো Codex TTL-কে নতুন ডিফল্ট হিসেবে বহন করে।

#7274-এর আগে, `resolveSessionAffinityTtlMs()` `codex` ছাড়া প্রতিটি প্রোভাইডারের জন্য অবিলম্বে `0` ফেরত দিত, ফলে পিনিং ব্যবস্থা এবং হেডার এক্সট্র্যাকশন আগে থেকেই প্রোভাইডার-স্বাধীন হওয়া সত্ত্বেও TTL সেটিংটি (এবং সেশন হেডারগুলো) অন্য কোথাও কোনো প্রভাব ফেলত না। সংশোধনটি সেই আগাম রিটার্ন সরিয়ে দিয়েছে; TTL এখন গ্লোবালি `0`-এর বেশি মানে সেট করা হলে প্রতিটি প্রোভাইডারের ক্ষেত্রে সমানভাবে প্রযোজ্য হয়।

তিনটি সেশন-অ্যাফিনিটি হেডার কখনোই আপস্ট্রিমে ফরওয়ার্ড করা হয় না — এক্সিকিউটরগুলো ক্লায়েন্ট হেডার সরাসরি পাঠানোর বদলে শুরু থেকে নিজেদের আপস্ট্রিম হেডার তৈরি করে, তাই এটি কেবল একটি অভ্যন্তরীণ কোরিলেশন আইডি হিসেবেই থাকে।
...
**উদ্দেশ্য:** যেসব ক্লায়েন্টের একাধিক অনুরোধজুড়ে কঠোর রাউটিং
সীমানা প্রয়োজন, তাদের জন্য টেকসই এক্সক্লুসিভ সংযোগ-মালিকানা প্রদান করা। এটি সেশন অ্যাফিনিটি থেকে আলাদা, যা ধারাবাহিকতার একটি নমনীয় অগ্রাধিকার:
একটি এক্সক্লুসিভ লিজ SQLite-এ লাইফসাইকেল স্টেট স্থায়ীভাবে সংরক্ষণ করে, গ্লোবাল সক্রিয়-মালিক ও
সক্রিয়-সংযোগের অনন্যতা বলবৎ করে এবং প্রোভাইডার ডিসপ্যাচের আগে একটি অচল জেনারেশন প্রত্যাখ্যান করে।

ফিচারটি প্রতি API key-এর জন্য ঐচ্ছিক। একটি ম্যানেজড কী-এর অবশ্যই `lease:exclusive` স্কোপ এবং একটি
স্পষ্ট, ফাঁকা নয় এমন `allowedConnections` তালিকা থাকতে হবে। যেকোনো HTTP ক্লায়েন্ট লাইফসাইকেল এন্ডপয়েন্ট ব্যবহার করতে পারে; কোনো
ক্লায়েন্টের নাম, user-agent, প্রোভাইডার, OAuth পদ্ধতি বা মডেলের প্রয়োজন নেই। লিজটি একটি সংযোগের মালিক হয়,
কোনো মডেলের নয়; তাই সংযোগটি সাধারণভাবে যোগ্য থাকা পর্যন্ত মডেল পরিবর্তন হলেও বাইন্ডিং বজায় থাকে।
স্বাভাবিক মডেল, কোটা, স্বাস্থ্য, কুলডাউন ও অ্যালাউলিস্টের নিয়মগুলো কর্তৃত্বপূর্ণ থাকে এবং
একই জেনারেশনকে অন্য কোনো মুক্ত ও যোগ্য সংযোগে স্থানান্তর করতে পারে।

লাইফসাইকেলটি হলো `POST /api/v1/session-leases`, যেখানে JSON অ্যাকশন হিসেবে `acquire`, `renew` এবং `release` ব্যবহৃত হয়।
ম্যানেজড ইনফারেন্স অনুরোধে অস্বচ্ছ `X-OmniRoute-Lease-Owner` মান এবং হুবহু
`X-OmniRoute-Lease-Generation` উপস্থাপন করা হয়। ওনারে `vlo_`-এর পর 43টি base64url অক্ষর থাকে; শুধু
এর SHA-256 হ্যাশ সংরক্ষণ করা হয়। প্রতিটি চূড়ান্ত ডিসপ্যাচ ফেন্স প্রমাণীকৃত API key ID এবং
সক্রিয় connection ID-কেও আবদ্ধ করে। লিজ কন্ট্রোল হেডারগুলো লগ, সংরক্ষিত অনুরোধ স্ন্যাপশট এবং
আপস্ট্রিম এক্সিকিউটর হেডার থেকে সরিয়ে দেওয়া হয়।

সাধারণ রাউটিংয়ে যোগ্য ম্যানেজড ক্যান্ডিডেট থাকা সত্ত্বেও যদি প্রতিটি মুক্ত ক্যান্ডিডেট কোনো
বহিরাগত সক্রিয় লিজের দখলে থাকে, তবে OmniRoute HTTP `429`, lease-capacity-unavailable কোড, একটি
ক্যাপাসিটির-জন্য-অপেক্ষমাণ অবস্থা এবং প্রাসঙ্গিক সবচেয়ে নিকটবর্তী মেয়াদোত্তীর্ণ সময় থেকে নির্ধারিত সীমাবদ্ধ `Retry-After` ফেরত দেয়।
সাধারণভাবে কোনো যোগ্য সংযোগ না থাকা লিজ কনটেনশন নয় এবং সে ক্ষেত্রে বিদ্যমান রাউটিং ত্রুটির আচরণ বজায় থাকে।
...
- এক্সক্লুসিভ ম্যানেজড সেশন লিজ হলো জেনারেশন ফেন্সসহ টেকসই লাইফসাইকেল মালিকানা।
...
**উদ্দেশ্য:** কেবল একটি মডেল অনুপলভ্য বা কোটা-সীমাবদ্ধ হলে সম্পূর্ণ connection নিষ্ক্রিয় করা এড়ানো।

**উদাহরণ:**

- প্রতি-মডেল কোটা provider-এর 429 ফেরত দেওয়া
- একটি অনুপস্থিত মডেলের জন্য local provider-এর 404 ফেরত দেওয়া
- Provider-নির্দিষ্ট mode/model অনুমতি-সংক্রান্ত ব্যর্থতা (যেমন, Grok mode)
...
সক্রিয় লকআউটগুলো এই তথ্যসহ তালিকাভুক্ত করে: provider, connection, model, reason, expiresAt। অপারেটররা কার্ড থেকে কোনো মডেলকে ম্যানুয়ালি পুনরায় সক্রিয় করতে পারেন।

**REST API:**

- `GET /api/resilience/model-cooldowns` — সক্রিয় লকআউটের তালিকা
- `DELETE /api/resilience/model-cooldowns` — ম্যানুয়াল পুনঃসক্রিয়করণ। বডি: `{provider, connection, model}`। প্রমাণীকরণ: management।

### লকআউট সেটিংস UI + সফলতা-ক্ষয় পুনরুদ্ধার (v3.8.23)

মডেল লকআউট সর্বদা-সক্রিয় হার্ডকোডেড আচরণ থেকে নিজস্ব সেটিংস কার্ড ও স্বয়ংক্রিয়ভাবে পুনরুদ্ধারক্ষম একটি পথসহ সম্পূর্ণ কনফিগারযোগ্য,
অপ্ট-ইন ফিচারে রূপান্তরিত হয়েছে।

**সেটিংস কার্ড:** Settings → Model Lockout
(`src/app/(dashboard)/dashboard/settings/components/ModelLockoutCard.tsx`)।
এটি উপরের শুধু-পঠনযোগ্য `ModelCooldownsCard` থেকে **স্বতন্ত্র** (যা শুধু
সক্রিয় লকআউটগুলো _তালিকাভুক্ত_ করে) — নতুন কার্ডটি _প্যারামিটারগুলো কনফিগার করে_। ডিফল্ট মানগুলো
`DEFAULT_MODEL_LOCKOUT_SETTINGS`-এ রয়েছে
(`src/lib/resilience/modelLockoutSettings.ts`):

| সেটিং                   | ডিফল্ট                           | অর্থ                                                              |
| ----------------------- | -------------------------------- | ----------------------------------------------------------------- |
| `enabled`               | `false`                          | মাস্টার টগল — মডেল লকআউট **ডিফল্টভাবে বন্ধ**।                     |
| `errorCodes`            | `[403, 404, 429, 502, 503, 504]` | যেসব upstream status মডেল-স্কোপড ব্যর্থতা হিসেবে গণ্য হয়।        |
| `baseCooldownMs`        | `120_000` (120 সেকেন্ড)          | প্রথম ব্যর্থতার জন্য প্রাথমিক লকআউটের সময়কাল।                    |
| `maxCooldownMs`         | `1_800_000` (30 মিনিট)           | ক্রমবর্ধমান কুলডাউনের সর্বোচ্চ সীমা।                              |
| `maxBackoffSteps`       | `10`                             | এক্সপোনেনশিয়াল-ব্যাকঅফ বৃদ্ধির সর্বোচ্চ ধাপ।                     |
| `useExponentialBackoff` | `true`                           | পুনরাবৃত্ত ব্যর্থতায় কুলডাউন এক্সপোনেনশিয়ালি বৃদ্ধি পাবে কি না। |

সেটিংসগুলো স্বাভাবিক settings store-এর মাধ্যমে সংরক্ষিত থাকে এবং
resilience settings schema-এর মাধ্যমে যাচাই করা হয়; কার্ডটি `baseCooldownMs`/`maxCooldownMs`
(`maxCooldownMs ≥ baseCooldownMs` সহ) এবং `maxBackoffSteps`-এর সীমা নির্ধারণ করে।

**সফলতা-ক্ষয় পুনরুদ্ধার:** পুনরুদ্ধার **শুধু** টাইমারের মেয়াদ শেষ হওয়ার ওপর নির্ভরশীল নয়। একটি সুস্থ
response মডেলটির ব্যর্থতার সংখ্যা ধাপে ধাপে কমায়, যাতে সময়সীমার মাঝপথে পুনরুদ্ধার হওয়া কোনো মডেলের
টাইমার শেষ হওয়ার আগেই বৃদ্ধি থেমে যায় (এবং লকআউট মুছে যায়)। কোনো combo target সফল হলে, `open-sse/services/combo.ts` থেকে `decayModelFailureCount()`
(`open-sse/services/accountFallback.ts`) কল করা হয়, যা সংরক্ষিত
`failureCount`-কে **অর্ধেক** করে (`Math.floor(failureCount / 2)`); এটি `0`-এ পৌঁছালে লকআউট
এন্ট্রিটি সম্পূর্ণভাবে মুছে ফেলা হয়। এর বিপরীতে `recordModelLockoutFailure()`
escalation window-এর মধ্যে ব্যর্থতা ঘটলে গণনা বাড়ায় (এবং কুলডাউন বৃদ্ধি করে)।
এই সফলতা-ক্ষয় সাধারণ টাইমার মেয়াদোত্তীর্ণতার অতিরিক্ত — যেকোনো পথেই একটি মডেল পুনরায় সক্রিয় হতে পারে।

**স্টেট:** লকআউটগুলো DB-তে সংরক্ষিত নয়, বরং **in-memory**-তে রাখা হয় (প্রতি-process `Map`-এ
`provider:connectionId:model` দ্বারা কীকৃত `ModelLockoutEntry`) —
restart হলে এগুলো হারিয়ে যায়। _সেটিংস_ সংরক্ষিত থাকে; সক্রিয়
লকআউটের _স্টেট_ সাময়িক।
...
- কোনো প্রোভাইডারের সব কী বাদ পড়লে → সার্কিট ব্রেকারের অবস্থা এবং প্রতিটি সংযোগের `rateLimitedUntil`/`testStatus`—উভয়ই পরীক্ষা করুন।
- রিসেট উইন্ডোর পরও প্রোভাইডার স্থায়ীভাবে বাদ থাকলে → কোডটি `getStatus()`/`canExecute()`-এর পরিবর্তে সরাসরি `state` পড়ছে।
- একটি কী ব্যর্থ হলেও অন্যগুলো কাজ করার কথা → সার্কিট ব্রেকারের পরিবর্তে সংযোগ কুলডাউনকে অগ্রাধিকার দিন।
- শুধু একটি মডেল ব্যর্থ হলে → সংযোগ কুলডাউনের পরিবর্তে মডেল লকআউটকে অগ্রাধিকার দিন।
- স্টেটের নিজে থেকে পুনরুদ্ধার হওয়ার কথা, কিন্তু তা না হলে → ভবিষ্যতের টাইমস্ট্যাম্প এবং মেয়াদোত্তীর্ণ স্টেট রিফ্রেশ করে এমন রিড পাথ পরীক্ষা করুন। স্থায়ী স্ট্যাটাসের জন্য ম্যানুয়াল পরিবর্তন প্রয়োজন।
