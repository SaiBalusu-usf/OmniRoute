...
## Inventori Gerbang (~90 skrip)

Skrip terletak di bawah `scripts/check/` (gerbang dasar) dan `scripts/quality/` (enjin ratchet).
...
penyumbang terus bergerak dengan gerbang pantas yang ditapis mengikut laluan, serta satu isyarat binaan pengeluaran
bersifat nasihat untuk perubahan kod:

| Tugas                                            | Skop                                                                                                                                                                                                                             | Menyekat                                                                                                     |
| ------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ |
| `Build (advisory)`                               | PR kod bukan draf dan cawangan baris gilir Mergify; Node 24, `npm-ci-retry`, `check:node-runtime`, `npm run build` dengan `OMNIROUTE_USE_TURBOPACK=1`; tiada muat naik artifak kerana tiada tugas kualiti hiliran menggunakannya | **Nasihat** (`continue-on-error: true`; alih keluar selepas satu minggu pelaksanaan PR keluaran yang stabil) |
...
| `Forgotten sibling tests`                        | PR kod; modul yang diubah dijejak kepada pengguna statik dan ujian saudara calon; laluan barrel dan import dinamik dilaporkan sebagai diagnostik bersifat nasihat, dengan pengecualian senarai dibenarkan yang dirujuk           | **Nasihat**                                                                                                  |
| `Vitest (fast-path)`                             | PR kod; suit vitest pantas                                                                                                                                                                                                       | Ya                                                                                                           |
| `Unit Tests fast-path`                           | PR kod; suit unit 4 serpihan                                                                                                                                                                                                     | Ya                                                                                                           |
| `No new ESLint warnings`                         | PR kod; pengawal lint yang mengambil kira penindasan                                                                                                                                                                             | Ya untuk asal sendiri, bersifat nasihat untuk fork                                                           |
| `Merge integrity (changelog + generated skills)` | PR bukan draf; penyegerakan log perubahan dan kemahiran yang dijana                                                                                                                                                              | Ya untuk asal sendiri, bersifat nasihat untuk fork                                                           |
...
`modul/simbol diubah -> pengguna statik -> ujian saudara calon` yang deterministik apabila ujian calon
tiada dalam perbezaan permintaan tarik. Ringkasan Markdown dan hasil JSON disimpan sebagai
artifak aliran kerja `forgotten-sibling-tests` untuk penentukuran sebelum sebarang pelaksanaan menyekat.

Eksport semula barrel dan import dinamik hanyalah diagnostik penyelesaian; ia tidak sekali-kali menghasilkan
dapatan menyekat. Pengecualian yang telah disemak terletak dalam
`config/quality/forgotten-sibling-allowlist.json`. Setiap entri mesti menamakan pengguna dan ujian
calon, memberikan rasional khusus dan memautkan isu atau permintaan tarik GitHub. Entri yang rosak akan gagal
secara tertutup. Pengecualian tidak boleh menindas ujian calon yang dipadam atau perbezaan yang menambahkan `.skip`/`.todo`;
pelemahan penegasan dan penyamaran lain kekal di bawah tanggungjawab gerbang
...
| `check:model-lifecycle`           | Tiga jadual penghalaan yang diselenggara secara manual kekal konsisten dengan petikan kitar hayat yang disimpan dalam repositori (#11503): `FITNESS_TABLE` (`taskFitness.ts`) tidak memberikan skor kepada mana-mana id yang telah ditamatkan tetapi masih boleh dihalakan oleh `REGISTRY`; setiap sasaran `BUILT_IN_ALIASES` terdapat dalam `REGISTRY` dan tiada dalam petikan id yang telah ditamatkan; setiap id yang telah ditamatkan tetapi masih berada dalam `REGISTRY` dimajukan atau disenaraikan dalam `allowedRetiredInCatalog`; dan tiada sumber atau sasaran `DEFAULT_DEGRADATION_MAP` ditandai sebagai telah ditamatkan dalam petikan tersebut. Ini tidak membuktikan bahawa sesuatu model sedang disediakan oleh huluan aktif. Luar talian — dibandingkan dengan `config/quality/model-lifecycle.json`, yang dimuat semula secara manual menggunakan `npm run quality:refresh-model-lifecycle` (rangkaian; tidak disepadukan ke dalam CI). `allowedRetiredInCatalog` ialah mekanisme pengetatan beransur-ansur: tambah entri hanya berserta isu penjejakan. | Ya                                      |
| `check:fetch-targets`             | Setiap `fetch("/api/...")` dalam `src/` pada bahagian klien dipadankan kepada `route.ts` yang sebenar                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | Ya                                      |
| `check:deps`                      | Semua kebergantungan yang boleh dipasang dengan `npm install` merentas setiap `package.json` dalam repositori terdapat dalam `dependency-allowlist.json`; pakej baharu yang tidak ditetapkan versinya atau merupakan hasil slopsquatting akan ditandai                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | Ya                                      |
| `audit:deps`                      | `npm audit` (akar + electron) — tiada nasihat keselamatan tahap tinggi/kritikal (bertindih dengan osv `check:vuln-ratchet`; lihat Senarai Tertunggak Rasionalisasi)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | Ya                                      |
| `check:lockfile`                  | Integriti `package-lock.json` — pendaftar https, cincangan integriti, tiada penggantian hos                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | Ya                                      |
...
| `check:public-creds`              | Tiada `client_id`/`client_secret` OAuth literal atau kekunci Web Firebase di luar `publicCreds.ts` (Peraturan Tegas #11)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | Ya                                      |
...
| `check:route-guard-membership`    | Setiap laluan yang mencetuskan proses anak diklasifikasikan oleh `isLocalOnlyPath()` (Peraturan Tegas #15/#17)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | Ya                                      |
...
| `lint:json`                       | Fail konfigurasi JSON dihuraikan dan mematuhi peraturan lint repositori                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| `check:dashboard-typecheck`       | `tsc` yang dikhususkan kepada `src/app/(dashboard)/**` (#7033) — senarai izin 27 fail terpilih bagi `typecheck:core` tidak merangkumi mana-mana TSX papan pemuka, dan `next build` juga tidak pernah menyemak jenisnya (`next.config.mjs` menetapkan `ignoreBuildErrors: true`), maka regresi pengecam yatim di situ (#6625/#6909) tidak dapat dikesan oleh CI. Perbezaan dibandingkan dengan garis dasar kiraan beku bagi setiap fail/setiap kod TS (`config/quality/dashboard-typecheck-baseline.json`, corak penguatkuasaan lapuk yang sama seperti `check:known-symbols`) — hanya ralat BAHARU yang melebihi kiraan garis dasar akan menyebabkan pagar gagal; turunkan ambang secara berperingkat dengan `--update` apabila ralat sedia ada dibetulkan.                                                                                                                                                                                                                                                                                                                | Ya                                      |
...
| ---------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------- |
| `quality:collect`            | Menghasilkan `quality-metrics.json` (kiraan amaran ESLint, liputan daripada laporan serpihan yang digabungkan)                                                                                                       | Ya (huluan bagi mekanisme ambang berperingkat) |
...
| `check:complexity`           | Kerumitan siklomatik pada peringkat fail tidak melebihi had (peraturan teras ESLint `complexity` + `max-lines-per-function`)                                                                                         | Ya                                             |
| `check:cognitive-complexity` | Mekanisme ambang berperingkat bagi kerumitan kognitif (`eslint-plugin-sonarjs`) — laluan ESLint yang berasingan; CI menjalankan kedua-duanya secara gabungan sebagai satu langkah `check:complexity-ratchets`        | Ya                                             |
| `check:dead-code`            | Mekanisme ambang berperingkat bagi eksport / fail yang tidak digunakan (knip) tidak merosot berbanding garis dasar                                                                                                   | Ya                                             |
...
| `check:type-coverage`        | Mekanisme ambang berperingkat bagi peratusan yang mempunyai jenis (`type-coverage`) tidak merosot; sebahagian besarnya merangkumi `typecheck:noimplicit:core`                                                        | Ya                                             |
| `check:codeql-ratchet`       | Kiraan isyarat CodeQL terbuka tidak merosot (dibaca melalui `gh api`; dilangkau dengan selamat tanpa token) — untuk kekerapan muat semula dan pencetus manual: lihat "Mekanisme ambang berperingkat CodeQL" di bawah | Ya                                             |
...
| `check:bundle-size`      | Saiz bundle tidak melebihi had                                                                                                                                                                              | **Nasihat** |
...
| `check:openapi-breaking` | Perubahan pemecah kepada kontrak API awam (`openapi.yaml`) berbanding cabang asas (oasdiff) — menghasilkan `openapiBreaking=N`; dilangkau jika oasdiff tiada atau spesifikasi asas tidak dapat diselesaikan | **Nasihat** |
...
| ↳ `check:docs-counts`          | Kiraan dalam prosa (bilangan penyedia, bilangan migrasi, dll.) berada dalam tetingkap ratchet bagi kiraan sebenar                                                                                          | Ya                               |
...
| ↳ `check:doc-links`            | Pautan markdown dalaman dalam dokumentasi menghala kepada fail sebenar (format `[text]`/`(path)`)                                                                                                          | Ya                               |
| ↳ `check:fabricated-docs`      | Laluan, pemboleh ubah persekitaran, perintah CLI, nama hook dan laluan fail yang disebut dalam dokumentasi wujud dalam pangkalan kod. Gerbang keras melalui `--strict`; gagal secara lembut tanpa bendera. | Ya (melalui `--strict` dalam CI) |
| `check:cli-i18n`               | Rentetan perintah CLI terdapat dalam semua fail tempatan i18n                                                                                                                                              | Ya                               |
| `check:openapi-coverage`       | Spesifikasi OpenAPI merangkumi sekurang-kurangnya aras minimum laluan sebenar yang diratchet                                                                                                               | Ya                               |
...
| `i18n translation drift`       | Kekunci yang belum diterjemahkan dalam fail tempatan i18n — amaran sahaja                                                                                                                                  | **Nasihat**                      |
...
| `check-new-key-coverage` (sebaris) | Kunci bahasa Inggeris **baharu** tersedia dalam setiap lokal                                                                                                                                                            | Ya          |
| `check-translation-ratio`          | Nisbah terjemahan sebenar bagi setiap lokal (sama dengan bahasa Inggeris / ruang letak / daun yang tiada di luar senarai dibenarkan) tidak boleh melebihi `config/quality/i18n-translation-baseline.json` + kelonggaran | **Nasihat** |

Memerlukan `fetch-depth: 0` — gerbang hanyutan nilai membandingkan perbezaan `en.json` dengan pangkalan gabungan.
...
ditulis semula tetapi terjemahan yang berasal daripada bahasa Inggeris _sebelumnya_ masih kekal, menyebabkan
pengguna bukan berbahasa Inggeris terus membaca teks yang kedengaran meyakinkan tetapi kini salah.

Perkara ini benar-benar telah dihantar. `oauthModal.googleOAuthWarning` ditulis semula apabila pembantu log masuk
Antigravity diperkenalkan (#5203); **39 daripada 43 lokal** mengekalkan teks yang mengarahkan pengendali supaya
"salin URL penuh dan tampalkannya di bawah" — aliran yang tidak dapat diselesaikan untuk penyedia tersebut. Perkara
ini tidak disedari sehingga #8463 kerana:

- `sync-ui-keys` hanya mengisi semula kunci yang **tiada**, bukan kunci yang **lapuk**;
- `check-ui-keys-coverage` mengira _kehadiran_ kunci, jadi terjemahan lapuk dikira sebagai diliputi;
- `check-translation-drift` menjejaki cerminan dokumentasi `docs/i18n/<locale>/**.md` —
  ia langsung tidak membaca `src/i18n/messages/*.json`. Menyekat dalam tugas `docs-sync-strict` sejak
  penyegerakan semula 2026-09: sunting dokumen teras → `npm run i18n:run -- --files=<doc>` (peringkat bahagian, murah).

**Peka perbezaan, bukan disokong garis dasar.** Ia membandingkan `en.json` pada pangkalan gabungan dengan
pepohon kerja; bagi setiap kunci yang nilai bahasa Inggerisnya berubah, mana-mana lokal yang masih mempunyai
terjemahan yang tidak disentuh dianggap lapuk. Ini sengaja **membekukan hutang sedia ada** — perbezaan
tidak dapat menunjukkan versi bahasa Inggeris lama yang menjadi sumber sesuatu terjemahan lama, jadi gerbang
hanya menilai perkara yang disentuh oleh perubahan semasa. Pilihan lain (garis dasar cincangan setiap kunci) akan
menghasilkan fail terjana kira-kira 600 KB, 3× lebih besar daripada garis dasar sedia ada yang terbesar, serta
berubah-ubah pada setiap PR i18n.

Dua cara untuk mematuhinya:

1. kemas kini terjemahan yang terjejas, atau
2. tetapkannya kepada `__MISSING__:<new english>` — masa jalan kemudiannya menyediakan bahasa Inggeris yang dibetulkan
   (`src/i18n/request.ts::deepMergeFallback`, #7258) dan kunci tersebut dimasukkan dalam baris gilir untuk diterjemahkan.

Jika **makna** rentetan berubah, sebaik-baiknya **namakan semula kunci**: kunci baharu tidak boleh mewarisi
...
Keluar dengan kod 0 serta `SKIP reason=base-unresolved` apabila katalog asas tidak dapat dibaca (klon cetek
tanpa rujukan asas), selaras dengan `check-openapi-breaking`.

### Tugas: `i18n`

Matriks pengesahan i18n penuh (satu tugas bagi setiap lokal). Keseluruhan tugas bersifat nasihat.

| Skrip                           | Mengesahkan                              | Menyekat                                                       |
| ------------------------------- | ---------------------------------------- | -------------------------------------------------------------- |
| `validate_translation.py quick` | Kelengkapan terjemahan bagi setiap lokal | **Nasihat** (`continue-on-error: true` pada keseluruhan tugas) |
...
| `check:test-masking`   | Fail ujian yang diubah tidak mengurangkan jumlah bersih penegasan atau menambah tautologi `assert.ok(true)`                                          | Ya       |
| `check:pr-evidence`    | Kandungan PR menyebut bukti ujian/VPS bagi perubahan tersebut (mengautomasikan Peraturan Tegas #18 dengan grep pada prosa PR — rapuh, lihat Backlog) | Ya       |

### Tugas: `test-vitest`

Dijalankan selepas `build`. Menyekat penggabungan jika gagal.

| Suite            | Mengesahkan                                                 | Menyekat                                                                                                                          |
| ---------------- | ----------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------- |
| `test:vitest`    | Pelayan MCP (110 alat), autoCombo, cache — pelaksana vitest | Ya                                                                                                                                |
| `test:vitest:ui` | Ujian komponen UI — pelaksana vitest                        | **Menyekat** — kegagalan sedia ada dikecualikan secara jelas dalam `vitest.config.ts`; kegagalan baharu menyebabkan tugasan gagal |

### Aliran kerja setiap malam (berjadual, berbentuk nasihat)

Aliran kerja ini dijalankan mengikut jadual cron (dan `workflow_dispatch`), tidak pernah pada PR. Semuanya berbentuk nasihat.

| Aliran kerja           | Mengesahkan                                                                                                                                                                                   | Menyekat              |
| ---------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------- |
| `nightly-property`     | Ujian sifat fast-check dengan benih rawak + bilangan pelaksanaan yang tinggi                                                                                                                  | **Berbentuk nasihat** |
| `nightly-resilience`   | Gerbang pertumbuhan heap, suntikan kerosakan chaos, beban/rendaman k6                                                                                                                         | **Berbentuk nasihat** |
| `nightly-llm-security` | Pengadang suntikan promptfoo (mod sekat) + prob garak (dilangkau tanpa rahsia penyedia)                                                                                                       | **Berbentuk nasihat** |
| `nightly-schemathesis` | Pengujian fuzz kontrak OpenAPI (schemathesis) terhadap OmniRoute aktif menggunakan `docs/openapi.yaml` — mendedahkan pelanggaran spesifikasi / ralat 500 yang tidak dikendalikan (Fasa 8 B.4) | **Berbentuk nasihat** |
| `nightly-mutation`     | Skor ujian mutasi Stryker pada laluan unit pantas — mutan yang terselamat mendedahkan penegasan yang lemah                                                                                    | **Berbentuk nasihat** |
| `nightly-compat`       | Matriks keserasian enjin Node merentas julat `engines.node` yang disokong                                                                                                                     | **Berbentuk nasihat** |
...
#### `check-key-completeness` — get pariti set kunci

`scripts/i18n/check-key-completeness.mjs` (`npm run i18n:check-keys`, tugas `i18n-ui-coverage`).
Membandingkan set kunci terminal bagi setiap `src/i18n/messages/<locale>.json` dengan `en.json` dan gagal
jika terdapat sebarang kunci terminal yang tiada atau berlebihan, tanpa mengira bila kunci itu ditambahkan. Ruang letak `__MISSING__:`
dikira sebagai wujud (kandungannya ialah urusan get nisbah). Ia merupakan pelengkap mutlak
kepada dua get berasaskan perbezaan/peratusan: `check-ui-keys-coverage` menguatkuasakan paras minimum 80 % bagi setiap
tempat peristiwa (43 kunci yang tiada daripada ~13,000 masih dibaca sebagai 99.7 %) dan `check-new-key-coverage` menilai
hanya kunci yang ditambahkan oleh PR kepada `en.json`. Kelompok tempat peristiwa dijana daripada `en.json` pada hari
cabangnya dicipta dan diterjemahkan selama beberapa hari sementara asas terus menambahkan kunci; PR kelompok itu sendiri tidak
menambahkan sebarang kunci, maka kedua-dua get berkaitan kekal senyap apabila kelompok 1 (#13044) digabungkan dengan kekurangan 43 kunci dalam sembilan
tempat peristiwa dan kelompok 2 (#13660) dengan kekurangan 10 kunci dalam lapan tempat peristiwa (2026-09-15). Baiki kegagalan dengan
`node scripts/i18n/sync-ui-keys.mjs --locale=<codes> --translate-markers`; kunci terminal `extra`
bermaksud sumber telah menggugurkannya — padamkannya daripada tempat peristiwa tersebut. `--warn` melaporkan tanpa menggagalkan.
...
#### `check-new-key-coverage` — get i18n kunci baharu

Get berkaitan bagi `check-ui-value-drift`. Get tersebut mengesan nilai bahasa Inggeris yang telah **ditulis semula**
sedangkan terjemahannya tidak dikemas kini; get ini mengesan kunci bahasa Inggeris yang telah **ditambahkan**
sedangkan sesetengah tempat peristiwa tidak pernah menerimanya.

`check-ui-keys-coverage` tidak dapat mengesan kelas ini: ia menguatkuasakan paras minimum peratusan bagi setiap tempat peristiwa, dan
sebelas kunci yang tiada daripada ~13,000 menghasilkan liputan 99.9%. Peratusan bagi setiap bahasa tidak dapat
menyatakan "ciri ini diterbitkan tanpa terjemahan" — keseluruhan ciri boleh dimasukkan ke dalam tempat peristiwa baharu tanpa sebarang
teks dan langsung tidak mengubah angka tersebut.

Insiden yang dirakamkannya: Fasa 3 Orchestration Canvas menterjemahkan sebelas kuncinya merentas
42 tempat peristiwa yang wujud pada masa itu. Beberapa jam kemudian, kelompok bahasa EU (#13044) meningkatkan bilangan
tempat peristiwa repositori kepada 51, dan sembilan tempat peristiwa baharu (`el`, `et`, `ga`, `hr`, `lt`, `lv`, `mt`, `sl`, `sr`) tidak pernah
menerimanya. `deepMergeFallback` menggantikan kunci yang tiada dengan bahasa Inggeris, maka mod kegagalannya ialah
UI yang tidak diterjemahkan dan bukannya UI kosong — masalah sebenar yang sememangnya berlaku secara senyap.

Seperti get berkaitannya, ia **peka terhadap perbezaan**, dengan membandingkan bahasa Inggeris pada asas gabungan dengan pepohon
kerja, maka jurang sedia ada kekal dibekukan dan get ini tidak memerlukan migrasi untuk diaktifkan. Jalan keluar:
`__MISSING__:<english>` menangguhkan terjemahan sambil memastikan masa jalan kekal betul. `vi` melarang
ruang letak (`tests/unit/i18n-vi-completeness.test.ts`) dan memerlukan terjemahan sebenar.

#### `check-vitest-exclusions` — get ujian yang diketepikan

Fail dalam senarai `exclude` bagi `vitest.config.ts` ialah ujian yang tidak dijalankan, namun kelihatan seperti
liputan kepada sesiapa yang membaca pepohon tersebut. Enam puluh dua fail terkumpul di sebalik komen
`// #8618 — pre-existing failure; remove this exclusion when fixed`. Isu #8618 telah ditutup pada
2026-08-11 sementara senarai yang dijejakinya berkembang daripada 45 entri kepada 62, dengan setiap entri baharu mewarisi komen
yang merujuk kepada isu yang telah ditutup. Apabila senarai tersebut akhirnya diukur fail demi fail (#13204), **51 daripada 62 fail
lulus terhadap pepohon semasa tanpa sebarang perubahan sumber**.

Get ini mewajibkan setiap pengecualian yang merujuk kepada fail sebenar untuk (a) menyatakan isu penjejakan dan
(b) muncul dalam `config/quality/vitest-exclusions.json` bersama status yang telah diukur, supaya penambahan pengecualian menjadi
perbezaan yang boleh disemak dalam fail khusus dan bukannya satu lagi baris dalam tatasusunan 60 entri. Ia sengaja
tidak menjalankan semula ujian yang dikecualikan — tindakan itu mengambil masa ~10 minit dan sepatutnya dilakukan dalam tugas berkala; inventori
merekodkan bila setiap ujian kali terakhir diukur.
