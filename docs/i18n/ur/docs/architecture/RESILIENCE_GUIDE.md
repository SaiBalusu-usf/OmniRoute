...
**مقصد:** ایک خراب کلید کو نظر انداز کرنا، جبکہ اسی فراہم کنندہ کے دیگر کنکشن سروس فراہم کرتے رہیں۔
...
- `rateLimitedUntil` — وہ ٹائم اسٹیمپ جب تک کول ڈاؤن ختم نہیں ہوتا
...
- API کلید کی بنیادی مدت: 3s
- API کلید کے لیے 429: اپ اسٹریم `Retry-After`/ری سیٹ ہیڈرز/قابلِ تجزیہ ری سیٹ متن کو ترجیح دیتا ہے
- بیک آف: `baseCooldownMs * 2 ** failureIndex`

**اینٹی تھنڈرنگ ہرڈ حفاظتی بندوبست:** بیک وقت ہونے والی ناکامیوں کو کول ڈاؤن ضرورت سے زیادہ بڑھانے یا `backoffLevel` کو دو بار بڑھانے سے روکتا ہے۔

**اختتامی حالتیں (کول ڈاؤن نہیں):**

- `banned` — ممنوعہ کلیدی لفظ / اکاؤنٹ پابندی کی شناخت سے مقرر ہوتی ہے (دیکھیے [BAN_DETECTION](../security/BAN_DETECTION.md))
...
یہ حالتیں اس وقت تک برقرار رہتی ہیں جب تک اسناد تبدیل نہ ہوں یا کوئی آپریٹر انہیں ری سیٹ نہ کرے۔ عارضی کول ڈاؤن حالت سے اختتامی حالتوں کو اوور رائٹ نہ کریں۔

**لیزی ریکوری:** جب `rateLimitedUntil` گزر چکا ہو تو کنکشن دوبارہ اہل ہو جاتا ہے۔ کامیاب استعمال پر `clearAccountError()` خرابی کے تمام فیلڈز صاف کر دیتا ہے۔

### سیشن وابستگی (#7274)

**دائرۂ کار:** ایک کلائنٹ سیشن (`X-Session-Id` / `x-codex-session-id` / `x-omniroute-session` ہیڈر) جو **کسی بھی** فراہم کنندہ کے لیے ایک کنکشن سے منسلک ہو۔

**مقصد:** متعدد تبادلوں والے ایجنٹ (Claude Code، aider، حسبِ ضرورت ایجنٹس) کو درخواستوں کے دوران اسی اکاؤنٹ پر برقرار رکھنا، تاکہ مختلف اکاؤنٹس کے درمیان سیاق و سباق کا نقصان اور فی اکاؤنٹ سیشن حالت رکھنے والے فراہم کنندگان پر بار بار کولڈ اسٹارٹ 429s کم ہوں۔
...
- محفوظ شدہ پن ٹیبل: `sessionAccountAffinity` (`src/lib/db/sessionAccountAffinity.ts`)
- ترتیب: `sessionAffinityTtlMs` (ملی سیکنڈز میں عالمی TTL، `0` اسے غیر فعال کرتا ہے) — `src/lib/db/settings.ts`۔ اسے صرف Codex کے لیے مخصوص `codexSessionAffinityTtlMs` سے مائیگریشن `124_generic_session_affinity_ttl.sql` کے ذریعے نیا نام دیا گیا، جو پہلے سے ترتیب دی گئی کسی بھی Codex TTL کو نئے ڈیفالٹ کے طور پر منتقل کرتی ہے۔

#7274 سے پہلے، `resolveSessionAffinityTtlMs()` فراہم کنندہ `codex` کے علاوہ ہر فراہم کنندہ کے لیے فوراً `0` واپس کر دیتا تھا، اس لیے TTL ترتیب (اور سیشن ہیڈرز) کا کہیں اور کوئی اثر نہیں ہوتا تھا، حالاں کہ پننگ کا طریقۂ کار اور ہیڈر اخذ کرنے کا عمل پہلے ہی فراہم کنندہ سے غیر متعلق تھے۔ اس اصلاح نے وہ ابتدائی واپسی ہٹا دی؛ اب TTL عالمی طور پر `0` سے زیادہ مقرر ہونے کے بعد ہر فراہم کنندہ پر یکساں طور پر لاگو ہوتا ہے۔

سیشن وابستگی کے یہ تینوں ہیڈرز کبھی اپ اسٹریم فارورڈ نہیں کیے جاتے — ایگزیکیوٹرز کلائنٹ ہیڈرز کو آگے بھیجنے کے بجائے اپنے اپ اسٹریم ہیڈرز ابتدا سے بناتے ہیں، اس لیے یہ صرف ایک داخلی کوریلیشن آئی ڈی رہتا ہے۔

### خصوصی منظم سیشن کنکشن لیزز

**دائرۂ کار:** ایک فعال منظم HTTP کلائنٹ/سیشن ایک اہل OmniRoute کنکشن کی ملکیت رکھتا ہے۔

**مقصد:** ان کلائنٹس کے لیے کنکشن کی پائیدار خصوصی ملکیت فراہم کرنا جنہیں درخواستوں کے درمیان ایک سخت روٹنگ
حد درکار ہو۔ یہ سیشن وابستگی سے مختلف ہے، جو تسلسل کی ایک نرم ترجیح ہے:
ایک خصوصی لیز SQLite میں لائف سائیکل کی حالت برقرار رکھتی ہے، عالمی فعال مالک اور
فعال کنکشن کی یکتائی نافذ کرتی ہے، اور فراہم کنندہ کو بھیجنے سے پہلے پرانی جنریشن مسترد کر دیتی ہے۔

یہ خصوصیت ہر API کلید کے لیے اختیاری ہے۔ منظم کلید کے پاس لازماً `lease:exclusive` اسکوپ اور ایک
واضح، غیر خالی `allowedConnections` فہرست ہونی چاہیے۔ کوئی بھی HTTP کلائنٹ لائف سائیکل اینڈ پوائنٹ استعمال کر سکتا ہے؛ کسی
کلائنٹ نام، یوزر ایجنٹ، فراہم کنندہ، OAuth طریقے، یا ماڈل کی ضرورت نہیں۔ لیز کنکشن کی ملکیت رکھتی ہے،
ماڈل کی نہیں، اس لیے ماڈل کی تبدیلی سے بائنڈنگ برقرار رہتی ہے، بشرطیکہ کنکشن معمول کے مطابق
اہل رہے۔ ماڈل، کوٹہ، صحت، کول ڈاؤن، اور اجازت فہرست کے معمول کے قواعد بدستور حتمی اختیار رکھتے ہیں اور
اسی جنریشن کو کسی دوسرے آزاد اہل کنکشن پر منتقل کر سکتے ہیں۔

لائف سائیکل `POST /api/v1/session-leases` ہے، جس کے JSON ایکشنز `acquire`، `renew`، اور `release` ہیں۔
منظم انفرنس درخواستیں مبہم `X-OmniRoute-Lease-Owner` قدر اور عین
`X-OmniRoute-Lease-Generation` پیش کرتی ہیں۔ مالک `vlo_` کے بعد 43 base64url حروف استعمال کرتا ہے؛ صرف
اس کا SHA-256 ہیش محفوظ کیا جاتا ہے۔ ہر حتمی ڈسپیچ حد توثیق شدہ API کلید آئی ڈی اور
فعال کنکشن آئی ڈی کو بھی بائنڈ کرتی ہے۔ لیز کنٹرول ہیڈرز لاگز، محفوظ شدہ درخواست اسنیپ شاٹس، اور
اپ اسٹریم ایگزیکیوٹر ہیڈرز سے ہٹا دیے جاتے ہیں۔

اگر معمول کی روٹنگ کے پاس اہل منظم امیدوار موجود ہوں لیکن ہر آزاد امیدوار کسی
غیر متعلق فعال لیز کے زیرِ قبضہ ہو، تو OmniRoute HTTP `429`، lease-capacity-unavailable کوڈ، ایک
...
- OAuth سیشن آکوپینسی، OAuth اکاؤنٹس کے لیے پراسیس-لوکل نرم تقسیم ہے۔
- اکاؤنٹ سیمی فورز درخواست کی ہم وقتی کارروائی کے اجازت نامے دیتے ہیں اور درخواست مکمل ہونے پر ختم ہو جاتے ہیں۔
- خصوصی منظم سیشن لیزز جنریشن حد کے ساتھ پائیدار لائف سائیکل ملکیت ہیں۔

---

## 3. ماڈل لاک آؤٹ

**دائرۂ کار:** فراہم کنندہ + کنکشن + ماڈل کی تکڑی۔

**مقصد:** جب صرف ایک ماڈل دستیاب نہ ہو یا اس کا کوٹا محدود ہو تو پورے کنکشن کو غیر فعال کرنے سے بچنا۔

**مثالیں:**

- فی ماڈل کوٹا رکھنے والے فراہم کنندگان کی جانب سے 429 واپس آنا
- کسی ایک غیر موجود ماڈل کے لیے مقامی فراہم کنندگان کی جانب سے 404 واپس آنا
- فراہم کنندہ سے مخصوص موڈ/ماڈل کی اجازت کی ناکامیاں (مثلاً Grok موڈز)

**عمل درآمد:** `open-sse/services/accountFallback.ts` — `lockModel()`، `clearModelLock()`، `getAllModelLockouts()`۔

### ماڈل کول ڈاؤنز ڈیش بورڈ (v3.8.0)

UI: ترتیبات → ماڈل کول ڈاؤنز (`src/app/(dashboard)/dashboard/settings/components/ModelCooldownsCard.tsx`)

فعال لاک آؤٹس کو ان معلومات کے ساتھ فہرست کرتا ہے: فراہم کنندہ، کنکشن، ماڈل، وجہ، expiresAt۔ آپریٹرز کارڈ سے کسی ماڈل کو دستی طور پر دوبارہ فعال کر سکتے ہیں۔
...
### لاک آؤٹ ترتیبات کا UI + کامیابی پر مبنی تدریجی بحالی (v3.8.23)

ماڈل لاک آؤٹ ہمیشہ فعال رہنے والے ہارڈ کوڈڈ رویے سے تبدیل ہو کر ایک مکمل طور پر قابلِ تشکیل،
اختیاری خصوصیت بن گیا ہے، جس کا اپنا ترتیباتی کارڈ اور خودکار بحالی کا راستہ ہے۔

**ترتیباتی کارڈ:** ترتیبات → ماڈل لاک آؤٹ
(`src/app/(dashboard)/dashboard/settings/components/ModelLockoutCard.tsx`)۔
یہ اوپر موجود صرف پڑھنے کے قابل `ModelCooldownsCard` سے **الگ** ہے (جو صرف فعال
لاک آؤٹس کی _فہرست_ دکھاتا ہے) — نیا کارڈ _پیرامیٹرز کو تشکیل دیتا ہے_۔ طے شدہ اقدار
`DEFAULT_MODEL_LOCKOUT_SETTINGS`
(`src/lib/resilience/modelLockoutSettings.ts`) میں موجود ہیں:

| ترتیب                   | طے شدہ                           | مطلب                                                         |
| ----------------------- | -------------------------------- | ------------------------------------------------------------ |
| `enabled`               | `false`                          | بنیادی ٹوگل — ماڈل لاک آؤٹ **بطور طے شدہ بند** ہے۔           |
| `errorCodes`            | `[403, 404, 429, 502, 503, 504]` | اپ اسٹریم اسٹیٹسز جو ماڈل کے دائرے کی ناکامی شمار ہوتے ہیں۔  |
| `baseCooldownMs`        | `120_000` (120 سیکنڈ)            | پہلی ناکامی کے لیے لاک آؤٹ کا ابتدائی دورانیہ۔               |
| `maxCooldownMs`         | `1_800_000` (30 منٹ)             | بڑھائے گئے کول ڈاؤن کی بالائی حد۔                            |
| `maxBackoffSteps`       | `10`                             | ایکسپونینشل بیک آف میں اضافے کے زیادہ سے زیادہ مراحل۔        |
| `useExponentialBackoff` | `true`                           | آیا بار بار کی ناکامیاں کول ڈاؤن کو ایکسپونینشلی بڑھائیں گی۔ |

ترتیبات معمول کے ترتیباتی اسٹور کے ذریعے برقرار رہتی ہیں اور لچک پذیری کی
ترتیباتی اسکیما کے ذریعے توثیق ہوتی ہیں؛ کارڈ `baseCooldownMs`/`maxCooldownMs`
(`maxCooldownMs ≥ baseCooldownMs` کے ساتھ) اور `maxBackoffSteps` کو حدود میں رکھتا ہے۔

**کامیابی پر مبنی تدریجی بحالی:** بحالی **صرف** ٹائمر ختم ہونے پر منحصر نہیں ہے۔ ایک صحت مند
جواب ماڈل کی ناکامیوں کی تعداد کو بتدریج کم کرتا ہے، تاکہ دورانیے کے درمیان بحال ہونے والا ماڈل
اپنے ٹائمر کے ختم ہونے سے پہلے مزید اضافے کو روک دے (اور لاک آؤٹ صاف کر دے)۔ کسی کامیاب
کمبو ہدف پر، `open-sse/services/combo.ts`، `decayModelFailureCount()`
(`open-sse/services/accountFallback.ts`) کو کال کرتا ہے، جو محفوظ کردہ
`failureCount` کو **نصف** کر دیتا ہے (`Math.floor(failureCount / 2)`)؛ جب یہ `0` تک
پہنچتا ہے تو لاک آؤٹ اندراج مکمل طور پر حذف کر دیا جاتا ہے۔ اس کا ہم منصب `recordModelLockoutFailure()`
اضافے کی مدت کے اندر ہونے والی ناکامیوں پر تعداد بڑھاتا ہے (اور کول ڈاؤن میں اضافہ کرتا ہے)۔
کامیابی پر مبنی یہ تدریجی کمی، عام ٹائمر کے اختتام کے علاوہ ہے —
دونوں میں سے کوئی بھی راستہ ماڈل کو دوبارہ فعال کر سکتا ہے۔

**حالت:** لاک آؤٹس **ان میموری** رکھے جاتے ہیں (ہر پراسیس کے `Map`s میں
`ModelLockoutEntry`، جس کی کلید `provider:connectionId:model` ہے)، انہیں
DB میں برقرار نہیں رکھا جاتا — ری اسٹارٹ ہونے پر یہ ضائع ہو جاتے ہیں۔ _ترتیبات_ برقرار
رکھی جاتی ہیں؛ فعال لاک آؤٹ کی _حالت_ عارضی ہوتی ہے۔
...
- کسی فراہم کنندہ کی تمام کیز چھوڑ دی جائیں → سرکٹ بریکر کی حالت اور ہر کنکشن کا `rateLimitedUntil`/`testStatus`، دونوں چیک کریں۔
- ری سیٹ ونڈو کے بعد فراہم کنندہ مستقل طور پر خارج رہے → کوڈ `getStatus()`/`canExecute()` کے بجائے خام `state` پڑھ رہا ہے۔
- ایک کی ناکام ہو، مگر دیگر کو کام کرنا چاہیے → سرکٹ بریکر کے بجائے کنکشن کول ڈاؤن کو ترجیح دیں۔
- صرف ایک ماڈل ناکام ہو → کنکشن کول ڈاؤن کے بجائے ماڈل لاک آؤٹ کو ترجیح دیں۔
- حالت کو خود بخود بحال ہونا چاہیے مگر ایسا نہیں ہوتا → مستقبل کے ٹائم اسٹیمپ اور اس ریڈ پاتھ کو چیک کریں جو میعاد ختم شدہ حالت کو ریفریش کرتا ہے۔ مستقل اسٹیٹسز کے لیے دستی تبدیلیاں درکار ہوتی ہیں۔
