...
**ዓላማ:** ለተመሳሳይ አቅራቢ ያሉ ሌሎች ግንኙነቶች አገልግሎት መስጠታቸውን ሲቀጥሉ አንድ ችግር ያለበትን ቁልፍ መዝለል።

**አተገባበር:**

- የማይገኝ በማለት ምልክት ማድረግ፦ `src/sse/services/auth.ts::markAccountUnavailable()`
...
- `rateLimitedUntil` — የማቀዝቀዣ ጊዜው እስከሚያበቃበት ድረስ ያለው የጊዜ ማህተም
- `testStatus: "unavailable"`
- `lastError`, `lastErrorType`, `errorCode`
- `backoffLevel` — የርቢ የመጠበቂያ ጊዜ ቆጣሪ
...
- የመጠበቂያ ጊዜ፦ `baseCooldownMs * 2 ** failureIndex`

**በአንድ ጊዜ የሚፈጠሩ ብዙ ጥያቄዎችን መከላከያ:** በተመሳሳይ ጊዜ የሚከሰቱ ውድቀቶች የማቀዝቀዣ ጊዜውን ከልክ በላይ እንዳያራዝሙ ወይም `backoffLevel`ን ሁለት ጊዜ እንዳይጨምሩ ይከላከላል።

**የመጨረሻ ሁኔታዎች (የማቀዝቀዣ ጊዜዎች አይደሉም):**

- `banned` — በታገደ-ቁልፍ-ቃል / የመለያ-እገዳ ማወቂያ የሚዘጋጅ (ይመልከቱ [BAN_DETECTION](../security/BAN_DETECTION.md))
- `expired` (ከተወሰነ የድጋሚ ሙከራ ብዛት በኋላ ወደ መጨረሻ ሁኔታ ይሸጋገራል — `EXPIRED_RETRY_MAX = 3` ከርቢ የመጠበቂያ ጊዜ ጋር — ስለዚህ ጊዜያዊ የOAuth ስህተቶች መለያው በቋሚነት ከመቦዘኑ በፊት በራሳቸው ሊያገግሙ ይችላሉ)
- `credits_exhausted`

እነዚህ ማረጋገጫዎች እስኪለወጡ ወይም ኦፕሬተር ዳግም እስኪያስጀምራቸው ድረስ ይቆያሉ። የመጨረሻ ሁኔታዎችን በጊዜያዊ የማቀዝቀዣ ሁኔታ አይተኩ።

**የዘገየ ማገገም:** `rateLimitedUntil` ካለፈ በኋላ ግንኙነቱ ዳግም ለምርጫ ብቁ ይሆናል። በተሳካ ሁኔታ ጥቅም ላይ ሲውል፣ `clearAccountError()` ሁሉንም የስህተት መስኮች ያጸዳል።

### የክፍለ-ጊዜ ትስስር (#7274)

**ወሰን:** አንድ የደንበኛ ክፍለ-ጊዜ (`X-Session-Id` / `x-codex-session-id` / `x-omniroute-session` ራስጌ) ለ**ማንኛውም** አቅራቢ ከአንድ ግንኙነት ጋር ይሰካል።

**ዓላማ:** ባለብዙ-ዙር ወኪልን (Claude Code, aider, ብጁ ወኪሎች) በተለያዩ ጥያቄዎች ላይ በተመሳሳይ መለያ ማቆየት፣ የመለያ-ተሻጋሪ ዐውድ መጥፋትን እና በእያንዳንዱ መለያ የክፍለ-ጊዜ ሁኔታ ባላቸው አቅራቢዎች ላይ ተደጋጋሚ የቀዝቃዛ-ጅምር 429ዎችን መቀነስ።

**አተገባበር:**

- የTTL መወሰኛ፦ `src/sse/services/sessionAffinityPin.ts::resolveSessionAffinityTtlMs()`
- የማሰሪያ ምርጫ/ፈጠራ፦ `src/sse/services/sessionAffinityPin.ts::selectSessionAffinityConnection()`
- ራስጌ ማውጣት (አጠቃላይ፣ ለማንኛውም አቅራቢ)፦ `src/sse/services/auth.ts::extractSessionAffinityKey()`
- በቋሚነት የሚከማች የማሰሪያ ሰንጠረዥ፦ `sessionAccountAffinity` (`src/lib/db/sessionAccountAffinity.ts`)
- ቅንብር፦ `sessionAffinityTtlMs` (ዓለም አቀፍ TTL በms፣ `0` ያሰናክለዋል) — `src/lib/db/settings.ts`። ቀደም ብሎ የተዋቀረውን የCodex TTL እንደ አዲሱ ነባሪ በሚያስተላልፈው ፍልሰት `124_generic_session_affinity_ttl.sql`፣ የCodex-ብቻ `codexSessionAffinityTtlMs` ስሙ ተቀይሯል።

ከ#7274 በፊት፣ `resolveSessionAffinityTtlMs()` ከ`codex` በስተቀር ለሁሉም አቅራቢዎች ወዲያውኑ `0` በመመለስ ይቋረጥ ነበር፤ ስለዚህ የማሰሪያ ዘዴው እና የራስጌ ማውጣቱ ቀድሞውኑ ከአቅራቢ ነጻ ቢሆኑም፣ የTTL ቅንብሩ (እና የክፍለ-ጊዜ ራስጌዎቹ) በሌላ ቦታ ምንም ተጽዕኖ አልነበራቸውም። ማስተካከያው ያንን ቀደምት መመለሻ አስወገደው፤ TTL አሁን በዓለም አቀፍ ደረጃ ከ`0` በላይ ከተቀናበረ በኋላ ለእያንዳንዱ አቅራቢ በእኩልነት ይተገበራል።

ሦስቱ የክፍለ-ጊዜ ትስስር ራስጌዎች ወደ ላይኛው አገልግሎት ፈጽሞ አይተላለፉም — አስፈጻሚዎች የደንበኛ ራስጌዎችን ከማስተላለፍ ይልቅ የራሳቸውን የላይኛው አገልግሎት ራስጌዎች ከባዶ ይገነባሉ፤ ስለዚህ ይህ የውስጥ ማዛመጃ መለያ ብቻ ሆኖ ይቆያል።

### ብቸኛ የሚተዳደር የክፍለ-ጊዜ ግንኙነት ኪራዮች

**ወሰን:** አንድ ንቁ የሚተዳደር HTTP ደንበኛ/ክፍለ-ጊዜ አንድ ብቁ የOmniRoute ግንኙነትን ይይዛል።

**ዓላማ:** በተለያዩ ጥያቄዎች ላይ ጥብቅ የማዞሪያ
ወሰን ለሚፈልጉ ደንበኞች ዘላቂ ብቸኛ የግንኙነት ባለቤትነትን ማቅረብ። ይህ ለስላሳ የቀጣይነት ምርጫ ከሆነው የክፍለ-ጊዜ ትስስር የተለየ ነው፦
ብቸኛ ኪራይ በSQLite ውስጥ የሕይወት-ዑደት ሁኔታን በቋሚነት ያከማቻል፣ ዓለም አቀፍ የንቁ-ባለቤትና
የንቁ-ግንኙነት ልዩነትን ያስገድዳል፣ እንዲሁም ወደ አቅራቢው ከመላኩ በፊት ጊዜው ያለፈበትን ትውልድ ውድቅ ያደርጋል።

ባህሪው ለእያንዳንዱ API ቁልፍ በምርጫ የሚነቃ ነው። የሚተዳደር ቁልፍ የ`lease:exclusive` ወሰን እና
በግልጽ የተገለጸ ባዶ ያልሆነ የ`allowedConnections` ዝርዝር ሊኖረው ይገባል። ማንኛውም የHTTP ደንበኛ የሕይወት-ዑደት መዳረሻውን መጠቀም ይችላል፤
የደንበኛ ስም፣ user-agent፣ አቅራቢ፣ የOAuth ዘዴ ወይም ሞዴል አያስፈልግም። ኪራዩ የሚይዘው ግንኙነት እንጂ
ሞዴል አይደለም፤ ስለዚህ ግንኙነቱ በመደበኛነት ብቁ ሆኖ እስከቆየ ድረስ የሞዴል ለውጥ ትስስሩን ያቆያል።
መደበኛ የሞዴል፣ ኮታ፣ ጤና፣ የማቀዝቀዣ ጊዜ እና የፈቃድ ዝርዝር ደንቦች በዋናነት መተግበራቸውን ይቀጥላሉ፣ እንዲሁም
ተመሳሳዩን ትውልድ ወደ ሌላ ነጻና ብቁ ግንኙነት ሊያሸጋግሩ ይችላሉ።

የሕይወት-ዑደቱ `POST /api/v1/session-leases` ሲሆን የJSON እርምጃዎቹ `acquire`፣ `renew` እና `release` ናቸው።
የሚተዳደሩ የግምት ጥያቄዎች ግልጽ ያልሆነውን የ`X-OmniRoute-Lease-Owner` እሴት እና ትክክለኛውን
`X-OmniRoute-Lease-Generation` ያቀርባሉ። ባለቤቱ `vlo_`ን ተከትሎ 43 base64url ቁምፊዎችን ይጠቀማል፤
የሚከማቸው የእሱ SHA-256 hash ብቻ ነው። እያንዳንዱ የመጨረሻ መላኪያ ወሰን እንዲሁም የተረጋገጠውን የAPI ቁልፍ ID እና
የንቁውን ግንኙነት ID ያስተሳስራል። የኪራይ መቆጣጠሪያ ራስጌዎች ከመዝገቦች፣ ከተያዙ የጥያቄ ቅጽበታዊ ቅጂዎች እና
ከላይኛው አገልግሎት አስፈጻሚ ራስጌዎች ይወገዳሉ።

መደበኛ ማዞሪያ ብቁ የሚተዳደሩ ዕጩዎች ካሉት፣ ነገር ግን እያንዳንዱ ነጻ ዕጩ በሌላ ንቁ ኪራይ የተያዘ ከሆነ፣
OmniRoute HTTP `429`ን፣ የኪራይ-አቅም-አይገኝም ኮድን፣
አቅም-በመጠበቅ-ላይ ሁኔታን እና ከሚመለከተው ቀደምት የማብቂያ ጊዜ የተወሰደ የተገደበ `Retry-After`ን ይመልሳል።
መደበኛ ባዶ ብቁነት የኪራይ ፉክክር አይደለም፣ እና ነባሩን የማዞሪያ ስህተት ትርጉም እንዳለ ያቆያል።

ተዛማጅ ዘዴዎች ተለያይተው ይቆያሉ፦

- የOAuth ክፍለ-ጊዜ ይዞታ ለOAuth መለያዎች በሂደት-ውስጥ ብቻ ያለ ለስላሳ ስርጭት ነው።
- የመለያ ሴማፎሮች የጥያቄ-ትይዩነት ፈቃዶችን ይሰጣሉ፣ እና ጥያቄው ሲጠናቀቅ ያበቃሉ።
- ብቸኛ የሚተዳደሩ የክፍለ-ጊዜ ኪራዮች የትውልድ ወሰን ያለው ዘላቂ የሕይወት-ዑደት ባለቤትነት ናቸው።

---

## 3. የሞዴል መቆለፊያ

**ወሰን፦** የአቅራቢ + የግንኙነት + የሞዴል ሦስትዮሽ።

**ዓላማ፦** አንድ ሞዴል ብቻ የማይገኝ ወይም በኮታ የተገደበ ሲሆን ሙሉ ግንኙነቱን ከማሰናከል መቆጠብ።

**ምሳሌዎች፦**

- በየሞዴሉ ኮታ የሚያስቀምጡ እና 429 የሚመልሱ አቅራቢዎች
- ለአንድ የጎደለ ሞዴል 404 የሚመልሱ አካባቢያዊ አቅራቢዎች
- ለአቅራቢው የተለዩ የሁነታ/ሞዴል ፈቃድ አለመሳካቶች (ለምሳሌ፣ Grok ሁነታዎች)

**ትግበራ፦** `open-sse/services/accountFallback.ts` — `lockModel()`፣ `clearModelLock()`፣ `getAllModelLockouts()`።

### የሞዴል የማቀዝቀዣ ጊዜዎች ዳሽቦርድ (v3.8.0)

UI፦ ቅንብሮች → የሞዴል የማቀዝቀዣ ጊዜዎች (`src/app/(dashboard)/dashboard/settings/components/ModelCooldownsCard.tsx`)

ንቁ መቆለፊያዎችን ከሚከተሉት ጋር ይዘረዝራል፦ አቅራቢ፣ ግንኙነት፣ ሞዴል፣ ምክንያት፣ expiresAt። ኦፕሬተሮች ሞዴልን ከካርዱ ላይ በእጅ ዳግም ማንቃት ይችላሉ።

**REST API፦**

- `GET /api/resilience/model-cooldowns` — ንቁ መቆለፊያዎችን ዘርዝር
- `DELETE /api/resilience/model-cooldowns` — በእጅ ዳግም ማንቃት። የጥያቄ ይዘት፦ `{provider, connection, model}`። ማረጋገጫ፦ አስተዳደር።

### የመቆለፊያ ቅንብሮች UI + በስኬት-መቀነስ መልሶ ማግኛ (v3.8.23)

የሞዴል መቆለፊያ ሁልጊዜ ከነቃ እና በኮድ ውስጥ ከተወሰነ ባህሪ ወደ ሙሉ በሙሉ ሊዋቀር የሚችል፣
በምርጫ የሚነቃ እና የራሱ የቅንብሮች ካርድ እንዲሁም ራሱን የሚጠግን የመልሶ ማግኛ መንገድ ያለው ባህሪ ሆኗል።

**የቅንብሮች ካርድ፦** ቅንብሮች → የሞዴል መቆለፊያ
(`src/app/(dashboard)/dashboard/settings/components/ModelLockoutCard.tsx`)።
ይህ ከላይ ካለው ለንባብ ብቻ የሆነ `ModelCooldownsCard` (ንቁ መቆለፊያዎችን ብቻ
_ከሚዘረዝር_) **የተለየ** ነው — አዲሱ ካርድ _መለኪያዎቹን ያዋቅራል_። ነባሪዎች በ
`DEFAULT_MODEL_LOCKOUT_SETTINGS`
(`src/lib/resilience/modelLockoutSettings.ts`) ውስጥ ይገኛሉ፦

| ቅንብር                    | ነባሪ                              | ትርጉም                                          |
| ----------------------- | -------------------------------- | --------------------------------------------- |
| `enabled`               | `false`                          | ዋና መቀያየሪያ — የሞዴል መቆለፊያ **በነባሪ ጠፍቷል**።         |
| `errorCodes`            | `[403, 404, 429, 502, 503, 504]` | በሞዴል ወሰን እንደተከሰተ ውድቀት የሚቆጠሩ የላይኛው ስርዓት ሁኔታዎች። |
| `baseCooldownMs`        | `120_000` (120 s)                | ለመጀመሪያው ውድቀት የመነሻ መቆለፊያ ቆይታ።                  |
| `maxCooldownMs`         | `1_800_000` (30 min)             | በመጨመር ላይ ላለው የማቀዝቀዣ ጊዜ ከፍተኛ ገደብ።              |
| `maxBackoffSteps`       | `10`                             | ከፍተኛው የገላጭ-ዕድገት መዘግየት የመጨመሪያ ደረጃዎች።           |
| `useExponentialBackoff` | `true`                           | ተደጋጋሚ ውድቀቶች የማቀዝቀዣ ጊዜውን በገላጭ ዕድገት ያሳድጉ እንደሆነ። |

ቅንብሮች በመደበኛው የቅንብሮች ማከማቻ በኩል ተጠብቀው ይቆያሉ፣ እና በመቋቋም
ቅንብሮች መርሐግብር በኩል ይረጋገጣሉ፤ ካርዱ `baseCooldownMs`/`maxCooldownMs`ን
(`maxCooldownMs ≥ baseCooldownMs` እንዲሆን) እና `maxBackoffSteps`ን በወሰን ውስጥ ያቆያል።

**በስኬት-መቀነስ መልሶ ማግኛ፦** መልሶ ማግኛው በጊዜ ቆጣሪው ማብቃት ላይ ብቻ የተመሠረተ **አይደለም**። ጤናማ
ምላሽ የሞዴሉን የውድቀት ብዛት ቀስ በቀስ ወደ ታች ስለሚቀንስ፣ በመስኮቱ መሀል ያገገመ ሞዴል
ጊዜ ቆጣሪው ከማብቃቱ በፊት መጨመሩን ያቆማል (እና ይጸዳል)። በተሳካ የጥምር
ዒላማ ላይ፣ `open-sse/services/combo.ts` ውስጥ `decayModelFailureCount()`
(`open-sse/services/accountFallback.ts`) ይጠራል፤ ይህም የተከማቸውን
`failureCount` (`Math.floor(failureCount / 2)`) **በግማሽ ይቀንሳል**፤ `0` ሲደርስ የመቆለፊያ
ግቤቱ ሙሉ በሙሉ ይሰረዛል። ተጓዳኙ `recordModelLockoutFailure()`
በመጨመሪያ መስኮቱ ውስጥ ውድቀቶች ሲከሰቱ ብዛቱን ያሳድጋል (እንዲሁም የማቀዝቀዣ ጊዜውን ይጨምራል)።
ይህ በስኬት-መቀነስ ተራ የጊዜ ቆጣሪ ማብቂያን የሚጨምር ነው —
ከሁለቱ መንገዶች ማንኛውም ሞዴልን ዳግም ማንቃት ይችላል።

**ሁኔታ፦** መቆለፊያዎች **በማህደረ ትውስታ ውስጥ** (በየሂደቱ `Map`ዎች ውስጥ
በ`provider:connectionId:model` ቁልፍ የተደረጉ `ModelLockoutEntry`ዎች) ይያዛሉ፣ በ
DB ውስጥ አይጠበቁም — ዳግም ሲጀመር ይጠፋሉ። _ቅንብሮቹ_ ተጠብቀው ይቆያሉ፤ ንቁው
...
- የአንድ አቅራቢ ቁልፎች በሙሉ ተዘለሉ → የወረዳ ቆራጩን ሁኔታ እና የእያንዳንዱን ግንኙነት `rateLimitedUntil`/`testStatus` ይፈትሹ።
- ከዳግም ማስጀመሪያ ጊዜ መስኮት በኋላ አቅራቢው በቋሚነት ተገለለ → ኮዱ `getStatus()`/`canExecute()`ን ከመጠቀም ይልቅ ጥሬ `state`ን እያነበበ ነው።
- አንድ ቁልፍ አይሠራም፣ ሌሎቹ መሥራት አለባቸው → ከወረዳ ቆራጭ ይልቅ የግንኙነት ማቀዝቀዣን ይምረጡ።
- አንድ ሞዴል ብቻ አይሠራም → ከግንኙነት ማቀዝቀዣ ይልቅ የሞዴል መቆለፊያን ይምረጡ።
- ሁኔታው በራሱ መመለስ አለበት ግን አይመለስም → የወደፊት የጊዜ ማህተም + ጊዜው ያለፈበትን ሁኔታ የሚያድስ የንባብ ዱካ መኖሩን ይፈትሹ። ቋሚ ሁኔታዎች በእጅ ለውጦችን ይፈልጋሉ።
