...
`.github/workflows/quality.yml` 会在目标为 `release/**` 的 PR 上运行。它通过按路径筛选的快速门禁保持贡献者分支的推进，并针对代码更改额外提供一个建议性的生产构建信号：

| 作业                                             | 范围                                                                                                                                                                              | 阻塞性                                                              |
| ------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------- |
| `Build (advisory)`                               | 非草稿代码 PR 和 Mergify 队列分支；Node 24、`npm-ci-retry`、`check:node-runtime`，并使用 `OMNIROUTE_USE_TURBOPACK=1` 执行 `npm run build`；不上传构件，因为没有下游质量作业使用它 | **建议性**（`continue-on-error: true`；发布 PR 稳定运行一周后移除） |
| `Docs Gates (fast-path)`                         | 文档/代码 PR；API 文档引用和 docs-all                                                                                                                                             | 是                                                                  |
| `Fast Quality Gates`                             | 代码 PR；静态检查、类型检查、仪表板类型检查、受影响的单元测试                                                                                                                     | 是                                                                  |
| `Forgotten sibling tests`                        | 代码 PR；将已更改模块追踪至静态使用方和候选同级测试；桶文件和动态导入路径作为建议性诊断进行报告，并包含引用的允许列表例外                                                         | **建议性**                                                          |
| `Vitest (fast-path)`                             | 代码 PR；快速 vitest 测试套件                                                                                                                                                     | 是                                                                  |
| `Unit Tests fast-path`                           | 代码 PR；4 分片单元测试套件                                                                                                                                                       | 是                                                                  |
| `No new ESLint warnings`                         | 代码 PR；可感知抑制规则的 lint 防护                                                                                                                                               | 对同源 PR 为是，对分叉 PR 为建议性                                  |
| `Merge integrity (changelog + generated skills)` | 非草稿 PR；变更日志和生成的技能同步                                                                                                                                               | 对同源 PR 为是，对分叉 PR 为建议性                                  |

#### 遗漏的同级测试报告

`npm run check:forgotten-sibling-tests` 复用了测试影响映射背后的导入解析器。
对于每个已更改的生产模块，当候选测试未出现在拉取请求的差异中时，它会报告确定性的
`已更改模块/符号 -> 静态使用方 -> 候选同级测试` 链。在任何阻塞式推广之前，Markdown 摘要和 JSON 结果会作为
`forgotten-sibling-tests` 工作流构件予以保留，以供校准。

桶式重新导出和动态导入仅作为解析诊断；它们绝不会产生阻塞性发现。经过审查的例外位于
`config/quality/forgotten-sibling-allowlist.json` 中。每个条目都必须指定使用方和候选测试、给出具体理由，并链接到 GitHub issue 或拉取请求。格式错误的条目将以关闭方式失败。
例外不能抑制已删除的候选测试，也不能抑制新增 `.skip`/`.todo` 的差异；断言弱化和其他掩盖行为仍由独立的阻塞门禁
`check:test-masking` 负责。

### 作业：`lint`

在每个目标为 `main` 的 PR 上运行。失败时阻止合并。

| 脚本 (`npm run ...`)              | 验证内容                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | 阻断性                                  |
| --------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------- |
| `check:node-runtime`              | Node.js 版本是否在支持的范围内                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | 是                                      |
| `check:cycles`                    | 循环导入——所有 `src/` + `open-sse/` 模块                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | 是                                      |
| `check:route-validation:t06`      | 所有路由是否都存在 Zod schema（Tier 6 策略）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | 是                                      |
| `check:any-budget:t11`            | `@ts-expect-error // any` 的数量不超过预算（Tier 11 catraca）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | 是                                      |
| `check:provider-consistency`      | `providers.ts` 中的每个提供者在 `providerRegistry.ts` 中都有对应条目（反之亦然，但仅限允许列表范围内）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | 是                                      |
| `check:model-lifecycle`           | 三个手动维护的路由表与已检入的生命周期快照 (#11503) 保持一致：`FITNESS_TABLE` (`taskFitness.ts`) 不会为任何 `REGISTRY` 可路由的已退役 ID 评分；每个 `BUILT_IN_ALIASES` 目标都存在于 `REGISTRY` 中，且不在已退役 ID 快照中；`REGISTRY` 中仍然存在的每个已退役 ID 都会被转发，或列在 `allowedRetiredInCatalog` 中；并且 `DEFAULT_DEGRADATION_MAP` 的源或目标均未在该快照中显示为已退役。这并不能证明某个模型当前正由实时上游提供服务。离线检查——与 `config/quality/model-lifecycle.json` 进行比较；该文件通过 `npm run quality:refresh-model-lifecycle` 手动刷新（需要网络；未接入 CI）。`allowedRetiredInCatalog` 是一种逐步清零的棘轮机制：仅在有关联的跟踪议题时才添加条目。 | 是                                      |
| `check:fetch-targets`             | 客户端 `src/` 中的每个 `fetch("/api/...")` 都会解析到真实的 `route.ts`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | 是                                      |
| `check:deps`                      | 仓库中每个 `package.json` 里所有可通过 `npm install` 安装的依赖都包含在 `dependency-allowlist.json` 中；新增的未固定版本或疑似名称抢注依赖会被标记                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | 是                                      |
| `audit:deps`                      | `npm audit`（根目录 + electron）——不存在高危/严重级别的安全通告（与 osv `check:vuln-ratchet` 重叠；参见合理化待办事项）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | 是                                      |
...
| `check:file-size`                 | 源文件不得超过各扩展名对应的大小上限（棘轮机制：大型文件冻结在 `frozen` 列表中）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | 是                                      |
| `check:error-helper`              | 执行器/处理程序中的错误响应使用 `buildErrorBody()` / `sanitizeErrorMessage()`（硬性规则 #12）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | 是                                      |
| `check:migration-numbering`       | 迁移 SQL 文件须按顺序编号，不得存在编号缺失或重复                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | 是                                      |
| `check:public-creds`              | 除 `publicCreds.ts` 外，不得存在字面量 OAuth `client_id`/`client_secret` 或 Firebase Web 密钥（硬性规则 #11）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | 是                                      |
| `check:db-rules`                  | `src/lib/db/` 模块之外不得存在原始 SQL；不得从 `localDb.ts` 进行桶式导入（硬性规则 #2/#5）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | 是                                      |
| `check:known-symbols`             | 在其分发表中注册的提供者执行器、路由策略和转换器必须与磁盘上的文件匹配，不得存在孤立或未声明的符号                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | 是                                      |
...
| `check:agent-skills-sync`         | 生成的智能体技能产物必须与其源目录保持一致（不得漂移）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `check:provider-asset-provenance` | 提供者徽标/资产必须具有已记录的来源条目                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `lint:json`                       | JSON 配置文件可正确解析并满足仓库的 lint 规则                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `typecheck:core`                  | TypeScript 编译无错误（仅允许提示性警告）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | 是                                      |
| `typecheck:noimplicit:core`       | 严格的 `noImplicitAny`——面向未来；许多既有调用位置仍需添加类型注解                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | **提示性**（`continue-on-error: true`） |
| `check:dashboard-typecheck`       | 作用域限定为 `src/app/(dashboard)/**` 的 `tsc`（#7033）——`typecheck:core` 精选的 27 文件允许列表不包含任何仪表板 TSX，且 `next build` 也从不对其进行类型检查（`next.config.mjs` 设置了 `ignoreBuildErrors: true`），因此其中的孤立标识符回归（#6625/#6909）对 CI 不可见。与按文件/TS 代码计数冻结的基线（`config/quality/dashboard-typecheck-baseline.json`，采用与 `check:known-symbols` 相同的过期强制执行模式）进行差异比较——只有超出基线计数的新增错误才会导致门禁失败；修复既有错误时，使用 `--update` 逐步下调基线。                                                                                                                                                    | 是                                      |
...
| 脚本                         | 验证内容                                                                                                                    | 阻断性                   |
| ---------------------------- | --------------------------------------------------------------------------------------------------------------------------- | ------------------------ |
| `quality:collect`            | 生成 `quality-metrics.json`（ESLint 警告数量、来自合并分片报告的覆盖率）                                                    | 是（棘轮检查的上游步骤） |
...
| `check:cognitive-complexity` | 认知复杂度棘轮检查（`eslint-plugin-sonarjs`）——独立的 ESLint 检查；CI 将两者合并为单个 `check:complexity-ratchets` 步骤运行 | 是                       |
| `check:dead-code`            | 未使用导出/文件的棘轮检查（knip）相较基线没有退化                                                                           | 是                       |
| `check:compression-budget`   | 压缩基准预算——每种引擎的令牌节省下限不得退化                                                                                | 是                       |
| `check:type-coverage`        | 类型标注百分比棘轮检查（`type-coverage`）没有退化；基本涵盖 `typecheck:noimplicit:core`                                     | 是                       |
| `check:codeql-ratchet`       | 未解决的 CodeQL 警报数量没有增加（通过 `gh api` 读取；无令牌时优雅跳过）——刷新频率和手动触发方式：参见下文“CodeQL 棘轮检查” | 是                       |

### 作业：`quality-extended`

整个作业均为建议性检查（`continue-on-error: true`）。基于 npm 的棘轮检查会实际运行；外部扫描器通过 `gh release download` 安装，并在二进制文件仍不存在时自行跳过（退出码为 0）。
...
| `check:vuln-ratchet`     | 依赖项漏洞（osv-scanner）不发生回退 — 如果二进制文件不存在则跳过                                                                                        | **建议性** |
| `check:workflows`        | 工作流 lint（actionlint + zizmor）— 如果二进制文件不存在则跳过                                                                                          | **建议性** |
| `check:openapi-breaking` | 对比基础分支，检查公共 API 契约（`openapi.yaml`）是否存在破坏性变更（oasdiff）— 输出 `openapiBreaking=N`；如果 oasdiff 不存在或无法解析基础规范，则跳过 | **建议性** |
...
| ↳ `check:docs-counts`          | 文档正文中的数量（提供者数量、迁移数量等）处于实际数量的棘轮窗口范围内                                                       | 是                         |
| ↳ `check:env-doc-sync`         | `.env.example` 中的每个环境变量都已记录在文档表格中，反之亦然                                                                | 是                         |
| ↳ `check:deprecated-versions`  | 文档中不存在已弃用的版本字符串                                                                                               | 是                         |
| ↳ `check:doc-links`            | 文档中的内部 Markdown 链接可解析到实际文件（`[text]`/`(path)` 形式）                                                         | 是                         |
| ↳ `check:fabricated-docs`      | 文档中引用的路由、环境变量、CLI 命令、钩子名称和文件路径均存在于代码库中。通过 `--strict` 实施硬门禁；不带该标志时为软失败。 | 是（CI 中通过 `--strict`） |
| `check:cli-i18n`               | 所有 i18n 语言区域文件中都包含 CLI 命令字符串                                                                                | 是                         |
| `check:openapi-coverage`       | OpenAPI 规范对实际路由的覆盖率至少达到通过棘轮机制设定的下限                                                                 | 是                         |
| `check:openapi-security-tiers` | `openapi.yaml` 中的安全层级注解与 `routeGuard.ts` 中的分类一致                                                               | **建议性**                 |
| `check:openapi-routes`         | `openapi.yaml` 中的每个路径都可解析到实际的 `route.ts`（防止虚构）                                                           | 是                         |
| `check:docs-symbols`           | `docs/**/*.md` 中的每个 `/api/...` 引用都可解析到实际的 `route.ts`（防止虚构）                                               | 是                         |
| `i18n translation drift`       | i18n 语言区域文件中未翻译的键 — 仅警告                                                                                       | **建议性**                 |
...
| `check-ui-value-drift`（内联）   | 重写后的英文**值**不会留下过时翻译                                                                                                              | 是         |
| `check-new-key-coverage`（内联） | **新增的**英文键已覆盖每个语言区域                                                                                                              | 是         |
| `check-translation-ratio`        | 每个语言区域的真实翻译比例（与英文相同 / 占位符 / 不在允许列表中的缺失叶节点）不得超过 `config/quality/i18n-translation-baseline.json` + 宽限值 | **建议性** |

需要 `fetch-depth: 0`——值漂移门禁会将 `en.json` 与合并基准进行差异比较。

#### `check-ui-value-drift`——过时翻译门禁

捕获其他门禁在结构上无法发现的一类 i18n 回归：英文值被重写，但从_先前_英文派生的翻译仍然保留，导致非英语用户继续看到措辞笃定、但现已错误的文案。

这确实曾发布到生产环境。Antigravity 登录辅助程序上线时（#5203），`oauthModal.googleOAuthWarning` 被重写；**43 个语言区域中有 39 个**仍保留着要求运维人员“复制完整 URL 并将其粘贴到下方”的文本——对于该提供者，此流程根本无法完成。这个问题直到 #8463 才被发现，原因如下：

- `sync-ui-keys` 只回填**缺失**的键，绝不会处理**过时**的键；
- `check-ui-keys-coverage` 只统计键是否_存在_，因此过时翻译仍会被计为已覆盖；
- `check-translation-drift` 跟踪的是 `docs/i18n/<locale>/**.md` 文档镜像——
  它从不读取 `src/i18n/messages/*.json`。自 2026-09 重新同步后，在作业 `docs-sync-strict` 中阻塞：
  编辑核心文档 → `npm run i18n:run -- --files=<doc>`（按章节执行，成本低）。

**感知差异，而非基于基线。**它会将合并基准处的 `en.json` 与工作树进行比较；对于英文值已更改的每个键，如果任何语言区域仍保留未经修改的翻译，则该翻译即为过时翻译。这有意**冻结了既有欠账**——差异无法揭示长期存在的翻译源自哪个旧英文版本，因此该门禁只评判当前变更所触及的内容。另一种方案（按键维护哈希基线）会产生约 600 KB 的生成文件，是现有最大基线的 3 倍，并且每个 i18n PR 都会导致该文件频繁变动。

有两种方法可满足此门禁：

1. 更新受影响的翻译，或
2. 将其设置为 `__MISSING__:<new english>`——随后运行时会提供修正后的英文
   （`src/i18n/request.ts::deepMergeFallback`，#7258），并将该键加入待翻译队列。

如果字符串的**含义**发生了变化，最好**重命名该键**：新键不会继承过时翻译。这正是 #8463 所采用的模式。
...
当无法读取基础目录（浅克隆中没有基础引用）时，以状态码 0 退出，并输出 `SKIP reason=base-unresolved`，行为与 `check-openapi-breaking` 一致。
...
| 脚本                   | 验证内容                                                                                                   | 阻塞性 |
| ---------------------- | ---------------------------------------------------------------------------------------------------------- | ------ |
| `check:pr-test-policy` | 更改 `src/`、`open-sse/`、`electron/` 或 `bin/` 中生产代码的 PR 必须包含或更新测试（硬性规则 #8）          | 是     |
| `check:test-masking`   | 发生变更的测试文件不会减少断言净数量，也不会添加 `assert.ok(true)` 这类永真断言                            | 是     |
| `check:pr-evidence`    | PR 正文引用了针对该变更的测试/VPS 证据（通过 grep 检索 PR 文本来实现硬性规则 #18——较为脆弱，参见 Backlog） | 是     |
...
| `test:vitest:ui` | UI 组件测试 — vitest 运行器                               | **阻断** — `vitest.config.ts` 中已明确排除现有失败；新的失败会导致作业失败 |
...
| `nightly-schemathesis` | 使用 `docs/openapi.yaml` 对实时 OmniRoute 执行 OpenAPI 契约模糊测试（schemathesis）— 揭示规范违规/未处理的 500 错误（阶段 8 B.4） | **建议性** |
| `nightly-mutation`     | 对快速单元测试通道执行 Stryker 变异测试评分 — 存活的变异体会揭示薄弱的断言                                                        | **建议性** |
| `nightly-compat`       | 在支持的 `engines.node` 范围内执行 Node 引擎兼容性矩阵测试                                                                        | **建议性** |
...
它会将每个 `src/i18n/messages/<locale>.json` 的叶级键集合与 `en.json` 进行比较，只要有任何缺失或多余的叶级键就会失败，无论该键是何时添加的。`__MISSING__:` 占位符算作已存在（其内容由比例门禁负责）。它是另外两个基于差异/百分比的门禁的绝对补充：`check-ui-keys-coverage` 对每个区域设置强制执行 80 % 的下限（在约 13,000 个键中缺少 43 个，覆盖率仍显示为 99.7 %），而 `check-new-key-coverage` 仅检查 PR 添加到 `en.json` 的键。区域批次基于创建其分支当天的 `en.json` 生成，并在翻译期间持续数日，而基础分支仍在不断添加键；批次 PR 本身没有添加任何键，因此，当批次 1（#13044）合入时，九个区域缺少 43 个键，而批次 2（#13660）合入时，八个区域缺少 10 个键（2026-09-15），这两个同级门禁都未发出警报。可使用 `node scripts/i18n/sync-ui-keys.mjs --locale=<codes> --translate-markers` 修复失败；出现 `extra` 叶级键意味着源语言中已删除该键——请将其从对应区域中删除。`--warn` 仅报告而不会导致失败。`--catalog=cli` 会对 `bin/cli/locales` 执行相同的比较（`npm run i18n:check-keys:cli`）；这两个步骤都位于作业 `i18n-ui-coverage` 中。

#### `check-new-key-coverage` — 新键 i18n 门禁

`check-ui-value-drift` 的同级门禁。后者会捕获英文值已被**重写**但其翻译未同步更新的情况；本门禁则捕获新增了英文键但某些区域从未收到该键的情况。

`check-ui-keys-coverage` 无法发现此类问题：它对每个区域强制执行覆盖率下限，而在约 13,000 个叶级键中缺少十一个键，覆盖率仍有 99.9%。按语言计算的百分比无法表达“此功能发布时未翻译”——一个完整功能可能在新区域中完全没有文本，却不会对这个数字产生明显影响。

它所记录的事件如下：Orchestration Canvas 的第 3 阶段在当时已有的 42 个区域中翻译了其十一个键。数小时后，欧盟语言批次（#13044）将仓库扩展至 51 个区域，而新增的九个区域（`el`、`et`、`ga`、`hr`、`lt`、`lv`、`mt`、`sl`、`sr`）从未收到这些键。`deepMergeFallback` 会用英文替换缺失的键，因此故障表现为界面未翻译，而不是界面空白——这是一个真实存在、且按设计不会发出警报的问题。

与其同级门禁一样，它也**感知差异**，会将合并基础处的英文内容与工作树进行比较，因此已有缺口会保持冻结状态，启用该门禁时无需进行迁移。应急机制：`__MISSING__:<english>` 可在保持运行时正确性的同时推迟翻译。`vi` 禁止使用占位符（`tests/unit/i18n-vi-completeness.test.ts`），必须提供真实翻译。

#### `check-vitest-exclusions` — 暂停测试门禁

位于 `vitest.config.ts` 的 `exclude` 列表中的文件是不会运行的测试，但对于查看代码树的人来说，它看起来仍像覆盖率的一部分。共有六十二个文件被逐渐添加到注释 `// #8618 — pre-existing failure; remove this exclusion when fixed` 之后。问题 #8618 已于 2026-08-11 关闭，但它所跟踪的列表却从 45 项增长到 62 项，每个新增项都继承了一条指向已关闭问题的注释。最终逐个文件测量该列表时（#13204），**62 个文件中有 51 个无需修改源代码便能在当前代码树上通过**。

该门禁要求每个能够解析为真实文件的排除项都必须：(a) 指明一个跟踪问题；并且 (b) 连同其测量状态一起出现在 `config/quality/vitest-exclusions.json` 中。这样，添加排除项时，会在专用文件中形成可审查的差异，而不是在包含 60 个条目的数组中再增加一行。它特意不会重新运行被排除的测试——这需要约 10 分钟，应由周期性作业完成；清单会记录每项上次测量的时间。
