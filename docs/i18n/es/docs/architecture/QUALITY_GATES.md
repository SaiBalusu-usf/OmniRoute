...
## Inventario de controles (~90 scripts)

Los scripts se encuentran en `scripts/check/` (controles de políticas) y `scripts/quality/` (motor de ratchet).
La fuente de verdad de CI es `.github/workflows/ci.yml`.

### Vía rápida para PRs de release (`quality.yml`)

`.github/workflows/quality.yml` se ejecuta en PRs dirigidas a `release/**`. Mantiene en movimiento las ramas
de los colaboradores mediante controles rápidos filtrados por rutas, además de una señal consultiva de compilación
de producción para cambios de código:

| Trabajo                                          | Alcance                                                                                                                                                                                                                                                                   | Bloqueante                                                                                                           |
| ------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| `Build (advisory)`                               | PRs de código que no sean borradores y ramas de cola de Mergify; Node 24, `npm-ci-retry`, `check:node-runtime`, `npm run build` con `OMNIROUTE_USE_TURBOPACK=1`; sin carga de artefactos porque ningún trabajo de calidad posterior los consume                           | **Consultivo** (`continue-on-error: true`; eliminar después de una semana de ejecuciones estables de PRs de release) |
| `Docs Gates (fast-path)`                         | PRs de documentación/código; referencias de documentación de la API y docs-all                                                                                                                                                                                            | Sí                                                                                                                   |
| `Fast Quality Gates`                             | PRs de código; comprobaciones estáticas, comprobación de tipos, comprobación de tipos del panel, pruebas unitarias afectadas                                                                                                                                              | Sí                                                                                                                   |
| `Forgotten sibling tests`                        | PRs de código; módulos modificados rastreados hasta consumidores estáticos y posibles pruebas hermanas; las rutas de archivos de barril e importaciones dinámicas se notifican como diagnósticos consultivos, con las excepciones referenciadas de la lista de permitidos | **Consultivo**                                                                                                       |
| `Vitest (fast-path)`                             | PRs de código; conjunto rápido de Vitest                                                                                                                                                                                                                                  | Sí                                                                                                                   |
| `Unit Tests fast-path`                           | PRs de código; conjunto de pruebas unitarias en 4 fragmentos                                                                                                                                                                                                              | Sí                                                                                                                   |
| `No new ESLint warnings`                         | PRs de código; control de lint que tiene en cuenta las supresiones                                                                                                                                                                                                        | Sí para el origen propio, consultivo para forks                                                                      |
| `Merge integrity (changelog + generated skills)` | PRs que no sean borradores; sincronización del registro de cambios y de las habilidades generadas                                                                                                                                                                         | Sí para el origen propio, consultivo para forks                                                                      |

#### Informe de pruebas hermanas olvidadas

`npm run check:forgotten-sibling-tests` reutiliza el resolutor de importaciones subyacente al mapa de impacto de pruebas.
...
no está presente en el diff de la solicitud de incorporación de cambios. El resumen en Markdown y el resultado JSON se conservan como
el artefacto de flujo de trabajo `forgotten-sibling-tests` para su calibración antes de cualquier implantación bloqueante.

Las reexportaciones mediante archivos de barril y las importaciones dinámicas son únicamente diagnósticos de resolución; nunca generan
un hallazgo bloqueante. Las excepciones revisadas se encuentran en
`config/quality/forgotten-sibling-allowlist.json`. Cada entrada debe nombrar al consumidor y la prueba candidata,
proporcionar una justificación específica y enlazar una incidencia o solicitud de incorporación de cambios de GitHub. Las entradas malformadas fallan
de forma cerrada. Las excepciones no pueden suprimir una prueba candidata eliminada ni un diff que añada `.skip`/`.todo`;
el debilitamiento de aserciones y otros mecanismos de enmascaramiento siguen siendo responsabilidad del control bloqueante independiente
`check:test-masking`.

### Trabajo: `lint`

Se ejecuta en cada PR dirigida a `main`. Bloquea la fusión en caso de fallo.

| Script (`npm run ...`)            | Valida                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | Bloqueante                                  |
| --------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------- |
| `check:node-runtime`              | La versión de Node.js está dentro del intervalo compatible                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | Sí                                          |
| `check:cycles`                    | Importaciones circulares en todos los módulos de `src/` + `open-sse/`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | Sí                                          |
| `check:route-validation:t06`      | Hay esquemas de Zod presentes en todas las rutas (política del nivel 6)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | Sí                                          |
| `check:any-budget:t11`            | El recuento de `@ts-expect-error // any` no supera el límite (catraca del nivel 11)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | Sí                                          |
...
| `check:model-lifecycle`           | Las tres tablas de enrutamiento mantenidas manualmente permanecen coherentes con la instantánea del ciclo de vida incluida en el repositorio (#11503): `FITNESS_TABLE` (`taskFitness.ts`) no puntúa ningún id retirado que `REGISTRY` pueda enrutar; cada destino de `BUILT_IN_ALIASES` está presente en `REGISTRY` y ausente de la instantánea de ids retirados; cada id retirado que aún esté en `REGISTRY` se reenvía o figura en `allowedRetiredInCatalog`; y ningún origen ni destino de `DEFAULT_DEGRADATION_MAP` aparece como retirado en esa instantánea. Esto no demuestra que un modelo sea atendido actualmente por un servicio ascendente activo. Sin conexión: compara con `config/quality/model-lifecycle.json`, que se actualiza manualmente mediante `npm run quality:refresh-model-lifecycle` (requiere red; no está integrado en CI). `allowedRetiredInCatalog` es un mecanismo de reducción progresiva: añada una entrada únicamente con una incidencia de seguimiento. | Sí                                          |
| `check:fetch-targets`             | Cada `fetch("/api/...")` del código cliente en `src/` se resuelve a un `route.ts` real                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | Sí                                          |
| `check:deps`                      | Todas las dependencias instalables mediante `npm install` de cada `package.json` del repositorio están en `dependency-allowlist.json`; se señalan los paquetes nuevos sin versión fijada o víctimas de slopsquatting                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | Sí                                          |
| `audit:deps`                      | `npm audit` (raíz + electron): sin avisos de gravedad alta/crítica (se solapa con `check:vuln-ratchet` de osv; consulte la lista de tareas de racionalización)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | Sí                                          |
| `check:lockfile`                  | Integridad de `package-lock.json`: registro https, hashes de integridad y sin sustituciones de host                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | Sí                                          |
| `check:licenses`                  | Lista de licencias SPDX permitidas para las dependencias de producción                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | Sí                                          |
| `check:tracked-artifacts`         | Sin artefactos de compilación ni enlaces simbólicos de `node_modules` confirmados en el repositorio (también se ejecuta en el pre-commit de husky; el pre-push es intencionadamente ligero — #6716)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | Sí                                          |
...
| `check:file-size`                 | Ningún archivo de código fuente supera el límite por extensión (mecanismo de reducción progresiva: archivos grandes congelados en la lista `frozen`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | Sí                                          |
...
| `check:db-rules`                  | No hay SQL sin procesar fuera de los módulos de `src/lib/db/`; no hay importaciones de barril desde `localDb.ts` (reglas estrictas n.º 2/n.º 5)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | Sí                                          |
...
| `check:route-guard-membership`    | Cada ruta que inicia un proceso hijo está clasificada por `isLocalOnlyPath()` (reglas estrictas n.º 15/n.º 17)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | Sí                                          |
| `check:test-discovery`            | Cada archivo `*.test.ts` / `*.spec.ts` del repositorio es recopilado por al menos un ejecutor de pruebas (mecanismo de reducción progresiva: la lista de huérfanos en `test-discovery-baseline.json` solo puede reducirse)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | Sí                                          |
...
| `typecheck:noimplicit:core`       | `noImplicitAny` estricto: orientado al futuro; muchos puntos de llamada preexistentes todavía necesitan anotaciones                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | **Informativo** (`continue-on-error: true`) |
| `check:dashboard-typecheck`       | `tsc` limitado a `src/app/(dashboard)/**` (#7033): la lista de 27 archivos permitidos seleccionados de `typecheck:core` no incluye ningún TSX del panel, y `next build` tampoco comprueba nunca sus tipos (`next.config.mjs` establece `ignoreBuildErrors: true`), por lo que las regresiones de identificadores huérfanos allí (#6625/#6909) eran invisibles para CI. Compara con una línea base congelada de recuentos por archivo y por código TS (`config/quality/dashboard-typecheck-baseline.json`, con el mismo patrón de aplicación ante obsolescencia que `check:known-symbols`): solo los errores NUEVOS que superen el recuento de la línea base hacen fallar la comprobación; reduzca progresivamente la línea base con `--update` cuando se corrija un error preexistente.                                                                                                                                                                                                    | Sí                                          |

### Trabajo: `quality-gate`

Se ejecuta después de `test-coverage`. Bloquea la fusión en caso de error.

| Script                       | Valida                                                                                                                                                                                                                      | Bloqueante                        |
| ---------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------- |
| `quality:collect`            | Genera `quality-metrics.json` (recuento de advertencias de ESLint y cobertura del informe combinado de los fragmentos)                                                                                                      | Sí (precede al ajuste progresivo) |
...
| `check:cognitive-complexity` | Ajuste progresivo de la complejidad cognitiva (`eslint-plugin-sonarjs`): pasada de ESLint independiente; CI ejecuta ambas combinadas como el único paso `check:complexity-ratchets`                                         | Sí                                |
| `check:dead-code`            | El ajuste progresivo de exportaciones/archivos sin usar (knip) no empeora respecto a la línea base                                                                                                                          | Sí                                |
...
| `check:type-coverage`        | El ajuste progresivo del porcentaje tipado (`type-coverage`) no empeora; sustituye en gran medida a `typecheck:noimplicit:core`                                                                                             | Sí                                |
| `check:codeql-ratchet`       | El recuento de alertas abiertas de CodeQL no empeora (se consulta mediante `gh api`; se omite de forma segura sin token): frecuencia de actualización y activación manual: consulte «Ajuste progresivo de CodeQL» más abajo | Sí                                |

### Trabajo: `quality-extended`

El trabajo completo es informativo (`continue-on-error: true`). Los ajustes progresivos
basados en npm se ejecutan realmente; los analizadores externos se instalan mediante
`gh release download` y se omiten automáticamente (código de salida 0) cuando aún falta
algún binario.

| Script                   | Valida                                                                                                                                                                                                         | Bloqueante      |
| ------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------- |
| `check:circular-deps`    | No hay dependencias circulares (dpdm)                                                                                                                                                                          | **Informativo** |
| `check:bundle-size`      | El tamaño del paquete no supera el límite                                                                                                                                                                      | **Informativo** |
| `check:secrets`          | Análisis de secretos (gitleaks): se omite si falta el binario                                                                                                                                                  | **Informativo** |
| `check:vuln-ratchet`     | Las vulnerabilidades de las dependencias (osv-scanner) no empeoran: se omite si falta el binario                                                                                                               | **Informativo** |
| `check:workflows`        | Análisis de workflows (actionlint + zizmor): se omite si faltan los binarios                                                                                                                                   | **Informativo** |
| `check:openapi-breaking` | Cambios incompatibles en el contrato de la API pública (`openapi.yaml`) respecto a la rama base (oasdiff): genera `openapiBreaking=N`; se omite si falta oasdiff o no se puede resolver la especificación base | **Informativo** |

### Trabajo: `docs-sync-strict`

Se ejecuta en cada PR dirigida a `main`. Bloquea la fusión en caso de error.
...
| ↳ `check:docs-counts`          | Los recuentos en el texto (número de proveedores, número de migraciones, etc.) están dentro del intervalo de ajuste respecto a los recuentos reales                                                                    | Sí                             |
...
| ↳ `check:fabricated-docs`      | Las rutas, variables de entorno, órdenes de CLI, nombres de hooks y rutas de archivos citados en la documentación existen en el código fuente. Puerta estricta mediante `--strict`; fallo no bloqueante sin la opción. | Sí (mediante `--strict` en CI) |
| `check:cli-i18n`               | Las cadenas de órdenes de la CLI están presentes en todos los archivos de configuración regional de i18n                                                                                                               | Sí                             |
| `check:openapi-coverage`       | La especificación OpenAPI cubre al menos un mínimo ajustable de rutas reales                                                                                                                                           | Sí                             |
...
| `i18n translation drift`       | Claves sin traducir en los archivos de configuración regional de i18n; solo genera una advertencia                                                                                                                     | **Consultivo**                 |
...
| `check-ui-value-drift` (en línea)   | Un **valor** en inglés reescrito no deja ninguna traducción obsoleta                                                                                                                                                                                | Sí             |
| `check-new-key-coverage` (en línea) | Una **nueva** clave en inglés llega a todas las configuraciones regionales                                                                                                                                                                          | Sí             |
| `check-translation-ratio`           | La proporción de traducciones reales por configuración regional (valores idénticos al inglés, marcadores de posición o elementos ausentes fuera de la lista de permitidos) no debe superar `config/quality/i18n-translation-baseline.json` + margen | **Consultivo** |

Requiere `fetch-depth: 0`, ya que la puerta de detección de valores obsoletos compara `en.json` con la base de fusión.

#### `check-ui-value-drift` — puerta de detección de traducciones obsoletas
...
Esto llegó a producción. `oauthModal.googleOAuthWarning` se reescribió cuando se incorporó el asistente de inicio de sesión Antigravity (#5203); **39 de 43 configuraciones regionales** conservaron un texto que indicaba a los operadores que debían «copiar la URL completa y pegarla a continuación», un flujo que no puede completarse con ese proveedor. Pasó inadvertido hasta #8463 porque:
...
- `check-translation-drift` supervisa las réplicas de documentación `docs/i18n/<locale>/**.md` —
  nunca lee `src/i18n/messages/*.json`. Es bloqueante en el trabajo `docs-sync-strict` desde la
  resincronización de 2026-09: edite un documento principal → `npm run i18n:run -- --files=<doc>` (por sección, rápido).

**Consciente de las diferencias, no respaldado por una línea base.** Compara `en.json` en la base de fusión con el
...
traducción sin modificar estará desactualizada. Esto **congela deliberadamente la deuda preexistente**: una diferencia
no puede revelar de qué versión antigua del inglés procede una traducción existente desde hace tiempo, por lo que el control evalúa
únicamente lo que afecta el cambio actual. La alternativa (una línea base de hashes por clave) supondría
...
2. establecerlas en `__MISSING__:<nuevo inglés>`; el entorno de ejecución sirve entonces el inglés corregido
...
una traducción desactualizada. Ese es el patrón que utilizó #8463.

```bash
npm run i18n:check-value-drift          # estricto (lo que ejecuta CI)
npm run i18n:check-value-drift:warn     # solo genera el informe
...
| Script                 | Valida                                                                                                                                                                     | Bloqueo |
| ---------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------- |
| `check:pr-test-policy` | Las PR que cambien código de producción en `src/`, `open-sse/`, `electron/` o `bin/` deben incluir o actualizar pruebas (Regla estricta n.º 8)                             | Sí      |
...
| `check:pr-evidence`    | El cuerpo de la PR cita pruebas o evidencias de VPS para el cambio (mecaniza la Regla estricta n.º 18 buscando patrones en el texto de la PR; es frágil, véase el backlog) | Sí      |
...
| `test:vitest:ui` | Pruebas de componentes de la interfaz de usuario: ejecutor vitest   | **Bloqueante**: los fallos preexistentes se excluyen explícitamente en `vitest.config.ts`; los fallos nuevos hacen fallar el trabajo |
...
| `nightly-resilience`   | Control del crecimiento del heap, inyección caótica de fallos y pruebas de carga/prolongadas con k6                                                                                                    | **Informativo** |
...
| `nightly-schemathesis` | Fuzzing del contrato OpenAPI (schemathesis) contra una instancia activa de OmniRoute mediante `docs/openapi.yaml`: detecta infracciones de la especificación y errores 500 no controlados (Fase 8 B.4) | **Informativo** |
| `nightly-mutation`     | Puntuación de pruebas de mutación de Stryker sobre el carril rápido de pruebas unitarias; los mutantes supervivientes revelan aserciones débiles                                                       | **Informativo** |
...
#### `check-key-completeness` — barrera de paridad del conjunto de claves
...
ante cualquier clave hoja ausente o adicional, independientemente de cuándo se añadiera. Los marcadores
`__MISSING__:` cuentan como presentes (su contenido es competencia de la barrera de proporción). Es el complemento
absoluto de las dos barreras basadas en diferencias/porcentajes: `check-ui-keys-coverage` impone un mínimo del 80 %
por configuración regional (43 claves ausentes de unas 13 000 siguen dando un 99,7 %) y `check-new-key-coverage` evalúa
únicamente las claves que una PR añade a `en.json`. Un lote de configuraciones regionales se genera a partir del `en.json` del día
en que se crea su rama y se traduce durante días mientras la base sigue añadiendo claves; la PR del lote no añade ninguna
clave por sí misma, por lo que ambas barreras relacionadas permanecieron en silencio cuando el lote 1 (#13044) llegó con 43 claves ausentes en nueve
configuraciones regionales y el lote 2 (#13660), con 10 claves ausentes en ocho (2026-09-15). Corrija un fallo con
`node scripts/i18n/sync-ui-keys.mjs --locale=<codes> --translate-markers`; una clave hoja `extra`
significa que el origen la eliminó: elimínela de la configuración regional. `--warn` informa sin provocar un fallo.
`--catalog=cli` ejecuta la misma comparación sobre `bin/cli/locales` (`npm run i18n:check-keys:cli`);
ambos pasos forman parte del trabajo `i18n-ui-coverage`.

#### `check-new-key-coverage` — barrera de i18n para claves nuevas

Barrera relacionada con `check-ui-value-drift`. Esta última detecta un valor en inglés que se ha **reescrito**
mientras sus traducciones se han quedado atrás; esta detecta una clave en inglés que se ha **añadido**
sin que algunas configuraciones regionales llegaran a recibirla.

`check-ui-keys-coverage` no puede detectar este caso: impone un porcentaje mínimo por configuración regional, y
once claves ausentes de unas 13 000 dejan la cobertura en el 99,9 %. Un porcentaje por idioma no puede
expresar que «esta funcionalidad se publicó sin traducir»: una funcionalidad completa puede incorporarse a una nueva configuración regional sin
texto y no alterar nunca la cifra.

El incidente que codifica: la fase 3 de Orchestration Canvas tradujo sus once claves a las
42 configuraciones regionales que existían en ese momento. Horas más tarde, el lote de idiomas de la UE (#13044) elevó el repositorio
a 51 configuraciones regionales, y las nueve recién llegadas (`el`, `et`, `ga`, `hr`, `lt`, `lv`, `mt`, `sl`, `sr`) nunca
las recibieron. `deepMergeFallback` sustituye una clave ausente por el inglés, por lo que el modo de fallo fue
una interfaz de usuario sin traducir en lugar de una interfaz en blanco: un problema real y silencioso por diseño.

Al igual que su barrera relacionada, **tiene en cuenta las diferencias** y compara el inglés en la base de fusión con el árbol de
trabajo, de modo que las carencias preexistentes permanecen congeladas y la barrera no necesitó ninguna migración para activarse. Vía de escape:
`__MISSING__:<english>` aplaza una traducción y mantiene correcto el comportamiento en tiempo de ejecución. `vi` prohíbe
los marcadores (`tests/unit/i18n-vi-completeness.test.ts`) y exige una traducción real.

#### `check-vitest-exclusions` — barrera para pruebas apartadas

Un archivo incluido en la lista `exclude` de `vitest.config.ts` es una prueba que no se ejecuta, aunque parece
cobertura para quien consulta el árbol. Sesenta y dos archivos se acumularon tras el comentario
`// #8618 — pre-existing failure; remove this exclusion when fixed`. La incidencia #8618 se cerró el
2026-08-11 mientras la lista que rastreaba crecía de 45 entradas a 62, y cada nueva entrada heredaba un comentario
que apuntaba a una incidencia cerrada. Cuando finalmente se evaluó la lista archivo por archivo (#13204), **51 de los 62
pasaban con el árbol actual sin ningún cambio en el código fuente**.

La barrera exige que toda exclusión que corresponda a un archivo real (a) indique una incidencia de seguimiento y
(b) aparezca en `config/quality/vitest-exclusions.json` con su estado medido, de modo que añadir una constituya una
diferencia revisable en un archivo específico en lugar de ser una línea más en un array de 60 entradas. Deliberadamente,
no vuelve a ejecutar las pruebas excluidas: eso tarda unos 10 minutos y corresponde a un trabajo periódico; el
inventario registra cuándo se midió cada una por última vez.
