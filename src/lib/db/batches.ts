 ...
 *
 * Batches whose `api_key_id` IS NULL are intentionally OUT of a key-scoped
 * sweep. This diverges from `scopeCheck` in
 * `src/app/api/v1/batches/[id]/route.ts`, which lets any key read/delete a
 * single unowned batch by id: a bulk destructive sweep must never reach
 * records the key does not own, so unowned batches are only swept by
 * `{ allTenants: true }`.
 *
